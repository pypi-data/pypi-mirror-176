...
class EC2Manager:
    ...
    def _install_python(self, name):
        self._logger.info(f'Installing python...')
    ...
    def _install_git(self, name):
        self._logger.info(f'Installing git...')
        if self._platform == 'amzn2':
            self.run_command(
                name,
    ...
    def _install_docker(self, name):
        self._logger.info(f'Installing docker...')
        if self._platform == 'amzn2':
            self.run_command(
                name,
                [
    ...
    def _install_docker_compose(self, name):
        self._logger.info(f'Installing docker compose...')
        if self._platform == 'amzn2':
            self.run_command(
                name,
                [
    ...
    def _install_dependencies(self, name, data):
        self._install_git(name)
        self._install_python(name)
        self._install_docker(name)
        self._install_docker_compose(name)
        self._expose_docker_daemon(name)
    ...
    def install_dependencies(self):
        """
        Installs dependencies on the instances.
        """
        self._call_on_all(
            callable_instance=self._install_dependencies,
