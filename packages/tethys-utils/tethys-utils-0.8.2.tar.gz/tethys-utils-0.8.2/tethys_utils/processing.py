...
base_encoding = {'lon': {'dtype': 'int32', '_FillValue': -2147483648, 'scale_factor': 0.000001},
                 'lat': {'dtype': 'int32', '_FillValue': -2147483648, 'scale_factor': 0.000001},
                 'altitude': {'dtype': 'int32', '_FillValue': -2147483648, 'scale_factor': 0.001},
                 'height': {'dtype': 'int32', '_FillValue': -2147483648, 'scale_factor': 0.001},
                 'time': {'_FillValue': -9223372036854775808, 'units': "seconds since 1970-01-01 00:00:00", 'dtype': 'int64', 'calendar':'gregorian'},
                 'modified_date': {'_FillValue': -9223372036854775808, 'units': "seconds since 1970-01-01 00:00:00", 'dtype': 'int64', 'calendar':'gregorian'},
                 'band': {'dtype': 'int8', '_FillValue': -99, 'scale_factor': 1},
                 'chunk_day': {'dtype': 'int32'},
                 'chunk_date': {'_FillValue': -2147483648, 'units': "days since 1970-01-01 00:00:00", 'dtype': 'int32', 'calendar':'gregorian'},
    ...
    for e, val in encoding1.items():
        if e in results:
            if ('dtype' in val) and (not 'scale_factor' in val):
                if ('int' in val['dtype']) and ('datetime' not in results[e].dtype.name):
...
def update_compare_results(previous_result, metadata, version_date, new_path):
    ...
    hash_id = parts1[-1].split('.')[1]

    version_date_key = make_run_date_key(version_date)

    glob_str = '{ds_id}_{vd}_{stn_id}_{chunk_id}_*_results.h5'.format(ds_id=ds_id, stn_id=stn_id, chunk_id=chunk_id, vd=version_date_key)
    new_results = glob.glob(os.path.join(new_path, glob_str))

    old_xr = load_dataset(previous_result)

    if not new_results:
        raise ValueError('No new results found to compare to old results. How did this happen?')

    new_xr = load_dataset(new_results[0])
    ...
    up1 = add_extra_chunk_data(up1)

    new_hash_id = up1.chunk_hash.values.flatten()[0]

    if new_hash_id == hash_id:
        raise ValueError('The new and old chunk hashes are the same. This should not happen...')

    new_result_path = os.path.join(new_path, '{ds_id}_{vd}_{stn_id}_{chunk_id}_{hash_id}_results.h5'.format(ds_id=ds_id, stn_id=stn_id, chunk_id=chunk_id, vd=version_date_key, hash_id=hash_id))

    xr_to_hdf5(up1, new_result_path, compression='zstd')

    if new_results:
        if new_results[0] != new_result_path:
            os.remove(new_results[0])
...
def process_sparse_stations_from_df(stns, precision=6):
    """
...
def get_station_data_from_xr(data):
    ...
    if 'station_geometry' in dims1:
        ...
        geo1 = geojson.Point([lon, lat], True, 6)
...
def preprocess_data_structure(nc_path, time_name, x_name, y_name, variables, time_index_bool=None, projected_coordinates=True):
    ...
    for v in variables:
        ...
        # xr_to_hdf5(xr2.to_dataset(), new_path)
