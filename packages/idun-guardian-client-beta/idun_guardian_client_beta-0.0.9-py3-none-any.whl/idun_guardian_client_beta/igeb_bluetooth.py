...
class GuardianBLE:
    ...
    async def run_ble_record(
        ...
        async def stop_recording_timeout():
            ...
            if self.debug:
                logging.info("[BLE]: Sending stop comman to device")
            await asyncio.sleep(self.ble_delay)
            await self.client.write_gatt_char(
                self.command_id, utf_8_encode(self.stop_cmd)[0]
            )
            ...
            # ------------------ API should send final package in this time  ------------------
            await asyncio.sleep(
            ...
            if self.debug:
                logging.info("[BLE]: Sending disconnect command")
        ...
        async def stop_recording_cancelled_script():
            ...
            if self.debug:
                logging.info("[BLE]: Sending stop command")
            await asyncio.sleep(self.ble_delay)
            await self.client.write_gatt_char(
                self.command_id, utf_8_encode(self.stop_cmd)[0]
            )
        ...
        async def stop_recording_device_lost():
            ...
            # ------------------ Sending stop commands to cloud ------------------
            if self.debug:
        ...
        def exit_script():
            """Exit the script."""
            if self.debug:
                logging.info("[BLE]: Exiting script")
            try:
                sys.exit(0)
            except SystemExit:
                os._exit(0)
            ...
            try:
                ...
                if self.client.is_connected:
                    ...
                    while (
                        ...
                        await asyncio.sleep(self.ble_delay)  # sleep so that everything can happen
                ...
                logging.error("[BLE]: Error in bluetooth client: %s", e)
    ...
    async def read_battery_level(self, interval=1) -> None:
