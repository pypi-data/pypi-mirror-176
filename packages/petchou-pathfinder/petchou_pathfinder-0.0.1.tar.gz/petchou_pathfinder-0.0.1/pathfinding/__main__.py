...
class PathFinder:
    def __init__(self, grid: str | list[str], start: str, end: str, walkable: str, non_walkable: str, debug_mode=False):
        self._grid = [line for line in grid if line != ""] if isinstance(grid, list) else grid.split("\n")
        self._start = start
        self._end = end
    ...
    def optimize_path(self):
        ...
        # ajout d'un décalage au fur et à mesure que la liste se réduit
        offset = 0
    ...
    def show_path(self, show_grid=True, animate=False):
        ...
        if show_grid:
            ...
            grid.insert(0, [" "] + [ALPHABET[i] + " " for i in range(2, len(grid[0]) + 2)])
            for i, row in enumerate(grid):
                row.insert(0, str(i - 1 )if i > 0 else " ")
        ...
        if animate:
            ...
            for node in path:
                grid[node[1]+2][node[0]+1] = path_node
