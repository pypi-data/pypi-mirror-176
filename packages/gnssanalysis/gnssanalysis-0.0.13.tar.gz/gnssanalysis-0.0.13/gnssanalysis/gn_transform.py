...
def xyz2enu(xyz,xyz_origin):
    """returns 2d ndarray with east, north, up columns and rows corresponding to the input xyz"""
    llh_origin_rad = xyz2llh_heik(xyz_origin,deg=False)
    phi = llh_origin_rad[:,0]
    lam = llh_origin_rad[:,1]
    rot = llh2rot(phi=phi, lamb=lam, enu_to_ecef=False)
    return (rot @ (xyz - xyz_origin)[:,:,None]).reshape((xyz_origin.shape[0],-1))
