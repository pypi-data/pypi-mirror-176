...
def yydoysec2datetime(snxdate: _np.ndarray or _pd.Series, recenter: bool = True, as_j2000: bool = True) -> _np.ndarray:
    ...
    time = _gn_const.SEC_IN_DAY // 2 if recenter else secs_day
...
def datetime2yydoysec(datetime):
    ...
    yydoysec_no_nans = (
        _pd.Series(date_years.astype(str)).str.slice(2).values  # need U4+1 for [Y] so str
        + ":"
...
def j20002j2000days(array: _np.ndarray) -> _np.ndarray:
    return ((array - _gn_const.SEC_IN_DAY // 2) / _gn_const.SEC_IN_DAY).astype(
        int
    )  # SEC_IN_DAY//2 is needed to account for J2000 origin at 12:00
