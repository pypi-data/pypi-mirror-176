...
def _diff2msg(diff, tol=None, dt_as_gpsweek=False):
    ...
    if (diff_count[mask] > 1).sum() > 0:
        msg["MAX_DIFF"] = (
            diff_max.round(4).astype(str).str.rjust(7)
...
def _compare_sv_states(diffstd, log_lvl, tol=None, plot=False):
    ...
    if bad_sv_states is not None:
        _logging.log(
            msg=f':diffutil found SV states diffs above {"the extracted STDs" if tol is None else f"{tol:.1E} tolerance"}:\n{bad_sv_states.to_string(justify="center")}\n',
            level=log_lvl,
        )
        return -1
    _logging.log(
        msg=f':diffutil [OK] SVs states diffs within the {"extracted STDs" if tol is None else f"{tol:.1E} tolerance"}',
        level=_logging.INFO,
    )
    return 0
...
def _compare_nonsv_states(diffstd, log_lvl, tol=None):
    ...
    if bad_nonsv_states is not None:
        _logging.log(
            msg=f':diffutil non-SVs states diffs above {"the extracted STDs" if tol is None else f"{tol:.1E} tolerance"}:\n{bad_nonsv_states.to_string(justify="center")}\n',
            level=log_lvl,
        )
        return -1
    _logging.log(
        msg=f':diffutil [OK] non-SVs states diffs within the {"extracted STDs" if tol is None else f"{tol:.1E} tolerance"}',
        level=_logging.INFO,
    )
    return 0
...
def _compare_sat_postfit_residuals(diffstd, log_lvl, tol=None):
    ...
    if bad_sv_residuals is not None:
        _logging.log(
            msg=f':diffutil found SVs residuals diffs above {"the extracted STDs" if tol is None else f"{tol:.1E} tolerance"}:\n{bad_sv_residuals.to_string(justify="center")}\n',
            level=log_lvl,
        )
        return -1
    _logging.log(
        msg=f':diffutil [OK] SVs residuals diffs within the {"extracted STDs" if tol is None else f"{tol:.1E} tolerance"}',
        level=_logging.INFO,
    )
    return 0
...
def _compare_nonsat_postfit_residuals(diffstd, log_lvl, tol=None):
    ...
    if bad_nonsv_residuals is not None:
        _logging.log(
            msg=f':diffutil found non-SVs residuals diffs above {"the extracted STDs" if tol is None else f"{tol:.1E} tolerance"}:\n{bad_nonsv_residuals.to_string(justify="center")}\n',
            level=log_lvl,
        )
        return -1
    _logging.log(
        msg=f':diffutil [OK] non-SVs residuals diffs within the {"extracted STDs" if tol is None else f"{tol:.1E} tolerance"}',
        level=_logging.INFO,
    )
    return 0
