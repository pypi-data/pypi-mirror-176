...
def get_logger(debug) -> logging.Logger:
    ...
    log = logging.getLogger("aiotinyrpc")
...
class EncryptedSocketClientTransport(ClientTransport):
    ...
    def __init__(self, address: str, port: int, debug: bool = False):
        ...
        self.log = get_logger(self.debug)
...
class EncryptedSocketServerTransport(ServerTransport):
    ...
    async def handle_client(self, reader, writer):
        peer = writer.get_extra_info("peername")
        self.log.info("Peer connected:", peer)
        ...
        while running:
            ...
            if message:
                self.log.debug("Message received (decrypted):", message)
