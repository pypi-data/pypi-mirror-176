...
def add_job_task(
    ...
    :param str analyzerName: The name of the analyzer \=\> \[a\-zA\-Z0\-9\_\:\\\-\\.\]\*
    :param str state: The initial state of the job task \(default enqueued\)
    :param json: return the response's body as a ``dict`` parsed from json. ``True`` by
      default. If set to false, the raw ``requests.Response`` object will be returned.
    :param verify: path to a certificate bundle or boolean indicating whether SSL
      verification should be performed.
...
def search_jobs(
    ...
    :param list state: Only return jobs in either of these states \(default \[enqueued\, executing\]\)
    ...
    if state is not None:
        body.update({"state": state})
    # Only send sampleSha256 if the argument was provided, dont send null values
...
def update_job_task(
    ...
    :param str analysisID: The ID of the analysis produced by this task. Must be set when updating the state to \'success\'
    ...
    if state is not None:
        body.update({"state": state})
    # Only send message if the argument was provided, dont send null values
