...
def bulk_schedule_sensor_downtime(
    ...
    :param bool acknowledge: Whether to acknowledge the Sensor at the same time as scheduling downtime. \(default false\)
    :param json: return the response's body as a ``dict`` parsed from json. ``True`` by
      default. If set to false, the raw ``requests.Response`` object will be returned.
    :param verify: path to a certificate bundle or boolean indicating whether SSL
      verification should be performed.
    ...
    if comment is not None:
        body.update({"comment": comment})
    # Only send acknowledge if the argument was provided, dont send null values
    if acknowledge is not None:
        body.update({"acknowledge": acknowledge})
...
def restore_sensor(
    ...
    body = body or {}
    # Only send enable if the argument was provided, dont send null values
...
def submit_sensor_status_bulk(
    ...
    """Submit sensor status records in a bulk operation (DEV)
