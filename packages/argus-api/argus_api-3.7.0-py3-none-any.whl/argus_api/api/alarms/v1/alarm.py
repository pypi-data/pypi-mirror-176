...
def add_alarm_mitre_relationship(
    ...
    # Only send categoryModel if the argument was provided, dont send null values
    if categoryModel is not None:
        body.update({"categoryModel": categoryModel})
    # Only send categoryVersion if the argument was provided, dont send null values
...
def list_alarm_mitre_relationships(
    ...
    query_parameters = {}
    # Only send categoryType if the argument was provided, dont send null values
...
def update_alarm(
    ...
    :param list addReferences: Add vulnerability references \(CVE\-numbers\, BID\-numbers\, URLs etc\) \=\> Sanitize by regex \^\(\(\?\!\(https\?\|ftp\|gopher\|telnet\|file\|javascript\|mailto\)\:\)\)\(\[\\s\\w\\\{\\\}\\\$\\\-\\\(\\\)\\.\\\[\\\]\"\\\'\_\/\\\\\,\\\*\\\+\\\#\:\@\!\?\;\=\]\)\*\|\(\(https\?\|ftp\|gopher\|telnet\|file\)\:\(\(\/\)\|\(\\\\\)\)\+\[\\w\\d\:\\\#\@\%\/\;\$\(\)\~\_\?\\\\\+\-\=\\\\\\.\&\]\*\)
