...
from hestia_earth.utils.tools import flatten, list_sum, safe_parse_float
...
from hestia_earth.validation.utils import (
    ...
    _find_linked_node, _is_before_today, run_model_from_node, run_model, _value_from_model, _get_dict_key,
...
def validate_area(node: dict):
    ...
    try:
        ...
        return delta < threshold or {
            'level': 'error',
            'dataPath': '.area',
            'message': f"must be equal to boundary (~{expected_value})",
            'params': {
                'current': value,
