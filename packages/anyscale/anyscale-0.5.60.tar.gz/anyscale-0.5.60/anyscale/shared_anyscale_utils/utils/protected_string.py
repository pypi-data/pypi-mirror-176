...
from shared_anyscale_utils.conf import ANYSCALE_ENV
...
class UserPassword(ProtectedString):
    ...
    def verify_against_hash(self, other_hash: "UserPasswordHash") -> bool:
        return cast(
            bool,  # black: no
            hasher.verify(other_hash._UNSAFE_DO_NOT_USE, self._UNSAFE_DO_NOT_USE),
        )
...
def protect_strings(
    ...
    for key in keys:
        ...
        if allow_optional_strings:
            assert (
                isinstance(old_string, str) or old_string is None
            ), "Tried to protect a non-string"
            if old_string is None:
                result_dict[key] = None
            else:
                result_dict[key] = protected_string_type(old_string)
        else:
            assert isinstance(old_string, str), "Tried to protect a non-string"
            result_dict[key] = protected_string_type(old_string)
    return result_dict
