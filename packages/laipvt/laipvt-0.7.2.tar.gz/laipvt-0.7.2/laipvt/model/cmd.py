...
class DockerImageModel(DockerModel):
    ...
    def _parse_tag(self, image: str) -> dict:
        ...
        try:
            t = image.split(":")
            return {"image_name": t[0], "image_tag": self.tag_version}
        except AttributeError as e:
    ...
    def run(self) -> bool:
        ...
        for image in image_name_withtag:
            ...
            if self.tag(image, path_join(self.repo_path, new_image_name), image_name["image_tag"]):
                self.new_image_addr = path_join(self.repo_path, new_image_name)
                self.push(path_join(self.repo_path, new_image_name), image_name["image_tag"], auth_config=self.auth_config)
        return False
