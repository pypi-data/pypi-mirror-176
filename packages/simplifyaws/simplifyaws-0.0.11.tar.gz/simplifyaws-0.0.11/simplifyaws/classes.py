...
import boto3
from simplifyaws.SQS import SQS

from simplifyaws.constants import FilterOptions, Services


class SimplifyAWS:
    def __init__(self,services:list[Services],region:str=os.environ.get('AWS_DEFAULT_REGION'),aws_access_key:str=os.environ.get('AWS_ACCESS_KEY_ID'),aws_secret_key:str=os.environ.get('AWS_SECRET_ACCESS_KEY'),secrets_name:str=os.environ.get('AWS_SECRET_ACCESS_KEY')):
        self.__clients={}
    ...
    def get_secrets(self,secrets_name=None):
        if self.__clients.get(Services.SECRETSMANAGER):
            try:
                if not secrets_name:
                    secrets_name=self.secrets_name
                response=self.__clients.get(Services.SECRETSMANAGER).get_secret_value(SecretId=secrets_name)
    ...
    def publish_sns(self,body,SNS_ARN,usecaseStringValue,MessageGroupId,usecaseData_type:str='String',MessageDeduplicationId=uuid.uuid4().hex):
        if self.__clients.get(Services.SNS):
            response = self.__clients.get(Services.SNS).publish(
                    TargetArn=SNS_ARN,
    ...
    def create_sqs_loop(self,sqs_url,callback,name:str='Loop',sleep_time=0.2,visibility_timeout=20,wait_time_seconds=0,max_messages=10,filters:list[str]=[],filter_type:FilterOptions=FilterOptions.ANY):
        if self.__clients.get(Services.SQS):
            try:
                sqs_loop=SQS(sqs_url,self.__clients[Services.SQS],callback,name,sleep_time,visibility_timeout,wait_time_seconds,max_messages,filters,filter_type)
