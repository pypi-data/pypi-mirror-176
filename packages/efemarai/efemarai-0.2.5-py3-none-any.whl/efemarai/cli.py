        ...
        with console.status(
            ...
            for dataset in result["datasets"]:
                ...
                if dataset.loading_failed:
                    if verbose:
                        console.print(f":poop: Dataset {dataset} failed to load.")
                    exit(1)
...
def model_version_list(model):
    ...
    if model.endswith((".yaml", ".yml", ".")):
        ...
        models = [m for m in project.models if m.name == model or m.id == model]
...
def dataset_download(dataset, file, format, path, num_samples, unzip, ignore_cache):
    ...
    if _check_for_multiple_entities(project.datasets, dataset):
        ...
        _print_table(datasets)
        console.print(
            f"\nRun the command with a specific dataset id: [bold green]$ efemarai dataset download {datasets[0].id}",
        )
        exit(1)
...
def test_run(definition_file, file, wait, sequential, verbose):
    ...
    for test_definition in test_definitions:
        ...
        if sequential:
            _wait_for_stress_tests([test])

    if not wait:
        return

    _wait_for_stress_tests(tests)


def _wait_for_stress_tests(tests):
...
def test_download(
    test, format, min_score, include_dataset, path, unzip, ignore_cache, file
):
...
def _check_for_multiple_entities(entities, name):
    return len([x for x in entities if x.name == name])
