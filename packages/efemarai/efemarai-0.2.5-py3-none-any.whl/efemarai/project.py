...
class Project:
    ...
    def _upload(self, from_url, endpoint):
        return self._session._upload(from_url, endpoint, self.id)
    ...
    def models(self):
        ...
        return [
            ...
            for model in self._get(f"api/models/{self.id}")
        ]
    ...
    def push_model(
        ...
        **kwargs,
        ...
        if existing_model:
            ...
            return Model.create(self, name, description, repository, files)
    ...
    def datasets(self):
        ...
        return [
            Dataset(
                ...
                dataset["stage"],
                dataset["data_url"],
                dataset.get("annotations_url"),
                dataset["states"][-1]["name"],
                Dataset._parse_classes(dataset["classes"]),
            )
            for dataset in self._get(f"api/datasets/{self.id}")
        ]
    ...
    def create_dataset(
        ...
        **kwargs,
        ...
        Args:
            ...
            stage (str): Dataset stage - one of [`training`, `validation`, `test`].
    ...
    def domains(self):
        ...
        return [
            ...
            for domain in self._get(f"api/domains/{self.id}")
        ]
    ...
    def create_domain(
        self, name, transformations, graph, exists_ok=False, **kwargs
    ) -> Domain:
        """
    ...
    def stress_tests(self):
        ...
        return [
            StressTest(
                ...
                test["dataset"]["id"],
                test["states"][-1]["name"],
                test["states"][-1].get("message"),
                test["reports"],
            )
            for test in self._get(f"api/getRuns/{self.id}")["objects"]
        ]
