...
def create_body(args):
    def search_lib_by_name(lib_name, lib_ver, lib_type, lib_ext):
        try:
            ...
            for lib_ in lib_lst['librariesInformation']:
                try:
                    if (lib_['artifactId'][lib_['artifactId'].rfind('.') + 1:]).lower() == lib_ext or lib_ext == "":
                        try:
                            sha1 = lib_['sha1']
                        except:
                            pass
            ...
            # logger.error(f"The error was produced. Library {lib_name}. Details: {err}")
            return "", ""
                ...
                for ext_ref in pck_ext:
                    if ext_ref['referenceCategory'] == "PACKAGE_MANAGER" or ext_ref[
                        ...
                        pkgname = re.search(r"pkg:(.*?)/", ext_ref['referenceLocator'], flags=re.DOTALL).group(
                            1).strip()
                        lang_type = SHA1CalcType.get_package_type(f_t=pkgname).language
                        ext = SHA1CalcType.get_package_type(f_t=pkgname).ext
                        lang_types.append({lang_type: ext})
                        break
            except:
                ext_name = os.path.splitext(package['packageFileName'])[1][1:]
                type_lst = SHA1CalcType.get_package_type_list_by_ext(ext_name)
                for type_lst_ in type_lst:
                    lang_type = type_lst_.language
                    ext = type_lst_.ext
                    lang_types.append({lang_type: ext})
                #    TODO Maybe need to check each package type, if we have not any data related package
                ...
                if sha1_ != "":
                    ...
                    if add_sha1 != "":
                        pck = {
                            ...
                            "systemPath": "",
                            "optional": False,
                            "filename": f"{lname_ if lname_ != '' else pkg_name}",
                            "additionalSha1": f"{add_sha1}",
                            "dependencyFile": ""
                        }
                    #else:
                    #    logger.warning(f"Impossible to find and import file {pkg_name}.")
...
def main():
    ...
    try:
        ...
        """
        try:
            json_to_csv_(json_file_name=args.sbom, csv_file_name="test.csv")
        except Exception as err:
            logger.error(f"Error opening SBOM file: {err}")
            exit(-1)
        """
        #outp = csv_to_json("test.csv")
        #exit(-1)
