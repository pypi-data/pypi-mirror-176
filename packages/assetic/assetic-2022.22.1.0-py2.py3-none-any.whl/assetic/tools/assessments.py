...
class AssessmentHelper(object):
    ...
    def create_form_from_csv(self, form_repr, csvfile, structure=None, exceed_limit=0):
        """
        ...
        if len(rows) == 0:
            ...
            return 1
        if len(rows) > 80 and exceed_limit == 0:
            self.logger.warning("If total rows are greater than 80, set exceed_limit to 1 in the prompter (ex. "
                                "assessment_form_create_prompter(exceed_limit = 1)")
        ...
        if exceed_limit == 0:
            ...
            new_form = self.assesstool.new_form(form_name, form_level, form_label)
            # add an empty tab with tab label provided by the user
            newtab = self.assesstool.new_tab(tab_label)
            new_form.form_tabs.append(newtab)
            newformid = self.assesstool.create_form(new_form)
            if newformid.lower() != form_name.lower():
                return newformid
            # get the form guid
            new_form_guid = self.assesstool.get_form_guid_internal_api(newformid)
            if new_form_guid is None:
                return 1
            # get the payload of the form that has just been created
            new_form_json = self.assesstool.get_form(newformid)
            # add guid to the payload
            new_form_json['Id'] = new_form_guid
            # add FormTabs to the payload provided by the user
            new_form_json['FormTabs'] = self.api_client.sanitize_for_serialization(form.form_tabs)
            # update the form with the new payload
            newformid = self.assesstool.update_form_internal_api(new_form_json)
        if newformid.lower() != form_name.lower():
            return newformid
        if len(self._comboboxrelations) > 0:
            ...
            if exceed_limit == 1:
                # use internal api
                self.logger.debug("Update form using internal API")
                final_savedform_json = self.api_client.sanitize_for_serialization(savedform)
                final_savedform_json['Id'] = new_form_guid
                chk = self.assesstool.update_form_internal_api(final_savedform_json)
            else:
    ...
    def assessment_form_create_prompter(self, exceed_limit=0):
        """X
        ...
        ##create the form
        try:
            result = self.create_form_from_csv(form_repr, csvfile, csv_structure, exceed_limit)
        except Exception as ex:
            self.user_input(
...
class AssessmentFormCreateRepresentation(object):
    ...
    def __init__(self, form_name=None, form_label=None, form_level=None, version=1, can_add_attachment=True,
                 tab_label="Tab", layout_pattern="Single", widget_labels=["Data"], label_formatter=None):
        """
...
class AssessmentTools(object):
    ...
    def new_form(self, form_name, form_level, label, version=1, can_add_attachment=True, tabs=None,
    ...
    def update_form_internal_api(self, form):
        """
           update an assessment form
           :param form: The complete form representation
           AssessmentFormPageRepresentation
           :returns: >0=error (API status code), 0 = Success
           This API is used when Assessment API failed to create Assessment Form.
        """
        try:
            import requests
            import json
            ...
            return None
        try:
            response = requests.put(url=self.api_client.configuration.host + '/api/FormApi',
                                    headers=self.api_client.default_headers, data=json.dumps(form))
    ...
    def get_form_guid_internal_api(self, form_name):

        """
        Get an assessment form guid
        : param form_name: get the form from API using form id/form name
        returns: None= error, guid = Success
        ...
        try:
            ...
            return None
        try:
            response = requests.get(url=self.api_client.configuration.host + '/api/maintenance/authtoken',
        ...
        try:
            response = requests.get(
                self.api_client.configuration.host + '/maintenanceapi/assessmentforms?filters=name=={0}'.format(
                    form_name),
        ...
        if form_details['total_results'] != 1:
            self.logger.error("Total results returned for form {0} must be equal to 1.".format(form_name))
            return None
        else:
            form = {}
            for result in form_details['results']:
                for key, value in result.items():
                    form[key] = value
            if 'id' not in form:
                self.logger.error('id not found in the form {0}'.format(form))
                return None
            return form['id']

    def add_tab_with_widgets_to_form(self, form, tab_label, widgetlabels, pattern_name="Single", visible=True,
                                     taborder=None, collapsed=False):
        """
    ...
    def new_multiline_text_control(self, label, name=None, length=1000, order=1, hint=None, visible=True,
                                   required=False):
        """
    ...
    def new_stepper_control(self, label, name=None, minvalue=0, maxvalue=1000, step=1, mask="n0", order=1, hint=None,
