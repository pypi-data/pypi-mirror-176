...
class NetboxManager:
    """
    NetboxManager manages the object that hosts infrastruc ture data from NetBox host.
    ...
    def inject_host_data_master(self, host):
        ...
        host.username = ""
        host.password = ""
        if 'virtual-machines' in host['url']:
            port = self._nb.secrets.secrets.get(
                virtual_machine_id=host["id"], 
                name="port"
            )
        ...
        #host.connection_options["netmiko"] = {"extras": {"timeout": 60}}
