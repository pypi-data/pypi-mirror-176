...
from .port_mapping import (
    get_junos_lo_configs,
    get_linux_lo_configs
)
...
def los_config_generate(topo=None):
    ...
    for host in nr_inv.hosts:
        ...
        # print_json(host_data) #################################################################
        #manufacturer = host_data['device_type']['manufacturer']['name']
        platform = host_data['platform']['slug']
        con_ints=[]
        if topo:
            for link in topo:
                host_a = link['endpoint_a']['host']['name']
                host_z = link['endpoint_z']['host']['name']
        ...
        if platform == 'junos':
            ...
            #TO-DO create get linux configs based on interface
            lo_configs = get_linux_lo_configs(nornir=nr, dev_name=host).splitlines()
