...
def process_mimic_data(data_dir):
    ...
    patients['real_anchor_year_sample'] = patients.anchor_year_group.apply(lambda x: get_anchor_year(x))
...
class MIMICBase(Dataset):
    def __init__(self, args):
        ...
        self.args = args
        if not os.path.exists(args.data_dir + '/mimic_stay_dict.pkl'):
            get_stay_dict(args.data_dir)
        data = pickle.load(open(args.data_dir + '/mimic_stay_dict.pkl', 'rb'))
        preprocess_MIMIC(data, args)
