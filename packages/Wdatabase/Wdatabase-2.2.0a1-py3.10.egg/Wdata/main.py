...
class WdataMain():
    ...
    def Save_file(self, filename: str, type=JSON, *excelmessage) -> None:
        """
        ExcelMessage:
            1. Sheet        Default File Name
            2. Row or Column (True or False)
        WARNING: BEFORE USE EXCEL, YOU SHOULD CREATE EXCEL FILE AND CLOSE IT
        """
        ...
        if type == JSON:
            ...
            for i in range(len(years_list)):
                if excelmessage[1] == True:
                    sheet.cell(row=i, column=0).value = years_list[i]
                    sheet.cell(row=i, column=1).value = value_list[i] 
                else:
                    sheet.cell(row=0, column=i).value = years_list[i]
                    sheet.cell(row=1, column=i).value = value_list[i]
        elif type == CSV:
