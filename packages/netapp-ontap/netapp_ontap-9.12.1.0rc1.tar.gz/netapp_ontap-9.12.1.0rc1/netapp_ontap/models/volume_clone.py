...
class VolumeCloneSchema(ResourceSchema, metaclass=ResourceSchemaMeta):
    ...
    inherited_physical_used = Size(data_key="inherited_physical_used")
    r""" Inherited physical used from the clone's base snapshot. """

    inherited_savings = Size(data_key="inherited_savings")
    ...
    """inherited_physical_used,inherited_savings,is_flexclone,parent_snapshot.links,parent_snapshot.name,parent_snapshot.uuid,parent_svm.links,parent_svm.name,parent_svm.uuid,parent_volume.links,parent_volume.name,parent_volume.uuid,split_complete_percent,split_estimate,split_initiated,"""
