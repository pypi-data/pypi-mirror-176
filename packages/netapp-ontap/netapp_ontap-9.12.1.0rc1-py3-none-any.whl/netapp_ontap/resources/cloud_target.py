...
* s3_snapmirror"""
    ...
    r""" Type of cloud provider. Allowed values depend on owner type. For FabricPool, AliCloud, AWS_S3, Azure_Cloud, GoggleCloud, IBM_COS, SGWS, and ONTAP_S3 are allowed. For SnapMirror, the valid values are AWS_S3 or SGWS. For FabricLink, AWS_S3, SGWS, S3_Compatible, S3EMU, LOOPBACK and ONTAP_S3 are allowed."""
...
* virtual_hosted_style"""
    ...
    """links,access_key,authentication_type,azure_account,cap_url,certificate_validation_enabled,cluster,container,ipspace.links,ipspace.name,ipspace.uuid,name,owner,port,provider_type,scope,server,server_side_encryption,snapmirror_use,ssl_enabled,svm.links,svm.name,svm.uuid,url_style,use_http_proxy,used,uuid,"""
    ...
    """access_key,authentication_type,azure_account,azure_private_key,azure_sas_token,cap_url,certificate_validation_enabled,cluster,name,port,secret_password,server,server_side_encryption,snapmirror_use,ssl_enabled,svm.name,svm.uuid,url_style,use_http_proxy,"""
    ...
    """access_key,authentication_type,azure_account,azure_private_key,azure_sas_token,cap_url,certificate_validation_enabled,cluster,container,ipspace.name,ipspace.uuid,name,owner,port,provider_type,secret_password,server,server_side_encryption,snapmirror_use,ssl_enabled,svm.name,svm.uuid,url_style,use_http_proxy,"""
...
* [`DOC /cloud/targets`](#docs-cloud-cloud_targets)"""
    ...
    if RECLINE_INSTALLED:
        ...
        def cloud_target_show(
            fields: List[Choices.define(["access_key", "authentication_type", "azure_account", "azure_private_key", "azure_sas_token", "cap_url", "certificate_validation_enabled", "container", "name", "owner", "port", "provider_type", "scope", "secret_password", "server", "server_side_encryption", "snapmirror_use", "ssl_enabled", "url_style", "use_http_proxy", "used", "uuid", "*"])]=None,
        ) -> ResourceTable:
            ...
            Args:
                ...
                provider_type: Type of cloud provider. Allowed values depend on owner type. For FabricPool, AliCloud, AWS_S3, Azure_Cloud, GoggleCloud, IBM_COS, SGWS, and ONTAP_S3 are allowed. For SnapMirror, the valid values are AWS_S3 or SGWS. For FabricLink, AWS_S3, SGWS, S3_Compatible, S3EMU, LOOPBACK and ONTAP_S3 are allowed.
