...
def StressReduction(G, label1,label2,numSGD):
  for n in G.nodes():
      nodeStress = 0
      neigList = list(G.neighbors(n))
      a = np.zeros(numSDG)
...
def graphCalliberation(numRounds,EpsilonStress,crop,statesList,numSDG,final,attribute_list,taluk_attri_Dict,states):
  ...
  for n in G3.nodes(): 
    statesDict[G3.nodes[n]["name"]].append(G3.nodes[n]["sdgvec"])
  for n in G3.nodes():
    df[G3.nodes[n]["name"]] = statesDict[G3.nodes[n]["name"]] 
  final[crop] = df
  data_to_append= []
  for n in G3.nodes():
...
def StressModelling(numNodes,numSDG,numOfRounds,BeforeATEFile,AfterATEFile,adjList):
    ...
    df2=pd.read_excel('/content/sample_data/ATEBefore.xlsx')
    ...
    taluk_attri_Dict = dict(zip(df.agroClimaticZone, df.sdgvec))
