...
class Runner():
    ...
    def process_run_command(self, args):
        ...
        if not self.backend.provides_container_support():
            ...
            if docker_name and docker_name != "none":
                self.build_docker_cmd(docker_name, compute, service_type, bootstrap_dir, job_secret, setup_name, args)
        ...
        self.create_job_info(job_id, search_style, total_run_count, runs_by_box, experiment, job_guid, job_secret, repeat_count, 
            compute_def, workspace, secrets_by_node, args)
        ...
        if not fake_submit:
            ...
            self.update_job_info(job_id, workspace, service_job_info, service_info_by_node)
    ...
    def update_job_info(self, job_id, workspace, service_job_info, service_info_by_node):
...
def update_compute_def_from_cmd_options(compute_def, hold=False):
    ...
    compute_props = ["vm-size", "azure-image", "low-pri", "box-class", "docker", "setup", "boxes", 
        "vc", "cluster", "queue", "sku", "nodes", "sla", "aml_compute", "service"]
