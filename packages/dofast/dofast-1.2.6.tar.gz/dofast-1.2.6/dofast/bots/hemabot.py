...
from dofast.bots.bothelper import CMCCCookieUpdater
    ...
    def text_handler(self, update: Update, context: CallbackContext) -> None:
        ...
        try:
            if re.findall(r'[\u4e00-\u9fff]+', text):     # contain chinese
                ...
                resp = cf.net.post('http://localhost:5000',
                                   json={
                                       'text': text.lower()
                                   }).json()
        ...
        PROC = PROC_MAP.get(label, None)
        if PROC:
            update.message.reply_text('Match {}'.format(PROC.__name__))
            PROC().process(text, update)
        else:
