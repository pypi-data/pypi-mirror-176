...
def main() -> None:
    ...
    parser.add_argument(
        "--version",
        action="store_true",
        help="Show the program version (0.0.0-master)",
    )
    parser.add_argument(
        "-st",
        "--set-tag",
    ...
    if argv.version:
        print(cyan("version-manager: 0.0.0-master"))
        sys.exit(0)
    ...
    for resolved_name, version_patterns in files_to_process.items():
        ...
        for version_pattern in version_patterns:
            ...
            if version_pattern.match_count != version_pattern.expected_count:
                print(
                    red(
                        "Got %d matches instead of %d trying to find %s"
                        % (version_pattern.match_count, version_pattern.expected_count, version_pattern)
                    )
                )
                sys.exit(3)
