...
from tuxbake.utils import repo_init, git_init, copy_artifacts
...
class OEBuild(Base):
    ...
    def do_build(self):
        ...
        cmd = [
            "bash",
            "-c",
            f"rm -f {self.build_dir}/conf/bblayers.conf && rm -f {self.build_dir}/conf/local.conf && source {self.envsetup} {self.build_dir} && cat ../extra_local.conf >> conf/local.conf && cat ../bblayers.conf >> conf/bblayers.conf || true && echo 'Dumping local.conf..' && cat conf/local.conf && bitbake -e > bitbake-environment",
        ]
    ...
    def publish_artifacts(self):
        if self.distro.startswith("openbmc"):
            bitbake_env_file = (
                f"{self.src_dir}/openbmc/{self.build_dir}/bitbake-environment"
            )
        else:
            bitbake_env_file = f"{self.src_dir}/{self.build_dir}/bitbake-environment"
        artifacts_dir = f"{self.src_dir}/{self.artifacts_dir}"
        copy_artifacts(self.artifacts, self.src_dir, artifacts_dir, bitbake_env_file)
