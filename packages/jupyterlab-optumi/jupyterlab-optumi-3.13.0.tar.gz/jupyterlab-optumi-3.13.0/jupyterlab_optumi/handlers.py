...
class StopNotebookHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            module = data["module"]
            response = await IOLoop.current().run_in_executor(
                None, optumi.core.stop_notebook, workload, module
            )
            self.write(response.text)
            if response.status_code >= 300:
                jupyter_log.info(
...
class TeardownNotebookHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            module = data["module"]
            response = await IOLoop.current().run_in_executor(
                None, optumi.core.teardown_notebook, workload, module
            )
            self.write(response.text)
            if response.status_code >= 300:
                jupyter_log.info(
...
def setup_handlers(server_app):
    ...
    web_app.add_handlers(
        ...
        [
            ...
            (
                url_path_join(base_url, "/optumi/preview-notebook"),
                PreviewNotebookHandler,
            ),
            (url_path_join(base_url, "/optumi/setup-notebook"), SetupNotebookHandler),
            (url_path_join(base_url, "/optumi/launch-notebook"), LaunchNotebookHandler),
            (
            ...
            (url_path_join(base_url, "/optumi/stop-notebook"), StopNotebookHandler),
            (
                url_path_join(base_url, "/optumi/teardown-notebook"),
            ...
            (url_path_join(base_url, "/optumi/delete-files"), DeleteFilesHandler),
            ...
            (url_path_join(base_url, "/optumi/delete-machine"), DeleteMachineHandler),
