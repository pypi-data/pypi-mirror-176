...
@strawberry.federation.type(description='Information to assist with pagination.', shareable=True)
class PageInfo:
...
class Connection(Generic[T]):
    ...
    def load(cls, data: Pagination, counts: Tuple[int, int]):
        ...
        return Connection(
            edges=[Edge(node=item, cursor=to_cursor_hash(item.created_at)) for item in data.items],
            page_info=PageInfo(
                start_cursor=to_cursor_hash(data.items[0].created_at) if data.items else None,
                end_cursor=to_cursor_hash(data.items[len(data.items) - 1].created_at) if data.items else None,
