...
from .generic import _shutdown_instance
...
class DependencyContainer(contracts.DependencyContainer):
    ...
    def register_shutdown_callback(
        ...
        self._callbacks[contract] = contracts.CallbackRecord(contracts.EventType.ON_SHUTDOWN, callback)
    ...
    async def shutdown(self) -> typing.NoReturn:
        for contract, record in self._callbacks.items():
            if record.type == contracts.EventType.ON_SHUTDOWN:
                singleton_instance = self._get_from_registry(contract)
                await _shutdown_instance(record.instance, singleton_instance)
