...
class ECGTrainerBuilder(IService):
    ...
    def build(self):
        ...
        for i in range(1, self.builder_parameters.cv + 1):
            ...
            self.serialize_data_set(training_data_set, self.builder_parameters.training_set_file_path)
            self.serialize_data_set(validation_data_set, self.builder_parameters.validation_set_file_path)
    ...
    def _load_images_data(self) -> list:
        ...
        for image_file in image_files:
            ...
            mask_image = imread(str(mask_file_file_path[0])) / 255.0
            # TODO check what this function is for
            if np.sum(mask_image) == 0:
                continue

            image = imread(str(image_file)) / 255.0
            print(f"create image set {i}")
            image_set = self._create_image_set(image=image, mask_image=mask_image)
            print(f"dataset append set {i}")
    ...
    def serialize_data_set(self, data_set: list, file_path: Path):
        print(f"Start serialize dataset start {file_path}")
        data = pd.DataFrame(data_set)
        data.to_json(path_or_buf=file_path)
        print(f"Finish serialize dataset start {file_path}")
