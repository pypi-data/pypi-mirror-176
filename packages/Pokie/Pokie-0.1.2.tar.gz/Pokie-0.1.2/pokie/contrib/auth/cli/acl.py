...
class DescriptionRequest(RequestRecord):
    fields = {
        'description': field(validators="required|minlen:1|maxlen:200", error="invalid description")
    }


class IdDescriptionRequest(RequestRecord):
    fields = {
        'id_resource': field(validators="required|minlen:1|maxlen:200", error="invalid resource id"),
        'description': field(validators="required|minlen:1|maxlen:200", error="invalid description")
    }
...
class UserRoleRequest(RequestRecord):
    fields = {
        ...
        'id_role': field(validators="required|numeric|pk:acl_role", error="invalid role id")
    }
...
class RoleResourceRequest(RequestRecord):
    fields = {
        'id_role': field(validators="required|numeric|pk:acl_role", error="invalid role id"),
        'id_resource': field(validators="required|pk:acl_resource", error="invalid resource id")
    }
...
class AclRoleCreateCmd(AclCommand):
    ...
    def arguments(self, parser: ArgumentParser):
        parser.add_argument('description', type=str, help="Role description for new role")
    ...
    def run(self, args) -> bool:
        ...
        for record in self.svc_acl.list_roles():
            if record.description.lower() == lower:
                self.tty.write(self.tty.colorizer.red("Error: role with same description already exists"))
                return False

        id = self.svc_acl.add_role(args.description)
        self.tty.write("Created role #{} with description '{}'".format(str(id), args.description))
...
class AclRoleRemoveCmd(AclCommand):
    ...
    def arguments(self, parser: ArgumentParser):
        ...
        parser.add_argument('-f', '--force', action='store_true', help="Force removal even if role is in use")

    def run(self, args) -> bool:
        id = int(args.id_role)
        record = self.svc_acl.get_role(id)
        if record is None:
            self.tty.write(self.tty.colorizer.red("Error: role id #{} not found".format(str(args.id_role))))
            return False

        if not self.svc_acl.can_remove_role(id) and args.force is False:
            self.tty.write(self.tty.colorizer.red("Error: role is in use; use -f to truncate records"))
            return False
    ...
    def run(self, args) -> bool:
        ...
        for record in self.svc_acl.list_resources():
            if record.id.lower() == id_lower:
                self.tty.write(self.tty.colorizer.red("Error: resource with specified id already exists"))
                return False
            if record.description.lower() == lower:
                self.tty.write(self.tty.colorizer.red("Error: resource with specified description already exists"))
                return False

        id = self.svc_acl.add_resource(args.id, args.description)
        self.tty.write("Created resource '{}' with description '{}'".format(str(id), args.description))
...
class AclResourceLinkCmd(AclCommand):
    ...
    def run(self, args) -> bool:
        ...
        self.tty.write("Associated resource '{}' with role #{}".format(args.id_resource, id_role))
...
class AclResourceUnlinkCmd(AclCommand):
    ...
    def run(self, args) -> bool:
        ...
        self.tty.write("Removed association of resource '{}' with role #{}".format(args.id_resource, id_role))
...
class AclRoleLinkCmd(AclCommand):
    ...
    def run(self, args) -> bool:
        ...
        if user is None:
            self.tty.write(self.tty.colorizer.red("Error: username '{}' not found".format(args.username)))
            return False

        id_role = int(args.id_role)
        self.svc_acl.add_user_role(user.id, id_role)
        self.tty.write("Associated user '{}' with role #{}".format(user.username, id_role))
