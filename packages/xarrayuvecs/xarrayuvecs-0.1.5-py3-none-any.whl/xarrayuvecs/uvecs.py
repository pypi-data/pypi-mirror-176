    ...
    def plotly_ODF(self,nbr=10000,bw=0.2,plot_cm=True,plotOT=True,cf=True,cline=10,xoz_plane=False,res=400,add_dot=None):
            ...
            if plot_cm:
                ...
                if cf:
                    ...
                    fig1=px.imshow(w0,origin='lower')
                    fig.add_trace(fig1.data[0])

            if add_dot is not None:
                proj_dot=odfplot.vector2plane(self._obj,xoz_plane=xoz_plane)
                ss=proj_dot.shape
                proj_dot=proj_dot.reshape([ss[0]*ss[1],2])   
                proj_dot=proj_dot[np.unique(np.random.randint(0,ss[0]*ss[1],size=add_dot)),:]
                fig=odfplot.add_orientation(proj_dot,fig,name='ori',size=2,color='Red')
            ...
            if plotOT:
                ...
                for i in list(range(3)): # Loop on the 3 eigenvalue
                    ...
                    fig=odfplot.add_orientation(np.array([[xxv,yyv]]),fig,name='e'+str(i+1),text=str(round(eigvalue[i],2)))
