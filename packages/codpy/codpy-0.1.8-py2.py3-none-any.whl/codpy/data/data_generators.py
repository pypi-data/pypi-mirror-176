...
def get_box(x):
    if isinstance(x,list):
        temp = [np.asarray(get_box(y)) for y in x]
...
def get_cartesian_mesh(x, steps):
    import itertools
    box = np.asarray(get_box(x))
    box = np.asarray([np.arange(start = box[n][0], stop = box[n][1], step =(box[n][1]-box[n][0])/float(steps[n])) for n in range(len(box))])
    if box.ndim==1:return box
    if box.ndim==2:return np.asarray(itertools.product(box[0],box[1]))
    if box.ndim==3:return np.asarray(itertools.product(box[0],box[1],box[2]))
    test = list(itertools.product(box[0],box[1]))
