...
def mnist(**codpy_param):
    set_kernel = set_gaussian_kernel
    scenarios_list = [ (-1, 4096, 2**i,-1) for i in np.arange(5,10,1)]
    validator_compute=['accuracy_score','discrepancy_error','inertia']
    scenario_generator_ = scenario_generator()
    mnist_impl(scenario_generator_,scenarios_list,MNIST_data_generator(),scikitClusterClassifier(set_kernel = set_kernel),cluster_accumulator(),
