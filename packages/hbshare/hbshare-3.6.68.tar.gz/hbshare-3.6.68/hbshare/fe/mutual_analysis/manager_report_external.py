...
class QuantitativeReport:
    def __init__(self, file_path, manager_code, fund_code=None):
        ...
        for code in self.manager_fund['FUND_CODE'].unique().tolist():
            ...
            manager_fund_scale_code = manager_fund_scale_code if len(manager_fund_scale_code) > 0 else pd.DataFrame(columns=['ZCJZ', 'BBLB1', 'JSRQ', 'JJDM', 'GGRQ', 'ROW_ID'])
            manager_fund_scale_code = manager_fund_scale_code[manager_fund_scale_code['BBLB1'] == 13]
    ...
    def get_representative_fund(self):
        ...
        if self.fund_code is not None:
            representative_fund = manager_fund[manager_fund['FUND_CODE'] == self.fund_code][['FUND_CODE', 'BEGIN_DATE', 'END_DATE']]
        else:
    ...
    def get_manager_info(self):
        ...
        manager_fund_cp = manager_fund.merge(latest_scale[['FUND_CODE', 'AUM']], on=['FUND_CODE'], how='left')
        manager_fund_cp = manager_fund_cp[['FUND_FULL_NAME', 'AUM']].groupby('FUND_FULL_NAME').sum().reset_index()
        manager_fund = manager_fund.merge(manager_fund_cp, on=['FUND_FULL_NAME'], how='left')
        manager_fund = manager_fund.sort_values(['BEGIN_DATE', 'END_DATE', 'FUND_CODE']).drop_duplicates('FUND_FULL_NAME')
        manager_fund['FUND_SHORT_NAME'] = manager_fund['FUND_SHORT_NAME'].apply(lambda x: x.replace('A', '').replace('C', ''))
        ...
        manager_fund = manager_fund[['FUND_SHORT_NAME', 'FUND_TYPE_2ND', 'BEGIN_DATE', 'END_DATE', 'REASON', 'AUM']]
        manager_fund.columns = ['基金名称', '基金类型', '任职日期', '离任日期', '离任原因', '最新规模（A、C份额合计，亿元）']
    ...
    def get_manager_style(self):
        ...
        for idx, row in representative_fund.iterrows():
            ...
            hbs_style_sc_rank_history = hbs_style_sc_rank_history.rename(columns={'jjdm': 'FUND_CODE', 'jsrq': 'REPORT_DATE', 'shift_ratio_size': 'STYLE_SHIFT_RANK', 'c_level_size': 'STYLE_CEN_RANK'})
    ...
    def get_manager_brison(self):
        ...
        sns.lineplot(ax=ax, x='REPORT_DATE', y='总超额收益', data=fund_brison_disp, label='总超额收益', color='#F04950')
    ...
    def get_quantitative_report(self):
        ...
        document.add_paragraph(text=fund_summary)
        fund_summary = '8. 选股方法：{0}选股{1}{2}，{3}。'.format(self.manager_name, manager_industry_stock_text.split('%，')[2].replace('。', '，'), manager_left.iloc[1, 4], manager_new.iloc[1, 4]) if len(self.representative_fund) == 1 else '8. 选股方法：{0}代表基金{1}的左侧特征分别为{2}，新股/次新股偏好分别为{3}。'.format(manager_industry_stock_text.split('%，')[2].replace('。', '，'), '、'.join(manager_left.iloc[1:, 0].tolist()), '、'.join(manager_left.iloc[1:, 4].tolist()), '、'.join(manager_new.iloc[1:, 4].tolist()))
        document.add_paragraph(text=fund_summary)
