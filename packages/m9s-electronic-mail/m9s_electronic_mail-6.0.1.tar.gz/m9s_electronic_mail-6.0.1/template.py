...
from trytond.sendmail import SMTPDataManager, sendmail, sendmail_transactional
...
class Template(ModelSQL, ModelView):
    ...
    reports = fields.One2Many('electronic.mail.template.ir.action.report',
        'template', 'Reports')
    ...
    def eval(cls, expression, record, engine='genshi'):
        ...
        try:
            result = engine_method(expression, record)
        except:
            raise UserError(gettext(
                    'electronic_mail.msg_wrong_expression',
    ...
    def render(cls, template, records, data, engine='genshi', render_html=True):
        ...
        for record in records:
            ...
            for report_action in template.reports:
                report = Pool().get(
                    report_action.report.report_name, type='report')
                reports.append([report.execute(
                    [record.id], {
                                'id': record.id,
                                'action_id': report_action.report.id,
                                }),
                    cls.eval(report_action.report.email_filename, record,
                            engine)])
            # The boolean for direct print in the tuple is useless for emails
            val['reports'] = [
                (r[0][0], r[0][1], r[0][3], r[1]) for r in reports]
...
class TemplateReport(sequence_ordered(), ModelSQL, ModelView):
