...
class Extractor:
    ...
    def extract_financial_index(self, cut_res):
        cut_word = [k for k, v in cut_res]
        res = utils.max_backward_match(cut_word, self.financial_dict_keys, 10)
        res = [(elem[0], self.financial_dict[elem[0]]) for elem in res]
        return res


    def extract_general_model(self, text):
        pass
