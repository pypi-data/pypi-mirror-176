    ...
    def _make_request(
        ...
        try:
            ...
            if isinstance(result_type, type):
                ...
                converter = lambda x: result_type(client=client, **x)  # type: ignore
                ...
                converter = lambda x: [result_type(client=client, **v) for v in x]  # type: ignore
            else:
                if not content_type.startswith("application/json"):
                    with response:
...
class PaginatorDataModel(DataModel, Generic[P]):
    ...
    def scan(self, flatten: bool = True, method: str = "GET") -> Iterator[P]:
        """Iterate pages."""
        ...
        while True:
            ...
            with client.request(next_page, method, None) as response:
                result = type(self)(**response.json(), client=client)

    def fetch(self, method: str = "GET") -> Sequence[P]:
        """Fetch all data."""

        return type(self.data)(self.scan(True, method=method))
