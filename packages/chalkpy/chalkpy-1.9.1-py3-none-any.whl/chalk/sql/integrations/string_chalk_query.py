...
class StringChalkQuery(StringChalkQueryProtocol):
    ...
    def _execute_internal_snowflake(self, expected_features: Sequence[Feature]):
        ...
        with snowflake.connector.connect(
            ...
            cursor = con.cursor().execute(self._original_query)
            ...
            result_columns = [c.name for c in cursor.description]
            mapping = _resolve_implicit_mappings(
                explicit_mappings=self._fields_normalized(),
                tuple_column_names=result_columns,
                expected_features=expected_features,
            )

            return DataFrame(
                from_arrow(cursor.fetch_arrow_all()).rename(
                    ...
                    {k.upper(): v.root_fqn for k, v in zip(result_columns, mapping) if v is not None}
                )
            )
    ...
    def _execute_internal_sql(self, expected_features: Sequence[Feature]):
        ...
        if self._finalizer == Finalizer.All:
            ...
            return _construct_features_df(mapping, rows)
        ...
        return _construct_features_single(mapping, row, _coerce_raw_values=True)
