...
def _normalize_for_implicit_mapping(name: str) -> str:
    """
    1. Case insensitive
...
def _resolve_implicit_col_feature_mappings(
    ...
    implicit_mappings = {_normalize_for_implicit_mapping(f.name): f for f in expected_features}
    return [
        explicit_mappings.get(col)
        if col in explicit_mappings
        else implicit_mappings.get(_normalize_for_implicit_mapping(col), None)
...
def _construct_features_single(
    ...
    if row is None:
        return features_class(**{f.attribute_name: None for f in col_feature_mappings})
