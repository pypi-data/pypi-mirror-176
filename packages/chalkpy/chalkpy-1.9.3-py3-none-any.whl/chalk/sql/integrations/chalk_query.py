...
def _resolve_implicit_mappings(
    explicit_mappings: Mapping[str, Feature],
    expected_features: Sequence[Feature],
    tuple_column_names: StrSequence,
) -> Sequence[Union[Feature, None]]:
    implicit_mappings = {f.name: f for f in expected_features}
    return [
        explicit_mappings.get(col)
        if col in explicit_mappings
        else implicit_mappings.get(col)
        if col in implicit_mappings
        else None
        for col in tuple_column_names
    ]
...
def _construct_features_single(
    column_mapping: Sequence[Union[Feature, None]], row: Optional[Row], _coerce_raw_values: bool
):
    root_ns = get_unique_item((f.root_namespace for f in column_mapping if f is not None), "root_ns")
    features_class = FeatureSetBase.registry[root_ns]

    if row is None:
        return features_class(**{f.attribute_name: None for f in column_mapping})

    assert len(row) == len(column_mapping)
...
class ChalkQuery(ChalkQueryProtocol):
    ...
    def incremental(self, lookback_period: Duration = "0s", incremental_column: Optional[Union[str, Feature]] = None):
        self._finalizer = Finalizer.All
