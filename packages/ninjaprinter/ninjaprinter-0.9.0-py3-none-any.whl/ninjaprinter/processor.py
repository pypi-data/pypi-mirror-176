...
FOLDER_PATH = os.environ.get("NP_TMP_DIR", "local/tmp/")
PAYMENT_URL = os.environ.get("NP_PAYMENT_URL")
PAGE_LIMIT = int(os.environ.get("NP_PAGE_LIMIT", "5"))
...
class Processor():
    ...
    def __init__(self, mailbox: MailBox, print_host: PrintHost):
    ...
    def process(self, *, 
        ...
        for msg_nr, msg in enumerate(messages):
            ...
            # print attachments
            process_job = self.ProcessJob(self, msg.attachments, file_extensions)
            process_job.process()
    ...
    class ProcessJob():
        ...
        def __init__(self, processor: Processor, attachments: list, file_extensions: list) -> None:
            self.processor = processor
            self.file_extensions = file_extensions
            self.print_items = []
            for attachment in attachments:
                file_path = self.create_file(attachment.payload, FOLDER_PATH, attachment.filename)
        ...
        class PrintItem():
            ...
            def __init__(self, process_job: Processor.ProcessJob, file_path: str) -> None:
                ...
                if re.search(".*\.pdf$", file_path, re.IGNORECASE):
                    ...
                    if  nr_pages > PAGE_LIMIT:
                        logger.info(f"pdf file exceed page limit of {PAGE_LIMIT}: {nr_pages} pages were reduced to {PAGE_LIMIT} pages")
