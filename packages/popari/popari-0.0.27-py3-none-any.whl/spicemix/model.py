        ...
        with torch.no_grad():
            ...
            for dataset_index, dataset  in enumerate(self.datasets):
                ...
                if not use_spatial:
                    ...
                    if (prior_x[0] != 0).all():
                        logZ_i_X +=  torch.full((N,), self.K * torch.log(prior_x[0]), **self.context)
                    ...
                    if (prior_x[0] != 0).all():
                        logZ_i_s = torch.full((N,), -self.K * torch.log(prior_x[0]) + torch.log(factorial(self.K-1, exact=True)), **self.context)
