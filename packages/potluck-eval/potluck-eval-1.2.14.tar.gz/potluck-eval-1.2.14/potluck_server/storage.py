...
def concepts_file(course, semester):
    ...
    return os.path.join(
        evaluation_directory(course, semester),
        _CONFIG.get("CONCEPTS_FILE", "concepts.json")
    )
...
def get_task_info(course, semester):
    ...
    exfmtmap = _CONFIG.get("EXERCISE_URLS", {}).get(course, {})
    if 'exercises' in result:
        for egroup in result["exercises"]:
            if 'url' not in egroup:
                for prefix in exfmtmap:
                    if egroup["group"].startswith(prefix):
                        egroup["url"] = exfmtmap[prefix].format(
                            semester=semester,
                            group=egroup["group"]
                        )

            # No prefix matches perhaps
            if 'url' not in egroup:
                egroup["url"] = '#'
...
def fetch_best_outcomes(
    ...
    for submission in reversed(fulls):  # iterate backwards chronologically
        ...
        if when <= deadline:
            ...
            submission["group_credit"] = submission.get(
                "group_credit",
                submission.get("credit", 0)
            )
        else:
            submission["on_time"] = False
            submission["group_credit"] = late_fraction * submission.get(
                "group_credit",
                submission.get("credit", 0)
            )
