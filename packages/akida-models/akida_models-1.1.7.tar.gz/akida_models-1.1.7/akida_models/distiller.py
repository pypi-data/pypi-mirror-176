...
class DeitDistiller(Distiller):
    ...
    def train_step(self, data):
        ...
        # Rename output_3_XX keys to XX in the result dictionary for a nicer display,
        # and leave this metric as the main one.
        results = {k.replace('output_3_', ''): v for k, v in super().train_step(data).items()}
        return results
    ...
    def test_step(self, data):
        ...
        # Rename output_3_XX keys to XX in the result dictionary for a nicer display,
        # and leave this metric as the main one.
        results = {k.replace('output_3_', ''): v for k, v in super().test_step(data).items()}
        return results
...
class KLDistillationLoss(KLDivergence):
    ...
    def __call__(self, y_true, y_pred, sample_weight=None):
        ...
        scale_factor = tf.constant(self.temperature ** 2)
