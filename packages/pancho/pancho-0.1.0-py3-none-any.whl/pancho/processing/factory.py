...
class ProcessorFactory(contracts.ProcessorFactory):
    def __init__(
        self,
    ...
    def get_instance(
        self,
        settings: contracts.ProcessorSettings = ProcessorSettings()
    ) -> Processor:
        return Processor(
    ...
    def _actor_repository_map(
        ...
        return self._dependency_container.get_bindings().filter(
            ...
            key_clause=lambda c, r: inspect.getfullargspec(getattr(r.instance, 'get')).annotations.get('return'),
