...
class PyMedQuery:
    ...
    def __set_extract_params(
            self, get_all: bool, get_affines: bool, format_params: Dict[str, Union[str, int, float, bool]],
        ...
        if get_all:
            ...
            sql_file_path: os.PathLike = config.SERIES_MASK_QUERY_DEFAULT if get_affines else config.SERIES_MASK_AFF_QUERY
            format_params: Union[Dict[str, str], Dict[any, any]] = {'project_id': str(project_id), 'limit': limit, 'sub': sub}
        else:
    ...
    def __pg_extract(
            ...
            verbose: bool, reset_connection: bool, post_close_connection: bool,
            batch_extract: bool
        ...
        pg_payload: Dict[Tuple[Any]] = self.crud.read(sql=sql_file_path, format_params=format_params, verbose=verbose)
    ...
    def __fetch_uids(
            self, get_all: bool, get_affines: bool, format_params: dict,
            sql_file_path: str, project_id: str, limit: str,
            include_mask: bool, verbose: bool, reset_connection: bool,
            post_close_connection: bool, batch_extract: bool
    ) -> List[str]:
        ...
        format_params, sql_file_path = self.__set_extract_params(
            get_all=get_all, get_affines=get_affines, format_params=format_params,
        ...
        all_uids, pg_payload = self.__pg_extract(
            sql_file_path=sql_file_path, format_params=format_params,
            verbose=verbose, reset_connection=reset_connection, post_close_connection=post_close_connection,
    ...
    def extract(
        ...
        try:
            ...
            all_uids, data_info = self.__fetch_uids(
                get_all=get_all, get_affines=get_affines, sql_file_path=sql_file_path,
                format_params=format_params, project_id=project_id,
                limit=limit, include_mask=include_mask, verbose=verbose,
                reset_connection=reset_connection, post_close_connection=post_close_connection,
                batch_extract=False
            )
            if not all_uids[0]:
            ...
            self.log.error(f'failed to extract data with uids {all_uids} and error: {e}')

    def batch_extract(
            self, get_all: bool, get_affines: bool = True, sql_file_path: Optional[str] = None,
            ...
            bucket_name: Optional[str] = 'multimodal-images', verbose: bool = True, image_extraction: bool = True,
            reset_connection: bool = False, post_close_connection: bool = False
    ...
    def upload(
        ...
        col_names = self.__fetch_columns(table_name=table_name, verbose=verbose, post_close_connection=post_close_connection)

        for rec in records.values():
            if len(rec) != 2:
                raise ValueError(f'I expected a dict value as tuple with lenght 2 corresponding to (record_value, blob | None), but got {rec}')
        ...
        try:
            ...
            self.log.error(f'I tried to connect to the MedQuery but failed to upload the record values {record_values} with the traceback: {e}')
