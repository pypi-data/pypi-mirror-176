...
def get_client():
    return stream.connect(
        os.environ.get("STREAMIO_KEY", ".."), os.environ.get("STREAMIO_SECRET", "..")
    )
...
class StreamObject:
        ...
        if feed_mapping is not None:
            for feed_slug, id_field in feed_mapping:
                ...
                if id is not None:
                    ...
                    if is_m2m:
                        for item in id.all():
                            to_field = f"{feed_slug}:{item.pk}"
                            to_mapping.add(to_field)
                    elif is_list:
                        for item in id:
                            to_field = f"{feed_slug}:{item}"
                            to_mapping.add(to_field)
                    else:
                        to_field = f"{feed_slug}:{id}"
                        to_mapping.add(to_field)
        ...
        if object_id_mapping is not None:
            ...
            for feed_slug, collection_name in object_id_mapping:
                for object_id in object_ids:
                    if object_id.startswith(f"{collection_name}:"):
                        if feed_slug == collection_name:
                            to_mapping.add(object_id)
                        else:
                            to_field = f'{feed_slug}:{object_id.split(":")[1]}'
                            to_mapping.add(to_field)
        return list(to_mapping)

    def get(self):
        return self.client.collections.get(self.collection_name, self.object_id)

    def get_feed(self, enrich=True):
        return self.client.feed(self.collection_name, self.object_id).get(enrich)

    def add_notification(
        self, users_to_notify, verb, message, forward, *args, **kwargs
    ):
        feed_group = get_notification_feed_group()
        object_reference = self.client.collections.create_reference(
            self.collection_name, self.object_id
        )
        activities = []
    ...
    def perform_action(
        ...
        activity_id = self.generate_foreign_key(
            verb, is_onceoff_action=is_onceoff_action, date_field=date_field
        )
        activity_data = {
            "actor": user_reference,
            "verb": verb,
        ...
        if extra_data:
            activity_data.update(extra_data)
    ...
    def enrich(self, serializer=None, force_update=False):
        ...
        try:
            return self.client.collections.add(
                self.collection_name, serialized, id=self.object_id
            )
        except exceptions.StreamApiException as e:
            if force_update:
                return self.client.collections.update(
                    self.collection_name, self.object_id, serialized
                )
            else:
                return self.get()
...
class StreamUser:
    ...
    def get_or_create(data={}):
        ...
        user = self.client.users.add(self.user_id, data, get_or_create=True)

    def perform_action(self, object, verb, time=None):
        actor_id = self.user_id
        user_reference = self.client.users.create_reference(actor_id)
        object_reference = self.client.collections.create_reference(
            object.collection, str(object.id)
        )
        activity_data = {
            "actor": user_reference,
            "verb": verb,
