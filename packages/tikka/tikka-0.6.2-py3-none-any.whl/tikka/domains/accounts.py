...
class Accounts:
    ...
    def create_new_account(
        ...
        """
        Create a root account (read-only if derivation not empty) with mnemonic, and a transparent derivation account as per RFC 0019,
        ...
        if root_account is None:
            ...
            if derivation == "":
                ...
                self.create_new_root_account(mnemonic, language_code, None, None)
            root_account = self.get_by_address(root_keypair.ss58_address)
        else:
    ...
    def create_new_root_account(
        ...
        """
        Create a read-only root account with mnemonic
    ...
    def get_available_derivation_list(self, root_account: Account) -> List[str]:
        ...
        derivation_number = 0
        member_derivation_exists = False
        for account in derived_accounts:
            ...
            if match:
                ...
                if account_derivation_number > 0 and account_derivation_number % 2 == 0:
                    derivation_number = max(
                        derivation_number, account_derivation_number
                    )
                if account_derivation_number == 0:
                    member_derivation_exists = True

        # add next available transparent derivation path
        available_derivation_list.append(f"//{derivation_number + 2}")

        if not member_derivation_exists:
            available_derivation_list.append("//0")
