...
class Sqlite3AccountsRepository(
        ...
        if filters is not None:
            for key, value in filters.items():
                if key == AccountsRepositoryInterface.COLUMN_ROOT:
                    if value is True:
                        conditions.append(sql_accounts_table.root is None)
                    elif value is False:
                        conditions.append(sql_accounts_table.root is not None)
    ...
    def table_view(
        ...
        # SELECT "e"."address", "a"."identity_index", "a"."name", "a"."address", "a"."path", "a"."root", "a"."crypto_type",
        ...
        for row in result_set:
            table_view_row = AccountsRepositoryInterface.TableViewRow(
                *row[
                    : len(
                        AccountsRepositoryInterface.TableViewRow.__dataclass_fields__  # type: ignore # pylint: disable=no-member
                    )
                ],
            )
...
def get_account_from_row(row: tuple) -> Account:
    """
    Return a Account instance from a result set row
