...
class LexicalAnalyzer():
    ...
    def get_tokens(self, string):
        ...
        while i < len(string) and j <= len(string):
            ...
            if  self.is_arithmatic_operator(temp) or self.is_symbol(temp) or self.isUnaryOperator(temp)\
                    or self.isRelationalOperator(temp) or self.isBooleanOperator(temp) or self.isComparisonOperator(temp):
                temp = temp[0: -1]
    ...
    def is_identifier(self, string):
        ...
        for i in range(0, len(string)):
            if st == 0:
                ...
                if 48 <= ord(string[i]) <= 57 or 65 <= ord(string[i]) <= 90 or 97 <= ord(string[i]) <= 122 or string[i] == '_':
                    st = 1
                else:
                    return True
        return False
