...
INVERSE_GAMMA_ALIASES = ['invgamma', 'ig', 'inversegamma', 'invg', 'inverseg', 'inv_gamma', 'ing_g', 'inverse_g', 'i_g']
...
DIST_ALIAS_LIST = [NORMAL_ALIASES, TRUNCNORM_ALIASES, HALFNORMAL_ALIASES, GAMMA_ALIASES, INVERSE_GAMMA_ALIASES,
                   UNIFORM_ALIASES, BETA_ALIASES]


class CompositeDistribution:
    def __init__(self, dist, **parameters):
        defined_params = {param: value for param, value in parameters.items() if isinstance(value, (int, float))}
    ...
    def _unpack_pdf_dict(self, point_dict):
        ...
        assert set(param_dict.keys()).union(set(self.rv_params.keys())) == set(self.rv_params.keys())

        point_dict = {param: value for param, value in point_dict.items() if param not in param_dict.keys()}
...
class BaseDistributionParser(ABC):
    ...
    def _parse_parameter(self, param_dict: Dict[str, str],
                         aliases: List[str],
                         canon_name: str,
    ...
    def _verify_distribution_parameterization(self, param_dict):
        ...
        if (n_constraints + n_params_passed) > self.n_params:
            raise DistributionOverDefinedException(self.variable_name, self.d_name, self.n_params,
                                                   n_params_passed, n_constraints)

        if self._has_std_constraint():
            if self.std_constraint <= 0:
                raise InvalidParameterException(self.variable_name, self.d_name,
                                                self.std_constraint, self.std_constraint, f'{self.std_constraint} > 0')

        if self.scale_param_name in declared_params and param_dict[self.scale_param_name] <= 0:
            raise InvalidParameterException(self.variable_name, self.d_name,
                                            self.scale_param_name, self.scale_param_name,
    ...
    def _parse_parameter_candidates(self, canon_param_name: str,
...
class NormalDistributionParser(BaseDistributionParser):
    ...
    def _postprocess_parameters(self, param_dict: Dict[str, float]) -> Dict[str, float]:
        ...
        # Handle cases with bounds
            ...
            a = param_dict[self.lower_bound_param_name] if self.lower_bound_param_name in parameters else -np.inf
            b = param_dict[self.upper_bound_param_name] if self.upper_bound_param_name in parameters else np.inf
            ...
            # Case 2: Mean constraint and scale parameter
                ...
                loc = optimize.root_scalar(match_mean,
                                           args=(mean, scale, a, b),
                                           bracket=[-100, 100],
                                           method='brenth').root
            ...
            # Case 3: Scale constraint and loc parameter
                ...
                scale = optimize.root_scalar(match_std,
                                             args=(std, loc, a, b),
                                             bracket=[1e-4, 100],
                                             method='brenth').root
...
class HalfNormalDistributionParser(BaseDistributionParser):
    ...
    def _postprocess_parameters(self, param_dict: Dict[str, float]) -> Dict[str, float]:
        if self._has_mean_constraint() and self._has_std_constraint():
            ...
            loc, scale = match_first_two_moments(target_mean=mean,
                                                 target_std=std,
                                                 dist_object=halfnorm)
...
class InverseGammaDistributionParser(BaseDistributionParser):
    ...
    def _parse_parameters(self, param_dict: Dict[str, str]) -> Dict[str, float]:
        ...
        parse_shape_parameter = partial(self._parse_parameter,
    ...
    def _postprocess_parameters(self, param_dict: Dict[str, float]) -> Dict[str, float]:
        ...
        # Case 3: Std constraint only
            ...
            if user_passed_shape:
                ...
                param_dict[self.shape_param_name] = optimize.root_scalar(solve_for_shape,
                                                                         args=(std, b),
                                                                         bracket=[2 + 1e-4, 100],
                                                                         method='brenth').root
        return param_dict
...
class BetaDistributionParser(BaseDistributionParser):
    ...
    def _parse_parameters(self, param_dict: Dict[str, str]) -> Dict[str, float]:
        ...
        parse_scale_parameter = partial(self._parse_parameter,
                                        canon_name=self.scale_param_name,
                                        aliases=[self.scale_param_name])
        parse_shape_parameter_1 = partial(self._parse_parameter,
                                          canon_name=self.shape_param_name_1,
                                          aliases=BETA_SHAPE_ALIASES_1)
        parse_shape_parameter_2 = partial(self._parse_parameter,
    ...
    def _postprocess_parameters(self, param_dict: Dict[str, float]) -> Dict[str, float]:
        ...
        if self._has_mean_constraint() and self._has_std_constraint():
            mean, std = self.mean_constraint, self.std_constraint
            if (mean > 1) or (mean < 0):
                raise InvalidParameterException(self.variable_name, self.d_name, 'mean', 'mean', '0 < mean < 1')
            ...
            if ((1 - mean) ** 2 * mean) < (std ** 2):
                used_name = list(set(used_parameters).intersection(set(STD_ALIASES)))[0]
                raise InvalidParameterException(self.variable_name, self.d_name, 'mean, std', f'mean, {used_name}',
                                                '((1 - mean) ** 2 * mean) < (std ** 2)')

            x = (1 - mean) / mean
            param_dict[self.shape_param_name_1] = alpha = x / (std ** 2 * (x + 1) ** 3) - (1 / (x + 1))
        ...
        # Case 4: Std constraints and one shape parameter
            ...
            if user_passed_alpha:
                ...
                a = param_dict[self.shape_param_name_1]
                b = optimize.root_scalar(solve_for_beta,
                                         args=(a, std),
                                         bracket=[1.001, 100],
                                         method='brenth').root
                ...
                b = param_dict[self.shape_param_name_2]
                a = optimize.root_scalar(solve_for_alpha,
                                         args=(b, std),
                                         bracket=[1.001, 100],
                                         method='brenth').root
...
class GammaDistributionParser(BaseDistributionParser):
    ...
    def _postprocess_parameters(self, param_dict: Dict[str, float]) -> Dict[str, float]:
        ...
        if self._has_mean_constraint() and self._has_std_constraint():
            mean, std = self.mean_constraint, self.std_constraint
            if mean < 0:
                raise InvalidParameterException(self.variable_name, self.d_name, 'mean', 'mean', 'mean >= 0')
            if std <= 0:
                raise InvalidParameterException(self.variable_name, self.d_name, 'std', 'std', 'std >= 0')
...
def match_first_two_moments(target_mean: float, target_std: float, dist_object: rv_continuous) -> Tuple[float, float]:
    def moment_errors(x, target_mean: float, target_std: float, dist_object: rv_continuous) -> float:
        loc_approx, scale_approx = x
...
def preprocess_prior_dict(raw_prior_dict: Dict[str, str]) -> Tuple[List[str], List[str], List[Dict[str, str]]]:
    """
...
def rename_dict_keys_with_value_transform(d: Dict,
                                          to_rename: List[str],
                                          new_key: str,
                                          variable_name: str,
                                          d_name: str,
                                          transformation: Callable = lambda name, value: value) -> Dict[str, Any]:
    result = {}
...
def split_out_composite_distributions(variable_names: List[str],
                                      d_names: List[str],
                                      param_dicts: List[Dict[str, str]]) -> Tuple[Dict[str, Tuple[str, Dict[str, str]]],
                                                                                  Dict[
...
def composite_distribution_factory(variable_name,
    ...
    if d_name == 'normal':
        ...
        param_dict = rename_dict_keys_with_value_transform(param_dict,
                                                           NORMAL_LOC_ALIASES + MEAN_ALIASES,
                                                           'loc',
                                                           variable_name,
                                                           d_name)
        param_dict = rename_dict_keys_with_value_transform(param_dict,
