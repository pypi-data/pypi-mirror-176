...
class WebSocket(object):
    def __init__(self: any, auth: str) -> None:
        #self.auth: str = auth
        self.data: dict = {'api_version': '4', 'auth': auth, 'data_enc': '', 'method': 'handShake'}
        self.enc = Encryption(auth)
        self.__WSaddress: list = list(loads(urlopen('https://shgetdcmess.iranlms.ir/').read().decode('utf-8')).get('data').get('socket').values())

    async def handShake(self, urI: str) -> dict:
    ...
    async def handler(self, chat_updates=False, message_updates=True, show_notifications=False) -> dict:
