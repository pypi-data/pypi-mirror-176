...
from bluearchive.models.student import Student, StudentDetails
from bluearchive.query import BuruakaQuery
...
class BuruakaClient:
    ...
    async def single_chara(self, chara: str) -> StudentDetails:
        async with self._session.get(url=f"/bluearchive/character/{chara}") as resp:
    ...
    async def all_chara(self) -> list[Student]:
        async with self._session.get(url="/bluearchive/character") as resp:
            json = await resp.json()
            pprint(json)
