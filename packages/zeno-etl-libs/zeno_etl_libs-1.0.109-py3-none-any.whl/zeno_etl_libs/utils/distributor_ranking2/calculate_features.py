...
def calculate_features(df_features, reset_date, time_interval, group_cols):
    """
    ...
    print('finished fulfillment calculations')

    # ====================== DC-DIST VOLUME CALCULATION =======================
    ...
    df_vol["ff_requests"] = np.where(df_vol["ff_requests"] < 0, 0, df_vol["ff_requests"])
    ...
    df_vol.drop(["total_requests_dc_dist", "total_requests_dc"], axis=1, inplace=True)

    print('finished total requests calculations')
    ...
    features["request_volume_dc_dist"] = np.round(features["request_volume_dc_dist"], 3)
