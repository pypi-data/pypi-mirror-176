...
class SampleView(object):
    ...
    :class:`~.SampleView` instance are iterable.
    ...
    def __getitem__(self, index):
        ...
        else: raise IndexError('SampleView instances contain only three items')
...
class SequenceView(object):
    ...
    def find(self, motif, start=0, stop=None):
        ...
        :param start: position at which to start searching. The method
            will never return a value smaller than :fparam:`start`. By default,
        ...
        :param stop: position at which to stop search (the motif
            ...
            larger than ` stop - len(motif)``. By default, or if :fparam:`stop`
            is equal to
...
class Base(object):
    ...
    def create(cls, source, alphabet=None):
        ...
        The object :fparam:`source` can be:
        ...
        * a list containing acceptable items (as described in :meth:`~.Align.add_samples`).
        ...
        (but not that in the case of a :class:`.Container` to :class:`.Align`
    ...
    def fasta(self, fname=None, first=None, last=None,
        ...
        may use the :fparam:`alphabet` argument to pass a valid
        ...
        :param fname: name of the file to export data to. By default, the
            ...
            option :fparam:`append` is ``True``, data is appended at the end of the
            file (and it must exist). If :fparam:`fname` is ``None`` (default), no
        ...
        :param last: if only part of the sequences should be exported: index
            ...
            last (this is the default). If :fparam:`last` < :fparam:`first`, no sequences are
            exported.

        :param alphabet: exporting alphabet to use instead of the instance's alphabet. The provided
            ...
            alphabet based on their rank in theire respective alphabets.
        ...
        :return: If :fparam:`fname` is ``None``: a fasta-formatted string.
            Otherwise: ``None``.
        """
    ...
    def add_samples(self, items):
        ...
        :param items: must be a list, an :class:`.Align` instance, or a
            :class:`.Container` instance. If a list is passed, each item must be
            of length 2 or 3 and contain the sample name
            string, the list of data values and, if provided,
            the list of labels. See the method
    ...
    def find(self, name, regex=False, flags=None, multi=False, index=False):
        ...
        :param regex: a boolean indicating whether the value passed as
            :fparam:`name` is a regular expression. If so, the string is passed
        ...
        :param flags: list of flags to be passed to :py:func:`re.search`
            (only considered if :fparam:`regex` is ``True``). For example, when looking
        ...
        :return: The type of the returned value depends on the argument
            :fparam:`multi` and on whether any hits were found:

            =============== =============== ======================================
            :fparam:`multi` :fparam:`index` return value
            =============== =============== ======================================
    ...
    def find_motif(self, sample, motif, start=0, stop=None):
        ...
        :param stop: position at which to stop search (the motif
            cannot overlap this position). No returned value will be
            larger than ` stop - len(motif)``. By default, or if :fparam:`stop` is equal to
            or larger than the length of the sequence, search until the
            end of the sequence.
        """
    ...
    def encode(self, nbits=10):
        ...
        using :meth:`~.Base.rename`. This method is not affected by
        the presence of sequences with identical names in the original
        instance (and :meth:`~.Base.rename` will also work properly
        in that case).
        """
    ...
    def rename(self, mapping, liberal=False):
        ...
        :param liberal: if this argument is ``False`` and a name does not
            appear in :fparam:`mapping`, a :exc:`ValueError` is
            raised. If :fparam:`liberal` is ``True``, names that don't appear in
            :fparam:`mapping` are left unchanged.
    ...
    def subset(self, samples):
        ...
        if isinstance(samples, Structure):
            ...
            if samples.req_no > self._ns: raise ValueError('invalid structure')
...
class Align(Base):
    ...
    :param alphabet: a :class:`~.Alphabet` instance. There must be at
    ...
    def insert_columns(self, position, values):
        ...
        :param position: the position at which to insert sites. Sites
            ...
            length of the alignment, or ``None``. If :fparam:`position` is
    ...
    def extract(self, *args):
        ...
        :param stop: stop position for the range to extract. *This
            ...
            smaller than :fparam:`start`, empty sequences are generated. If this
    ...
    def fix_ends(self, replaced='-', replacing='?'):
        ...
        specified by :fparam:`replaced` by the one specified by :fparam:`replacing`.
    ...
    def filter(self, ratio, relative=False):
        ...
        :param ratio: limit threshold, expressed as a proportion of
            ...
            (if the :fparam:`relative` argument is ``True``).
    ...
    def phyml(self):
        ...
        The alphabet must be of type character (or :py:obj:`.alphabets.DNA`).
        """
    ...
    def phylip(self, format='I'):
        """
        Return a phyml-formatted string representing the content of the
        instance. Raise a :exc:`ValueError` if any
...
class Container(Base):
    ...
    :param alphabet: a :class:`~.Alphabet` instance.
    """
    ...
    def insert_sites(self, sample, position, values):
        ...
        :param position: the position at which to insert sites. Sites
            ...
            can use 0 to insert sites at the beginning of the sequence.
            To insert sites at the end of the sequence, pass the current
            length of the sequence. If :fparam:`position` is larger than the
...
class encode(object):
    ...
    This is a context manager that can be used to perform some code using
    a :class:`.Align` or :class:`.Container` instance
    with temporarily renamed samples. The names are encoded (using the
    :meth:`~.Align.encode` method) before executing user code and then
    decoded (using :meth:`~.Align.rename`) as the end of the block (even
    if an exception occurs). Example::

        with egglib.encode(obj) as mapping:
            name = obj.get_name(0)
        print(obj.get_name(0))
        print(mapping[name])
...
def struct_from_labels(data, lvl_clust=None, lvl_pop=None, lvl_indiv=None,
    ...
    Create a new :class:`.Structure` instance based on the labels
    ...
    :param lvl_indiv: index of individual labels. If ``None``, the
        ...
        :fparam:`lvl_indiv` is ``None``, any additional label of outgroup individuals
    ...
    :param ploidy: indicate the ploidy. Must be a positive number and,
        ...
        ignored if :fparam:`lvl_indiv` is ``None``.
...
def struct_from_dict(ingroup, outgroup):
    ...
    Create a new :class:`.Structure` instance based on the
    ...
    :param outgroup: a dictionary of outgroup samples indices, or
...
class Structure(object):
    ...
    def as_dict(self):
        ...
        Return a tuple of two :class:`dict` instances representing,
        ...
        ``d[clt][pop][idv]`` will yield a list of sample indices.
    ...
    def make_auxiliary(self):
        ...
        that have been filtered using the original structure and with the
    ...
    def subset(self, *pops, outgroup=True):
        """ subset(pop1[, pop2[, ...], outgroup=True)
        ...
        *pop1*, *pop2*, etc. are a list of at least one valid population
        labels. The order and possible replicates are not significant.
        *outgroup* specifies if the outgroup must be included.

        .. versionadded:: 3.2
        """
        if len(pops) == 0: raise ValueError('there must be at least one population')
        ret = Structure()
        res = ret._obj.subset(self._obj, '\n'.join(pops) + '\n', outgroup)
    ...
    def make_sorted_auxiliary(self):
        ...
        the original structure but where the individual level has not
        been collapsed).
        """
        dest = Structure()
        cur = 0
    ...
    def req_ns(self):
        """
        Required number of ingroup samples. Equal to the largest index
        overall, plus one.
        """
        return self._obj.get_ni_req()

    @property
    def req_no(self):
        """
        Required number of ougroup samples. Equal to the largest index
        overall, plus one.
        """
    ...
    def shuffle(self, mode='it', nr=None, rnd=None):
        ...
        structuration. It is not allowed modify the structure object
        ...
        :return: A context manager, which itself is iterable if :fparam:`nr`
            is not ``None``.
        """
