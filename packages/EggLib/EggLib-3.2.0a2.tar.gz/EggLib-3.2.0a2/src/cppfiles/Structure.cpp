...
namespace egglib {
    ...
    const char * StructureHolder::subset(const StructureHolder& src, char * popstring, bool outgroup) {
        bool flags[src._num_pop];
        memset(flags, 0, src._num_pop);
        char * str = strtok(popstring, "\n");
    ...
    void StructureHolder::add_sample_ingroup(unsigned int sam_idx, StructureIndiv * indiv) {
        _ni++;
        if (sam_idx+1 > _required_ni) _required_ni = sam_idx + 1;
    ...
    unsigned int StructureHolder::get_ni_req() const {
        return _required_ni;
    }
