...
class ComputeStats_test(unittest.TestCase):
    ...
    def test_process_align_E(self):
        CS = egglib.stats.ComputeStats()
        aln = egglib.io.from_fasta(os.path.join(path_F, 'c_file_1.fsa'), labels=True, alphabet=egglib.alphabets.DNA)
        CS.add_stats('Atot', 'Aing', 'R', 'He')
        with self.assertRaises(ValueError):
            pos=list(range(0, 100))
            CS.process_align(aln, positions=pos)

        aln_e= egglib.io.from_fasta(os.path.join(path_T, 'dmi3.fas'), labels=True, alphabet=egglib.alphabets.DNA)
    ...
    def test_alphabet_V(self):
        ...
        with self.assertRaisesRegexp(ValueError, 'cannot compute V, Ar, M, and Rst with this alphabet'):
...
class Statistics_test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        f = my_file_reader(os.path.join(path_stats, 'stats.txt'))
        self.files = []
        while True:
            line = f.readline()
    ...
    def _run_test(self, report_mode):
        cnt = {'OK': 0, '??': 0, 'FAIL': 0}

        for filedata in self.files:
            if filedata['type'] == 'LD':
                statkeys = 'd', 'D', 'Dp', 'r', 'rsq'
                aln = egglib.io.from_fasta(os.path.join(path_stats, filedata['fname']), labels=True, alphabet=egglib.alphabets.DNA)
                ...
                if report_mode:
                    print('-----------------------------------------------------------------------------------------------------')
                    print('{0[fname]:<18} {0[type]:<10}'.format(filedata))
                    print('-----------------------------------------------------------------------------------------------------')
                ...
                if report_mode:
                    print('-----------------------------------------------------------------------------------------------------')
                    print('{0[fname]:<18} {0[type]:<10} missing:{0[max_missing]} stop:{0[consider_stop]} mhits:{0[multiple_alleles]} mall:{0[multiple_hits]}'.format(filedata))
                    print('-----------------------------------------------------------------------------------------------------')
                ...
                f = open(os.path.join(path_stats, filedata['fname'] + '.hap'))
                for line in f:
                ...
                info = []
                f = open(os.path.join(path_stats, filedata['fname'] + '.inp'))
                for line in f:
                ...
                if report_mode:
                    print('-----------------------------------------------------------------------------------------------------')
                    print('{0[fname]:<18} {0[type]:<10} chr:{0[chr]} core:{0[core]} phased:{0[phased]} binary:{0[EHH_binary]} thr:{0[thr]}'.format(filedata))
                    print('-----------------------------------------------------------------------------------------------------')
                ...
                if report_mode:
                    print('-----------------------------------------------------------------------------------------------------')
                    print('{0[fname]:<18} {0[type]:<10} hapl: {0[haplotypes]:<2} max_missing: {0[max_missing]} multiple: {0[multiple]}'.format(filedata))
                    print('-----------------------------------------------------------------------------------------------------')
                ...
                if filedata['type'] == 'Align':
                    ...
                    if filedata['haplotypes']:
                        ...
                        cs.configure(multi_hits=filedata['multiple'], struct=struct.make_auxiliary())
                        stats = cs.process_site(hapls)
                    else:
                        cs.configure(multi_hits=filedata['multiple'], struct=struct)
                    ...
                    f = open(os.path.join(path_stats, filedata['fname']))
                    sites = []
                    pl = set()
                    for line in f:
                        hier = None
                    ...
                    if not report_mode:
                        if v is None: self.assertIsNone(stats_dict[k], msg=msg)
                            ...
                            if isinstance(v, int): self.assertEqual(stats_dict[k], v, msg=msg)
                                ...
                                for x, y in zip(stats_dict[k], v):
                                    self.assertAlmostEqual(x, y, msg=msg, places=pl)
                            else:
                        ...
                        print('    {0:<20} ctlr: {1:<25} egglib: {3:<25} --> {2:>5}'.format(k, rep1, msg, rep2))
                        if flag and 'OK' not in msg:
                            print('    ->', v, len(v))
                            print('    ->', stats_dict[k], len(stats_dict[k]))
                            print(' DIFF:', ' '.join(['{0}!={1}'.format(i, j) for i,j in zip(v, stats_dict[k]) if i!=j]))
                        cnt[msg.lstrip('~')] += 1
