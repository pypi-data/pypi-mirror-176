...
REND_TIME_TYPE_SINGLE = 1
REND_TIME_TYPE_ACTIVE = 2
REND_TIME_TYPE_RANGE = 3
REND_TIME_TYPE_FRAMES = 4
...
def scene_frame_spec():
    ...
    If the render time type is set to single frame, return a single frame spec, otherwise return a
    simple range spec.
    """
    ...
    if rend_time_type == REND_TIME_TYPE_SINGLE:
        frame =  int(rt.sliderTime)
        return "{:d}".format(frame)
    else:
        frame_range = rt.animationRange
        return "{:d}-{:d}".format(
            int(frame_range.start),
            int(frame_range.end)
        )
