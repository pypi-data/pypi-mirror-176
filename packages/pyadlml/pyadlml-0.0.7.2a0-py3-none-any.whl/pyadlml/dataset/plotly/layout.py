...
def acts_n_devs_layout(df_acts, df_devs, start_time, end_time, set_end_time, plot_height=350):
    ...
    if df_acts is None or df_devs is None:
        ...
        time_slider = _build_range_slider(df_acts, df_devs, start_time, end_time, set_end_time)
...
def activities_layout(df_acts, act_id=0, plot_height=350):
    ...
    layout_activities = dbc.Container([
        ...
        dbc.Row([
            dbc.Col(width=6,
                    children=[
                        dbc.Row(dcc.Graph(id=f'graph-bar-{act_id}',
                                          style=dict(height='100%',width='100%'),
                                          figure=fig_bar_count,
                                          config=dict(displaylogo=False,
                                                displayModeBar=True,
            ...
            html.Div(children='', id=f"dev-curr-sel-{act_id}", style=dict(display="none")),
            dcc.Store(id=f'act-curr-sel-store-{act_id}'),
            dcc.Store(id=f'dev-curr-sel-store-{act_id}'),
            ...
            dbc.Col(width=6,
                    children=[
                        dbc.Row(dcc.Graph(id=f'graph-boxplot-{act_id}',
                                  style=dict(height='100%',width='100%'),
                                  figure=fig_duration,
                                  config=dict(displayModeBar=True,
                                              modeBarButtonsToRemove=_buttons_to_use(
        ...
        dbc.Row([
            dbc.Col(width=6,
                    children=[
                        dbc.Row(dcc.Graph(id=f'graph-density-{act_id}',
                                  style=dict(height='100%',width='100%'),
                                  figure=fig_density,
                                  config=dict(
                                        modeBarButtonsToRemove=_buttons_to_use(
            ...
            dbc.Col(width=6,
                    children=[
                        dbc.Row(dcc.Graph(id=f'graph-transition-{act_id}',
                                  style=dict(height='100%',width='100%'),
