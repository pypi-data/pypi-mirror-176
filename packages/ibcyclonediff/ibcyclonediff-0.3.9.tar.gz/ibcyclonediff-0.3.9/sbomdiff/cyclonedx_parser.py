    ...
    def parse_cyclonedx_json(self, sbom_file):
        ...
        if "components" in data:
            for c in data["components"]:
                self.__find_licenses_in_components(c, packages)



        return packages

    def __find_licenses_in_components(self, c, packages):
        if c["type"] in ["library", "application", "operating-system"]:
            package = c["name"]
            version = c["version"]
            license = "NOT FOUND"

            # First check if there's an evidence tag enclosing licenses
            if "evidence" in c:
                e = c["evidence"]
                # Multiple ways of defining license data
                if "licenses" in e and len(e["licenses"]) > 0:
                    license_data = e["licenses"][0]
            ...
            # Also check if there are licenses directly under components
            if "licenses" in c and len(c["licenses"]) > 0:
                license_data = c["licenses"][0]
                if "license" in license_data:
                    if "id" in license_data["license"]:
                        license = license_data["license"]["id"]
                    elif "name" in license_data["license"]:
        ...
        # if current components has nested components tag then call recursively
        if "components" in c and len(c["components"]) > 0:
