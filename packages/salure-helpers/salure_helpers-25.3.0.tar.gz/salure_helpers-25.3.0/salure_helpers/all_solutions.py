...
class AllSolutions:
    ...
    def update_person(self, data: dict) -> json:
        ...
        allowed_fields = {
            ...
            "mobile_phone_work": "ma01.mobiel",
            "email_work": "ma01.email",
            "note": "ma01.notitie-edit"
        }
        ...
        if self.debug:
            print(json.dumps(payload))
        response = requests.put(url=f"{self.url}mrlprs/{data['employee_id']}",
                                headers=self._get_headers(),
                                data=json.dumps(payload))
        if self.debug:
            print(response.content)
