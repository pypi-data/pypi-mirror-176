...
class SalureConnect:
    ...
    def get_system_credential(self, system: str, reference: str = None, test_environment: bool = False) -> json:
        """
        ...
        if system == 'profit':
            ...
            credentials = [credential for credential in credentials if ('reference' in credential.keys() and credential['reference'] == reference) and ('testEnvironment' in credential.keys() and credential['testEnvironment'] is test_environment)]
    ...
    def refresh_system_credential(self, system: str, system_id: int) -> json:
        ...
        :return json response from salureconnect
        """
        response = requests.get(url=f'{self.url}connector/{system}/{system_id}/refresh', headers=self.__get_headers())
        response.raise_for_status()
        credentials = response.json()
