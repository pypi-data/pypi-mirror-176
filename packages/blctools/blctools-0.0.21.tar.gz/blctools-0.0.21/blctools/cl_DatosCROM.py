...
class DatosCROM(TablasVC):
    ...
    def __init__(
        ...
        if reporte_consolidado or consolidar_todo:
        ...
        if cargar_incidencias and explotar_incidencias:
            if isinstance(explotar_incidencias,str):
                ...
                if explotar_incidencias:
                    self.explotar_incidencias(granularidad = '1dia')
            else:
    ...
    def __iec61400_colapsar_nuevamente_a_incidencias(self,mensajes=False):
        ...
        if not lista_dfs:
            ...
            self.__rpt_iec61400_minutal = df
            self.__rpt_iec61400_incidencias = self.__super_deep_copy(df_short) # Asegurar una copia profunda, recursiva de todos los elementos, antes de eliminar el DataFrame minutal
    ...
    def __iec61400_ajustes_finales(self,mensajes=False,guardar_minutal=False):
        ...
        if df_long is None or df_short is None:
            self.__rpt_iec61400_incidencias = pd.DataFrame()
            
            if guardar_minutal:
                self.__rpt_iec61400_minutal = pd.DataFrame()
    ...
    def explotar_incidencias(self,
        ...
        if df.empty:
            self.__incidencias_explotadas = pd.DataFrame()
            return None
    ...
    def exportar(self,reportes='todos',ruta=None,nombre=None):
        ...
        tiene_datos_inc_aut = not(self.incidencias_autodetectadas is None)
