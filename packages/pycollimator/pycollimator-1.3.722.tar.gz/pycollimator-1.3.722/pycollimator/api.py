...
class Api:
    ...
    @classmethod
    def simulation_update(cls, model_uuid: str, simulation_uuid: str, body: dict) -> dict:
        api = f"{Api.BASE_PATH}/models/{model_uuid}/simulations/{simulation_uuid}"
        return cls.put(api, body)
    ...
    @classmethod
    def upload_file(cls, filepath: str, project_uuid: str = None, overwrite: bool = False) -> str:
        project_uuid = project_uuid or GlobalVariables.project_uuid()
        ...
        with open(filepath, "rb") as f:
            response = requests.put(s3_url, data=f, headers={"Content-Type": content_type, "Content-Length": str(size)})

        if response.status_code != 200:
            Log.debug("API error, status_code:", response.status_code, "response:", response.json)
            raise CollimatorApiError(f"Failed to upload to S3. Detailed error:\n{response.text}")

        return cls._call_api(f"{Api.BASE_PATH}/projects/{project_uuid}/files/{file_uuid}/process", method="POST")
