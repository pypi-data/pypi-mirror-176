...
from .protection_utils import dp_equivalent, dp_transform, pep_transform
...
async def pd_shape_dp(dataframe: Any, budget: Any, seed: int) -> Any:
    """Dummy implementation of DP shape."""
    n_rows, n_cols = dataframe.shape
    # TODO properly call DP primitives
    epsilon = budget["epsilons"][0]
    global_sensitivity = 1.0
    if epsilon == 0.0:
        raise ValueError("`epsilon` should be greater than 0.")
    np.random.seed(seed)
    noise = np.random.laplace(loc=0.0, scale=global_sensitivity / epsilon)
    n_rows = math.floor(n_rows + noise + 0.5)
