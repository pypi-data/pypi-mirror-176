...
def Integer(
    ...
    possible_values: t.Optional[int] = None,
...
def Float(
    ...
    possible_values: t.Optional[float] = None,
...
def Datetime(
    ...
    possible_values: t.Optional[str] = None,
...
def Date(
    ...
    max: t.Optional[str] = None,
    possible_values: t.Optional[str] = None,
    base: t.Optional[sdt.DateBase] = None,
    properties: t.Optional[t.Mapping[str, str]] = None,
) -> Type:
    ...
    return Type(
        sp.Type(
            name='Date',
            date=sp.Type.Date(
                format=format, min=min, max=max, base=base.value
            ),
            properties=properties,
        )
    )
...
def Time(
    ...
    max: t.Optional[str] = None,
    possible_values: t.Optional[str] = None,
    base: t.Optional[sdt.TimeBase] = None,
    properties: t.Optional[t.Mapping[str, str]] = None,
) -> Type:
    ...
    if format is None:
        format = '%H:%M:%S.%f'
    if base is None:
        base = sdt.TimeBase.INT64_NS
    if min is None:
        if base == sdt.TimeBase.INT64_NS:
            min = '00:00:00.000000000'
        else:
            min = '00:00:00.000000'
    if max is None:
        if base == sdt.TimeBase.INT64_NS:
...
def Duration(
    ...
    return Type(
        sp.Type(
            name='Duration',
            duration=sp.Type.Duration(unit=unit, min=bounds[0], max=bounds[1]),
            properties=properties,
        )
    )
