...
def emit_context_activations(contexts):
    ...
    if module_is_global:
        ...
        while executable.find("_") != -1:
            ...
            emit(2, "if not window: window = natlinkutils.matchWindow(moduleInfo,'" + \
                    executable + "','')\n")
        emit(2, "if not window: return\n")

    emit(2, "# Return if same window and title as before\n")
    emit(2, "if moduleInfo == self.currentModule: return\n")
    ...
    for context in contexts:
        ...
        if targets[0] == "":
            ...
            tests = " or ".join(["title.find('" + target + "') >= 0" for target in targets])
            emit(2, "status = (" + tests + ")\n")
...
def emit_top_command_actions(command):
    ...
    emit(1, "# ")
    OUT.append(unparse_terms (0, terms))
    emit(0, "\n")
...
def inline_a_term(unnamed):
    ...
    index = 0
    while (index < len(terms)) and (terms[index]["OPTIONAL"] or terms[index]["TYPE"] == "dictation"): index += 1
...
def emit_file_trailer():
    OUT.append('''thisGrammar = ThisGrammar()
