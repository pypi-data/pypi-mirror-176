...
class Jejudos(commands.Cog):
    ...
    @commands.group("jejudo",aliases=["jeju","jdo"])
    @commands.is_owner()
    ...
    async def jejudo_shell(self, ctx: ContextA, *, argument: codeblock_converter):
        """
        Run a shell command
        """
        if not argument:
            await ctx.send("Usage: `jejudo shell <command> <args>`")
            return
