...
class Jejudos(commands.Cog):
    ...
    @commands.group("jejudo",aliases=["jeju","jds"],invoke_without_command=True)
    @commands.is_owner()
    ...
    async def tag_tag(self, ctx, tag):
        try:
            ...
            await ctx.send(f"{dbb[str(tag)]['value']}")
        except:
            print(traceback.format_exc())
    ...
    async def tag_add(self, ctx, tag_name, *, tag_add):
        try:
            ...
            #     await ctx.send(f"`❌` Sorry.. This tag is already fixed.")
            ...
            await ctx.send(f"`✅` Successfully created tag\n> **Tag_Name** : {tag_name}\n> **Tag_Value** : {tag_add}")
        except:
            print(traceback.format_exc())
