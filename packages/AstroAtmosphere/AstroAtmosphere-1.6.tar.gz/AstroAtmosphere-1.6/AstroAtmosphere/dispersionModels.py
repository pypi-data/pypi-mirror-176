...
class dispersion(Observatory):
	...
	def independentCassiniError(self, n1, n2, zenith, dn1=0, dn2=0, dz=0, 
									lat=None, h=None):
		'''
			Calculates the uncertainty in the atmospheric dispersion, 
			following the Cassini model, without assumptions about the
			refractivity model.
			This involves a much shorter use the chain rule. 
			...
			----------
			n1 	: 	float
				Refractive index at the first wavelength of interest at the 
				observer
			n2 	: 	float
				Refractive index at the second wavelength of interest at the 
				observer
			zenith : float
				Angle of observation in degrees.
				zenith = 0 means the observation is directly overhead.
			dn1 : float
				Uncertainty in the provided value of the refractive index
		...
		# Some precalculation and variable setting
		_zenith = np.deg2rad(zenith)
		_dz   	= np.deg2rad(dz)
		_R 		= self.rc
		_H 		= self.H

		# Partial derivatives for derivation to n1, n2, and z
		_dddn1  = - _R * np.sin(_zenith) / ( (_R + _H) * np.sqrt(1 - (n1 * _R * np.sin(_zenith) / (_R + _H))**2 ) )
		_dddn2  =   _R * np.sin(_zenith) / ( (_R + _H) * np.sqrt(1 - (n2 * _R * np.sin(_zenith) / (_R + _H))**2 ) )
		_dddz 	= (_R * np.cos(_zenith)) / (_R + _H) * ( n1 / np.sqrt(1-(n1**2 * _R**2 * np.sin(_zenith)**2)/(_R + _H)**2) - n2 / np.sqrt(1-(n2**2 * _R**2 * np.sin(_zenith)**2)/(_R + _H)**2) )		
		
		# Combining the partial derivatives
		_dispersionError = np.sqrt( (_dddn1 * dn1)**2 + (_dddn2 * dn2)**2 + (_dddz * dz)**2)
