...
def dictionary_hash(my_file, my_hash, mask = False, minimum = 1, maximum = 10):
    clear()
...
def log_scanner(verify, file = "", folder = ""):
    verify_list = []

    clear()
    
    if folder != "" and file == "":
        list_logs = os.listdir(folder)
        for i in list_logs:
            print("checking: " + i)
            
            with open(folder + "/" + i, "r", errors = "ignore") as my_file:
                for ii in my_file:
                    result = re.search(verify, ii.lower())

                    if result:
                        my_result = re.findall("\S+" + verify, ii.lower())

                        for pwn in my_result:
                            verify_list.append(folder + "/" + i + ": " + pwn)
            ...
            with open(file, "r", errors = "ignore") as my_file:
                for ii in my_file:
                    ...
                    if result:
                        ...
                        for pwn in my_result:
                            verify_list.append(folder + "/" + file + ": " + pwn)

    verify_list = list(dict.fromkeys(verify_list))

    clear()

    print("hits: " + str(len(verify_list)))
...
def security_scanner(url, secure = True, bot = False):
    clear()
    my_port_scanner = port_scanner(url, bot)
    my_subdomain_takeover = subdomain_takeover(url, secure, bot)
    my_sql_injection_scanner = sql_injection_scanner(url, secure, bot)
    my_xss_scanner = xss_scanner(url, secure, bot)

    clear()

    print("open ports:")

    for i in my_port_scanner:
        print("port: " + str(i))
...
def subdomain_scanner(url, secure = True, bot = False):
    ...
    website = link_scanner(url, secure, bot)
...
def subdomain_takeover(url, secure = True, bot = False):
    result = subdomain_scanner(url, secure, bot)
