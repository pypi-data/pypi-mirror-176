    ...
    def generar_cfdi(self, configuracion, timbrado_prueba=None, pac=None):
        ...
        for rp in self.pagos:
            ...
            for documento in rp["documentos"]:
                ...
                tipo_cambio_dr = None
                equivalencia_dr = 1
                if documento['moneda'] != rp["MonedaP"] and cfdi.Version != "4.0":
        ...
        for imp_p in rp.get("impuestos_p", []):
            if str(imp_p['tasa0_cuota_p']) == "0.000000":
                iva_p_0 = "0.00"
                base_iva_0 += imp_p['base_p']
                impuesto_iva_0 += imp_p['importe_p']
            
            if str(imp_p['tasa0_cuota_p']) == "0.080000":
                iva_p_8 = "0.080000"
                base_iva_8 += imp_p['base_p']
                impuesto_iva_8 += imp_p['importe_p']
                
            if str(imp_p['tasa0_cuota_p']) == "0.160000":
                iva_p_16 = "0.160000"
                base_iva_16 += imp_p['base_p']
                impuesto_iva_16 += imp_p['importe_p']
                
            if str(imp_p['tasa0_cuota_p']) == "excento":
                iva_p_excento = "excento"
                base_iva_excento += imp_p['base_p']
                impuesto_iva_excento += imp_p['importe_p']
        ...
        for imp_p in rp.get("impuestos_p", []):
            if not imp_p["tasa0_cuota_p"] in IMPUESTOS_LIST:
                ...
                if str(imp_p["impuesto_p"]) == "002":
                    if str(imp_p['tasa0_cuota_p']) == "0.000000":
                        base = base_iva_0
                        impuesto = impuesto_iva_0
                    
                    if str(imp_p['tasa0_cuota_p']) == "0.080000":
                        base = base_iva_8
                        impuesto = impuesto_iva_8
                    
                    if str(imp_p['tasa0_cuota_p']) == "0.160000":
                        base = base_iva_16
                        impuesto = impuesto_iva_16
                           
                    if str(imp_p['tasa0_cuota_p']) == "excento":
                        base = base_iva_excento
                        impuesto = impuesto_iva_excento
        ...
        if iva_p_16:
            cp.TotalTrasladosBaseIVA16 = "%.2f" % (base_iva_16 * tc)
            cp.TotalTrasladosImpuestoIVA16 = "%.2f" % (impuesto_iva_16 * tc)

        if iva_p_8:
            cp.TotalTrasladosBaseIVA8 = "%.2f" % (base_iva_8* tc)
            cp.TotalTrasladosImpuestoIVA8 = "%.2f" % (impuesto_iva_8 * tc)
        
        if iva_p_0:
            cp.TotalTrasladosBaseIVA0 = "%.2f" % (base_iva_0 * tc)
            cp.TotalTrasladosImpuestoIVA0 = "%.2f" % (impuesto_iva_0 * tc)

        if iva_p_excento:
            cp.TotalTrasladosBaseIVA0 = "%.2f" % (base_iva_excento * tc)
            cp.TotalTrasladosImpuestoIVA0 = "%.2f" % (impuesto_iva_excento * tc)
