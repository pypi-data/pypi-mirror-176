...
from ..components import NodeVisualization
from ..data_transformation import *
from ...server.state import LayerType
from node_values import InputAsNodeValues, SetNodeValues
from ...globals import *
...
def UpdateNodeValues(new_input):
    currentServer.sync_state.layers[0].node_values = InputAsNodeValues(new_input)
    unweighted_layer = 0
    for i in range(1, currentServer.sync_state.GetLayerCount()):
        currentServer.sync_state.layers[i].node_values = SetNodeValues(
            NodeVisualization.PREV_WITH_ACTIVATION,
            currentVisualizer.GetModel(),
            currentVisualizer.GetModel().GetWeightedLayerSizes(),
            i - unweighted_layer - 1, i - 1, currentServer.sync_state.layers[i - 1],
            currentServer.sync_state.layers[i].layer_type)
        if currentServer.sync_state.layers[i].layer_type == LayerType.POOLING:
            unweighted_layer += 1
    currentServer.sync_state.UpdateNodeValues()
