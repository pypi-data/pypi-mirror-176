...
TO DO:
    ...
    combine statements; compound statement
        ...
        > simple to combine models
    perform groupby on 'Item' level - very difficult to do with Pyomo level b/c of change in Index dimension
        - seems like I have overcomplicated groupby anyway ... can just be done through `concat` operations in Elements object!?!?!?!?
    speed optimization using numba
    Pickle abstract model, store in database and reload to statement so that new instance data can be used
    `as_totals` method for `add_metrics` that loops and totals multiple accounts
    convert to account = fs.Account() structure to mirror pyomo


CLEAN UP ITEMS:
    What are time-series-factors????
    Can add_metrics() accept a ScalarVar or Param straight-up and not just EXPR?
    Speed up instance creation by stripping out non-meaningful index columns?

    IS THIS A FEATURE, not A BUG!?!??! Groupby: does NOT impact the lineitem nodes, so that any lineitem selected has to have groupby applied AGAIN; not true for resample
...
class StatGraph:
    ...
    def update_attr_in_all_nodes(self, attr, value):
        return self._graph.add_nodes_from(self._graph.nodes, **{attr:value})

    ### GRAPH FILTERING ###
    def filter_subgraph(self, attr:str, value, statname:str, graph:nx.Graph=None):
        # returns a subgraph containing only the nodes with attribute value specified
        """
        Params
        ------
        attr:       str; node attribute to be filtered
        value:      any; value of attr
        statname:   str; `short_name` attribute of statement to be filtered
        graph:      nx.Graph; If not provided, `graph` is pulled from the `statement`.
                    `graph` is passed when it has been previously filtered. Default is None.
        """
    ...
    def filter_graph_by_attribute(self, attr, value, graph=None):
        return self.filter_subgraph(attr, value, self._stat.short_name, graph)

    def filter_graph_by_attributes(self, **kwargs):
        graph = None
        for k, v in kwargs.items():
            graph = self.filter_graph_by_attribute(k, v, graph)
        return graph
    ...
    def filter_nodes_by_attributes(self, return_tuple=False, **kwargs):
        ...
        node_kwargs = {k: v for k, v in kwargs.items() if k not in filter_kwargs}
        graph = self.filter_graph_by_attributes(**filter_kwargs)

        for node in graph.nodes(**node_kwargs):
            node = node[1] if isinstance(node, tuple) and not return_tuple else node
            if 'data' in node_kwargs and node_kwargs['data'] == 'obj':
