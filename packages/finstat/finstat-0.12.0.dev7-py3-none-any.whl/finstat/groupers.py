...
######### ABOVE NOT TRUE FOR GROUPBY FROM MULTI TO SINGLE!!!!!!!!!!!
"""
...
class Grouper:
    def __init__(self, stat, *args, **kwargs):
        ...
        ### ALL OF THE BELOW IS AN OPERATION THAT COULD BE PERFORMED AS A `.copy()` function
        ### IN the FinancialStatement object
        self.grouped = stat._constructor(name=stat.name, short_name=stat.short_name, graph=stat.G._graph.copy(), *args, **kwargs)
        
        # Have to copy over every object in every lineitem
        for nodename in self.grouped.G.nodes:
            self.grouped.G.nodes[nodename]['obj'] = stat.G.nodes[nodename]['obj'].copy()
...
class FinStatGroupBy(Grouper):
    ...
    def _call(self, *args, **kwargs):
        ...
        """
        if self.grouped.G.graph['by'] == 'Item':
            import warnings
            warnings.warn('Groupby from multi to single is very complicated. Seems like you can simply pass the grouped statement with no operations!?!?')
            return self.grouped # Simple Statements should be able to do this automatically!!!!!!
