...
'''
These relationships are defined separately to the object/table declarations to allow them to
...
from .agent_contribution_activity import (AgentContributionActivity,
                                          agent_contribution_activity_table)
from .contribution_activity import ContributionActivity, contribution_activity_table
from .package_contribution_activity import (PackageContributionActivity,
...
def setup_relationships():
    ...
    if not manager or not manager.is_mapped:
        meta.mapper(ContributionActivity, contribution_activity_table, properties={
            'agent': relationship(Agent, secondary=agent_contribution_activity_table,
                                  uselist=False),
    ...
    if not manager or not manager.is_mapped:
        meta.mapper(AgentAffiliation, agent_affiliation_table, properties={
            '_agent_backref': relationship(Agent,
                                           ...
                                           primaryjoin=or_(
                                               agent_affiliation_table.c.agent_a_id.__eq__(
                                                   Agent.id),
                                               agent_affiliation_table.c.agent_b_id.__eq__(
                                                   Agent.id)),
                                           ),
