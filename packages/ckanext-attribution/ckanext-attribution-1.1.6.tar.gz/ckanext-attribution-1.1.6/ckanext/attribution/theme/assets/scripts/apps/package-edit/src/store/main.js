...
const store = new Vuex.Store(
    {
        ...
        actions: {
                ...
                return get('package_contributions_show', {
                    ...
                    .then(res => {
                        ...
                        Agent.insert({
                            data: res.contributions.map(r => {
                                ...
                                editedItems.forEach((itemMeta) => {
                                    ...
                                    if (factory.query().where('id', itemMeta.item_id).exists()) {
                                        updates.push(factory.update({ data: item })
                                                            .then(() => {
                                        ...
                                        delete item.meta;
                                        updates.push(factory.insert({ data: item }).then(() => {
                            ...
                            let isLastCitedPage = (res.offset + res.pageSize) >= res.citedTotal >= res.offset;
                            ...
                            return Agent.query()
                                    ...
                                    let isInRange = a.citation.order && (res.offset <= a.citation.order) && (a.citation.order <= (res.offset + res.pageSize));
                                    let higherThanTotalAtEnd = a.citation.order && (a.citation.order < context.state.results.citedTotal && isLastCitedPage);
                                    let show = uncitedOnLastPage || isInRange || higherThanTotalAtEnd;
            ...
            removeContributor(context, contributorId) {
                ...
                let promises = [];
                promises.push(Agent.updateMeta(contributorId, {
                    is_hidden: true,
                    to_delete: true
                }));
