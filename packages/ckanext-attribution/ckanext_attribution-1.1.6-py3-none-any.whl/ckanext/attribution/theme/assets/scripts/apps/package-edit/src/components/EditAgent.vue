<template>
    <div class="agent-edit-block agent-detail" :class="'agent-'+edits.agent_type.toLowerCase()">
        ...
        <div class="agent-id-edit attribution-row">
            <select-field v-model="edits.external_id_scheme"
                          :options="Object.entries(controlledLists.agentIdSchemes)"
                          :opt-label="o => o[1].label" :opt-value="o => o[0]"
                          :class="['wrap-small']">
                External ID scheme
            </select-field>
            <ValidatedField v-model="edits.external_id" :validator="validateExternalId"
                            ref="externalId"
        ...
        <template v-if="expand">
            <div class="agent-name-edit attribution-row">
                ...
                <div v-if="probableDuplicates.length > 0" style="grid-column: span 2;">
                    <small>
                        <b>Please ensure this contributor is not duplicating any of these
                           existing contributors:</b>
                    </small>
                    <div class="flex"><span v-for="d in probableDuplicates" style="margin-right: 5px;">{{ d.label }}</span>
                    </div>
                </div>

            </div>
            <div class="agent-user-edit attribution-row" v-if="edits.agent_type === 'person'">
                <autocomplete-field v-model="edits.user_id" @typing="updateUserOptions"
                                    :options="userOptions"
                                    label="Linked user account"
                                    :item-id="'agent-user-' + contributorId" v-if="!edits.user_id">
                </autocomplete-field>
...
export default {
    ...
    methods: {
        ...
        checkDuplicates() {
            ...
            get('agent_list', { q: this.displayName, mode: 'duplicates' }).then(agents => {
                    this.$set(this, 'probableDuplicates', agents.map(agent => {
        ...
        refresh() {
            ...
            this.affiliations = this.contributor.affiliations.filter(a => !a.meta.to_delete)
                                    .map(a => {
                                        return {
