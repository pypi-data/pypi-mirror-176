...
int tamodify_close()    {//读入一行内容，对应主结构里的close
...
static int setret(stru_ta_modify * tajk,int returncode,const char *fmt,...)  {//设置返回码、返回信息
    va_list ap;
    va_start(ap, fmt);
...
int tajk_closeall(stru_ta_modify * tajk){//关闭打开的文件等
