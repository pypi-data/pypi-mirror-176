...
class PageRecognizer(object):
    def __init__(self, cur_pages_dir, cur_page_idx, cur_page_width=2561, cur_page_height=1206):
    ...
    def recognize_rect(self, cur_key_word, crop_img_top, crop_img_bottom, crop_img_left, crop_img_right, post_replace={}, flag_force = False, ratio="2.0", flag_autocrop = True, app_name="img2txt"):
        cur_rec = ImageRecognizer("%s.png"%cur_key_word, crop_img_top, crop_img_bottom, crop_img_left, crop_img_right, self.cur_img_file(), self.dst_img_dir(), flag_force, ratio, flag_autocrop)
        cur_rec_data = cur_rec.recognize_text(app_name)
        for single_replace_key in post_replace:
            cur_rec_data['recognize_text'] = cur_rec_data['recognize_text'].replace(single_replace_key, post_replace[single_replace_key]) # ("\u2014", "-")        
        glue(cur_key_word, cur_rec_data['recognize_text'])
        self.page_elements[cur_key_word] = cur_rec_data        
        return cur_rec

    def generate_html_js(self, element_height={}):
        self.generate_js(element_height)
        self.generate_html()
        
    def generate_js(self, element_height={}):
        ...
        # cur_page_ratio = cur_page_width / 1280
        ...
        for single_element in self.page_elements:
            ...
            print(single_data)
            cur_page_elements_js = cur_page_elements_js + """
image_note.add(new DoneElement(%d / 2, %d / 2, %d / 2, %d / 2, "%s<br/>%s"));
""" % (single_data["crop_img_left"], single_data["crop_img_top"], 
       cur_page_width - single_data["crop_img_right"] - single_data["crop_img_left"], 
       cur_page_height - single_data["crop_img_bottom"] - single_data["crop_img_top"], 
       single_element, single_data["recognize_text"])
            cur_page_elements_js = cur_page_elements_js + """
image_note.add(new CommentElement(1300, 30+%d, 250, 44+%d, "Note_%s_%03d:<br/> - %s<br/> - %s"));
""" % (cur_page_element_top, cur_page_element_height - 44, self.page_idx, cur_page_element_idx, single_element, single_data["recognize_text"])
...
""" % (self.page_idx, cur_page_elements_js)
        util.create_text_file(file_name_page_js, file_content_page_js)
        # util.display_code(file_name_page_js)
        print("http://localhost:4507/xxx_dev_blog/%s/%s.png.html" % (cur_pages_dir, self.page_idx))
...
""" % (self.page_idx, self.page_idx, self.page_idx)
