...
def argument_parser():
    ...
    arg_parser = argparse.ArgumentParser(
        description='manatus - Metadata aggregation and transformation utility',
    ...
    harvest_parser = subcommand_parsers.add_parser('harvest', help='manatus harvest interactions',
                                                   parents=[config_parser])
    harvest_parser.add_argument('-r', '--run', action='store_true', help='run harvest for all config entries')

    # transform subcommand & args
    transformation_parser = subcommand_parsers.add_parser('transform', help='manatus transformation interactions',
                                                          parents=[config_parser])
...
def interactive_run(manatus_config, config_parser, subcommand, profile, *args, **kwargs):
    """
    Allows interactive selection of organization to harvest/transform

    :param profile:
    :param manatus_config:
    :param config_parser:
    :param subcommand:
    :param args:
    :param kwargs:
    ...
    if subcommand == 'harvest':
        ...
        transform(manatus_config, config_parser[config_parser.sections()[selection - 1]],
...
empty_manatus_cfg = r'''# Please see manatus configuration documentation: https://manatus.readthedocs.io"
...
empty_harvest_cfg = r'''# Please see manatus configuration documentation: https://manatus.readthedocs.io"

[manatus_provider_key]
oaiendpoint = 
setlist = 
metadataprefix =                     
'''

empty_scenario_cfg = r'''# Please see manatus configuration documentation: https://manatus.readthedocs.io"
