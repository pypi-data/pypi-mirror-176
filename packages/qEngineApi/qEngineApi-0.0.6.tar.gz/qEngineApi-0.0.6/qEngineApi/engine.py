...
class QlikEngine():
    ...
    def exportApp(self,dir, onlyPublished=True, streamId='ALL',appId="ALL",createFolderStructure = False):
        ...
        if appId == "ALL":
            for doc in docList:
                if createFolderStructure:
                    dir = os.path.join(dir,f'{doc["stream"]}({doc["streamId"]})')
                    if not os.path.isdir(dir):
                         Path(dir).mkdir(parents=True, exist_ok=True)
                file = os.path.join(dir,f'{doc["docName"]} ({doc["docId"]}).qvf')
