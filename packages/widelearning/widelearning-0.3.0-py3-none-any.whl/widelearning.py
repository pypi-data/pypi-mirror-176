...
def add_kernel(path, init_kernel):
 ...
 def top_bottom_class(train_path, top, other):
    # - функция определения верхнего класса
    # train_path - путь к обучающей выборке
    # top - верхний класс, задается списком
    # other - нижний класс, задается списком
...
def scale_weights_binary(up, down):
    ...
    #up = 0
    #for i in class_top['Scalar_calc']:
    #    if(i > class_others['Scalar_calc'].max()):
    #        up+=1
    #    else:
    #        break
    ...
    mx =  d_up#(d_up + class_down['Scalar_calc'].max())/2
    mn = d_down#(d_down + class_down['Scalar_calc'].min())/2
