...
from pdfform.config_win import config
...
class ApplicationForm:
    ...
    def add_step(self, action: dict):
        ...
        if action_type == "Skip":
            ...
            custom_pause = float(action.get("custom_pause")) or action.get("pause")
            if custom_pause:
                pause_time = config.get(custom_pause) or action.get("pause")
            else:
                pause_time = action.get("pause")
            control = TextField(
                action.get("data"), pause=pause_time, verbose=self.verbose
            )
            self._actions.append(control)
