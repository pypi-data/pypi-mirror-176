...
@pytest.mark.parametrize("ds_name,queries", [
    (COCO_DS_NAME, [
    ...
    (MNIST_DS_NAME, [
        ...
        ("SELECT * SAMPLE BY sum_weight(labels == 0: 10, labels == 1: 5) LIMIT 100", 100, None),
        ("SELECT * SAMPLE BY max_weight(labels == 0: 10, labels == 1: 5) LIMIT 100", 100, None),
        ("SELECT * SAMPLE BY labels", 60000, None),
        ("SELECT * SAMPLE BY sum_weight(labels > 5: 1, labels == 0: 3) REPLACE LIMIT 10000", 10000, None),
        ("SELECT * SAMPLE BY max_weight(labels > 5: 1, labels == 0: 3) REPLACE LIMIT 10000", 10000, None),
    ...
    (IMAGENET_DS_NAME, [
        ...
        ("SELECT * SAMPLE BY sum_weight(labels == 'fur coat': 10, labels == 'bikini': 3) limit 0.1 PERCENT", 1281, None),
        ("SELECT * SAMPLE BY max_weight(labels == 'fur coat': 10, labels == 'bikini': 3) limit 0.1 PERCENT", 1281, None),
        ("SELECT * SAMPLE BY sum_weight(labels == 'fur coat': 10, labels == 'bikini': 3) replace", 1281166, None),
        ("SELECT * SAMPLE BY sum_weight(labels == 'fur coat': 10, labels == 'bikini': 3) replace", 1281166, None),
        ("SELECT * SAMPLE BY max_weight(labels == 'fur coat': 10, True: 0.1)", 1281166, None),
    ]),
    # LAION_400_M_DS_NAME: [
