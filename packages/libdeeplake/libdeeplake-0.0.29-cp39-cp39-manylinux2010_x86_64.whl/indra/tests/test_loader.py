...
def test_on_with_s3_path_loader():
    ...
    hub_ds = deeplake.load("s3://hub-2.0-datasets-n/cars/")[0:100]
...
def test_mnist_batch_equality():
    ...
    hub_ds = deeplake.load(MNIST_DS_NAME)[0:100]
...
def test_cifar10_batch_equality():
    ...
    hub_ds = deeplake.load(CIFAR10_DS_NAME)[0:100]
...
def create_corrupted_collision():
    try:
        ...
        pass
    ds = deeplake.dataset(CORRUPTED_DS_PATH, overwrite=True)
    with ds:
...
@pytest.mark.parametrize("ds_name, slice", [
    (MNIST_DS_NAME, None),
...
@pytest.mark.parametrize("batch_size, num_threads", [
    (64, 2),
    (64, 4),
    (512, 2),
    (512, 4),
...
def test_imagenet_1k_batch_vs_threads(benchmark, batch_size, num_threads):
    ...
    def ds_loader():
        ...
        ds = ds[0:1000]
        l = ds.loader(shuffle=False, drop_last=True, batch_size=batch_size, num_threads=num_threads)
        for b in iter(l):
