...
# 미국 매크로
def get_US_macro(today_not_str):
    ...
    #  40영업일 래깅
    M2 = pdr.get_data_fred('M2SL', start='1978-01-01', end=today_not_str.strftime('%Y-%m-%d')).rename(columns={'M2SL': 'M2'})
    M2.index.name = 'date'
    ...
    # 30영업일 래깅
    unemployment = pdr.get_data_fred('UNRATE', start='1978-01-01', end=today_not_str.strftime('%Y-%m-%d')).rename(columns={'UNRATE': 'unemployment'})
    unemployment.index.name = 'date'

    # BEI: https://fred.stlouisfed.org/series/T10YIE # (Percent, Not Seasonally Adjusted, Daily)
    # (10-Year Breakeven Inflation Rate)
    # 래깅 필요 없음
    BEI = pdr.get_data_fred('T10YIE', start='1978-01-01', end=today_not_str.strftime('%Y-%m-%d')).rename(columns={'T10YIE': 'BEI'})
    BEI.index.name = 'date'
    ...
    # 50영업일래깅
    CPI = pdr.get_data_fred('CPALTT01USM657N', start='1978-01-01', end=today_not_str.strftime('%Y-%m-%d')).rename(columns={'CPALTT01USM657N': 'CPI'})
    CPI.index.name = 'date'
    ...
    DATA = pd.merge(SNP500, M2, how='outer', on='date')
    DATA = DATA.merge(NASDAQ, how='outer', on='date')
    DATA = DATA.merge(KOSPI, how='outer', on='date')
    ...
    DATA = DATA.merge(DI, how='outer', on='date')
    # DATA = DATA.merge(KRWYEN, how='outer', on='date')
    # DATA = DATA.merge(KRWUSD, how='outer', on='date')
    DATA = DATA.merge(unemployment, how='outer', on='date')
    DATA = DATA.merge(BEI, how='outer', on='date')
    DATA = DATA.merge(CPI, how='outer', on='date')
    DATA = DATA.merge(PCE, how='outer', on='date')
    DATA = DATA.merge(VIX, how='outer', on='date')
    DATA = DATA.merge(TEDspread, how='outer', on='date')
    DATA = DATA.merge(CLI, how='outer', on='date')
    DATA = DATA.merge(USBond_1Y, how='outer', on='date')
    DATA = DATA.merge(USBond_2Y, how='outer', on='date')
    DATA = DATA.merge(USBond_3Y, how='outer', on='date')
    DATA = DATA.merge(USBond_5Y, how='outer', on='date')
    DATA = DATA.merge(USBond_10Y, how='outer', on='date')
    DATA = DATA.merge(USBond_30Y, how='outer', on='date')
