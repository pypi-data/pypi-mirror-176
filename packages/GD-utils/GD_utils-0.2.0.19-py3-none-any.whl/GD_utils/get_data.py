...
def get_US_macro(today_not_str=pd.Timestamp.now()):
    ...
    CPI = pd.concat([CPI, CPI_YoY, CPI_YoY_z48, CPI_YoY_z60, CPI_YoY_z72], axis=1)
    ...
    US_GDP = pd.concat([US_GDP, End_Month_Dates], axis=1).fillna(method='ffill').groupby(pd.Grouper(freq='BM')).tail(1).drop('SNP500', axis=1)
    ...
    DATA = DATA.merge(US_GDP.add_suffix("(M)"), how='outer', on='date')
