...
@app.command()
def ls(
    directories: List[str] = Argument(os.getcwd()),
    include: str = Option(
        "", "-i", "--include", help="commma separated list of regexes to include"
    ),
    exclude: str = Option(
        "", "-e", "--exclude", help="commma separated list of regexes to exclude"
    ),
