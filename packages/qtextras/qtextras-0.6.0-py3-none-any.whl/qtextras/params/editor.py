...
from qtextras._funcparse import QtExtrasInteractor, RunOptions
...
class FileStateManager:
    ...
    def __init__(
        ...
        self.watcher: QtCore.QFileSystemWatcher | None = None
...
class ParameterEditor(QtWidgets.QWidget):
    ...
    defaultInteractor = QtExtrasInteractor()
        ...
        *,
        ...
        if stateDict is None:
            stateDict = (
                ...
                if (not addDefaults and not self.getDefaultStateName().exists())
                else self.getParameterDefaults()
            )
        if not blockWrite:
    ...
    def _parseAndUpdateCandidateParameters(
        ...
        if candidateParameters is None:
            candidateParameters = fns.flattenedParameters(self.rootParameter)
    ...
    def registerFunction(
        ...
        **kwargs
        ...
        self._interactAndHandleExistingParameters(
    ...
    @classmethod
    def _interactAndHandleExistingParameters(cls, interactor, function, **kwargs):
        existingParams = function.parameters.copy()
        # Set to nothing before interacting, so they can be added back without
        # being disconnected
        function.parameters = {}
        if alreadyRegistered := set(existingParams).intersection(kwargs):
            raise ValueError(
                f"Cannot pass parameters {alreadyRegistered} that are already "
                f"registered to {function}"
            )
        kwargs["ignores"] = set(kwargs.get("ignores", ())) | set(existingParams)
        interactor(function, **kwargs)
        function.parameters.update(existingParams)
        for ii, (name, param) in enumerate(existingParams.items()):
            kwargs["parent"].insertChild(ii, param)
            del function.extra[name]
