...
class ConversationTranscriptionResult(RecognitionResult):
    ...
    def __init__(self, handle: _spx_handle):
        ...
        _call_hr_fn(fn=_sdk_lib.conversation_transcription_result_get_user_id, *[handle, buffer, buffer_size])
        self._user_id = buffer.value.decode()
        _call_hr_fn(fn=_sdk_lib.conversation_transcription_result_get_utterance_id, *[handle, buffer, buffer_size])
...
class Conversation():
    ...
    def __init__(self, speech_config: SpeechConfig, conversation_id: OptionalStr = None):
        ...
        handle = _spx_handle(0)
        c_conversation_id = _c_str(conversation_id)
        _call_hr_fn(fn=_sdk_lib.conversation_create_from_config, *[ctypes.byref(handle), speech_config._handle, c_conversation_id])
        self.__handle = _Handle(handle, None, _sdk_lib.conversation_release_handle)
        propbag_handle = _spx_handle(0)
        _call_hr_fn(fn=_sdk_lib.conversation_get_property_bag, *[handle, ctypes.byref(propbag_handle)])
        self.__properties = PropertyCollection(propbag_handle)
        buffer = ctypes.create_string_buffer(1024 + 1)
        c_size = ctypes.c_size_t(1024 + 1)
        _call_hr_fn(fn=_sdk_lib.conversation_get_conversation_id, *[handle, buffer, c_size])
    ...
    def add_participant_async(self, participant: Optional[Participant] = None, user_id: OptionalStr = None) -> ResultFuture:
        ...
        def resolve_future(handle: _spx_handle):
            _call_hr_fn(fn=_sdk_lib.conversation_update_participant, *[self._handle, ctypes.c_bool(True), participant._handle])
            return participant
    ...
    def remove_participant_async(self, participant: Optional[Participant] = None, user_id: OptionalStr = None) -> ResultFuture:
        ...
        def resolve_future(handle: _spx_handle):
            ...
            if participant is not None:
                _call_hr_fn(fn=_sdk_lib.conversation_update_participant, *[self._handle, c_false, participant._handle])
            elif user_id is not None:
                c_user_id = _c_str(user_id)
                _call_hr_fn(fn=_sdk_lib.conversation_update_participant_by_user_id, *[self._handle, c_false, c_user_id])
    ...
    def end_conversation_async(self) -> ResultFuture:
        ...
        :returns: A future that is fulfilled once conversation has been ended.
        """
        def resolve_future(handle: _spx_handle):
            _call_hr_fn(fn=_sdk_lib.conversation_end_conversation, *[self._handle])
            return None
        return ResultFuture(None, resolve_future, None)
    ...
    def start_conversation_async(self) -> ResultFuture:
        ...
        :returns: A future that is fulfilled once conversation has been started.
        """
        def resolve_future(handle: _spx_handle):
            _call_hr_fn(fn=_sdk_lib.conversation_start_conversation, *[self._handle])
            return None
        return ResultFuture(None, resolve_future, None)
    ...
    def delete_conversation_async(self) -> ResultFuture:
        ...
        :returns: A future that is fulfilled once conversation has been deleted.
        """
        def resolve_future(handle: _spx_handle):
            _call_hr_fn(fn=_sdk_lib.conversation_delete_conversation, *[self._handle])
            return None
        return ResultFuture(None, resolve_future, None)
    ...
    def lock_conversation_async(self) -> ResultFuture:
        ...
        :returns: A future that is fulfilled once conversation has been locked.
        """
        def resolve_future(handle: _spx_handle):
            _call_hr_fn(fn=_sdk_lib.conversation_lock_conversation, *[self._handle])
            return None
        return ResultFuture(None, resolve_future, None)
    ...
    def unlock_conversation_async(self) -> ResultFuture:
        ...
        :returns: A future that is fulfilled once conversation has been unlocked.
        """
        def resolve_future(handle: _spx_handle):
            _call_hr_fn(fn=_sdk_lib.conversation_unlock_conversation, *[self._handle])
            return None
        return ResultFuture(None, resolve_future, None)
    ...
    def mute_all_participants_async(self) -> ResultFuture:
        ...
        """
        def resolve_future(handle: _spx_handle):
            _call_hr_fn(fn=_sdk_lib.conversation_mute_all_participants, *[self._handle])
            return None
        return ResultFuture(None, resolve_future, None)
    ...
    def unmute_all_participants_async(self) -> ResultFuture:
        ...
        """
        def resolve_future(handle: _spx_handle):
            _call_hr_fn(fn=_sdk_lib.conversation_unmute_all_participants, *[self._handle])
            return None
        return ResultFuture(None, resolve_future, None)
    ...
    def mute_participant_async(self, participant_id: str) -> ResultFuture:
        ...
        def resolve_future(handle: _spx_handle):
            ...
            _call_hr_fn(fn=_sdk_lib.conversation_mute_participant, *[self._handle, c_participant])
            return None
        return ResultFuture(None, resolve_future, None)
    ...
    def unmute_participant_async(self, participant_id: str) -> ResultFuture:
        ...
        def resolve_future(handle: _spx_handle):
            c_participant = _c_str(participant_id)
            _call_hr_fn(fn=_sdk_lib.conversation_unmute_participant, *[self._handle, c_participant])
            return None
        return ResultFuture(None, resolve_future, None)
...
class ConversationTranscriber(Recognizer):
    ...
    def __init__(self, audio_config: Optional[AudioConfig] = None):
        ...
        _call_hr_fn(fn=_sdk_lib.recognizer_create_conversation_transcriber_from_config, *[ctypes.byref(handle), audio_handle])
        super().__init__(handle)
    ...
    def join_conversation_async(self, conversation: Conversation) -> ResultFuture:
        ...
        :returns: A future that is fulfilled once joined the conversation.
        """
        def resolve_future(handle: _spx_handle):
            _call_hr_fn(fn=_sdk_lib.recognizer_join_conversation, *[conversation._handle, self._handle])
            None
        return ResultFuture(None, resolve_future, None)
    ...
    def leave_conversation_async(self) -> ResultFuture:
        ...
        :returns: A future that is fulfilled once left the conversation.
        """
        def resolve_future(handle: _spx_handle):
            _call_hr_fn(fn=_sdk_lib.recognizer_leave_conversation, *[self._handle])
            None
        return ResultFuture(None, resolve_future, None)
