...
from opentf.tools.ctlcommons import _is_command, _get_value, _error, _ensure_options
...
def _get_prefix(service, default):
    prefix = (
        _get_value(f'--orchestrator-{service}-prefix=')
...
def set_credentials(user):
    ...
    try:
        ...
        if step_depth := _get_value('--step-depth='):
            try:
                step_depth = int(step_depth)
                users[user]['user']['step-depth'] = int(step_depth)
            except ValueError:
                _fatal_cannot_modify_configuration_file(
                    conf_filename, 'step depth must be an integer'
                )
        if job_depth := _get_value('--job-depth='):
            try:
                job_depth = int(job_depth)
                users[user]['user']['job-depth'] = int(job_depth)
            except ValueError:
                _fatal_cannot_modify_configuration_file(
                    conf_filename, 'job depth must be an integer'
                )
        if max_command_length := _get_value('--max-command-length='):
            try:
                max_command_length = int(max_command_length)
                users[user]['user']['max-command-length'] = int(max_command_length)
            except ValueError:
                _fatal_cannot_modify_configuration_file(
...
def set_orchestrator(orchestrator):
    ...
    try:
        ...
        for svc in [
            ...
            if port := _get_value(f'--{svc}-port='):
                try:
                    ...
                    sys.exit(2)
                ports[svc] = port
            if prefix := _get_value(f'--{svc}-prefix='):
                prefixes[svc] = prefix
            if baseurl := _get_value(f'--{svc}-force-base-url='):
        ...
        if ports or prefixes or forcebaseurls:
            if 'ports' in orchestrators[orchestrator]['orchestrator']:
                services = {
                    svc: {'port': value}
                    for svc, value in orchestrators[orchestrator]['orchestrator'][
                        'ports'
                    ].items()
                }
                del orchestrators[orchestrator]['orchestrator']['ports']
            else:
                services = {}
            orchestrators[orchestrator]['orchestrator'].setdefault('services', services)
            for svc, port in ports.items():
                orchestrators[orchestrator]['orchestrator']['services'].setdefault(
                    svc, {}
                )
                orchestrators[orchestrator]['orchestrator']['services'][svc][
                    'port'
                ] = port
