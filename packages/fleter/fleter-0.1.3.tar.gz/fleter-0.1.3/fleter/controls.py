    ...
    def option(self, list):
        ...
        for item in self._options:
            ...
            if item_type == "list" or item_type == "tuple":
                disabled = item[1]
                self._option.append(Option(item[0], disabled=disabled))
            else:
...
class ColorPalette(object):
    def __init__(self):
        self._light_background = "#ffffff"
        self._light_foreground = "#101010"
        self._light_accent_background = "#3c7bfc"
        self._light_accent_foreground = "#ffffff"
        self._dark_background = "#101010"
        self._dark_foreground = "#ffffff"
        self._dark_accent_background = "#84e9ff"
        self._dark_accent_foreground = "#ffffff"
    ...
    def get_light(self):
        return {
            "background": self._light_background,
            "foreground": self._light_foreground,
            "accent_background": self._light_accent_background,
            "accent_foreground": self._light_accent_foreground,
        }

    def get_dark(self):
        return {
            "background": self._dark_background,
            "foreground": self._dark_foreground,
            "accent_background": self._dark_accent_background,
            "accent_foreground": self._dark_accent_foreground,
        }
...
class NoteBook(flet.Tabs):
    ...
    def add(self, text: str = "", content=None, note_content=None, *args, **kwargs):
        tab = flet.Tab(text=text, content=content, tab_content=note_content, *args, **kwargs)
        self._tabs.append(tab)
        self.tabs = self._tabs
        return tab

    def add_with_can_close(self, text: str = "", content=None, tooltip="close control", on_close=None,
                           close_tip: bool = False, close_tip_title="Close tab", close_tip_shape=flet.CountinuosRectangleBorder(radius=18),
                           close_tip_message="Are you sure you want to close this tab", close_tip_yes="Yes", close_tip_no="No",
                           *args, **kwargs):
        if on_close is None:
            if close_tip:
                def close(evt):
...
def mainwindow(page: flet.Page):
    headerbar = HeaderBar(page)
    headerbar.show()
    page.update()
...
if __name__ == '__main__':
    def main(page: flet.Page):
        ...
        disable_material3_theme(page)
        notebook = NoteBook()
        for item in range(9):
            notebook.add(f"Item {item + 1}")
        for item in range(9):
            notebook.add_with_can_close(f"Item {item + 10}")
        page.add(
            notebook
        )
        page.update()
