...
class ChapterPaperGenerator(object):
    ...
    def chapter_name_normalize(self, n):
        r = re.compile(r'(\d+)')
        ps = r.split(n)
        if len(ps) != 3:
            return n
    ...
    def questions_group_by_memo(self, qs):
        gs = {}
        for q in qs:
    ...
    def group_split_by_memo(self, gs):
        rs = OrderedDict()
        for gn, qs in gs.items():
            ngs = self.questions_group_by_memo(qs)
            if len(ngs) <= 1:
                nqs = ngs.get('')
                if nqs:
    ...
    def normalize(self, gs):
        ...
        ogs.update(gs)
        return self.group_split_by_memo(ogs)

    def gen_paper(self, title, cond, order_number=0):
        from .models import Paper, Content
        qts = self.store.gen_paper(cond)
    ...
    def generate(self):
        ...
        for n in self.owner:
            ...
            for cn, cts in categories:
                pname = '%s %s（%s）' % (self.chapter_name_normalize(n.prefix), n.name, cn)
                cond = {
    ...
    def get_source_papers(self):
        return models.Paper.objects.filter(owner_id=self.owner.pk, owner_type=self.owner_type, tags='历年真题')
