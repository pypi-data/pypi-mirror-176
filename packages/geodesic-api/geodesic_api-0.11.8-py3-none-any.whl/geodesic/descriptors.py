...
class _ListDescr(_BaseDescr):
    ...
    Args:
        item_type (tuple or type): A type or tuple of types that are allowed as the inputs to this descriptor.
            If you also set coerce to True, then the output type of the list (when you do a get) will be coerced
            to the first element of the tuple. If you would like to use this to pass in one type (typeA) and return another (typeB)
            then item_type should be `(typeB, typeA)` as well as `coerce=True`.
