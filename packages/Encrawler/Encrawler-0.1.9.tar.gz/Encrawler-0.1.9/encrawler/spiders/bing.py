...
class BingSpider():
    ...
    def search(self, keyword, start_page:int = 0, end_page: int = 1, item_num:int = None, **kwargs):
        ...
        """
        all_item_list = []
        if kwargs.get('proxies') != None and kwargs.get('proxies') != {}: # 若获取代理不为None或不为空字典则启用代理，允许传入空字典，则不启用代理
            ...
            for i in range(start_page, end_page):
                ...
                if self.validator(response):
                    item_list = self.parser(response, **{"keyword": keyword, "page_num": i+1, "item_num": item_num})
        ...
        if item_num:
            logger.info(f"根据item num返回{item_num}个")
            return all_item_list[item_num:]
        else:
            logger.info(f"返回全部, 共计{len(all_item_list)}个")
            return all_item_list
...
class BingNewsSpider(EngineBaseSpdier):
    ...
    def request(self, keyword:str, page_num: int=0, proxies: Optional[Dict] = {}, cookies: Optional[Dict] = {}, **kwargs):
        """
        @summary:单次请求
        ---------
        @param url: 请求url
        ...
        try:
            ...
            if proxies == {} or proxies == None: # 无proxy
                if cookies == {} or cookies == None: # 无cookie
                    ...
                    response = requests.get(url=self.url, headers=self.headers, params=self.params, cookies=cookies, timeout=10)
            else: # 有proxy
                proxies = kwargs.get("proxies")
                if cookies == {} or cookies == None: # 无cookie
                    response = requests.get(
                    ...
                    response = requests.get(url=self.url, headers=self.headers, params=self.params, cookies=cookies, timeout=10, proxies=proxies, verify=False, allow_redirects=False)
        except Exception as e:
            logger.error(e)
            raise e
        return response
    ...
    def parser(self, response, **kwargs):
        ...
        for i, item in enumerate(res):
            ...
            content = item.xpath('.//div[contains(@class,"snippet")]/text()')[0]
            raw_source = item.xpath(".//div[@class='source biglogo']/a[@class='biglogo_link']/@aria-label")
            source = item.xpath(".//div[@class='source biglogo']/a[@class='biglogo_link']/@aria-label")[0].replace("Search news from ", "") if len(raw_source) >= 1 else ""
            raw_publish_time = item.xpath(".//div[@class='source biglogo']/div/span[2]/@aria-label")
            publish_time = raw_publish_time[0] if len(raw_publish_time) >= 1 else ""
            # 注意这里time还没有处理，还是以28d，5h，1m的形式存在
            page_num = kwargs.get("page_num")
            page_rank = i
            validate_item = SearchItem(title=title, url=url, content=content, search_keyword=keyword, time=publish_time, source = source, page_num=page_num, page_rank=page_rank, crawltime=crawltime)
        ...
        for i in range(start_page, end_page):
            ...
            if proxy == {} or proxy == None:
                if kwargs.get('cookie') != None and kwargs.get('cookie') != {}:
                    ...
                    response = self.request(keyword, i, cookies=cookie)
                else:
                    response = self.request(keyword, i)
            # 有代理
            else:
                if kwargs.get('cookie') != None and kwargs.get('cookie') != {}:
                    cookie = kwargs.get("cookie")
                    response = self.request(keyword, i, proxies=proxy, cookies=cookie)
                else:
                    response = self.request(keyword, i, proxies=proxy)
                    
            # 解析
            search_item_list = self.parser(response, **{"keyword": keyword, "page_num": i+1})
...
if __name__ == "__main__":
    ...
    res = spider.search("原神 舆情",0, 1, **kwargs)
    for item in res:
