...
class Button:
    def __init__(self, screen: Screen, x: int, y: int, label: str, *args):
        ...
        for i in range(0, self.width+1):
            ...
            screen.addstr(y+2, x+i, "─", *args)

        screen.addstr(y, x, "┌")
        screen.addstr(y, x+self.width, "┐", *args)
        screen.addstr(y+1, x, "│")
        screen.addstr(y+1, x+self.width, "│", *args)
        screen.addstr(y+2, x, "└")
        screen.addstr(y+2, x+self.width, "┘", *args)

        screen.addstr(y+1, x+1, label, *args)
