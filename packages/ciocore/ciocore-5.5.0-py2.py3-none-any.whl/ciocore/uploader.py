...
from ciocore import api_client, common, worker, client_db, loggeria, file_utils
...
class MD5Worker(worker.ThreadWorker):
    ...
    def get_md5(self, filepath):
        ...
        if not self.md5_caching:
            return common.generate_md5(filepath, poll_seconds=5)
...
class MD5OutputWorker(worker.ThreadWorker):
    ...
    def target(self, thread_int):
        ...
        while not common.SIGINT_EXIT:
            ...
            # This happens if no new messages are put into the queue after waiting for
...
class FileStatWorker(worker.ThreadWorker):
    ...
    def do_work(self, job, thread_int):
        ...
        for singlepart_upload in job.get("singlePartURLs", []):
            ...
            self.put_job((path, file_size, upload_url, SINGLEPART, job.get("kmsKeyName")))
        ...
        for multipart_upload in job.get("multiPartURLs", []):
            ...
            # KMS is currently unsupported on AWS (multi-part), including here for consistency.
            self.put_job((path, file_size, multipart_upload, MULTIPART, job.get("kmsKeyName")))
...
def resolve_args(args):
    """
    Resolve all arguments, reconciling differences between command line args and config args.
    """
    cfg = config.config().config

    if args.get("md5_caching") is None:
        args["md5_caching"] = cfg["md5_caching"]

    if args.get("thread_count") is None:
        args["thread_count"] = cfg["thread_count"]

    args["database_filepath"] = args.get("database_filepath")
    args["location"] = args.get("location")
    logger.debug("Resolved Uploader Args are %s", args)
    return args
