...
class FilePlugin(object):
    ...
    def excel_to_dict(self, file_name, sheet: str='Sheet1', _replace: bool=True, file_path: str=TEST_DATA_PATH,
                      _functionName=None, _suitName=None, _storyId=None, _title=None):
        """
        ...
        data_list = excel_to_list(path.join(file_path, file_name,), sheet, _replace)
        data_list_new = []
        for case_data in data_list:
            if DataProcess.isNotNull(get_system_key(Constant.TEST_CASE_NAME_LIST)):
                if case_data[Constant.CASE_TITLE] in eval(get_system_key(Constant.TEST_CASE_NAME_LIST)):
                    data_list_new.append(case_data)
            if DataProcess.isNotNull(get_system_key(Constant.TEST_CASE_MARK)):
                if case_data[Constant.CASE_PRIORITY] in eval(get_system_key(Constant.TEST_CASE_MARK)):
                    data_list_new.append(case_data)
            if not DataProcess.isNotNull(get_system_key(Constant.TEST_CASE_NAME_LIST))\
                    and not DataProcess.isNotNull(get_system_key(Constant.TEST_CASE_MARK)):
                data_list_new.append(case_data)
