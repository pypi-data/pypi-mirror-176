...
class DataPlugin(object):
    ...
    @classmethod
    def jira_convert_allure(self, _caseName, _jiraKey):
        allure_title(_caseName)
        case_jira_key = JiraPlatForm.getJiraTestCaseKey(_caseName, _jiraKey)
        jira_key, case_name, case_link, case_priority, case_model, case_suitName, case_story_id,case_story_name,case_story_link = JiraPlatForm.getCaseInfo(
            case_jira_key)
        if DataProcess.isNotNull(case_link):
            allure_testcase(case_link, case_name)
            allure_severity(case_priority)
            allure_feature(case_model)
            allure_story(case_story_name,case_story_link)
            allure_suite(case_suitName)
