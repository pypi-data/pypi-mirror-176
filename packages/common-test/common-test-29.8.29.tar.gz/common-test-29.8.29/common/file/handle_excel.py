...
def excel_to_list(data_file, sheet,_index:int=1,_replace: bool=True):
    """
    ...
    for row in range(_index, sh.nrows):
        ...
        for col in range(0, table.ncols):
            ...
            # 获取当前单元格对应的列名
            col_title = table.cell_value(0, col).strip()
            # 当前列：值
            d[col_title] = cell_data
            d[col_title+'_row'] = row
            d[col_title+'_col'] = col
            d[col_title+'_sheetName'] = sheet
            d[col_title+'_excelPath'] = data_file
        #d = dict(zip(header, sh.row_values(i)))
        # 列表嵌套字典格式，每个元素是一个字典
        if check_excel_data(d):
            if _replace:
                data_list.append(DataBus.get_data(d))
            data_list.append(d)
    return data_list

def check_excel_data(testdata:dict):
