...
class RobotToPractiTest_Adapter(st.DataStatistics):
    ...
    def start_test_set(self, name, version, post_process: Callable = None, tags: list = None, **test_set_info):
        try:
            ...
            if post_process:
                post_process()
            logger.console(f"Suite '{name}' started as TestSet#{self.get_active_test_set_id()}")
        except Exception as e:
    ...
    def create_test_instance(self, robot_name, pt_name, tags, pure_tags, path, description, version, *steps,
                             **custom_fields):
        test_data = self._get_or_create_test(robot_name, pt_name, tags, path, description, version, *steps,
                                             **custom_fields)

        self.set_tests(st.TestStates.PractiTest, robot_name, status=st.TestStatuses.Pending, path=path, update=pt_name)
        ...
        for id_ in full_tests_list:
            try:
                ...
                logger.console(f"Test '{robot_name}': Run Instance created with name '{pt_name}' (Id: {test_id})")
            except Exception as e:
                f, w = utils.get_error_info()
                logger.error(
    ...
    def _get_or_create_test(self, robot_name, pt_name, tags, description, version, *steps, **custom_fields):
        ...
        while True:
            ...
            if test_data:
                logger.info(f"Test {pt_name} already existing")
                return test_data
            try:
                assert attempts <= rb.helpers.TEST_ALLOWED_CREATE_ATTEMPTS, \
                    f"Cannot get or create test '{robot_name}' during {attempts} attempts"
                self._client.create_test_case(pt_name, version=version, description=description, tags=tags,
                                              steps=steps, ignore_cache=True, **custom_fields)
                logger.console(f"Test '{robot_name}' created as '{pt_name}' successfully")
