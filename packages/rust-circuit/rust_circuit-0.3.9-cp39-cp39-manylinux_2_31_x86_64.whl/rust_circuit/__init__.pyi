...
class Index(Circuit):
    ...
    @staticmethod
    def index_synchronized_to_start(
...
class ModuleNodeArgSpec(object):
    ...
    @staticmethod
    def just_name(circuit: Circuit, batchable: bool = True, expandable: bool = True) -> ModuleNodeArgSpec: ...
    @property
...
class PrintOptions(object):
    ...
    def print(self, circuit: Circuit) -> None: ...
    def repr(self, circuit: Circuit) -> str: ...
    @staticmethod
    def repr_depth(circuit: Circuit, depth: int) -> str: ...
    @staticmethod
    def print_depth(circuit: Circuit, depth: int): ...
...
class ConstructModuleNodeExtractNotPresentError(ConstructError): ...
...
class ExpandModuleNodeRankReducedError(ExpandError): ...
...
def extract_rewrite(
    ...
    circuit_to_spec: Callable[[Circuit], ModuleNodeArgSpec],
) -> ModuleNode: ...
...
def extract_rewrite_raw(
    circuit: Circuit,
    input_specs: List[Tuple[Circuit, ModuleNodeArgSpec]],
    prefix_to_strip: Optional[str],
    module_name: Optional[str],
    spec_name: Optional[str],
...
class SimpFnSubset:
    ...
    # to generate below 3 fns, `cargo run --bin print_simp_fn_stub_part`
    @staticmethod
...
def add_flatten(add: Add, traversal: Optional[IterativeMatcher] = None) -> Add: ...
...
def nest_adds(add: Add, spec: NestAddsSpecSub, traversal: Optional[IterativeMatcher] = None) -> Add:
    """
...
def default_index_traversal() -> IterativeMatcher:
    """
    ``IterativeMatcher.noop_traversal().filter(term_early_at=[Index, ArrayConstant])``
    """

def push_down_index(
    index: Index,
    traversal: Optional[IterativeMatcherIn] = default_index_traversal(),
    suffix: Optional[str] = None,
    allow_partial_pushdown=False,
) -> Circuit: ...
def traverse_until_depth(depth: Optional[int]) -> IterativeMatcher:
    """
...
def distribute(
    einsum: Einsum,
    operand_idx: int,
    traversal: Optional[IterativeMatcherIn] = traverse_until_depth(1),
    suffix: Optional[str] = None,
    allow_partial_distribute: Optional[bool] = True,  # noop when fail to distribute at one point
    do_broadcasts: Optional[bool] = True,
) -> Circuit:
    """
