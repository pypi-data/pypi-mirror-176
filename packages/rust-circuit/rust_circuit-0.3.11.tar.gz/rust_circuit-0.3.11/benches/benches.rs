...
use circuit_base::{parsing::parse_circuit, CircuitRc};
...
use itertools::izip;
use pyo3::Python;
use rr_util::{opt_einsum::EinsumSpec, py_types::HASH_TENSOR, rrfs::get_rrfs_dir, timed};
extern crate test;
...
fn bench_schedule(circuits: &[CircuitRc]) {
    ...
    for circuit in circuits {
        let mut context = OptimizationContext::new_settings_circuit(settings, circuit.clone());
...
fn get_optimized(circuits: &[CircuitRc]) -> Vec<(CircuitRc, OptimizationContext)> {
    ...
    circuits
        .iter()
        .map(|c| {
            let mut context = OptimizationContext::new_settings_circuit(settings, c.clone());
            (optimize_circuit(c.clone(), &mut context), context)
        })
        .collect()
}
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let paths: Vec<_> = fs::read_dir(format!(
        ...
        // "{}/ryan/compiler_benches_is_was_10_14",
        get_rrfs_dir()
    ))
    .unwrap()
    .map(|d| d.unwrap().path())
    ...
    for (c, (_o, _ctx), p) in izip!(circuits, optimized, paths) {
