...
use anyhow::Result;
use circuit_base::{
    parsing::parse_circuit, print::PrintOptions, CircuitNode, IrreducibleNode, ScalarConstant,
};
...
impl TryFrom<&Schedule> for ScheduleToSend {
    ...
    fn try_from(x: &Schedule) -> Result<Self, Self::Error> {
        fn options() -> PrintOptions {
            PrintOptions::new(true, false, None, false, false).unwrap()
        }
        ...
        let out = Self {
            ...
            scalar_constants: map_to_str(&x.scalar_constants)?,
