...
use circuit_base::{prelude::*, Add, Concat, Einsum, Index, Rearrange, ScalarConstant, Scatter};
use pyo3::prelude::*;
use rr_util::{
...
pub fn einsum_pull_scatter(einsum: &Einsum) -> Option<CircuitRc> {
        ...
        .any(|(_i, slice)| slice.start == slice.stop)
    {
        ...
        let new_args = einsum
            ...
            .collect();
        let new_einsum = Einsum::nrc(new_args, einsum.out_axes.clone(), einsum.name_cloned());
        Some(Scatter::nrc(
            new_einsum,
...
pub fn add_pull_scatter(add: &Add) -> Option<Scatter> {
    ...
    if zip(&slices, &add.info().shape).all(|(s, l)| s.start == 0 && s.stop == *l) {
        ...
        let new_operands = add
            ...
            .collect();
        let new_add = Add::nrc(new_operands, add.name_cloned());
        Some(Scatter::new(
            new_add,
    ...
    if let Circuit::Einsum(inner) = &**node.node {
        ...
        if !did_anything {
            ...
            let new_args = inner
                ...
                .collect();
            let new_einsum = Einsum::nrc(new_args, inner.out_axes.clone(), inner.name_cloned());
