use std::{collections::HashSet, fmt, iter::zip};
...
use crate::{
    circuit_node_auto_impl, circuit_node_extra_impl, new_rc_unwrap, prelude::*, ArrayConstant,
        ...
        let mut run_sample = |num_batch_dims, random_inputs: bool| {
            ...
            // only run random on < orig
            {
                ...
                let tensors: Vec<_> = shapes
                    .iter()
                    .map(|shape| ArrayConstant::randn(shape.clone()).value)
                    .collect();
...
pub enum GeneralFunctionShapeError {
    ...
    #[error("ndim={ndim} < num_non_batchable_output_dims={num_non_batchable_output_dims} + removed_from_end={removed_from_end}")]
    NDimTooSmall {
        ndim: usize,
        num_non_batchable_output_dims: usize,
        removed_from_end: usize,
    ...
    #[error("x_shape={x_shape:?} index_shape={index_shape:?} batch_x={batch_x}")]
    IndexShapeInvalid {
        x_shape: Shape,
        index_shape: Shape,
        batch_x: bool,
