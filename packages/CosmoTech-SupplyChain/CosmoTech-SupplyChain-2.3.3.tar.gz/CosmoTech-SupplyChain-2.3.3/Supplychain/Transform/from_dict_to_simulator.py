...
class FromDictToSimulatorConverter(Timer):
    ...
    def __stock(self):
        ...
        for stock in stock_csv:
            ...
            stock_item = {
                ...
                "PurchasingUnitCostSchedule": convert_dict(int, float, stock["PurchasingUnitCosts"]),
                "CO2UnitEmissionsSchedule": convert_dict(int, float, stock["CO2UnitEmissions"]),
    ...
    def __transport(self):
        ...
        for transport in transport_csv:
            ...
            initial_quantities = convert_dict(int, float, transport['InitialTransportedQuantities'])
            initial_values = convert_dict(int, float, transport['InitialTransportedValues'])
            co2_unit_emissions = transport_item['CO2UnitEmissionsSchedule'].get(
                0,
                variables_default_values['Transport']['CO2UnitEmissions']
            )
            default_value = variables_default_values['Transport']['InitialTransportedValues']
            time_steps = reversed([
                (-i, t)
                for i, t
                in enumerate(
                    sorted(
                        (
                            t
                            for t, q
                            in initial_quantities.items() if 0.0 < q
                        ),
                        reverse=True
                    ),
                    1
