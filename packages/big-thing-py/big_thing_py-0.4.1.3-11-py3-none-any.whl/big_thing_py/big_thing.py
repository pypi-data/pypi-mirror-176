...
class SoPBigThing(SoPThing):
    def __init__(self, name: str, service_list: List[SoPService], alive_cycle: float, is_super: bool = False, is_parallel: bool = True,
                 ip: str = None, port: int = None, ssl_ca_path: str = f'{get_project_root()}/CA', ssl_enable: bool = True, log_name: str = None, log_enable: bool = True, log_mode: SoPPrintMode = SoPPrintMode.ABBR, append_mac_address: bool = True):
    ...
    def _handle_mqtt_message(self, msg: mqtt.MQTTMessage) -> None:
        ...
        print(f'[_hamdle_mqtt_message] recv_time: {msg.timestamp}')
    ...
    def _send_TM_RESULT_EXECUTE(self, function_name: str, thing_name: str,
                                return_type: SoPType, return_value: Union[str, float, bool], scenario: str, error: SoPErrorType) -> None:
        self._publish_queue.put(SoPExecuteResultMessage(
            function_name, thing_name, scenario, return_type, return_value, error).mqtt_message())
    ...
    def _subscribe_init_topics(self, thing_name: str):
        topic_list = [
            SoPProtocolType.Base.MT_RESULT_REGISTER.value % thing_name,
            SoPProtocolType.Base.MT_RESULT_UNREGISTER.value % thing_name,
            SoPProtocolType.Base.MT_RESULT_BINARY_VALUE.value % thing_name]
    ...
    def _subscribe_service_topics(self, thing_name: str, function_list: List[SoPFunction]):
        ...
        for function in function_list:
            ...
            topic_list += [SoPProtocolType.Base.MT_EXECUTE.value % (function_name, thing_name, '+', '#'),
                           (SoPProtocolType.Base.MT_EXECUTE.value % (function_name, thing_name, '', '')).rstrip('/')]
