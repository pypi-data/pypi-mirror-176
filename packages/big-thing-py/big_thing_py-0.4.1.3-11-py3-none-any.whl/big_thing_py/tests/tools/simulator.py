...
class Simulator():
        ...
        for middleware in self._middleware_list:
            for scenario in middleware._scenario_list:
                ...
                if len(scenario_latency_list) > 0:
                    ...
                    raise Exception(
                        f'scenario latency list is empty. scenario: {scenario._name}')
    ...
    def print_simulation_result(self, target_middleware: SoPMiddlewareElement):
        ...
        while True:
            ...
            if user_input == 0:
                ...
                # print(
                #     f'== Service Utilization ==     (*은 해당 service를 호출하는 local scenario가 현재 미들웨어에 없음을 의미)')
                print(
                    f'== Service Utilization ==')
                header = [
                    'thing(is_parallel)', 'service(call count)', 'energy consumption', 'utilization']
                table = []
                ...
                header = ['time', 'duration', 'event_type', 'level', 'middleware',
                          'thing', 'service(delay)', 'scenario(period)', 'result', 'return_value', 'return_type']
                table = []
    ...
    def export_simulation_result(self):
        ...
        for middleware in self._middleware_list:
            ...
            for thing in middleware._thing_list:
                for function in thing._function_list:
                    utilization = function['utilization']
                    if utilization > 0:
                        service_utilization_table.append(utilization)
            middleware_result.append(np.var(service_utilization_table))
            ...
            middleware_result.append(0 if (len(super_scenario_duty_ratio_table) == 0) else np.average(
                super_scenario_duty_ratio_table))
            middleware_result.append(
                False if False in super_scenario_fail_table else None if (len(super_scenario_fail_table) == 0) else True)
        ...
        f = open(
            f'{os.path.dirname(self._simulation_file_path)}/{os.path.basename(self._simulation_file_path)}_result.csv', 'w')
        wr = csv.writer(f)
        ...
        try:
            service_utilization_avg = f'{np.average([result[1] for result in simulation_result_table].remove(0.0)):0.5f}'
        except ValueError:
            service_utilization_avg = f'{np.average([result[1] for result in simulation_result_table]):0.5f}'

        try:
            scenario_duty_ratio_avg = f'{np.average([result[2] for result in simulation_result_table].remove(0.0)):0.5f}'
        except ValueError:
            scenario_duty_ratio_avg = f'{np.average([result[2] for result in simulation_result_table]):0.5f}'
        super_scenario_duty_ratio_avg = f'{np.average(simulation_result_table[0][4]):0.5f}'
        wr.writerow([simulation_config_name,
                     service_utilization_avg,
                     scenario_duty_ratio_avg,
                     super_scenario_duty_ratio_avg])
        f.close()
    ...
    def show_simulation_result(self, user_interaction: bool = True):
        ...
        while user_interaction:
            ...
            self.print_simulation_result(middleware)
