...
async def package_oci_archive(request: OciArchiveRequest, skopeo: SkopeoTool) -> OciArchive:
    skopeo = await Get(
        DownloadedExternalTool,
        ExternalToolRequest,
        skopeo.get_request(Platform.current),
    )
...
async def package_oci_image(field_set: ImageFieldSet) -> BuiltPackage:
    ...
    artifact = BuiltPackageArtifact(relpath=f"{name}.{suffix}")
