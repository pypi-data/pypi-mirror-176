...
def test_deeply_focussed_backtrack_bug():
    sample_json ={
        "workspaces": [
            {
                "wsid": "1",
    ...
    results = [{'wsid': '1', 'wsname': 'firstworkspace', 'reportname': 'r1ws1'},
               {'wsid': '1', 'wsname': 'firstworkspace', 'reportname': 'r2ws1'},
               {'wsid': '1', 'wsname': 'firstworkspace', 'reportname': 'r1ws1'},
               {'wsid': '2', 'wsname': 'secondworkspace', 'reportname': 'r1ws2'},
               {'wsid': '2', 'wsname': 'secondworkspace', 'reportname': 'r2ws2'},
               {'wsid': '2', 'wsname': 'secondworkspace', 'reportname': 'r1ws2'}]

    for match, result in zip(jertl.match_all(pattern, sample_json), results):
        assert match.bindings == result
