...
class AddDllDirectory:
    ...
    def open(self):
        ...
        self.add_dll_dir = add_dll_directory(self.dir) if USE_ADD_DLL else None
        self.active = True
...
def _import_ganessa(f, bth, dlls, test=True):
    ...
    try:
        if f == dlls[1]:
            ...
            TRIALS = ('_pygan_th2021', '_pygan_th2018', '_pygan_th2017',
                      '_pygan_th2016b', '_pygan_th2016', '_pygan_th2015',
                ) if bth else (
                      '_pygansim2022', '_pygansim2021',
        ...
        return False if test else (False, '')
    else:
        if debuglevel > 0:
