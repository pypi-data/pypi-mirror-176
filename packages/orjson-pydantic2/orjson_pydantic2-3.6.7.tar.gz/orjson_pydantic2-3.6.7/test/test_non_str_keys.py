...
class NonStrKeyTests(unittest.TestCase):
    def test_dict_keys_duplicate(self):
        ...
        """
        self.assertEqual(
            orjson_pydantic.dumps({"1": True, 1: False}, option=orjson_pydantic.OPT_NON_STR_KEYS),
            b'{"1":true,"1":false}',
        )

    def test_dict_keys_int(self):
        self.assertEqual(
            orjson_pydantic.dumps({1: True, 2: False}, option=orjson_pydantic.OPT_NON_STR_KEYS),
            b'{"1":true,"2":false}',
        )

    def test_dict_keys_substr(self):
        self.assertEqual(
            orjson_pydantic.dumps({SubStr("aaa"): True}, option=orjson_pydantic.OPT_NON_STR_KEYS),
            b'{"aaa":true}',
        )
    ...
    def test_dict_keys_substr_passthrough(self):
        ...
        self.assertEqual(
            orjson_pydantic.dumps(
                {SubStr("aaa"): True},
                option=orjson_pydantic.OPT_NON_STR_KEYS | orjson_pydantic.OPT_PASSTHROUGH_SUBCLASS,
            ),
            b'{"aaa":true}',
        )
    ...
    def test_dict_keys_substr_invalid(self):
        with self.assertRaises(orjson_pydantic.JSONEncodeError):
            orjson_pydantic.dumps({SubStr("\ud800"): True}, option=orjson_pydantic.OPT_NON_STR_KEYS)
    ...
    def test_dict_keys_strict(self):
        ...
        self.assertEqual(
            orjson_pydantic.dumps(
                ...
                option=orjson_pydantic.OPT_NON_STR_KEYS | orjson_pydantic.OPT_STRICT_INTEGER,
            ),
    ...
    def test_dict_keys_int_range_invalid(self):
        ...
        """
        with self.assertRaises(orjson_pydantic.JSONEncodeError):
            orjson_pydantic.dumps({-9223372036854775809: True}, option=orjson_pydantic.OPT_NON_STR_KEYS)
        with self.assertRaises(orjson_pydantic.JSONEncodeError):
            orjson_pydantic.dumps({18446744073709551616: True}, option=orjson_pydantic.OPT_NON_STR_KEYS)

    def test_dict_keys_float(self):
        self.assertEqual(
            orjson_pydantic.dumps({1.1: True, 2.2: False}, option=orjson_pydantic.OPT_NON_STR_KEYS),
            b'{"1.1":true,"2.2":false}',
        )

    def test_dict_keys_inf(self):
        self.assertEqual(
            orjson_pydantic.dumps({float("Infinity"): True}, option=orjson_pydantic.OPT_NON_STR_KEYS),
            b'{"null":true}',
        )
        self.assertEqual(
            orjson_pydantic.dumps({float("-Infinity"): True}, option=orjson_pydantic.OPT_NON_STR_KEYS),
    ...
    def test_dict_keys_nan(self):
        self.assertEqual(
            orjson_pydantic.dumps({float("NaN"): True}, option=orjson_pydantic.OPT_NON_STR_KEYS),
            b'{"null":true}',
        )

    def test_dict_keys_bool(self):
        self.assertEqual(
            orjson_pydantic.dumps({True: True, False: False}, option=orjson_pydantic.OPT_NON_STR_KEYS),
    ...
    def test_dict_keys_datetime_passthrough(self):
        ...
        self.assertEqual(
            orjson_pydantic.dumps(
                {datetime.datetime(2000, 1, 1, 2, 3, 4, 123): True},
                option=orjson_pydantic.OPT_NON_STR_KEYS | orjson_pydantic.OPT_PASSTHROUGH_DATETIME,
            ),
            b'{"2000-01-01T02:03:04.000123":true}',
        )
    ...
    def test_dict_keys_date(self):
        self.assertEqual(
            orjson_pydantic.dumps(
                {datetime.date(1970, 1, 1): True}, option=orjson_pydantic.OPT_NON_STR_KEYS
            ),
            b'{"1970-01-01":true}',
        )
    ...
    def test_dict_keys_time_err(self):
        ...
        with self.assertRaises(orjson_pydantic.JSONEncodeError):
            orjson_pydantic.dumps({val: True}, option=orjson_pydantic.OPT_NON_STR_KEYS)

    def test_dict_keys_str(self):
        self.assertEqual(
            orjson_pydantic.dumps({"1": True}, option=orjson_pydantic.OPT_NON_STR_KEYS),
            b'{"1":true}',
        )
    ...
    def test_dict_keys_dataclass_hash(self):
        ...
        with self.assertRaises(orjson_pydantic.JSONEncodeError):
            orjson_pydantic.dumps(obj, option=orjson_pydantic.OPT_NON_STR_KEYS)
    ...
    def test_dict_keys_unknown(self):
        obj = {frozenset(): True}
        with self.assertRaises(orjson_pydantic.JSONEncodeError):
            orjson_pydantic.dumps({frozenset(): True}, option=orjson_pydantic.OPT_NON_STR_KEYS)
