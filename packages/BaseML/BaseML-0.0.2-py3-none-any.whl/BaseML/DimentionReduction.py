...
class rdc(baseml):  # reduction
    """BaseML中的降维模块,包含['PCA', 'LDA', 'LLE']降维算法.

    Attributes:
        algorithm: 算法名称
        model: 实例化的模型
    """
    def __init__(self, algorithm = 'PCA', n_components = 2, params = {}):
        """rdc类的构造函数

        Args:
            algorithm (str, optional): 采用的降维算法. Defaults to 'PCA'.
            n_components (int, optional): 降维后保留的特征数. Defaults to 2.
            params (dict, optional): params (dict, optional): 参数字典,可自定义参数放入,参数名称详见sklearn官方文档. Defaults to {}.
        """

        super(rdc,self).__init__()   # 继承父类的构造方法
