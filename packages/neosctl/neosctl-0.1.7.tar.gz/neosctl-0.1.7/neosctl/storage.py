...
def _load_statement(statement: typing.Optional[str], statement_filepath: typing.Optional[str]):
    ...
    if statement_filepath:
        fp = pathlib.Path(statement_filepath)
        if not fp.exists():
            util.send_output(
...
def _load_params(params_filepath: typing.Optional[str]):
    params = None
    if params_filepath:
        fp = pathlib.Path(params_filepath)
        if not fp.exists():
            util.send_output(
                msg="Can not find file: {}".format(fp),
                exit_code=1,
        ...
        with fp.open() as f:
            try:
                ...
                util.send_output(
                    msg="Invalid params file, must be json format.",
                    exit_code=1,
                )
    return params
...
def fetch(
    ctx: typer.Context,
    statement: str = typer.Option(None, "--statement", "-s", help="pSQL statement"),
    statement_filepath: str = typer.Option(
        None,
        "--statement-filepath",
        "-sf",
        help="Filepath for statement sql file.",
    ),
    params_filepath: str = typer.Option(
        None,
        "--params-filepath",
        "-pf",
        help="Filepath for statement params json file.",
