...
def prepare_report_data_host(serials_dict: dict, nornir: bool = False) -> dict:
    ...
    for item in serials_dict.values():
        data["host"].append(item["host"])

    # Return the data dict if Nornir is not used
    if not nornir:
        return data

    # Define dict key for the Nornir data
    data["switch_member"] = []
    data["desired_version"] = []
    data["current_version"] = []
    # Append the Nornir data values for each defined dict key
    for header in data:
        success = False
        for item in serials_dict.values():
            for key, value in item["nr_data"].items():
                if header == key:
                    if key in data:
                        ...
                        success = True
        # If nothing was appended to the data dict, append an empty string
        if not success:
            data[header].append("")
...
def generate_cisco_maintenance_report(
    ...
    title_text = "title_text_with_tss" if tss_report else "title_text"
    ...
    print(f"-> Saved information about {df.shape[0]} serials to {report_file}")
