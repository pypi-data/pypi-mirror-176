...
def _init_rest_tracking(session: Session):
    """init_rest_tracking.
    ...
    """
    host_creds_builder = session.get_mlfoundry_host_creds_builder()
    ...
    host_creds = host_creds_builder()
    mlflow.set_tracking_uri(host_creds.host)
...
def get_client(
    ...
    if tracking_uri and tracking_uri.startswith("file:"):
        ...
        session = Session()
        _init_rest_tracking(session=session)
