...
def populate_db_unit_dispatch(start_date, end_date, raw_data_cache):
    ...
    as_bid_metrics = \
        as_bid_metrics.loc[:, ["INTERVAL_DATETIME", "DUID", "MAXAVAIL", "ROCUP", "ROCDOWN", "PASAAVAILABILITY"]]
    after_dispatch_metrics = fetch_data.get_duid_availability_data(start_date, end_date, raw_data_cache)
    unit_time_series_metrics = preprocessing.calculate_unit_time_series_metrics(as_bid_metrics, after_dispatch_metrics)
    unit_time_series_metrics["INTERVAL_DATETIME"] = unit_time_series_metrics["INTERVAL_DATETIME"].dt.strftime(
        "%Y-%m-%d %X"
    )
...
if __name__ == "__main__":
    ...
    # populate_supabase_bid_table(
    #     "2020/01/01 00:00:00", "2020/02/01 00:00:00", raw_data_cache
    # )
    # populate_supabase_region_data(
    #     "2020/01/01 00:00:00", "2020/02/01 00:00:00", raw_data_cache
    # )
