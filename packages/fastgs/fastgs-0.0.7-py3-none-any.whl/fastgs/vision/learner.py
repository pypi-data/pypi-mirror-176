...
def _show_one_result(img: TensorImageMS, msk: TensorMask, out: TensorMask, row, mskovl, **kwargs):
    if mskovl:
        rowl: int = len(row) / 2
        return [msk.show(ctx=c, **kwargs) for c in img.show(ctxs=row[:rowl],**kwargs)]
        + [out.show(ctx=c, **kwargs) for c in img.show(ctxs=row[rowl:],**kwargs)]
    else:
