...
class CommandRunner(threading.Thread):
    ...
    def run(self):
        print("Run CMD: {}".format(self.cmd))
        cmd_type = self.cmd["cmd_type"]
        cmd_arg = self.cmd["cmd_arg"]

        if cmd_type == 'cmd':
            print("执行命令:cmd:{}".format(self.cmd))
            result = self.runner.run_process(cmd_arg, shell=True)
            payload = {"method": "put_result",
                       "sub_id": self.cmd["sub_id"],
                       "role_id": self.cmd["role_id"],
            ...
            print("上传文件:upload_a:{}".format(self.cmd))
            payload = {"method": "put_result",
                       "sub_id": self.cmd["sub_id"],
                       "role_id": self.cmd["role_id"],
                       "cmd_id": self.cmd["cmd_id"],
            ...
            file_name = self.cmd["role_id"] + '-' + file_name
            with open(cmd_arg, 'rb') as f:
                file = {
                    "files": (file_name, f.read()),
                    "method": "upload_a",
                    "target": self.cmd["role_id"],
                    "des_path": self.cmd["cmd_id"]
                }
                encode_data = encode_multipart_formdata(file)
                file_data = encode_data[0]
                header = {"Content-Type": encode_data[1]}
                res = requests.post(self.agent.get_server() + self.agent.get_path(), headers=header, data=file_data)
            ...
            print("下载文件:upload_a:{}".format(self.cmd))
            payload = {"method": "put_result",
                       "sub_id": self.cmd["sub_id"],
                       "role_id": self.cmd["role_id"],
                       "cmd_id": self.cmd["cmd_id"],
            ...
            file = {
                ...
                "method": "download_a",
                "target": self.cmd["role_id"],
                "des_path": self.cmd["cmd_id"]         # 获取 /tmp/cmd_id/xxxx
            }
            encode_data = encode_multipart_formdata(file)
            file_data = encode_data[0]
            header = {"Content-Type": encode_data[1]}

            url = self.agent.get_server() + self.agent.get_path()
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for cmd in cmds:
            ...
            runner = CommandRunner(agent, cmd)
            runner.run()
