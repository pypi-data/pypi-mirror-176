...
from .scripts import merge_ipynb
    ...
    async def post(self):
        ...
        try:
            ...
            if base_content is not None:
                ...
                file_path = Path(os.path.expanduser(data.get("fileAbsPath")))

                with tempfile.TemporaryDirectory() as tempdir:
                    base_path = os.path.join(tempdir, f"{file_path.name}-base")
                    current_path = os.path.join(tempdir, f"{file_path.name}-current")
                    incoming_path = os.path.join(tempdir, f"{file_path.name}-incoming")

                    with open(base_path, "w") as f:
                        json.dump(json.loads(base_content), f)
                    with open(current_path, "w") as f:
                        json.dump(json.loads(prev_content), f)
                    with open(incoming_path, "w") as f:
                        json.dump(json.loads(curr_content), f)

                    merge_ipynb.main(
                        base_file_path=base_path, current_file_path=current_path, incoming_file_path=incoming_path, file_path=str(file_path)
                    )
