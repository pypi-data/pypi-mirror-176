...
def Seismic_Waterfall(data,t,xlim=[0,15],ylim=[0,1200],AmpMult=10,recInt=10,winX=15,winY=10,color='k'):
    '''
...
def Seismic_SEL(data,rng,Fs,sens=1e5/19.7,plotFlag=True,logFlag=True):
    ...
    if plotFlag == True:
        if logFlag == True:
            ...
            plt.ylim(yLims)
            if MammalFlag == True:
                plt.plot(rng,168*np.ones(len(rng)),'r--',linewidth=3)
                plt.plot(rng,183*np.ones(len(rng)),'r:',linewidth=3)
                plt.title('SEL (Marine Mammal Weighted)')
                plt.legend(['SEL-Weighted','TTS','PTS'])
            else:
                plt.title('Sound Exposure Level')
            plt.show()
        else:
            fig = plt.figure()
            ax = plt.gca()
            ...
            plt.ylim(yLims)
            if MammalFlag == True:
                plt.plot(rng,168*np.ones(len(rng)))
                plt.plot(rng,183*np.ones(len(rng)))
                plt.title('SEL (Marine Mammal Weighted)')
                plt.legend(['SEL-Weighted','TTS','PTS'])
            else:
                plt.title('Sound Exposure Level')
...
def time_stacking(nav,data,DATA,SSP,d_layer):
    ...
    time = np.linspace(0,(data.shape[0]-1)/Fs,data.shape[0])
    f = np.linspace(0,Fs,nfft)

    # srcBath = nav.depth[ssp_i]
    Time = str(header.hour)+':'+str(header.minute)+':'+str(header.second)
    JulianDay = header.julian_day
    Year = header.year
...
def ref_coeff(data,offset,depth,Fs,spreadFlag = False, beamdata = False, c = 1485.0, tleft = 0.25, tright = 1.75):
    ...
    for i in range(nrec):
        ...
        freq_energy = np.abs(np.fft.fft(data[i,:],samples,1))[0:int(samples/2)]
        ...
        freq_direnergy = np.abs(np.fft.fft(datatmp,tsamp,1))
        ...
        freq_refenergy = np.abs(np.fft.fft(datatmp,tsamp,1))
        ...
        tsamp = ti[1]-ti[0]
        datatmp = np.append(data[i,ti[0]:ti[1]],np.zeros(samples-tsamp))
        beamtmp = np.interp(np.linspace(0,Fs,tsamp), np.arange(0,Fs/2,Fs/2), beamdata[angle_i2,:])
        freq_ref2energy = np.abs(np.fft.fft(datatmp,tsamp,1))
