...
class Download:
    ...
    def download_nucleus_data(self, fid=None, sa=None, length=None) -> bool:
        ...
        if status:
            #self.logger.set_path(path=self._path + '/nucleus')

            #folder = self.logger.start()
    ...
    def convert_nucleus_data(self, path) -> bool:
        ...
        self.logger.set_path(path=path_folder + '/nucleus_converted')

        folder = self.logger.start(_converting=True)

        # TODO: THIS RELIES ON GET ALL DATA BEING IN THE SAME FILE AS BINARY DATA
        with open(path, 'rb') as raw_file:
            ...
            '''
            get_all = b''
            get_all_detected = False
            for _ in range(3000):

                #byte = raw_file.read(1)
                get_all += raw_file.read(1)

                #if byte == b'':
                #    self.messages.write_warning(message='Reached end of file without detecting GET_ALL')
                #    return False

                #get_all += byte

                #if len(get_all) > 3000:
                #    self.messages.write_message(message='Did not find GEL_ALL info at the start of the binary file. proceeding with parsing')
                #    break

                if b'\r\n\x00' in get_all:
                    get_all_detected = True
                    break

            else:
                self.messages.write_message(message='Did not find GEL_ALL info at the start of the binary file. proceeding with data parsing')

            if get_all_detected:
                ascii_queue_status = self.parser.get_queuing()['ascii']
                self.parser.set_queuing(ascii=True)

                self.parser.add_data(get_all)
                nucleus_get_all = None

                while True:
                    data = self.parser.read_ascii()

                    if data is None:
                        break
                    elif data['bytes'][0] == 0x20:
                        nucleus_get_all = data['bytes'][1:].rstrip(b'\x00')

                self.parser.set_queuing(ascii=ascii_queue_status)

                if nucleus_get_all is not None:
                    with open(folder + '/get_all.txt', 'w') as get_all_file:
                        get_all_file.writelines(nucleus_get_all.decode())
                else:
                    self.messages.write_warning('could not retrieve GET_ALL data from file')
                    return False
            ...
            packet_queue_status = self.parser.get_queuing()['packet']
            ascii_queue_status = self.parser.get_queuing()['ascii']
            self.parser.set_queuing(packet=False, ascii=True)
            ...
            while True:
                ...
                if not self.parser.ascii_queue.empty() and not get_all_detected:
                    data = self.parser.read_ascii()

                    if data['bytes'][0] == 0x20:
                        get_all = data['bytes'][1:].rstrip(b'\x00')
                        with open(folder + '/get_all.txt', 'w') as get_all_file:
                            get_all_file.writelines(get_all.decode())

                        get_all_detected = True

            if not get_all_detected:
                self.messages.write_warning('GEL_ALL data was not detected in binary file!')

            self.logger.stop()

            self.parser.set_queuing(packet=packet_queue_status, ascii=ascii_queue_status)
