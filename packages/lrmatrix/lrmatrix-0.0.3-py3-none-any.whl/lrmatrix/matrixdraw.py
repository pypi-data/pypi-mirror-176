	...
	for r in range( n_rows_ ):
		...
		if legend.ndim >= 2:
				...
				for c in range( legend.shape[ 1 ] ):
					...
					# if draw_box: g_si.append( draw.Rectangle( 0 + pad + width_sum, str( row_scale( r ) ), width, rec_height, shape_rendering = 'crispEdges', fill = fill, stroke = stroke, stroke_width = stroke_width ) )
...
# 	cols_top_legend, cols_top_legend_title,

# 	rows_left_legend_1 = None, rows_left_legend_title_1 = None,

# 	rows_right_legend_1 = None, rows_right_legend_1_title = None, rows_right_legend_2 = None, rows_right_legend_2_title = None,
...
def draw_range_matrix( matrix, col_values_min, col_values_max, matrix_row_categories, category_names, # change to draw_matrix
	...
	rec_width_fix = 1.5, rec_height_fix = 1.5, background_color = '#ffffff', cell_background = 'none', cell_background_color = 'auto', draw_range_box = False, draw_box_frame = True, draw_legend = True, stroke = '#000000', stroke_width = 2,
			...
			if ( alpha != 0.0 ) or ( beta != 0.0 ):
				...
				if not draw_deltas:
					...
					if draw_distribution == True:
						...
						b_str = ( c * bins )
						b_end = ( c * bins ) + bins
						g_si.append( Title( text = np.array2string( distribution_matrix[ r, b_str:b_end ], precision = precision_, separator = ',', suppress_small = True ) ) )
					...
					if ( draw_distribution == False ) or ( draw_range_box == True ):
						...
						if rec_width_fix > rec_width_p: 

							rec_width_p += rec_width_fix # range too small, not even showing
						...
						if 1 - rec_width_p / rec_width < 0.05: rec_width_p -= rec_width_fix # range too big, leading to the conclusion of being the complete range, occupying all cell width
						...
						else: g_si.append( draw.Rectangle( c_p + ( inner_pad_col_ * c ), str( row_scale( r ) + ( inner_pad_row_ * r ) ), rec_width_p, rec_height, shape_rendering = 'crispEdges', fill = fill_aux, stroke = stroke, stroke_width = stroke_width * 0.5 ) )
	...
	# draw_rows_left_legend( g_left_legend, rows_left_legend_1, rec_height, rows_left_legend_1_title, pad = pad, draw_rows_line = draw_rows_line, font_size = font_size, stroke_width = stroke_width ) # ??
	...
	if rows_right_legend_2 is not None: 
		...
		# draw_rows_right_legend( g_right_legend, rows_right_legend_2, rec_height, rows_right_legend_2_title, pad = pad, draw_change_line = draw_change_line, font_size = font_size, stroke_width = stroke_width, draw_box = draw_box_row_right_legend, draw_frame = draw_frame_right_legend ) ??
		pad += 95
