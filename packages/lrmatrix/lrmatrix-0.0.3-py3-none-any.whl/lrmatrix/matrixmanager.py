...
class MatrixManager( RuleVectorMatrix ):
	...
	def rule_extration( self, tree_array, X, y, feature_importances = None, calc_support_coverage = False, calc_diameters = True, n_jobs = 1 ):
		...
		# if feature_importances is None: self.feature_importances_ = np.zeros( self.n_features_ ) # ??
		# else: self.feature_importances_ = feature_importances # ??
	...
	def __path_to_rule( self, skl_tree, tree_id , node, rule, rules_matrix ):
		...
		if ( left_child == tree._tree.TREE_LEAF ):
			...
			if self._verbose > 2: print( 'rule extracted ' + str( self.n_rules_ ), rule_copy )	
	...
	def calc_histograms( self, X, y, mode = 'used-features', norm = True, bins = None ):
		...
		# self.feature_values_min_ = np.array( [ np.amin( X[:, i] ) for i in range( X.shape[1] ) ] ) # see rule_extraction # ??
		# self.feature_values_max_ = np.array( [ np.amax( X[:, i] ) for i in range( X.shape[1] ) ] )
	...
	def explanation( self, exp_type = 'global', rules = None, x_k = None, X_s = None, r_model = None, r_node = None, r_support_min = None,  r_certainty_min = None, r_pvalue_max = None, r_class = None, r_order = 'raw', f_importance_min = None, f_order = 'raw', r_label_format = 'auto', data_coverage_max = None, info_text = None, show_rule_certainty = 'auto', show_rule_fisher_pvalue = 'auto', show_rule_data_coverage = 'auto', show_feature_importance = 'auto', show_feature_range = False, show_global_histograms = 'auto', draw_distribution = 'auto', show_info_text = True ):
		...
		if data_coverage_max is not None: # must be here because the coverage cut takes rules order in consideration
			...
			cumulative_data_coverage = np.copy( self.rules_matrix_[ rules, self.COVERAGE ].toarray()[ :, 0 ] )
		...
		rule_labels = [ r_label_format.replace( '#num', str( r + 1 ) ) for r in self.rules_matrix_[ rules, self.ID ].toarray()[ :, 0 ].astype(int) ]
		...
		if show_rule_data_coverage:
			...
			if cumulative_data_coverage is None: # ??
				...
				rule_data_coverages = np.copy( self.rules_matrix_[ rules, self.COVERAGE ].toarray()[ :, 0 ] )
		...
		else: rule_data_coverages = None # data_coverages ??
		...
		if show_rule_fisher_pvalue: rule_pvalues = self.rules_matrix_[ rules, self.FISHERPVALUE ].toarray()[ :, 0 ]
		...
		for f in features:
			...
			if show_feature_importance: label += ' | ' + str( np.round( self.feature_importances_[ f ], decimals = self.precision_ ) ) # and ( self.feature_importances_ is not None ) ??
		...
		if show_feature_importance: feature_importances = self.feature_importances_[ features ] #  and ( self.feature_importances_ is not None ) ??
		...
		try:
			...
			if npz_format == False:
				...
				if self.rules_ranges_diameter_ is not None:
					...
					with open( file + '-rules_ranges_diameter_.csv', 'w', encoding = 'utf-8' ) as csv_file:
				...
				if self.histograms_matrix_ is not None:
					...
					with open( file + '-histograms_matrix_.csv', 'w', encoding = 'utf-8' ) as csv_file:
				...
				if self.rules_ranges_diameter_ is not None:
					...
					ssp.save_npz( file + '-rules_ranges_diameter_.npz', self.rules_ranges_diameter_.tocoo() )					
				...
				if self.histograms_matrix_ is not None:
					...
					ssp.save_npz( file + '-histograms_matrix_.npz', self.histograms_matrix_.tocoo() )					
	...
	def load( self, file, npz_format = True ):
		...
		self.aggregation_metadata_ = np.loadtxt( file + '.csv', delimiter = ';', skiprows = 3, max_rows = 1, dtype = 'str' ).tolist()
		if self._verbose > 0: print( 'aggregation_metadata loaded', self.aggregation_metadata_ )


		self.feature_names_ = np.loadtxt( file + '.csv', delimiter = ';', skiprows = 5, max_rows = 1, dtype = str )
		if self._verbose > 0: print( 'feature_names loaded', self.feature_names_.shape )


		self.feature_values_min_ = np.loadtxt( file + '.csv', delimiter = ';', skiprows = 7, max_rows = 1 )
		if self._verbose > 0: print( 'feature_values_min loaded', self.feature_values_min_.shape )

		
		self.feature_values_max_ = np.loadtxt( file + '.csv', delimiter = ';', skiprows = 9, max_rows = 1 )
		if self._verbose > 0: print( 'feature_values_max loaded', self.feature_values_max_.shape )

		
		self.features_used_ = np.loadtxt( file + '.csv', delimiter = ';', skiprows = 11, max_rows = 1, dtype = int )
		if self._verbose > 0: print( 'features_used loaded', self.features_used_.shape )


		self.feature_importances_ = np.loadtxt( file + '.csv', delimiter = ';', skiprows = 13, max_rows = 1 )
		if self._verbose > 0: print( 'feature_importances loaded', self.feature_importances_.shape )
		

		self.class_names_ = np.loadtxt( file + '.csv', delimiter = ';', skiprows = 15, max_rows = 1, dtype = str )
		if self._verbose > 0: print( 'class_names loaded', self.class_names_.shape )


		self.class_instances_count_ = np.loadtxt( file + '.csv', delimiter = ';', skiprows = 17, max_rows = 1, dtype = int )
		...
		if npz_format == False:
			...
			self.rules_matrix_ = ssp.lil_matrix( np.loadtxt( file + '.csv', delimiter = ';', skiprows = 19, dtype = np.float64 ) )
		...
		if npz_format == False:
			...
			try:
				...
				self.histograms_matrix_ = ssp.lil_matrix( np.loadtxt( file + '-histograms_matrix_.csv', delimiter = ';' ), dtype = int )
			...
			try:
				...
				if self._verbose > 0: print( 'loading histograms_matrix_ ...' )
				self.histograms_matrix_ = ssp.load_npz( file + '-histograms_matrix_.npz' ).tolil()
				if self._verbose > 0: print( 'histograms_matrix_ loaded' )
