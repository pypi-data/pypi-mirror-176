...
class SymAPIUnauthorizedError(SymAPIError):
    def __init__(self, error_message: str) -> None:
        super().__init__(
            message="You are not authorized to perform this action.", hints=error_message
        )
...
class SymAPIUnknownError(SymAPIRequestError):
    ...
    def __init__(
        self, response_code: int, request_id: str, message: Optional[str] = None
    ) -> None:
        if message:
            super().__init__(
                f"An unexpected error occurred ({response_code}): {message}", request_id
            )
        else:
            super().__init__(
