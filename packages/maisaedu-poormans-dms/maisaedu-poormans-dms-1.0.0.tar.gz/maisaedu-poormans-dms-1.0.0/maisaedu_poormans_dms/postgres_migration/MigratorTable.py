...
class MigratorTable:
    ...
    def __get_source_create_table_code(self):
        query = f"""
            ...
            (
            '::text) || array_to_string(array_agg((((('    '::text || '"' || tabledefinition.column_name::text) || '"' || ' '::text) || tabledefinition.type) || ' '::text) || tabledefinition.not_null), ',
            '::text)) || '
            );
            '::text AS code
    ...
    def __create_target_table(self, code_create_table):
        ...
        execute(
            self.target_conn,
            f"ALTER TABLE {self.target_schema_name}.{self.target_table_name} OWNER TO dataeng;",
        )
    ...
    def create_table(self, source_table_name: str, target_table_name: str) -> None:
        ...
        if self.__check_if_target_table_already_exist():
            ...
            prefect.context.logger.info(f"Table {target_table_name} created")
            prefect.context.logger.info(
