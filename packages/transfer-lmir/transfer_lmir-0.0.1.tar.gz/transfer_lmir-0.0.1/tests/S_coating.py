...
# rg.colorspectra(X2,Y2)

# Autoscale
X1, xm1, _ =rg.autoscale(X1)
X2, xm2, _ =rg.autoscale(X2)
...
#         T, U, P, W, Q, B, Yhat, SSX, SSY = rg.PLS(Xcv,Ycv, nbPC)  
...
nbPC = 1
T, U, P, W, Q, B, Yhat, SSX, SSY = rg.PLS(X2c,Y2c,nbPC)
Wstar = W@np.linalg.inv(P.T@W)
beta = Wstar@B@Q.T

Yhat_c = (X2c@beta)*ys1+ym1
R2_c = rg.R2(Y2c_0,Yhat_c)
E = Yhat_c - Y2c_0
RMSEC = np.sqrt(np.mean(E**2))

Yhat_v = (X2v@beta)*ys1+ym1
R2_v = rg.R2(Y2v_0,Yhat_v)
E = Yhat_v - Y2v_0
RMSEV = np.sqrt(np.mean(E**2))

print('\nResults for PLS : ')
print('R2 calibration: ', np.round(R2_c,4))
print('RMSE calibration: ', np.round(RMSEC,4))

print('R2 test: ', np.round(R2_v,4))
print('RMSE test: ', np.round(RMSEV,4))
...
#         beta, all_scores, y_pred_final = PPLS(Xcv,Ycv,nbPC) 
...
MSEc = np.mean(E**2)
RMSEC = np.sqrt(np.mean(E**2))  
R2 = rg.R2(Y2c_0, ypred_cal2)

print('\nResults for LMIR')
print('Calibration')
print('MSE: ', np.round(MSEc, 4))
print('RMSE: ', np.round(RMSEC, 4))
...
MSEv = np.mean(E**2)
RMSEV = np.sqrt(np.mean(E**2))  
R2v = rg.R2(Y2v_0, ypred_all2)

print('Test')
print('MSE: ', np.round(MSEv, 4))
print('RMSE: ', np.round(RMSEV, 4))
print('R2: ', np.round(R2v, 3))
...
R2_1 = rg.R2(Y1_0, ypred1)

print('\nCalibration LMIR on probe 1')
print('MSE: ', np.round(MSE, 4))
print('RMSE: ', np.round(RMSEC1, 4))
print('R2: ', np.round(R2_1, 4))
...
MSEv = np.mean(E**2)
RMSEV = np.sqrt(np.mean(E**2))  
R2_v = rg.R2(Y2v_0, ypred_all)


#%% Results

print('\nResults for calibration transfer: ')
print('R2 calibration: ', np.round(R2_ct,4))
print('RMSE calibration: ', np.round(RMSECt,4))
print('R2 test: ', np.round(R2_v,4))
print('RMSE test: ', np.round(RMSEV,4))
