...
class Participant:
    def __init__(self, data, match):
        try:
            ...
            self.individualPosition = [None, "TOP_LANE", "JUNGLE", "MID_LANE", "BOT_LANE", "UTILITY", "TOP_LANE", "JUNGLE", "MID_LANE", "BOT_LANE", "UTILITY"][self.participantId]
            ...
            self.perks = []
            self.statRunes = [ddragon_factory.runeFromId(datav4["statPerk" + str(rid)], match.version) for rid in
                              range(3)]
            self.runes = [ddragon_factory.runeFromId(datav4["perk" + str(rid)], match.version) for rid in range(6)]
            ...
            self.teamId = data["teamId"]
            self.teamPosition = [None, "TOP_LANE", "JUNGLE", "MID_LANE", "BOT_LANE", "UTILITY", "TOP_LANE", "JUNGLE", "MID_LANE", "BOT_LANE", "UTILITY"][self.participantId]
            ...
            self.totalMinionsKilled = datav4["totalMinionsKilled"]
    ...
    def lane_opponent(self):
        ...
        for participant in self._match.participants():
            if participant.teamPosition == self.teamPosition and self.summonerName != participant.summonerName:
...
class Team:
    def __init__(self, data, match):
        ...
        try:
            ...
            self.objectives = {
                ...
                'firstRiftHerald': data["firstRiftHerald"]
            }
        self.teamId = data["teamId"]
        if self.teamId == 100:
            self.side = "Blue"
    ...
    def cumulated_stats(self, fields=(), save=False):
