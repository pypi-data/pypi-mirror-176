...
# scrolling should not stop instantly
...
class Editor(tkinter.Toplevel):
	...
	def __init__(self):
		...
		if pkg_resources.resource_exists(__name__, ICONPATH):
			...
			try:
				...
				print('Could not load icon-file %s' % (realpath))
				print(e)
		
		
		if pkg_resources.resource_exists(__name__, HELPPATH):
			realpath = pkg_resources.resource_filename(__name__, HELPPATH)
			...
			try:
				...
				print('Could not load help-file %s' % (realpath))
				print(e)
		...
		self.btn_test.config(font=self.menufont, state='disabled')
		...
		self.ln = tkinter.Text(self, width=4, padx=10)
		...
		if pkg_resources.resource_exists(__name__, CONFPATH):
			realpath = pkg_resources.resource_filename(__name__, CONFPATH)
			...
			try:
				...
				print('\n Could not load existing configuration file %s' % realpath)
		...
		if self.tabindex == None:
			...
			self.btn_test.config(font=self.menufont)
			self.ln.config(font=self.font, foreground=self.fgcolor,
				background=self.bgcolor, state='disabled')
	...
	def updateLineNumbers(self):
		...
		# self.ln is linenumber-widget, ln in int-variable
		tt = self.ln
...
########## Configuration Related Begin
	...
	def save_config(self, event=None):
		...
		# at first launch this can be the case:
		if not pkg_resources.resource_exists(__name__, CONFPATH):
			
			# then build path from existing resource:
			realpath = pathlib.Path(pkg_resources.resource_filename(__name__, '__init__.py'))
			realpath = realpath.absolute()
			i = realpath.__str__().rindex(realpath.name)
			realpath = realpath.__str__()[:i] + CONFPATH
...
########## Theme Related Begin
	...
	def toggle_color(self, event=None):
		...
		self.ln.config(foreground=self.fgcolor, background=self.bgcolor)
