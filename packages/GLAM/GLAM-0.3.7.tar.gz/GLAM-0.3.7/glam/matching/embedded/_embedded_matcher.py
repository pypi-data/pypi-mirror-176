...
class EmbeddedMatcher(BaseMatcher):
    ...
    def __init__(self, data_dir):
        ...
        self.pc_path = os.path.join(self.data_dir,'PNF')
        if not os.path.exists(self.pc_path): 
            warnings.warn(f'PNF file not found at {self.pc_path}. Postcodes will not be used during matching')
            self.pc_path = None
    ...
    def match_addresses(self, addresses, build_dependencies = False):
        ...
        addresses = _predict.lookup_addresses(addresses, self.linz, self.trees, self.pc_path is not None)
        return addresses
