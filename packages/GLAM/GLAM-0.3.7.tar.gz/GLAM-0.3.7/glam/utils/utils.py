...
def download(path='glam-data'):

    _ = os.system(f'git clone https://github.com/lmor152/glam-data.git {path}')

    for fn in glob.glob(f'{path}/nz-street-address/*.zip'):
        with zipfile.ZipFile(fn, 'r') as zf:
            zf.extractall(f'{path}')

    df = pd.concat([pd.read_csv(fn, low_memory=False) for fn in glob.glob(f'{path}/nz-street-address/*.csv')], axis=1)
    df.to_csv(f'{path}/nz-street-address.csv', index=False)

    for fn in list(chain(glob.iglob(f'{path}/**'), glob.iglob(f'{path}/.**'))):
        if fn not in [f'{path}/nz-street-address.csv']:
            if os.path.isdir(fn):
                shutil.rmtree(fn)
            else:
                os.remove(fn)
