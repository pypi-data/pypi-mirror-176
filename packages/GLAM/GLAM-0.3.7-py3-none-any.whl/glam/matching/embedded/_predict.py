...
def lookup_addresses(addresses, linz, trees, postcodes, batch_size = 64):
    # embed addresses
    parsed_addresses = pd.DataFrame([x if x is not None else {} for x in addresses]).fillna('None')

    parsed_addresses['embedding'] = parsed_addresses.apply(lambda x: make_embeddings(x, postcode=postcodes), axis = 1)
...
def query_tree(embeddings, tree):
    if tree is None:
        return zip([-1]*len(embeddings),[-1]*len(embeddings))
    dist, idx = tree.query(embeddings.to_list())
    return zip(dist/tree.m,idx)
