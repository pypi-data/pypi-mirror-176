...
from krypton_cryptofeed.defines import BID, ASK, BITFINEX, BUY, CURRENCY, FUNDING, L2_BOOK, L3_BOOK, SELL, TICKER, TRADES, PERPETUAL
...
from krypton_cryptofeed.exchanges.mixins.bitfinex_rest import BitfinexRestMixin
from krypton_cryptofeed.types import Ticker, Trade, OrderBook
            ...
            for chan in set(channels or subscription):
                for pair in set(subscription[chan] if subscription else symbols or []):
                    ...
                    if (exch_sym[0] == 'f') == (chan != FUNDING):
                        LOG.warning('%s: No %s for symbol %s => krypton_cryptofeed will subscribe to the wrong channel', self.id, chan, pair)
