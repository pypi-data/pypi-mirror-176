...
from krypton_cryptofeed.connection import AsyncConnection, HTTPPoll, HTTPConcurrentPoll, RestEndpoint, Routes, WebsocketEndpoint
from krypton_cryptofeed.defines import ASK, BALANCES, BID, BINANCE, BUY, CANDLES, FUNDING, FUTURES, L2_BOOK, LIMIT, LIQUIDATIONS, MARKET, OPEN_INTEREST, ORDER_INFO, PERPETUAL, SELL, SPOT, TICKER, TRADES, FILLED, UNFILLED
from krypton_cryptofeed.feed import Feed
from krypton_cryptofeed.symbols import Symbol
from krypton_cryptofeed.exchanges.mixins.binance_rest import BinanceRestMixin
from krypton_cryptofeed.types import Trade, Ticker, Candle, Liquidation, Funding, OrderBook, OrderInfo, Balance
...
class Binance(Feed, BinanceRestMixin):
    id = BINANCE
    websocket_endpoints = [WebsocketEndpoint('wss://bnc-strm.krypton.center', sandbox='wss://testnet.binance.vision')]
    rest_endpoints = [RestEndpoint('https://binance.krypton.center', routes=Routes('/api/v3/exchangeInfo', l2book='/api/v3/depth?symbol={}&limit={}', authentication='/api/v3/userDataStream'), sandbox='https://testnet.binance.vision')]
