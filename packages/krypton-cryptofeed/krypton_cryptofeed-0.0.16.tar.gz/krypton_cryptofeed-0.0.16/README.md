...
[![Build Status](https://travis-ci.com/bmoscon/krypton_cryptofeed.svg?branch=master)](https://travis-ci.com/bmoscon/krypton_cryptofeed)
[![PyPi](https://img.shields.io/badge/PyPi-krypton_cryptofeed-brightgreen.svg)](https://pypi.python.org/pypi/krypton_cryptofeed)
[![Codacy Badge](https://api.codacy.com/project/badge/Grade/efa4e0d6e10b41d0b51454d08f7b33b1)](https://www.codacy.com/app/bmoscon/krypton_cryptofeed?utm_source=github.com&amp;utm_medium=referral&amp;utm_content=bmoscon/krypton_cryptofeed&amp;utm_campaign=Badge_Grade)
...
Please see the [examples](https://github.com/bmoscon/krypton_cryptofeed/tree/master/examples) for more code samples and the [documentation](https://github.com/bmoscon/krypton_cryptofeed/blob/master/docs/README.md) for more information about the library usage.


For an example of a containerized application using krypton_cryptofeed to store data to a backend, please see [Cryptostore](https://github.com/bmoscon/cryptostore).


## National Best Bid/Offer (NBBO)

krypton_cryptofeed also provides a synthetic [NBBO](examples/demo_nbbo.py) (National Best Bid/Offer) feed that aggregates the best bids and asks from the user specified feeds.
...
krypton_cryptofeed supports the following channels from exchanges:
...
krypton_cryptofeed supports `backend` callbacks that will write directly to storage or other interfaces.
...
**Note:** krypton_cryptofeed requires Python 3.8+

krypton_cryptofeed can be installed from PyPi. (It's recommended that you install in a virtual environment of your choosing).

    pip install krypton_cryptofeed

krypton_cryptofeed has optional dependencies, depending on the backends used. You can install them individually, or all at once. To install krypton_cryptofeed along with all its optional dependencies in one bundle:

    pip install krypton_cryptofeed[all]
...
See more discussion of package installation in [INSTALL.md](https://github.com/bmoscon/krypton_cryptofeed/blob/master/INSTALL.md).



## Rest API

krypton_cryptofeed supports some REST interfaces for retrieving real-time and historical data, as well as order placement and account management. These are integrated into the exchange classes directly. You can view the supported methods by calling the `info()` method on any exchange. The methods for interacting with the exchange RET endpoints exist in two flavors, the synchronous methods (suffixed with `_sync`) as well as the asynchronous which can be utilized with asyncio. For more information see the [documentation](docs/rest.md).


## Future Work

There are a lot of planned features, new exchanges, etc planned! If you'd like to discuss ongoing development, please join the [slack](https://join.slack.com/t/krypton_cryptofeed-dev/shared_invite/enQtNjY4ODIwODA1MzQ3LTIzMzY3Y2YxMGVhNmQ4YzFhYTc3ODU1MjQ5MDdmY2QyZjdhMGU5ZDFhZDlmMmYzOTUzOTdkYTZiOGUwNGIzYTk) or open a thread in the [discussions](https://github.com/bmoscon/krypton_cryptofeed/discussions) in GitHub.

## Contributing

Issues and PRs are welcomed!

krypton_cryptofeed wouldn't be possible without the help of many [contributors](AUTHORS.md)! I owe them and all other contributors my thanks!
