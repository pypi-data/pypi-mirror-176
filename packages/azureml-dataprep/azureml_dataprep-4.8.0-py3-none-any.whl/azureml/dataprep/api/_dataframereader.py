...
def _log_dataflow_executition_activity(
    ...
    def preppy_execution():
        ...
        try:
            ...
            if "clex_error" not in inner_telemetry_dict:
                ...
                inner_telemetry_dict["to_ndarray_with_clex_forced_error"] = repr(e)
            return UnexpectedError(e), inner_telemetry_dict
    ...
    try:
        ...
        if not force_clex:
            try:
                ...
                if not allow_fallback_to_clex:
                    logger.info(
                        "rslex failed with allow_fallback_to_clex set to false, failing the execution."
                    )
                    execution_succeeded = False
                    raise e
                ...
                logger.warn("rslex failed, falling back to clex.")
                pass
        ...
        if telemetry_dict is not None:
            ...
            _log_dataflow_executition_activity(
                activity,
                force_clex,
...
def get_partition_count_with_rslex(dataflow, span_context=None):
    _ ,partition_count, _ = _execute('_DataframeReader.get_partition_count_with_rslex', dataflow, span_context=span_context, collect_results=False, allow_fallback_to_clex=False)
                        ...
                        if column_type == pyarrow.bool_():
                            ...
                            if isinstance(column, pyarrow.ChunkedArray):
                                ...
                                try:
                                    ...
                                    message = "Failed to add colum to partition. Target type: {}, actual type: bool, partition id: {}, column idx: {}, error: {}, ".format(
                                        fields[j].type, i, j, e)
