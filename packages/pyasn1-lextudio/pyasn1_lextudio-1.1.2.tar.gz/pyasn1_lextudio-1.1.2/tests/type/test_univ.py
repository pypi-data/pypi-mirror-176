...
class BitStringTestCase(BaseTestCase):
    ...
    def testSet(self):
        ...
        assert self.b.clone(hexValue="A98A") == (
            1,
            0,
            1,
            0,
        ...
        assert self.b.clone("1010100110001010") == (
            1,
            0,
            1,
            0,
...
class OctetStringWithUnicodeMixIn(object):
    ...
    def testSerialised(self):
        assert (
            bytes(univ.OctetString(self.encodedPythonString, encoding=self.encoding))
            == self.encodedPythonString
        ), "__str__() fails"
    ...
    def testSeq(self):
        assert (
            univ.OctetString(self.encodedPythonString)[0] == self.encodedPythonString[0]
        ), "__getitem__() fails"
    ...
    def testAsInts(self):
        assert (
            univ.OctetString(self.encodedPythonString).asNumbers() == self.numbersString
        ), "testAsNumbers() fails"

    def testAdd(self):
        assert (
            univ.OctetString(self.encodedPythonString) + self.encodedPythonString
            == self.encodedPythonString + self.encodedPythonString
        ), "__add__() fails"

    def testRadd(self):
        assert (
            self.encodedPythonString + univ.OctetString(self.encodedPythonString)
            == self.encodedPythonString + self.encodedPythonString
        ), "__radd__() fails"
    ...
    def testReverse(self):
        assert list(reversed(univ.OctetString(self.encodedPythonString))) == list(
            reversed(self.encodedPythonString)
        )
...
class ObjectIdentifier(BaseTestCase):
    ...
    def testInput3(self):
        assert univ.ObjectIdentifier(univ.ObjectIdentifier("1.3") + (6,)) == (
            1,
            3,
            6,
        ), "prettyIn() fails"
...
class SequenceOf(BaseTestCase):
    def setUp(self):
        BaseTestCase.setUp(self)
        self.s1 = univ.SequenceOf(componentType=univ.OctetString(""))
    ...
    def testComponentTagsMatching(self):
        ...
        o = univ.OctetString("abc").subtype(
            explicitTag=tag.Tag(tag.tagClassPrivate, tag.tagFormatSimple, 12)
        )
        try:
...
class Sequence(BaseTestCase):
    ...
    def testItems(self):
        ...
        self.s1.setComponentByPosition(1, univ.OctetString("def"))
        self.s1.setComponentByPosition(2, univ.Integer(123))
        assert list(self.s1.items()) == [
            (x[0], str2octs(x[1])) for x in [("name", "abc"), ("nick", "def")]
        ] + [("age", 123)]
    ...
    def testUpdate(self):
        ...
        assert list(self.s1.items()) == [
            (x[0], str2octs(x[1])) for x in [("name", "CBA"), ("nick", "def")]
        ] + [("age", 123)]
...
class Set(BaseTestCase):
    ...
    def testByTypeWithPythonValue(self):
        ...
        assert self.s1.getComponentByType(univ.OctetString.tagSet) == str2octs(
            "abc"
        ), "set by name fails"
    ...
    def testGetTagMap(self):
        assert self.s1.tagMap.presentTypes == {univ.Set.tagSet: univ.Set().clear()}
...
class Choice(BaseTestCase):
    ...
    def testRepr(self):
        ...
        s = self.s1.clone().setComponents(
            sex=self.s1.setComponentByPosition(1)
            .getComponentByPosition(1)
            .clone()
            .setComponents(count=univ.Integer(123))
        )
    ...
    def testOuterByTypeWithPythonValue(self):
        self.s1.setComponentByType(univ.OctetString.tagSet, "abc")
        assert self.s1.getComponentByType(univ.OctetString.tagSet) == str2octs("abc")
