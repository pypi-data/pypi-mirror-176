...
#pragma warning( \
    disable : 4996) /* this function (strncat,sprintf,strcpy) or variable may be unsafe. */
#endif
...
void qh_outcoincident(qhT *qh, int coincidentpoints, double radius, int iscdd, double *coord,
                      int dim);
...
void qh_rboxpoints2(qhT *qh, char *rbox_command, double **simplex) {
    ...
    *command = '\0';
    ...
    while (*s) {
        ...
        switch (*s++) {
            case 'c':
                ...
                t = s;
                while (isspace(*t)) t++;
                if (*t == 'G') cube = qh_strtod(++t, &s);
                break;
            case 'd':
                adddiamond = 1;
                t = s;
                while (isspace(*t)) t++;
                if (*t == 'G') diamond = qh_strtod(++t, &s);
                break;
            case 'h':
                iscdd = 1;
                break;
            ...
            case 'B':
                ...
                break;
            case 'C':
                if (*s) coincidentpoints = qh_strtol(s, &s);
                if (*s == ',') {
                    ++s;
            ...
            case 'D':
                dim = qh_strtol(s, &s);
                if (dim < 1 || dim > MAXdim) {
                    qh_fprintf_rbox(qh, qh->ferr, 6189,
            ...
            case 'O':
                ...
                break;
            case 'P':
                if (!first_point) first_point = s - 1;
                addpoints++;
    ...
    if (qh->rbox_isinteger && !isbox) box = qh_DEFAULTzbox;
    if (addcube) {
        tempr = floor(ldexp(1.0, dim) + 0.5);
        cubesize = (int)tempr;
        if (cube == 0.0) cube = box;
    } else
        cubesize = 0;
    if (adddiamond) {
        diamondsize = 2 * dim;
        if (diamond == 0.0) diamond = box;
    } else
        diamondsize = 0;
    if (islens) {
        if (isaxis) {
        ...
        if (radius <= 1.0) {
            qh_fprintf_rbox(qh, qh->ferr, 6191,
    ...
    if (coincidentpoints > 0 && isregular) {
        qh_fprintf_rbox(qh, qh->ferr, 6423,
                        "rbox error: 'Cn,r,m' is not implemented for regular points ('r')\n");
        qh_errexit_rbox(qh, qh_ERRinput);
    }

    if (coincidenttotal == 0) coincidenttotal = numpoints;
    ...
    if (issimplex || ismesh)
        ...
        totpoints = numpoints;
        if (dim == 2) {
            if (islens) totpoints += numpoints - 2;
        } else if (dim == 3) {
            if (islens)
    ...
    if (iscdd)
        qh_fprintf_rbox(qh, qh->fout, 9391, "%s\nbegin\n        %d %d %s\n",
                        NOcommand ? "" : command, totpoints, dim + 1,
    ...
    if ((s = first_point)) {
        while (s && *s) { /* 'P' */
            count = 0;
            if (iscdd) qh_out1(qh, 1.0);
            while (*++s) {
                qh_out1(qh, qh_strtod(s, &s));
                count++;
                if (isspace(*s) || !*s) break;
                if (*s != ',') {
                    qh_fprintf_rbox(qh, qh->ferr, 6194,
                                    "rbox error: missing comma after coordinate in %s\n\n", s);
                    qh_errexit_rbox(qh, qh_ERRinput);
                }
            }
            if (count < dim) {
                for (k = dim - count; k--;) qh_out1(qh, 0.0);
            } else if (count > dim) {
                qh_fprintf_rbox(qh, qh->ferr, 6195,
    ...
    if (issimplex + issimplex2) {
        ...
        if (isregular) {
            for (i = 0; i < dim; i++) {
                for (k = 0; k < dim; k++) *(simplexp++) = i == k ? 1.0 : 0.0;
            }
            for (k = 0; k < dim; k++) *(simplexp++) = -1.0;
        } else {
            for (i = 0; i < dim + 1; i++) {
                for (k = 0; k < dim; k++) {
                    randr = qh_RANDOMint;
        ...
        if (issimplex2) {
            ...
            for (i = 0; i < dim + 1; i++) {
                if (iscdd) qh_out1(qh, 1.0);
                for (k = 0; k < dim; k++) qh_out1(qh, *(simplexp++) * box);
        ...
        for (j = 0; j < numpoints; j++) {
            if (iswidth)
                ...
                apex = -1;
            for (k = 0; k < dim; k++) coord[k] = 0.0;
            norm = 0.0;
            for (i = 0; i < dim + 1; i++) {
                randr = qh_RANDOMint;
                factor = randr / randmax;
                if (i == apex) factor *= width;
                norm += factor;
                for (k = 0; k < dim; k++) {
            ...
            for (k = 0; k < dim; k++) coord[k] *= box / norm;
    ...
    /* ============= regular points for 's' =============== */
        ...
        if (dim != 2 && dim != 3) {
            qh_fprintf_rbox(qh, qh->ferr, 6197,
    ...
    /* ============= regular points for 'r Ln D3' =============== */
        ...
        if (dim != 3) {
            qh_fprintf_rbox(qh, qh->ferr, 6198,
                            "rbox error: regular points can be used only in 2-d and 3-d\n\n");
            qh_errexit_rbox(qh, qh_ERRinput);
        }
        angle = 0.0;
    ...
    /* ============= apex of 'Zn' distribution + gendim =============== */
        ...
        if (isaxis) {
            gendim = dim - 1;
            if (iscdd) qh_out1(qh, 1.0);
            for (j = 0; j < gendim; j++) qh_out1(qh, 0.0);
            qh_out1(qh, -box);
        ...
        for (i = 0; i < numpoints; i++) {
            ...
            if (isaxis) {
                ...
                for (j = 0; j < gendim; j++) coord[j] = factor * coord[j];
                ...
                if (dim != 3) {
                    qh_fprintf_rbox(qh, qh->ferr, 6199,
                ...
                for (j = 0; j < dim; j++) coord[j] = factor * coord[j];
            }
            /* ============= project 'Zn s' point in to sphere =============== */
            if (isaxis && issphere) {
                coord[dim - 1] = 1.0;
                norm = 1.0;
                for (j = 0; j < gendim; j++) norm += coord[j] * coord[j];
                norm = sqrt(norm);
                for (j = 0; j < dim; j++) coord[j] = coord[j] / norm;
                if (iswidth) {
                    randr = qh_RANDOMint;
                ...
                for (j = 0, norm = 0; j < dim; j++) norm += coord[j] * coord[j];
                norm = sqrt(norm);
                for (j = 0; j < dim; j++) coord[j] = coord[j] * radius / norm;
                coord[dim - 1] -= lensbase;
                if (iswidth) {
                    randr = qh_RANDOMint;
                    coord[dim - 1] *= 1 - width * randr / randmax;
                }
                if (qh_RANDOMint > randmax / 2) coord[dim - 1] = -coord[dim - 1];
            ...
            /* ============= scale point to box =============== */
            for (k = 0; k < dim; k++) coord[k] = coord[k] * box;
...
int qh_roundi(qhT *qh, double a) {
    if (a < 0.0) {
        ...
        if (a + 0.5 > INT_MAX) {
            qh_fprintf_rbox(qh, qh->ferr, 6201,
...
void qh_out2n(qhT *qh, double a, double b) {
    if (qh->rbox_isinteger)
        qh_fprintf_rbox(qh, qh->fout, 9405, "%d %d\n", qh_roundi(qh, a + qh->rbox_out_offset),
                        qh_roundi(qh, b + qh->rbox_out_offset));
    else
        qh_fprintf_rbox(qh, qh->fout, 9406, qh_REAL_2n, a + qh->rbox_out_offset,
                        b + qh->rbox_out_offset);
} /* qh_out2n */
...
void qh_out3n(qhT *qh, double a, double b, double c) {
    if (qh->rbox_isinteger)
        qh_fprintf_rbox(qh, qh->fout, 9407, "%d %d %d\n", qh_roundi(qh, a + qh->rbox_out_offset),
                        qh_roundi(qh, b + qh->rbox_out_offset),
                        qh_roundi(qh, c + qh->rbox_out_offset));
    else
        qh_fprintf_rbox(qh, qh->fout, 9408, qh_REAL_3n, a + qh->rbox_out_offset,
                        b + qh->rbox_out_offset, c + qh->rbox_out_offset);
} /* qh_out3n */
