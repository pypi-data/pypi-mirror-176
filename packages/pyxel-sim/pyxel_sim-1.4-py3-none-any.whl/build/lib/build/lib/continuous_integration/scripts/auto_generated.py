...
import pathlib
import sys
from typing import Optional, Mapping, Any, Sequence, Iterator, Literal, Union, Tuple
...
class ModelChargeGenerationConversionWithQeMapArguments(Mapping[str, Any]):
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(('filename', 'position', 'align', 'binomial_sampling'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
class ModelChargeGenerationDarkCurrentArguments(Mapping[str, Any]):
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(('figure_of_merit', 'fixed_pattern_noise_factor', 'band_gap', 'band_gap_room_temperature', 'seed', 'temporal_noise'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
class ModelChargeGenerationDarkCurrentRule07Arguments(Mapping[str, Any]):
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(('cutoff_wavelength', 'fixed_pattern_noise_factor', 'seed', 'temporal_noise'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
class ModelChargeGenerationSimpleConversionArguments(Mapping[str, Any]):
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(('quantum_efficiency', 'binomial_sampling'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
#
# Model: Charge Collection / Fix Pattern Noise
#
@schema(title='Parameters')
@dataclass
class ModelChargeCollectionFixPatternNoiseArguments(Mapping[str, Any]):
    filename: Union[str, pathlib.Path] = field(
        metadata=schema(
            title='filename'
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(('filename', 'position', 'align'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
@schema(
    ...
    ,description=(
        'Add fix pattern noise caused by pixel non-uniformity during charge'
        'collection.'
    )
)
@dataclass
class ModelChargeCollectionFixPatternNoise:
    name: str
    arguments: ModelChargeCollectionFixPatternNoiseArguments
    func: Literal['pyxel.models.charge_collection.fix_pattern_noise'] = 'pyxel.models.charge_collection.fix_pattern_noise'
    enabled: bool = True
...
class ModelChargeMeasurementReadoutNoiseSaphiraArguments(Mapping[str, Any]):
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(('roic_readout_noise', 'controller_noise'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
class ModelChargeTransferArcticRemoveArguments(Mapping[str, Any]):
    ...
    instant_traps: Sequence[Mapping[str, float]] = field(
        metadata=schema(
            title='instant_traps'
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(('well_fill_power', 'instant_traps', 'num_iterations', 'express'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
class DetailedDetectionPipeline(DetectionPipeline):
    photon_generation: Optional[
        Sequence[Union[ModelPhotonGenerationIllumination, ModelPhotonGenerationLoadImage, ModelPhotonGenerationShotNoise, ModelPhotonGenerationStripePattern, ModelFunction]]
    ...
    charge_transfer: Optional[
        Sequence[Union[ModelChargeTransferArcticAdd, ModelChargeTransferArcticRemove, ModelChargeTransferCdm, ModelFunction]]
    ] = field(default=None, metadata=schema(title='Charge Transfer'))
    signal_transfer: Optional[
        Sequence[Union[ModelFunction]]
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help='Check if the JSON Schema is new.',
)
def create_json_schema(filename: pathlib.Path, check: bool):
    ...
    if not check:
        print(f'Generate JSON Schema file: {full_filename!s}')
        with full_filename.open('w') as fh:
            json.dump(obj=dct_schema, fp=fh, indent=2)
    else:
        with full_filename.open() as fh:
            previous_dct_schema = json.load(fh)

        diff_schema = diff(dct_schema, previous_dct_schema)
        if not diff_schema:
            print('JSON Schema file is unchanged.')
            sys.exit(0)
        else:
            print(
                "JSON Schema file must be regenerated !"
                "Please run 'tox -e json_schema'"
            )
            sys.exit(1)
