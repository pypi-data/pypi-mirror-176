...
import notion_get_properties as get_prop
import notion_update_properties as update_prop
...
###############
	def __init__(self, **kwargs):
		...
		self.update = update_prop.update_properties(users_list=self.all_users)
		self.verbose = True
...
############################################################################
	def addPageDatabase(self, dbId, name):
		url = self.url + '/pages'
		data = {
...
#####################################################
	def getProperties(self, elem):
		return (get_prop.get_properties()[elem['type']](elem))

##################################################
# Met à jour une propriété avec les elements voulu
##################################################
	def updateProperties(self, elem):
		return (self.update.update_properties()[elem['type']](elem))
