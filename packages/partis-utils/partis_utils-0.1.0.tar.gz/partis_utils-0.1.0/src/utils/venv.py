...
class VirtualEnv( ProcessEnv ):
  ...
  def __init__( self, *,
    ...
    path = osp.realpath( str(path) )

    bin_path = osp.join( path, 'bin' )
    ...
    interpreter = osp.realpath( shutil.which( interpreter ) )
    ...
    self._p_exec = osp.join(self.bin_path, 'python')
    self._p_mutex = mutex

    self._p_ctx_sys_path = None

    py_path_cur = osp.join(osp.dirname(sys.executable), osp.pardir)
    py_path_ori = osp.join(osp.dirname(self.interpreter), osp.pardir)

    if self.path in [ py_path_cur, py_path_ori]:
      # just set it to re-use the environment
      logger.info(f"Skipping creation of venv since it points to original interpreter: {self.path}")
      return

    # check that path is not being created inside the current one(s)
    if (
      osp.commonpath([self.path, py_path_cur]) in [self.path, py_path_cur]
      or osp.commonpath([self.path, py_path_ori]) in [self.path, py_path_ori] ):

      raise ValueError(
        f"Cannot create venv conflicting within another venv: {self.path}")

    with self._p_mutex:

      if osp.exists( self.path ):

        is_venv = osp.exists( self.exec )
      ...
      if not osp.exists( self.path ):
        logger.info(f"Creating venv: {self.path}")
      ...
      if bool(inherit_site_packages) or isinstance(inherit_site_packages, list):
        ...
        if isinstance(inherit_site_packages, list):
          for dir in inherit_site_packages:
            dir = osp.abspath(dir)
            exec = osp.join( dir, 'bin', 'python' )

            if not osp.isfile(exec):
        ...
        for path in sys_path:
          ...
          if path.endswith('site-packages'):
            ...
            site_dir = osp.join( *path.split(os.sep)[-3:] )

            if not osp.isdir( osp.join( self.path, site_dir ) ):
        ...
        for site_dir, paths in site_paths.items():
          ...
          inherit_pth = osp.join( self.path, site_dir, 'venv_inherited.pth' )
  ...
  def __enter__( self ):
    ...
    sys.path = sys.path + self.sys_path
