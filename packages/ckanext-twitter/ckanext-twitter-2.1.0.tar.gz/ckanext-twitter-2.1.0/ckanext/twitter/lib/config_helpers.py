...
def twitter_get_credentials():
    ...
    :return: (key, secret)
    '''
    consumer_key = toolkit.config.get('ckanext.twitter.consumer_key', 'no-consumer-key-set')
    consumer_secret = toolkit.config.get('ckanext.twitter.consumer_secret',
                                         'no-consumer-secret-set')
    token_key = toolkit.config.get('ckanext.twitter.token_key', 'no-token-key-set')
    token_secret = toolkit.config.get('ckanext.twitter.token_secret', 'no-token-secret-set')
...
def twitter_is_debug():
    ...
    :return: boolean
    '''
    return toolkit.config.get('ckanext.twitter.debug', toolkit.config.get('debug', False))
...
def twitter_new_format():
    ...
    '''
    return toolkit.config.get('ckanext.twitter.new',
...
def twitter_updated_format():
    ...
    :return: string with replaceable jinja2 tags
    '''
    return toolkit.config.get('ckanext.twitter.updated',
