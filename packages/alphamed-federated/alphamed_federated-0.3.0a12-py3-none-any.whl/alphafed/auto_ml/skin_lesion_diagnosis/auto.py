...
class AutoResNetBase(AutoModel, metaclass=ABCMeta):
    ...
    def _build_model(self):
        ...
        origin_num_classes = 1000
        self.num_classes = origin_num_classes
...
class AutoResNet(AutoResNetBase):
    ...
    def fine_tune(self,
        ...
        self._save_root = (os.path.join('/data/alphamed-federated', task_id)
                           ...
                           else os.path.join('models', 'skin_lesion_diagnosis'))
...
class ResNetFedAvgScheduler(FedAvgScheduler):
    ...
    def _prepare_task_output(self) -> Tuple[str, str]:
        ...
        with ZipFile(resource_zip_file, 'w') as resource_zip:
            param_file_path = self.auto_proxy.meta.param_file[len(resource_dir):]
...
class SkinLesionDiagnosisFamily(AutoModelFamily):
