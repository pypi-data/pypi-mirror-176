...
from ...fed_avg.secure_contractor import AutoSecureFedAvgContractor
...
class InceptionFedAvgScheduler(FedAvgScheduler):
    ...
    def _prepare_task_output(self) -> Tuple[str, str]:
        ...
        with ZipFile(resource_zip_file, 'w') as resource_zip:
            # remove prefix resource_dir path of the param_file path
...
class InceptionSecureFedAvgScheduler(SecureFedAvgScheduler):
    ...
    def _setup_context(self, id: str, task_id: str, is_initiator: bool = False):
        super()._setup_context(id=id, task_id=task_id, is_initiator=is_initiator)
        self.contractor = AutoSecureFedAvgContractor(task_id=task_id)
