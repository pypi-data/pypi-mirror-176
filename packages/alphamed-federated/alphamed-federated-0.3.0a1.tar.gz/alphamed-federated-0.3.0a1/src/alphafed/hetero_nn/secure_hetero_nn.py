...
from .hetero_nn import _SimplifiedOptimizer
...
class SecureHeteroNNHostScheduler(SecureHeteroNNScheduler):
    ...
    def _collect_noised_projection(self):
        ...
        self.contractor.notify_ready_for_noised_projection(round=self._round)
    ...
    def _collect_noised_w_grad(self):
        ...
        self.contractor.notify_ready_for_noised_w_grad(round=self._round)
    ...
    def _save_model(self):
        ...
        with open(f'{os.path.join(self._save_path, "feature_model_ckp.pt")}', 'wb') as f:
            torch.save(self.feature_model.state_dict(), f)
        with open(f'{os.path.join(self._save_path, "project_layer_ckp.pt")}', 'wb') as f:
            torch.save(self.project_layer.state_dict(), f)
        with open(f'{os.path.join(self._save_path, "infer_model_ckp.pt")}', 'wb') as f:
    ...
    def _prepare_task_output(self) -> Tuple[str, str]:
        ...
        feature_model_file = os.path.join(self._save_path,
                                          f'feature_model_{self.feature_key}.pt')
        with open(feature_model_file, 'wb') as f:
            torch.save(self.feature_model.state_dict(), f)
        project_layer_file = os.path.join(self._save_path, 'project_layer.pt')
        with open(project_layer_file, 'wb') as f:
            torch.save(self.project_layer.state_dict(), f)
        infer_model_file = f'{os.path.join(self._save_path, "infer_model.pt")}'
        with open(infer_model_file, 'wb') as f:
            torch.save(self.infer_model.state_dict(), f)
        model_file = os.path.join(self._save_path, 'model.zip')
