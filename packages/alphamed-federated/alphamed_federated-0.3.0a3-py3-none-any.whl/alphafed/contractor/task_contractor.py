...
class TaskContractor(Contractor):
    ...
    def query_nodes(self, is_auto_ml: bool = False) -> List[str]:
        """Query all nodes in this task."""
        post_data = {
            'task_id': self.task_id,
            'task_type': 2 if is_auto_ml else 1
        }
    ...
    def upload_file(self, fp, persistent: bool = False, upload_name: str = None) -> str:
        ...
        if isinstance(fp, str):
            ...
            assert (
                upload_name and isinstance(upload_name, str)
            ), f'Must specify a file name to save the stream, {upload_name} received.'
