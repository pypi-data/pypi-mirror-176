...
def push(session: Session, metadata, workbook_context: WorkbookContext, datasource_output, sync_token,
         row_filter: Callable, push_results_df: Optional[pd.DataFrame], status: Status):
    ...
    if push_results_df is None:
        ...
        original_index_name = push_results_df.index.name
        push_results_df.index.set_names([ORIGINAL_INDEX_COLUMN], inplace=True)
        push_results_df.reset_index(inplace=True)

        if 'Push Result' in push_results_df.columns:
        ...
        for index, row in push_results_df.iterrows():
            ...
            try:
                flush_now, last_scalar_datasource = \
                    _process_push_row(session, asset_batch_input, reified_assets, condition_batch_input, status,
                                      ...
                                      workbook_context, threshold_metric_inputs, display_template_inputs,
                                      display_inputs)
            ...
            if int(status.df['Overall']) % batch_size == 0 or flush_now:
                ...
                _flush(session, put_signals_input, put_scalars_input, condition_batch_input, threshold_metric_inputs,
                       display_template_inputs, display_inputs, asset_batch_input, tree_batch_input, push_results_df,
                       status)
    ...
    if ORIGINAL_INDEX_COLUMN in push_results_df.columns:
        push_results_df.set_index(ORIGINAL_INDEX_COLUMN, inplace=True)
        push_results_df.index.name = original_index_name
...
def _process_push_row(session: Session, asset_batch_input, reified_assets, condition_batch_input, status: Status,
                      datasource_output, flush_now, index, last_scalar_datasource, push_results_df, put_scalars_input,
                      put_signals_input, roots, row, sync_token, tree_batch_input, workbook_context: WorkbookContext,
                      threshold_metric_inputs, display_template_inputs, display_inputs):
    ...
    row_dict['Scoped To'] = workbook_context.workbook_id

    if not _common.present(row_dict, 'Type') or not _is_handled_type(row_dict['Type']):
        if not _common.present(row_dict, 'Formula'):
            _common.raise_or_catalog(status, df=push_results_df, column='Push Result', index=index,
    ...
    scoped_data_id = get_scoped_data_id(row_dict, workbook_context.workbook_id)
    ...
    if 'Signal' in row_dict['Type']:
        ...
        if _common.present(row_dict, 'ID'):
            ...
            try:
                ...
                _common.raise_or_catalog(status, df=push_results_df, index=index, column='Push Result', e=e)
        else:
        ...
        scalar_input.parameters = _process_formula_parameters(scalar_input.parameters, workbook_context.workbook_id,
                                                              push_results_df)
        ...
        condition_update_input.parameters = _process_formula_parameters(
            condition_update_input.parameters, workbook_context.workbook_id, push_results_df)
        row_dict['Formula Parameters'] = condition_update_input.parameters
        if len(condition_update_input.parameters) > 0:
            push_results_df.at[index, 'Formula Parameters'] = condition_update_input.parameters
        ...
        if _common.present(row_dict, 'ID'):
            ...
            condition_update_input.sync_token = sync_token
            setattr(condition_update_input, 'dataframe_index', index)
            status.df['Condition'] += _add_no_dupe(condition_batch_input.conditions, condition_update_input)
        ...
        reified_assets.add(scoped_data_id)
        ...
        if _common.get(row_dict, 'Formula') == '<ThresholdMetric>':
            ...
            if threshold_metric_input.measured_item:
                threshold_metric_input.measured_item = _item_id_from_parameter_value(
                    threshold_metric_input.measured_item, workbook_context.workbook_id, push_results_df)
            if threshold_metric_input.bounding_condition:
                threshold_metric_input.bounding_condition = _item_id_from_parameter_value(
                    threshold_metric_input.bounding_condition, workbook_context.workbook_id, push_results_df)

            threshold_metric_input.thresholds = _convert_thresholds_dict_to_input(
                threshold_metric_input.thresholds, workbook_context.workbook_id, push_results_df)
        ...
        if _common.present(row_dict, 'ID'):
            try:
                ...
                _common.raise_or_catalog(status, df=push_results_df, column='Push Result', index=index, e=e)
        else:
        ...
        try:
            ...
            if not _common.present(row_dict, 'Source Workstep ID') \
                ...
                if workbook_context.item_map is not None and workstep_id in workbook_context.item_map:
                    workstep_id = workbook_context.item_map[workstep_id]
            ...
            display_input.sync_token = sync_token
            ...
            if _common.present(row_dict, 'ID'):
                ...
                status.df['Display'] += _add_no_dupe(display_inputs, display_input)
            push_results_df.at[index, 'Template ID'] = template_output.id
        ...
        try:
            ...
            if _common.present(row_dict, 'ID'):
                ...
                display_template_inputs.append(display_template_input)
    ...
    if path:
        _reify_path(path, workbook_context, datasource_output, scoped_data_id, reified_assets, roots, asset_batch_input,
                    tree_batch_input, sync_token, status, index, push_results_df)
    return flush_now, last_scalar_datasource
...
def _set_push_result_string(dfi, iuo, status: Status, push_results_df):
    ...
    if isinstance(iuo, ItemUpdateOutputV1):
        if iuo.error_message is not None:
            ...
            push_results_df.loc[dfi, 'Datasource Class'] = iuo.datasource_class
            push_results_df.loc[dfi, 'Datasource ID'] = iuo.datasource_id
            push_results_df.loc[dfi, 'Data ID'] = iuo.data_id
        ...
        push_results_df.loc[dfi, 'Datasource Class'] = getattr(iuo, 'datasource_class', np.nan)
        push_results_df.loc[dfi, 'Datasource ID'] = getattr(iuo, 'datasource_id', np.nan)
        push_results_df.loc[dfi, 'Data ID'] = getattr(iuo, 'data_id', np.nan)
...
def _process_batch_output(session: Session, item_inputs, item_updates, status: Status, push_results_df):
    repost = False
    for i in range(0, len(item_inputs)):
        ...
        if hasattr(item_input, 'dataframe_index'):
            _set_push_result_string(item_input.dataframe_index, item_update_output, status, push_results_df)

    return repost


def _post_batch(session: Session, post_function, item_inputs, status: Status, push_results_df):
    item_batch_output = post_function()
    repost = _process_batch_output(session, item_inputs, item_batch_output.item_updates, status, push_results_df)
    if repost:
        ...
        item_batch_output = post_function()
        _process_batch_output(session, item_inputs, item_batch_output.item_updates, status, push_results_df)
...
def _flush(session: Session, put_signals_input, put_scalars_input, condition_batch_input, threshold_metric_inputs,
    ...
    if len(condition_batch_input.conditions) > 0:
        _post_batch(session, lambda: conditions_api.put_conditions(body=condition_batch_input),
                    condition_batch_input.conditions, status, push_results_df)
        condition_batch_input.conditions = list()
    ...
    if len(threshold_metric_inputs) > 0:
        ...
        _mock_post_batch(session, threshold_metric_inputs, metrics_api.create_threshold_metric,
                         _handle_metric_conflict, push_results_df, status)

    if len(display_template_inputs) > 0:
        _mock_post_batch(session, display_template_inputs, display_templates_api.create_display_template, None,
                         push_results_df, status)

    if len(display_inputs) > 0:
        ...
        _mock_post_batch(session, display_inputs, displays_api.create_display, _handle_display_conflict,
                         push_results_df, status)
    ...
    if len(tree_batch_input.relationships) > 0:
        _post_batch(session, lambda: trees_api.batch_move_nodes_to_parents(body=tree_batch_input),
                    tree_batch_input.relationships, status, push_results_df)  # type: ItemBatchOutputV1
        tree_batch_input.relationships = list()


def _mock_post_batch(session: Session, item_inputs, create_item_endpoint, action_on_data_triplet_match,
                     push_results_df, status: Status):
    """
    ...
    for item_input in item_inputs:
        ...
        _set_push_result_string(item_input.dataframe_index, item_push_output, status, push_results_df)
...
def _reify_path(path, workbook_context, datasource_output, scoped_data_id, reified_assets, roots, asset_batch_input,
                tree_batch_input, sync_token, status, index, push_results_df):
    ...
    for path_item in path_items:
        ...
        asset_input.scoped_to = workbook_context.workbook_id
        ...
        child_data_id = get_scoped_data_id(asset_dict, workbook_context.workbook_id)

        asset_input.data_id = child_data_id
        tree_input.child_data_id = child_data_id

        if asset_input.data_id not in reified_assets:
            # Look to see if this asset in the path has an entry in the push_results_df and if so, use its index to
            # store the results later when we push (using the 'dataframe_index' attribute to correlate to a row).
            clauses = (push_results_df['Type'] == 'Asset') & (push_results_df['Name'] == asset_dict['Asset'])
            if 'Path' in asset_dict:
                clauses &= (push_results_df['Path'] == asset_dict['Path'])
            existing_asset_rows = push_results_df.index[clauses].to_list()
            if len(existing_asset_rows) > 0:
                asset_index = existing_asset_rows[0]
            else:
                # No row was found, so add a row at the end
                asset_index = len(push_results_df.index)
                push_results_df.loc[asset_index] = asset_dict
            ...
            reified_assets.add(asset_input.data_id)
