...
def pull_tree(session: Session, node_id, workbook_id, datasource, status: Status):
    """
    ...
    df = _properties.process_properties(session,
                                        ...
                                        datasource, status, existing_tree_df=existing_tree_df)

    df = pull_all_children_of_all_nodes(session, df, workbook_id, datasource, status, existing_tree_df=existing_tree_df)
    _common.put_properties_on_df(df, types.SimpleNamespace(spy_tree=existing_tree_df is not None))

    return df


def pull_all_children_of_all_nodes(session: Session, df, workbook_id, datasource, status: Status,
...
def pull_all_children_of_node(session: Session, df, node_id, node_full_path, workbook_id,
    ...
    insert_df = _properties.process_properties(session, search_results, datasource,
