...
class Tree:
    ...
    def __init__(self, data, *, friendly_name=None, description=None, workbook=_common.DEFAULT_WORKBOOK_PATH,
                 datasource=None, quiet=False, errors=None, status=None, session: Optional[Session] = None):
        ...
        if isinstance(data, pd.DataFrame):
            ...
            df = _properties.process_properties(self.session, df, self._datasource,
            ...
            if existing_node_id:
                ...
                df = _pull.pull_tree(self.session, existing_node_id, self._workbook_id, self._datasource, self.status)
                _utils.increment_status_df(self.status, new_items=df)
    ...
    def insert(self, children=None, parent=None, *, name=None, formula=None, formula_parameters=None,
        ...
        children = _properties.process_properties(self.session, children, self._datasource,
                                                  status, keep_parent_column=True)

        # Pull children of items with pre-existing IDs
        children = _pull.pull_all_children_of_all_nodes(self.session, children, self._workbook_id,
    ...
    def push(self, *, errors=None, quiet=None, status=None):
        ...
        push_results = _push.push(metadata=df_to_push, workbook=self._workbook, datasource=self._datasource,
                                  archive=True, status=status, session=self.session)
