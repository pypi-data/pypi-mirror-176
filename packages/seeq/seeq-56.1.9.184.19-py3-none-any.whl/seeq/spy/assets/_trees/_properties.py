...
def process_properties(session: Session, df, datasource, status: Status, *, existing_tree_df=None,
    ...
    df.loc[row_type != 'pre-existing', :] = df.loc[row_type != 'pre-existing', :] \
        .apply(_safely(make_node_reference), axis=1, session=session)
...
def make_node_reference(row, session: Session) -> pd.Series:
    row = row.copy()
