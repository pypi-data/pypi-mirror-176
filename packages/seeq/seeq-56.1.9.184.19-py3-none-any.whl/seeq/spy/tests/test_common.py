...
def running_in_dev_compose():
    ...
    try:
        ...
        return False
    return len(docker_client.api.containers(filters={'name': 'crab_appserver-server_*'})) != 0


def get_appserver_server_container():
    matches = docker_client.api.containers(filters={'name': 'crab_appserver-server_*'})
    if len(matches) == 0:
        return None
...
def get_dev_compose_folder():
    return os.path.normpath(
        os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', '..', 'dev-compose'))
...
class VisualDiffTest:
    ...
    VISUAL_DIFF_FOLDER: ClassVar[bytes] = os.path.join(pathlib.Path(__file__).parent.resolve(), 'visual_diff')
    # In the case of a frontend update where certain baseline visual diff test images must be reset, set this
    #  class variables to `True` and run `pytest -m visual_diff`
    GENERATING_BASELINE_IMAGES: ClassVar[bool] = False

    def run(self):
        content_output = self.create_content()
        image_bytes = VisualDiffTest.get_image_bytes(content_output.id)
        with Image.open(io.BytesIO(image_bytes)) as im:
            subdirectory = 'current' if not VisualDiffTest.GENERATING_BASELINE_IMAGES else 'baseline'
            self.save_image(im, subdirectory)

        if not VisualDiffTest.GENERATING_BASELINE_IMAGES:
            self.diff_images()
    ...
    def save_image(self, image: Image, subdirectory: str, file_format: str = 'png'):
        directory = os.path.join(VisualDiffTest.VISUAL_DIFF_FOLDER, subdirectory)
        os.makedirs(directory, exist_ok=True)

        file_path = os.path.join(directory, f'{self.test_name}.{file_format}')
        image.save(file_path, file_format)

    def diff_images(self):
        file_name = f'{self.test_name}.png'
        baseline_file_name = os.path.join(VisualDiffTest.VISUAL_DIFF_FOLDER, 'baseline', file_name)
        actual_file_name = os.path.join(VisualDiffTest.VISUAL_DIFF_FOLDER, 'current', file_name)
        with Image.open(baseline_file_name).convert('RGB') as im_expected, \
            ...
            if pixel_error_count >= VisualDiffTest.PIXEL_ERROR_THRESHOLD:
                ...
                self.save_image(diff, 'difference')
    ...
    def remove_diff_image(self, file_format: str = 'png'):
        file_path = os.path.join(VisualDiffTest.VISUAL_DIFF_FOLDER, 'difference', f'{self.test_name}.{file_format}')
