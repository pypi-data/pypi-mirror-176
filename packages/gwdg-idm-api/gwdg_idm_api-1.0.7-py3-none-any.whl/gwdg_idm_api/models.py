...
class ChangeTemplate(pydantic.BaseModel):
    ...
    def validate_results(cls, provided_values):
        type_dict = {
            key: type(value.outer_type_()) for key, value in cls.__fields__.items()
        }
        # Early abort if the validator is called on a fastAPI serialize_request call
        if "pydantic" in str(cls):
