...
static void init_config_dma_multipass(HardwareDriver* driver, const Config& dma,
  ...
  reg = 0;
  uint32_t last_valid_descriptor = num_descriptors;
...
void enqueue_descriptor(HardwareDriver* driver, const Engine& dma,
  ...
  last_descriptor_id = (last_descriptor_id + 1) % num_descriptors;
...
void wait_config_dma_multipass_ready(HardwareDriver* driver,
                                     const Config& dma) {
