...
static void free_dma_descriptors(dma::Engine* dma, MemoryMgr* mem_mgr) {
  if (dma->descriptor_base_addr) {
    ...
    // Currently we have to set to 0 to mark we have correctly free
...
void HardwareDeviceImpl::program(const uint8_t* program, size_t size,
  ...
  if (reprogram) {
    ...
    alloc_dma_descriptors(&input_dma.engine, &mem_mgr_, dma::kMaxNbDescriptors);
    ...
    dma::init_default_dma(driver_, input_dma.engine, dma::kMaxNbDescriptors);
    ...
    if (multi_pass_en) {
      ...
      program::play_multi_pass(this, program, &multi_pass_memory_,
                               dma_config_.engine.descriptor_base_addr);
      // turn config dma on
      dma::toggle_engine(driver_, dma_config_.engine.reg_base_addr, true);
      ...
      // to complete
      dma::wait_config_dma_multipass_ready(driver_, dma_config_);
...
bool HardwareDeviceImpl::enqueue(const Tensor& input, const int32_t* label) {
  ...
  // put buffer on device, and get its address
  auto address_in = external_mem_.track_and_put_on_device_if_required(
      input.buffer()->data(), input.buffer()->size());
