...
class ProxyWorker(Connection):
    ...
    def createJob(self, job):
        return self.postThatReturnsObj("/proxyworker/jobs", job)

    def setSlotCount(self, num_slots):
        job = {"slot_count": num_slots}

        reply = self.put("/proxyworker/config", json.dumps(job))
        if self.lastReturnCode() != 200:
            return False
        return json.loads(reply)

    def getJob(self, job_id):
        return self.getThatReturnsObj("/proxyworker/jobs/" + job_id)

    def getAllJobs(self):
        return self.getThatReturnsObj("/proxyworker/jobs")

    def deleteJob(self, job_id):
        reply = self.delete("/proxyworker/jobs/" + job_id)
        if self.lastReturnCode() != 200:
            return False
