...
class Admin(Connection):
    ...
    def getMediaspaces(self):
        ...
        return self.getThatReturnsObj("/mediaspaces")
    ...
    def getMediaspace(self, name):
        ...
        url = "/mediaspaces?name=" + str(urllib.parse.quote(name))
        return self.getThatReturnsObj(url)

    def getMediaspaceAsAdmin(self, name):
        """Get a mediaspace by name"""

        url = "/mediaspaces?name=" + str(urllib.parse.quote(name)) + "&admin=true"
        return self.getThatReturnsObj(url)

    def find_mediaspace(self, name, as_admin=False):
    ...
    def getMediaspaceFolders(self, mediaspace_id, path=""):
        ...
        if path:
            escaped_path = urllib.parse.quote(str(path))
            url += "/" + escaped_path
        if True:
            url += "?database=false"
        return self.getThatReturnsObj(url)
    ...
    def mountpointByName(self, name):
        ...
        url = "/mountpoints?name=" + str(urllib.parse.quote(name))
        return self.getThatReturnsObj(url)
    ...
    def ensureSpaceExists(self, spacename: str, spacetype: str, esa_ip: str = None) -> FlowObject:
    ...
    def ensureUserIsInSpace(
        ...
        user_list = self.getSpaceUsers(space_obj["mediaspace_id"])
    ...
    def get_user_group(self, group_name: str) -> FlowObject:
        ...
        url = "/groups/" + str(urllib.parse.quote(group_name))
        return self.getThatReturnsObj(url)
    ...
    def update_user_group(self, group_data: FlowObject) -> FlowObject:
        ...
        url = "/groups/" + str(urllib.parse.quote(group_data["groupname"]))
    ...
    def get_group_users(self, group_name: str) -> List[str]:
        ...
        url = "/groups/" + str(urllib.parse.quote(group_name)) + "/users"
    ...
    def add_users_to_group(self, group_name: str, users: List[str]) -> bool:
        ...
        url = "/groups/" + str(urllib.parse.quote(group_name)) + "/users"
        return self.postThatReturnsBOOL(url, users)
