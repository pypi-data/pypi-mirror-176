...
class Connection:
    ...
    # pylint: disable=too-many-instance-attributes,too-many-public-methods,consider-using-f-string
    ...
    def do_request(self, verb, endpoint, data):
        ...
        if self._use_gateway:
            if endpoint.startswith("/" + self._service_name):
                endpoint = self._gateway_prefix[:-1] + endpoint
            else:
                endpoint = self._gateway_prefix + self._service_name + endpoint
        ...
        while True:
            ...
            try:
                ...
                print("socket.error exception! '{}' errno: {}. Retrying...".format(exc, exc.errno))
