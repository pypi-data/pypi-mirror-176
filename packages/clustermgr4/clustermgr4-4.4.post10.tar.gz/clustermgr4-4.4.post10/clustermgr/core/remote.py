...
log_to_file(os.path.join(Config.DATA_DIR, "logs", "paramiko.log"), level = "DEBUG")
...
def get_os_type(self):
    ...
    if "Ubuntu" in cout and "20.04" in cout:
        os_type = "Ubuntu 20"
    if "Ubuntu" in cout and "22.04" in cout:
        os_type = "Ubuntu 22"
...
class RemoteClient(object):
    ...
    def remove_file(self, filename):
        ...
        Args:
            filename (string): name of file to be removed

        Returns:
            tuple: True/False, file size / error
        """

        logger.debug("[%s] Removing file %s", self.host, filename)
