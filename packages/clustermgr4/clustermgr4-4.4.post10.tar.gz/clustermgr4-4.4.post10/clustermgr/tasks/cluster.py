...
def do_disable_replication(task_id, server, primary_server, app_conf):
    ...
    admin_pw_fn = '/root/.adminpw'
    admin_pw_fn_fp = os.path.join(installer.container, admin_pw_fn.lstrip('/'))
    installer.put_file(admin_pw_fn_fp, app_conf.replication_pw)

    cmd = ( 'OPENDJ_JAVA_HOME=/opt/jre '
            '/opt/opendj/bin/dsreplication disable --disableAll --port 4444 '
            '--hostname {} --adminUID admin --adminPasswordFile {} '
    ...
    installer.conn.remove_file(admin_pw_fn_fp)
...
def opendjenablereplication(self, server_id):
    ...
    if app_conf.modify_hosts:
        ...
        host_ip = [(ship.hostname, ship.ip) for ship in servers]

        for server in servers:
            wlogger.log(task_id, "Modifying /etc/hosts for server {}".format(server.hostname))
            si = Installer(
                        server, 
                        app_conf.gluu_version,
            ...
            modify_hosts(si, host_ip, inside=True)
            del si
    ...
    bind_pw_fn = '/root/.bindpw'
    bind_pw_fn_fp = os.path.join(installer.container, bind_pw_fn.lstrip('/'))
    installer.put_file(bind_pw_fn_fp, primary_server.ldap_password)
    admin_pw_fn = '/root/.adminpw'
    admin_pw_fn_fp = os.path.join(installer.container, admin_pw_fn.lstrip('/'))
    installer.put_file(admin_pw_fn_fp, app_conf.replication_pw)
    ...
    for server in servers:
        if not server.primary_server:
            ...
            for base in ['gluu', 'site', 'metric']:

                cmd = ( "OPENDJ_JAVA_HOME=/opt/jre "
                        "/opt/opendj/bin/dsreplication enable --host1 {} --port1 4444 "
                        "--bindDN1 'cn=directory manager' --bindPasswordFile1 {} "
                        "--replicationPort1 8989 --host2 {} --port2 4444 --bindDN2 "
                        "'cn=directory manager' --bindPasswordFile2 {} "
                        "--replicationPort2 8989 --adminUID admin --adminPasswordFile {} "
                ...
                installer.run(cmd, inside=True, error_exception='no base DNs available to enable replication')
            ...
            if not primary_server_secured:
                ...
                cmd = ( "OPENDJ_JAVA_HOME=/opt/jre "
                        ...
                        " -D  'cn=Directory Manager' -j {} --trustAll "
                        "-n set-crypto-manager-prop --set ssl-encryption:true"
                        ).format(primary_server.ip, bind_pw_fn)
    ...
    for base in ['gluu', 'site', 'metric']:
        cmd = ( "OPENDJ_JAVA_HOME=/opt/jre /opt/opendj/bin/dsreplication "
                ...
                "--adminPasswordFile {} --baseDN o={} --hostname {} "
    ...
    cmd = ( "OPENDJ_JAVA_HOME=/opt/jre "
            "/opt/opendj/bin/dsreplication status -n -X -h {} "
            "-p 4444 -I admin --adminPasswordFile {}").format(
                    primary_server.hostname,
                    admin_pw_fn)

    installer.run(cmd, inside=True, error_exception='no base DNs available to enable replication')
    installer.conn.remove_file(admin_pw_fn_fp)
    installer.conn.remove_file(bind_pw_fn_fp)
        ...
        if session_type == 'ip_hash':
            ...
            if 'ubuntu' in nginx_installer.server_os.lower():
                ...
                for package in ubuntu_sticky_packages[ubuntu_ver]:
                    ...
                    package_url = 'https://ox.gluu.org/icrby8xcvbcv/nginx/ubuntu{}/{}'.format(ubuntu_ver, package)
                    ...
                    nginx_installer.run('DEBIAN_FRONTEND=noninteractive apt-mark hold {}'.format(package_name), inside=False)
