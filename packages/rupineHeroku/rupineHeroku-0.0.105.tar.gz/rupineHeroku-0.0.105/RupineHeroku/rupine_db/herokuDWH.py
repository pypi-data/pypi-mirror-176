...
def getTxSortedAsc(connection, schema, public_address_list):
    ...
    queryStr = "SELECT id, tx_timestamp, tx_from, amount_fee_value, amount_fee_dwh_token_id,  \
    ...
    for tx in result:
        data = {
            ...
            , 'tx_hash': tx[2]
            , 'tx_fee_amount': tx[3]
            , 'tx_fee_token_id': tx[4]
            , 'tx_send_amount': tx[5]
            , 'tx_send_amount_usd': tx[6]
            , 'tx_send_token_id': tx[7]
            , 'tx_recv_amount': tx[8]
            , 'tx_recv_amount_usd': tx[9]
            , 'tx_recv_token_id': tx[10]
        }
