...
def getTxSortedAsc(connection, schema, public_address_list):
    ...
    queryStr = "SELECT id, tx_hash, tx_failed, tx_type_no, tx_timestamp, tx_from, amount_fee_value, amount_fee_dwh_token_id,  \
    ...
    for tx in result:
        data = {
            ...
            , 'tx_from': tx[5]
            , 'tx_fee_amount': tx[6]
            , 'tx_fee_token_id': tx[7]
            , 'tx_send_amount': tx[8]
            , 'tx_send_amount_usd': tx[9]
            , 'tx_send_token_id': tx[10]
            , 'tx_recv_amount': tx[11]
            , 'tx_recv_amount_usd': tx[12]
            , 'tx_recv_token_id': tx[13]
        }
