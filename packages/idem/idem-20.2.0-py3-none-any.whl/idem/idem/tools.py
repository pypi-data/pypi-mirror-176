...
def gen_tag(hub, chunk):
    ...
    """
    tag = f'{chunk["state"]}_|-{chunk["__id__"]}_|-{chunk["name"]}_|-{chunk["fun"]}'
    ...
    if chunk.get("name_prefix") and chunk.get("name_prefix") in chunk["name"]:
        tag = f'{chunk["state"]}_|-{chunk["__id__"]}_|-{chunk["name_prefix"]}_|-{chunk["fun"]}'
                ...
                if name in enforced_state:
                    ...
                    if enforced_state and name != "ctx":
                        msg = (
                            ...
                            f"but is not found in ESM cache for {gen_tag(hub, data)}"
                        )
