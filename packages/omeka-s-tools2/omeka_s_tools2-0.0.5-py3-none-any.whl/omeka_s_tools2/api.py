...
class OmekaAPIClient():
    ...
    def add_iiif_to_payload(self, payload, iiif_urls):
        '''
        ...
        * the modified payload dict
        ...
        payload['o:media'] = []
        files = {}
        for index, iiif_url in enumerate(iiif_urls):
            payload['o:media'].append({'o:ingester': 'upload', 'o:source': iiif_url, 'file_index': str(index), 'o:item': {}, 'dcterms:title': [{'property_id': 1, '@value': iiif_url, 'type': 'literal'}]})
        files['data'] = (None, json.dumps(payload), 'application/json')
        #files['data'] = (json.dumps(payload), 'application/json')
        return files
    ...
    def add_iiif_to_item(self, item_id, iiif_url, payload={}, template_id=None, class_id=None):
        '''
        Upload a media file and associate it with an existing item.
        ...
        * a dict providing a JSON-LD representation of the new media object
        ...
        url = iiif_url
        # For backwards compatibility
        if isinstance(iiif_url, dict):
            url = iiif_url['url']
            payload = iiif_url['title']
        if isinstance(payload, str):
            payload = self.prepare_item_payload({'dcterms:title': [payload]})
        if template_id:
            payload['o:resource_template'] = self.format_resource_id(template_id, 'resource_templates')
        ...
        payload.update(file_data)
        response = self.s.post(f'{self.api_url}/media', json=payload, params=self.params)
        data = self.process_response(response)
        return data
    ...
    def add_item_set(self, payload, template_id=None, class_id=None):
        '''
        Create a new item from the supplied payload, optionally uploading attached media files.
        ...
        * a dict providing the JSON-LD representation of the new item from Omeka
        ...
        if class_id:
            ...
            response = self.s.post(f'{self.api_url}/item_sets', json=payload, params=self.params)
        #print(response.text)
        data = self.process_response(response)
        return data
