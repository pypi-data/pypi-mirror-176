...
def _h5_data_to_np_array(h5f_input_data) -> Union[np.ndarray, List[np.ndarray]]:
    """
    ...
    # name of this input.
    if len(curr_batches) == 1:
        return curr_batches[0]
    else:
        return curr_batches
...
def dataset_entries_to_h5(data: DatasetEntries, h5f: h5py.File) -> h5py.File:
    ...
    for data_index, (key, data_list) in enumerate(data.items()):
        ...
        h5f[f"data/{data_index}"].attrs["format"] = "CORE_ML_MULTIARRAY"
