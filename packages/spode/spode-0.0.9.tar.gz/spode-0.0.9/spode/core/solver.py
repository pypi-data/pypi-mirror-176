...
class Circuit(object):
    ...
    def __init__(self, circuit_element: dict, mode_info: dict, omega: Union[List[float], np.ndarray],
        ...
        node_element: specify which circuit elements are connected to a node.
    ...
    def solve(self, require_grads: Optional = True) -> Union[Tuple[np.ndarray, np.ndarray], np.ndarray]:
        ...
        This function is the core of the Circuit class. The solver logic is to build a large system matrix Ax=b at
        one frequency point, with A and b embedding all S-matrix relations of all circuit elements, and x is a (2N,
        1) unknown vector represents the complex port magnitudes. Here N represents the number of ports, because for
        each port, the optical signals have two propagating directions. Derivative is calculated based on a first
        order Taylor expansion: delta_A * x + A * delta_x = 0 => delta_x = -inv(A)* delta_A * x. In implementation,
        the port magnitudes and derivatives are calculated in a batch manner: all angular frequency points are solved
        at the same time. Namely, A is of size (len(omega), 2N, 2N), etc.
    ...
    def update_attr(self, update_attr: str, new_value: Any, key_string: Optional[str] = None):
        ...
        and list attributes, this function provides a convenient interface to update any attribute.
