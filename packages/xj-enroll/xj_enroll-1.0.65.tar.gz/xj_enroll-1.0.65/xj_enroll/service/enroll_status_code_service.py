    ...
    def batcch_edit_code(enroll_id, code, edit_types=None):
        ...
        try:
            ...
            if "enroll_record" in edit_types:
                enroll_record_query_obj = EnrollRecord.objects.filter(enroll_id=enroll_id)
                enroll_record_query_obj.update(enroll_status_code=code)

            if "enroll_subitem" in edit_types:
                enroll_subitem_query_obj = EnrollSubitem.objects.filter(enroll_id=enroll_id)
                enroll_subitem_query_obj.update(enroll_subitem_status_code=code)

            if "enroll_subitem_record" in edit_types:
                enroll_subitem_record_query_obj = EnrollSubitemRecord.objects.annotate(enroll_id=F("enroll_record__enroll_id")).filter(enroll_id=enroll_id)
