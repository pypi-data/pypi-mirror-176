...
class EnrollRuleValuate(models.Model):
    ...
    field_choices = [
        ("thread_id", "thread_id(信息ID)"),
        ("category_id", "category_id(信息分类ID)"),
        ("user_id", "user_id(发布用户ID)"),
        ("trading_relate", "trading_relate(买卖关系)"),
        ("region_code", "region_code(行政区划编码)"),
        ("occupy_room", "occupy_room(占用房间号)"),
        ("enroll_status_code", "enroll_status_code(报名状态码)"),
        ("min_number", "min_number(最小起报人数)"),
        ("max_number", "max_number(最大报名人数)"),
        ("min_count_apiece", "min_count_apiece(每人最小购买数)"),
        ("max_count_apiece", "max_count_apiece(每人最大购买数)"),
        ("enroll_rule_group", "enroll_rule_group(报名规则组)"),
        ("price", "price(单价)"),
        ("count", "count(总数量)"),
        ("unit", "unit(单位)"),
        ("fee", "fee(小费)"),
        ("reduction", "reduction(商家减免)"),
        ("subitems_amount", "subitems_amount(分项合计)"),
        ("amount", "amount(总收)"),
        ("paid_amount", "paid_amount(已收)"),
        ("unpaid_amount", "unpaid_amount(未收)"),
        ("commision", "commision(佣金)"),
        ("deposit", "deposit(每份预付押金)"),
        ("hide_price", "hide_price(匿价)"),
        ("hide_user", "hide_user(匿名)"),
        ("has_repeat", "has_repeat(有重复报名)"),
        ("has_subitem", "has_subitem(有分项)"),
        ("has_audit", "has_audit(有审核)"),
        ("need_vouch", "need_vouch(需三方担保)"),
        ("need_deposit", "need_deposit(需押金)"),
        ("need_imprest", "need_imprest(需预付款)"),
        ("enable_pool", "enable_pool(启用报名池)"),
        ("pool_limit", "pool_limit(报名池容纳人数)"),
        ("pool_stopwatch", "pool_stopwatch(报名池倒计时)"),
        ("open_time", "open_time(报名开放时间)"),
        ("close_time", "close_time(报名关闭时间)"),
        ("launch_time", "launch_time(活动开始时间)"),
        ("finish_time", "finish_time(活动完成时间)"),
        ("spend_time", "spend_time(活动花费时间)"),
        ("create_time", "create_time(创建时间)"),
        ("update_time", "update_time(更新时间)"),
        ("snapshot", "snapshot(快照)"),
        ("remark", "remark(备注)"),
        ("subitem__name", "subitem__name(分项名称)"),
        ("subitem__price", "subitem__price(分项单价)"),
        ("subitem__count", "subitem__count(分项数量)"),
        ("subitem__unit", "subitem__unit(单位)"),
        ("subitem__amount", "subitem__amount(分项小计)"),
        ("subitem__description", "subitem__description(描述)"),
    ]
    ...
    expression_string = models.CharField(verbose_name='逻辑表达式', max_length=65535, blank=True, null=True, help_text='')
...
class EnrollSubitemExtendField(models.Model):
    ...
    id = models.AutoField(verbose_name='ID', primary_key=True, null=False, help_text='清单单价')
    category_id = models.IntegerField(verbose_name='信息类别ID', blank=True, null=True, help_text='')
    ...
    config = models.JSONField(verbose_name='数据配置', blank=True, null=True, help_text='')
    description = models.CharField(verbose_name='描述', max_length=255, blank=True, null=True, help_text='')
...
class EnrollRecord(models.Model):
    ...
    enroll_auth_status = models.ForeignKey(verbose_name='报名权限状态ID', to=EnrollAuthStatus, db_column='enroll_auth_status_id', related_name='+', on_delete=models.DO_NOTHING, blank=True, null=True,
                                           help_text='')
    enroll_pay_status = models.ForeignKey(verbose_name='报名支付状态ID', to=EnrollPayStatus, db_column='enroll_pay_status_id', related_name='+', on_delete=models.DO_NOTHING, blank=True, null=True,
                                          help_text='')
...
class EnrollSubitemRecord(models.Model):
    ...
    enroll_record = models.ForeignKey(
    ...
    enroll_subitem = models.ForeignKey(
