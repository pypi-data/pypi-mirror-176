...
class Predictor(BasePredictor, AnomalyDetector):
    ...
    def infer(self, img):
        ...
        return anomaly_map, resized_img

    def postprocess(self, img, anomaly_map, path):
        if isinstance(self.input_size, int):
            self.input_size = [self.input_size, self.input_size]
        ...
        img = cv2.resize(img, self.input_size).astype(np.uint8)
        ...
        for contour in contours:
            ...
            length_limit = int(pow(area_limit, 0.5) * 0.5)
        ...
        img_drawBox = img.copy()
        for rect in anomaly_area:
            x1, y1, x2, y2 = rect
            cv2.rectangle(img_drawBox, (x1, y1), (x2, y2), (0, 0, 255), 2)

        self.plot_fig(img, anomaly_map, coordinates, img_drawBox, path)
        if getattr(self, 'show_heatmap', False):
            self.plot_fig(img, anomaly_map,coordinates, path)
    ...
    def plot_fig(self, resize_img, anomaly_map,coordinates, img_drawBox, path):
        fig = plt.figure()
        ...
        ax4.axis('off')
        ax4.imshow(img_drawBox, cmap='gray', interpolation='none')
