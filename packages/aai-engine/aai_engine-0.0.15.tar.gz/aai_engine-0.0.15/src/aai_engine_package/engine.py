...
class Task():
    ...
    def __init__(self, name, cwd, script=None, standalone=False):
        ...
        log_time = datetime.strftime(datetime.now(), '%Y%m%d-%H%M%S')
        self.log_dir = sys.argv[1] if not standalone else os.path.join(cwd, 'logs', name, log_time)
        self.execution_logfile = os.path.join(self.log_dir, "execution_log.txt")
        self.zmq_logfile = os.path.join(self.log_dir, "zmq_log.txt")
