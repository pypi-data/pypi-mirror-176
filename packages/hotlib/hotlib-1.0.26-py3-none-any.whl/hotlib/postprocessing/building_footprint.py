...
class BuildingExtract(object):
    ...
    def extract(self, tile, mask):
        ...
        for outer, inner in features.items():
            ...
            feature = unary_union([Polygon(ring) for ring in rings])
            ...
            if type(feature) == MultiPolygon:
                ...
                final_polygon = Polygon(feature.exterior.coords)

                if not final_polygon.is_valid:
                    valid_geom = make_valid(final_polygon)

                    if hasattr(valid_geom, "__len__"):
                        for polygon in valid_geom.geoms:
                            if type(polygon) == Polygon and polygon.area > 0:
                                self.features.append(polygon)
                elif type(final_polygon) == Polygon:
                    self.features.append(final_polygon)
