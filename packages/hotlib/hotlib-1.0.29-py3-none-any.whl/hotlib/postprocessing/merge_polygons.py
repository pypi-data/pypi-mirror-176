...
def merge_polygons(polygons_path, new_polygons_path, distance_threshold):
    ...
    for i, shape in enumerate(
        tqdm(shapes, desc="Building graph", unit="shapes", ascii=True)
    ):
    ...
    for component in tqdm(
        components, desc="Merging components", unit="component", ascii=True
    ):
        ...
        if merged.is_valid:
            ...
            if isinstance(merged, shapely.geometry.polygon.Polygon):
                ...
                merged = [
                    shapely.geometry.polygon.orient(geom, sign=1.0)
                    for geom in merged.geoms
                ]
                merged = shapely.geometry.MultiPolygon(merged)
            else:
                print(
                    "Warning: merged feature is neither Polygon nor MultiPolygon, skipping",
                    file=sys.stderr,
                )
                continue
    ...
    # Uncomment these lines if you need to scale back to the original size
    # gs = GeoSeries([GeoSeries(x).scale(xfact=1/0.7, yfact=1/0.7)[0] for x in features])
    # gs.set_crs(SOURCE_CRS).simplify(TOLERANCE).to_file(new_polygons_path)
