...
def full_pfx(ctx, certificate, key, tmp_dir='/tmp', password=None):
    cert = NamedTemporaryFile(mode='w', delete=False)  # pylint: disable=consider-using-with
    f_key = NamedTemporaryFile(mode='w', delete=False)  # pylint: disable=consider-using-with
    f_pfx = NamedTemporaryFile(mode='wb', delete=False)  # pylint: disable=consider-using-with
    ...
    with open(f_pfx.name, 'rb') as f:
        data = f.read()
    os.remove(cert.name)
    os.remove(f_key.name)
    os.remove(f_pfx.name)
    return data
...
def install_cert_on_windows(
    ...
    filename = f.name
    print('importing pfx to cert store')
    with open(filename, 'wb') as f:
        f.write(pfx_data)
    cmdlet = """
    function Import-PfxCertificate {
        ...
        $pfx.import($filename, $password, 'Exportable,PersistKeySet,MachineKeySet')
        $p = new-object Security.Cryptography.X509Certificates.X509Store($store, $root_store)
        $p.open('MaxAllowed')
        $exists = $false
    ...
    if store:
        store_arg = f"-store '{store}'"
    lines = [
        cmdlet,
        f"$t = Import-PfxCertificate -filename '{filename}' {password} "
        f"{store_arg}",
        'Write-Host $t',
    ]
    c = '\n'.join(lines)
