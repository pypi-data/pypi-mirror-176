...
{% macro spark_cde__file_format_clause() %}
...
{% macro spark_cde__location_clause() %}
...
{% macro spark_cde__options_clause() -%}
...
{% macro spark_cde__comment_clause() %}
...
{% macro spark_cde__partition_cols(label, required=false) %}
...
{% macro spark_cde__clustered_cols(label, required=false) %}
...
{% macro spark_cde__create_temporary_view(relation, sql) -%}
  create temporary view {{ relation.include(schema=false) }} as
    {{ sql }}
{% endmacro %}


{% macro spark_cde__create_table_as(temporary, relation, sql) -%}
  {% if temporary -%}
...
{% macro spark_cde__create_view_as(relation, sql) -%}
...
{% macro spark_cde__create_schema(relation) -%}
  {%- call statement('create_schema') -%}
    create schema if not exists {{relation}}
  {% endcall %}
{% endmacro %}

{% macro spark_cde__drop_schema(relation) -%}
...
{% macro spark_cde__list_relations_without_caching(relation) %}
...
{% macro spark_cde__list_schemas(database) -%}
...
{% macro spark_cde__drop_relation(relation) -%}
...
{% macro spark_cde__generate_database_name(custom_database_name=none, node=none) -%}
  {% do return(None) %}
{%- endmacro %}

{% macro spark_cde__persist_docs(relation, model, for_relation, for_columns) -%}
  {% if for_columns and config.persist_column_docs() and model.columns %}
    {% do alter_column_comment(relation, model.columns) %}
  {% endif %}
{% endmacro %}

{% macro spark_cde__alter_column_comment(relation, column_dict) %}
...
{% macro spark_cde__make_temp_relation(base_relation, suffix) %}
...
{% macro spark_cde__alter_column_type(relation, column_name, new_column_type) -%}
...
{% macro spark_cde__alter_relation_add_remove_columns(relation, add_columns, remove_columns) %}
