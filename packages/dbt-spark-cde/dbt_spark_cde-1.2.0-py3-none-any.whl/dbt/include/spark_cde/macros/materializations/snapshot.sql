{% macro spark_cde__snapshot_hash_arguments(args) -%}
    md5({%- for arg in args -%}
...
{% macro spark_cde__snapshot_string_as_time(timestamp) -%}
    {%- set result = "to_timestamp('" ~ timestamp ~ "')" -%}
    {{ return(result) }}
{%- endmacro %}


{% macro spark_cde__snapshot_merge_sql(target, source, insert_cols) -%}
...
{% macro spark_cde__post_snapshot(staging_relation) %}
    {% do adapter.drop_relation(staging_relation) %}
{% endmacro %}


{% macro spark_cde__create_columns(relation, columns) %}
    {% if columns|length > 0 %}
    {% call statement() %}
