...
from softwares.utils.py_util import hash_url_to_filename
...
class ExtractManager:
    def __init__(self, cache_dir: Optional[str] = None):
        self.extract_dir = (
            os.path.join(cache_dir, config.EXTRACTED_SOFTWARE_DIR)
            if cache_dir
            else config.EXTRACTED_SOFTWARE_PATH
        )
        self.extractor = Extractor

    def _get_output_path(self, path: str) -> str:
        # Path where we extract compressed archives
        # We extract in the cache dir, and get the extracted path name by
        # hashing the original path"
        abs_path = os.path.abspath(path)
        return os.path.join(
            self.extract_dir, hash_url_to_filename(abs_path)
        )  # type: ignore

    def _do_extract(self, output_path: str, force_extract: bool):  # type: ignore
...
class BaseExtractor(ABC):
    ...
    @abstractmethod
    def extract(
        input_path: Union["pathlib.Path", str], output_path: Union["pathlib.Path", str]
    ) -> None:
        ...
...
class MagicNumberBaseExtractor(BaseExtractor, ABC):
    ...
    @classmethod
    def is_extractable(  # type: ignore
        cls, path: Union["pathlib.Path", str], magic_number: bytes = b""
    ) -> bool:
        if not magic_number:
            try:
    ...
    def safemembers(members):
        ...
        Desc:
            Directory traversal vulnerability in the (1) extract and (2) extractall
             functions in the tarfile module in Python allows user-assisted remote
              attackers to overwrite arbitrary files via a .. (dot dot)
        ...
        for finfo in members:
            if badpath(finfo.name, base):
                ...
                logger.error(
                    f"Extraction of {finfo.name} is blocked: Hard"
                    f" link to {finfo.linkname}"
                )
            elif finfo.islnk() and badlink(finfo, base):
                logger.error(
                    f"Extraction of {finfo.name} is blocked:"
                    f" Symlink to {finfo.linkname}"
                )
            else:
                yield finfo
...
class RarExtractor(BaseExtractor):
    ...
    def extract(
        ...
        if not config.RARFILE_AVAILABLE:  # type: ignore
...
class ZstdExtractor(MagicNumberBaseExtractor):
    ...
    def extract(
        ...
        if not config.ZSTANDARD_AVAILABLE:  # type: ignore
...
class SevenZipExtractor(MagicNumberBaseExtractor):
    ...
    def extract(
        ...
        if not config.PY7ZR_AVAILABLE:  # type: ignore
...
class Lz4Extractor(MagicNumberBaseExtractor):
    ...
    def extract(
        ...
        if not config.LZ4_AVAILABLE:  # type: ignore
...
class Extractor:
    ...
    #  (I guess it means: as tar or gzip)
    extractors = {
        "tar": TarExtractor,
        "gzip": GzipExtractor,
        "zip": ZipExtractor,
        ...
        for extractor in cls.extractors.values():
            if hasattr(extractor, "magic_number"):
                ...
                magic_number_max_length = (
                    magic_number_length
                    if magic_number_length > magic_number_max_length
                    else magic_number_max_length
                )
    ...
    @classmethod
    def is_extractable(
        cls, path: Union["pathlib.Path", str], return_extractor: bool = False
    ) -> bool:
        warnings.warn(
            "Method 'is_extractable' was deprecated in version 2.4.0 and"
        ...
        if extractor_format:
            return (
                ...
                else (True, cls.extractors[extractor_format])  # type: ignore
            )
        return False if not return_extractor else (False, None)  # type: ignore

    @classmethod
    def infer_extractor_format(
        cls, path: Union["pathlib.Path", str]
    ) -> str:  # <Added version="2.4.0"/>
    ...
    def extract(
        ...
        extractor: Optional[BaseExtractor] = "deprecated",  # type: ignore
    ) -> None:
        ...
        with FileLock(lock_path):  # type: ignore
            ...
            if extractor_format or extractor != "deprecated":
                if extractor != "deprecated" or not isinstance(
                    extractor_format, str
                ):  # passed as positional arg
                    warnings.warn(
                        "Parameter 'extractor' was deprecated in version 2.4.0 and"
                        " will be removed in 3.0.0. "
                    ...
                    extractor = (
                        extractor
                        if extractor != "deprecated"
                        else extractor_format  # type: ignore
                    )
                else:
                    extractor = cls.extractors[extractor_format]  # type: ignore # noqa
                return extractor.extract(input_path, output_path)  # type: ignore
            else:
                warnings.warn(
                    "Parameter 'extractor_format' was made required in"
                    " version 2.4.0 and not passing it will raise an "
                    "exception in 3.0.0.",
                    category=FutureWarning,
                )
                for extractor in cls.extractors.values():  # type: ignore
                    if extractor.is_extractable(input_path):  # type: ignore
                        return extractor.extract(input_path, output_path)  # type: ignore # noqa
