...
from softwares.artifacts.utils import test_artifacts_path
from softwares.load import (
...
from softwares.module_test import FooPromptware
from softwares.promptware import PromptConfig, Promptware
from softwares.utils.file_utils import DownloadConfig
...
class TestLoad(TestCase):
    ...
    def test_import_main_class(self):
        ...
        module_path = "softwares.module_test"
    ...
    def test_local_evaluation_module_factory(self):
        path = os.path.join(self.artifact_module_path, "boo_prompt.py")

        imported_module_config = LocalEvaluationModuleFactory(
            path=path,
            download_config=DownloadConfig(extract_compressed_file=True),
        ).get_module()
    ...
    def test_evaluation_module_factory(self):
        path = os.path.join(self.artifact_module_path, "boo_prompt.py")

        imported_module_config = evaluation_module_factory(
            path,
            download_config=DownloadConfig(extract_compressed_file=True),
        )
        self.assertEqual(
    ...
    def test_hub_evaluation_module_factory(self):

        imported_module_config = HubEvaluationModuleFactory(
            name="sentiment",
