...
from softwares.utils import logging
from softwares.utils.extract import ExtractManager
...
def ep_github_url(
    path: str, name: str, software=True, revision: Optional[str] = None
) -> str:

    revision = revision or os.getenv("EP_SCRIPTS_VERSION", config.SCRIPTS_VERSION)
    if software:
        return config.HUB_SOFTWARE_URL.format(revision=revision, path=path, name=name)
...
def cached_path(
    ...
    **download_kwargs,
    ...
    cache_dir = download_config.cache_dir or config.DOWNLOADED_SOFTWARE_PATH
...
def ep_hub_url(path: str, name: str, revision: Optional[str] = None) -> str:
    ...
    return config.HUB_SOFTWARE_URL.format(path=path, name=name, revision=revision)
...
def get_from_cache(
    ...
    if cache_dir is None:
        cache_dir = config.EP_SOFTWARE_CACHE
    if isinstance(cache_dir, Path):
        cache_dir = str(cache_dir)
