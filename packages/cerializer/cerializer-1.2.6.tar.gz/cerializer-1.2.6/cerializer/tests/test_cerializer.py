...
def test_fastavro_compatibility_serialize(
	...
	try:
		data_all = yaml.unsafe_load_all(
			open(os.path.join(schema_root, 'example.yaml'), encoding = 'utf-8'))
		SCHEMA_FAVRO = fastavro.parse_schema(
			yaml.load(
				open(os.path.join(schema_root, 'schema.yaml'), encoding = 'utf-8'), Loader = yaml.Loader
			)
		)
