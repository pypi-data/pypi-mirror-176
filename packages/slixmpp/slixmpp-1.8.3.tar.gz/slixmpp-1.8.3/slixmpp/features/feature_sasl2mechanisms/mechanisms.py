...
class FeatureSASL2Mechanisms(BasePlugin):

    name = 'feature_sasl2mechanisms'
    description = 'XEP-0388: Extensible SASL Profile'
    ...
    def plugin_init(self):
        ...
        register_stanza_plugin(StreamFeatures, stanza.SASL2Mechanisms)

        self.xmpp.register_stanza(stanza.SASL2Success)
        self.xmpp.register_stanza(stanza.SASL2Failure)
        self.xmpp.register_stanza(stanza.SASL2Auth)
        self.xmpp.register_stanza(stanza.SASL2Challenge)
        self.xmpp.register_stanza(stanza.SASL2Response)
        self.xmpp.register_stanza(stanza.SASL2Abort)

        self.xmpp.register_handler(
                Callback('SASL2 Success',
                         MatchXPath(stanza.SASL2Success.tag_name()),
                         self._handle_success,
                         instream=True))
        self.xmpp.register_handler(
                Callback('SASL2 Failure',
                         MatchXPath(stanza.SASL2Failure.tag_name()),
                         self._handle_fail,
                         instream=True))
        self.xmpp.register_handler(
                Callback('SASL2 Challenge',
                         MatchXPath(stanza.SASL2Challenge.tag_name()),
                         self._handle_challenge))

        self.xmpp.register_feature('sasl2mechanisms',
    ...
    def _handle_sasl_auth(self, features):
        """
        Handle authenticating using SASL2.
