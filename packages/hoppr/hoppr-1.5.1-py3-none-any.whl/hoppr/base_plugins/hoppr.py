    ...
    def wrapper(self, *args, **kwargs) -> Result:
        ...
        if comp is not None:
            if comp.purl is None:
                ...
                component_header = f"{comp.name}" + (
                    f"@{comp.version}" if comp.version is not None else ""
                )
...
def hoppr_rerunner(method: Callable) -> Callable:
    ...
    def wrapper(self, *args, **kwargs) -> Result:
        ...
        for attempt in range(self.context.max_attempts):
            log.info(
                msg=f"Processing component [attempt {attempt + 1} of {self.context.max_attempts}]",
                indent_level=1,
            )
            ...
            if attempt < self.context.max_attempts - 1:
                ...
                log.warning(
                    msg=f"Result message for attempt {attempt + 1}: {result.message}",
                    indent_level=1,
                )
        ...
        log.error(
            msg=f"Method {method.__name__} failed after {self.context.max_attempts} attempts",
        )

        log.error(
            msg=f"Result message for final attempt: {result.message}",
        )
...
class HopprPlugin(ABC):
    ...
    def run_command(
        self, command, password_list=None, cwd=None
        ...
        self.get_logger().debug(
            msg=f"Running command: '{obscured_command}'", indent_level=2
        )
        ...
        if result.returncode != 0:
            ...
            self.get_logger().debug(
                msg=f"{command[0]} command stdout content:\n{stdout_indented}",
                indent_level=2,
            )
            ...
            self.get_logger().error(
                msg=f"{command[0]} command failed with error:\n{stderr_indented}",
                indent_level=2,
            )
