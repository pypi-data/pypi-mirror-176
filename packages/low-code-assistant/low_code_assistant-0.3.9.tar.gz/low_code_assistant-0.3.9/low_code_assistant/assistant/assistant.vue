<template>
  <div>
    <div>Low Code Assistant™ initialized. (v{{ version }})</div>
...
module.exports = {
    ...
    code(value) {
      ...
      const afterActivate = () => {
        ...
        if (value.type_ && value.type_ === "markdown") {
          this._cell = LCA.changeCellType(this.notebookId, this._cell, "markdown")
        } else {
          this._cell = LCA.changeCellType(this.notebookId, this._cell, "code")
        }
        LCA.runAndAdvance(this.notebookId);
        this.injectCells();
        this.code = null;
      }
      // activate is async, but we cannot wait on it, ugly workaround
      setTimeout(afterActivate, 300);
    },
  ...
  methods: {
    ...
    show(cell) {
      ...
      this.syncStyle(cell)
      this.code_up = {code: cell.model.type === 'markdown' && cell.model.value.text, meta: cell.model.metadata.get("assistant") || null, type_: cell.model.type}
    },
    cleanup() {
