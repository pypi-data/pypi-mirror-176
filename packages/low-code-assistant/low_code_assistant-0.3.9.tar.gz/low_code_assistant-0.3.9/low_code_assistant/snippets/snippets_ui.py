...
from low_code_assistant.snippets.snippets import Snippet
...
def SnippetsPanel(
    ...
    reacton.use_memo(on_path_change, [path])
    ...
    with sol.Div(style_="height: 100%; display: flex; flex-direction: column;") as main:
        ...
        with sol.Div(style_="display: flex"):
            v.Tooltip(
                bottom=True,
                v_slots=[
                    {
        ...
        with v.Dialog(width="400px", v_model=delete_dialog, on_v_model=lambda v: set_delete_dialog(v)):
            with v.Card():
...
def AddDialog(writable_paths, open, on_open, on_add_snippet, path=[{}]):
    ...
    with v.Dialog(width="400px", v_model=open, on_v_model=lambda v: on_open(v)) as main:
        with v.Card():
