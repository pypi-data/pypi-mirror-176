...
class ActionOpen(Action):
    ...
    def render_code(self, df_var_in):
        ...
        fn_name = self.filename.lower().split(".")[-1]
        params = ""
        ...
        return textwrap.dedent(
            f"""\
            import pandas as pd
            from low_code_assistant import app
...
class ActionDemo(Action):
    ...
    def render_code(self, df_var_in):
        if self.df_name in dir(plotly.data):
            ...
            import_part = "import low_code_assistant as lca"
            pd_part = f"lca.data.{self.df_name}()"
...
class ActionDownloadDataSource(Action):
    ...
    def render_code(self, df_var_in):
        package = "low_code_assistant.domino_lab_mock" if in_dev_mode() else "domino.data_sources"

        if self.type_ in ["S3Config", "GCSConfig"]:
            assert self.database
            s3_key = self.database
            fn_name = s3_key.lower().split(".")[-1]
            return textwrap.dedent(
                f"""\
                import io
                import pandas as pd
                ...
                buffer = io.BytesIO()
                ds.download_fileobj("{s3_key}", buffer)

                buffer.seek(0)
                {self.df_var_out} = pd.read_{fn_name}(buffer)
                """
            )
...
class ActionFilter(Action):
    ...
    def render_code(self, df_var_in):
        if not self.is_string and self.value == "nan":
