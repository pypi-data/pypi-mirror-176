...
class TestCase(Trial):
    ...
    def checkReturnedStructure(self, expectedStructure):
        ...
        the result using `memoryReport`. The `expectedStructure` may be
        a string, in which case the `memoryReport` of the return value
        must match it, or if it's not a string, it's `memoryReport` will
        be compared against the `memoryReport` of the result.
        """
        results = self.fetchResults()
    ...
    def checkVariableStructure(self, varName, expectedStructure):
        ...
        if results["error"] is not None:
            ...
            if not isinstance(expectedStructure, str):
                expectedStructure = memoryReport(expectedStructure)
            actualStructure = memoryReport(value)
            firstDiff = findFirstDifference(
                actualStructure,
                expectedStructure
            )
...
def memoryMap(obj, assigned, count_from=0):
    """
    ...
    {
        <id1>: (0, [ MemReference(1), 3, MemReference(1) ]),
        <id2>: (1, [1, 2])
    }
    ...
    `MemReference` instance if it's large. Only tuples, lists, sets, and
    ...
    if id(obj) in assigned:
        return None, MemReference(assigned[id(obj)][0])
    ...
    if isinstance(obj, (int, float, complex, bool, type(None))):
        ...
        return count_from - 1, MemReference(original_n)
        ...
        return count_from, MemReference(count_from)


def memoryReport(obj):
    """
    ...
    """
    refs = {}
    _ = memoryMap(obj, refs, 0)  # modifies ref; we ignore the result
