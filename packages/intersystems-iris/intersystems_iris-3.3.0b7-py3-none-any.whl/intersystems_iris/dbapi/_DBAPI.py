...
from .._IRISNative import connect
...
class Cursor():
    ...
    def _preparse(self):
        ...
        for i, item in enumerate(self.params):
            ...
            elif intersystems_iris._DBList._DBList._set_switcher.get(type(item), None) != None:
                pass
            else:
                raise TypeError("Unsupported argument type: " + str(type(item)))
            if i == 0:
        ...
        parser = intersystems_iris.dbapi.preparser._PreParser._PreParser(self._connection._connection_info._delimited_ids)
        try:
        ...
        if self._statementType == StatementType.CALL or self._statementType == StatementType.CALLWITHRESULT:
            if len(self._params._params_list) != len(cached_statement._params._params_list):
                if (self._statementType == StatementType.CALL and
                    ...
                    self._params._params_list.insert(0, intersystems_iris.dbapi._Parameter._Parameter(ParameterMode.OUTPUT, 'c'))
                else:
