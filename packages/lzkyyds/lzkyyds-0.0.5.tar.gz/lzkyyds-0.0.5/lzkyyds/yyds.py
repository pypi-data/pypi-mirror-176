...
class Api:
    ...
    def api(uri: str, options=None):
        ...
        return yydsExt.http(uri, params)

    @staticmethod
    def sleep(delay: float):
        time.sleep(float(float))
...
class ToolClass(Api):
    ...
    def __get_screen_size(self):
        res = self.api("/shell", {"cmd": "dumpsys window displays | grep init"})
        res = res.lstrip().split(" ")[0].split("=")[1].split("x")
        self.device_width = float(res[0])
        self.device_height = float(res[1])
    ...
    def start_app(self, name: str, img):
        ...
        while True:
            ...
            if img.click(name).is_click(5):
                ...
                self.swipe(self.device_width * 0.85, self.device_height * 0.5, self.device_width * 0.2, self.device_height * 0.5, 250)
...
class ImageHandler(Api):
    ...
    def __init__(self, img_list: dict, img_url: str, project_name="test"):
        self.__isClick = False
        self.__img_dict: dict = img_list
        self.__img_url = img_url
        self.__to_path = "img/"
        self.__img_path = "/sdcard/Yyds.Py/" + project_name + "/img/"
    ...
    def __find_images(self, img_name):
        ...
        if res:
            ...
            for line in img_res_list:
                if line:
                    img_path, img_params = line.split('\t')
                    name = img_path.split('/')[-1]
                    prob, wh, point = img_params.split()
            ...
            result_list = self.__img_filter(temp_list)
            if type(img_name).__name__ == 'list':
                for name in img_name:
                    result = result_list.get(name)
                    if result:
                        break
            else:
                result = result_list.get(img_name)
        return result

    def click(self, img_name: str):
        self.__isClick = False
        ...
        if res:
            ...
            x = res["x"]
            y = res["y"]
            y += height * 0.25 + random.uniform(height * 0.25, height * 0.75)
            x += width * 0.25 + random.uniform(width * 0.25, width * 0.75)
            self.api("/touch", {"x": int(x), "y": int(y)})
            self.__isClick = True
        return self
