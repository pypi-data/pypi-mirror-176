...
class RotatingFileLogger:
    @staticmethod
    def GetLogger(filepath, minimumLogLevel=_logging.DEBUG, maxBytes=_Conversion.Bytes.MB * 100, maxRotations=10, useUTCTime=True):
...
class StdoutLogger:
    @staticmethod
    def GetLogger():
        stdoutLogger = _logging.getLogger("__STDOUTLOGGER__")
        if(stdoutLogger.hasHandlers()):
            return stdoutLogger
        handler = _logging.StreamHandler(_sys.stdout)
        handler.setLevel(_logging.DEBUG)
        handler.setFormatter(_BaseLogger.DefaultFormatter(useUTCTime=False))
