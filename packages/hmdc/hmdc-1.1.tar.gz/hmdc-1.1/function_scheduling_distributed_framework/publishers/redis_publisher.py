...
class RedisPublisher(AbstractPublisher, RedisMixin):
    ...
    def concrete_realization_of_publish(self, msg):
        conn = self.redis_db_frame.get_redis()
        self.conn.rpush(self._queue_name, msg)

    def clear(self):
        conn = self.redis_db_frame.get_redis()
        conn.delete(self._queue_name)
        conn.delete(f'{self._queue_name}__unack')
        self.logger.warning(f'清除 {self._queue_name} 队列中的消息成功')

    def get_message_count(self):
        # nb_print(self.redis_db7,self._queue_name)
        conn = self.redis_db_frame.get_redis()
        return conn.llen(self._queue_name)
