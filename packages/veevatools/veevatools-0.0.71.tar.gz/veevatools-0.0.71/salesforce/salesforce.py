...
#                     return pd.DataFrame(columns=results)
    ...
    async def field_permission_user_check(self, objects, permissions, users=[]):
        ...
        for profile in profiles:
                profile_result = task_result_list[profiles.index(profile)]
                if 'fieldPermissions' in profile_result:
                        ...
                        profile_metadata_dict["System Administrator" if profile == 'Admin' else profile] = pd.DataFrame()
