...
def _parse_github_tags(url: str, soup: BeautifulSoup) -> list:
    tag_versions: list = []
    tags: Optional[ResultSet] = soup.find_all(
...
def _parse_gitlab_tags(url: str, soup: BeautifulSoup) -> list:
    tag_versions: list = []
    tags: Optional[ResultSet] = soup.find_all(
        "li",
        class_="flex-row js-tag-list gl-white-space-normal! gl-align-items-flex-start!",
    )
    if not tags:
        raise Exception(f"No tags found for repo: {url}")
    for tag in tags:
...
def _parse_tags(repo: dict, soup: BeautifulSoup) -> list:
    url: str = repo["repo"]
    if "github.com" in url:
        tag_versions = _parse_github_tags(url, soup)
    elif "gitlab.com" in url:
        tag_versions = _parse_gitlab_tags(url, soup)
    else:
        raise Exception(f"Unsupported provider for repo: {url}")
    return tag_versions
...
async def run(dry_run: bool, all_versions: bool, verbose: bool) -> None:
    ...
    try:
        ...
        for i in range(len(data["repos"])):
            ...
            tag_versions: list = _parse_tags(rep, results[i])
