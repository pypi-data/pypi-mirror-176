...
from metlib.normet.cache import Cache
...
class TimeValues:
    ...
    def get_values(self,date: datetime, url, values):
        ...
        if "ilon" not in values:
            lons, lats = self.cache.get_root_variables(url, [lon_name,lat_name])
            ilon, ilat, lon_a, lat_a = TimeValues.__find_nearest(lons, lats,lon_pos, lat_pos)
            ...
            with nc.Dataset(url,"r") as f0:
                ...
                with nc.Dataset(date_cache_path, 'w', format='NETCDF4') as dst:
                    ...
                    for name in requested_values:
                        ...
                        vals = var_values[ilon,ilat,:]
                        dst.createVariable(name, np.float64, ["time"])[:] = vals
                        new_values[name]=vals
                    TimeValues.__concatenate_arrays(values, new_values)
        else:
            with nc.Dataset(date_cache_path,"r") as f0:
                TimeValues.__concatenate_variables(values, f0, new_values)
    ...
    def __concatenate_variables(values, f, arrays):
        ...
        TimeValues.__concatenate_arrays(values, arrays)
