    ...
    def decorator(function):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                data = {"func_name":funcName,"status":"SUCCESS","message":"START"}
                if CallBack:CallBack(data)
                ret = function(*args, **kwargs)
                
                if isinstance(ret,Trace):
                    if ret:
                        data = {"func_name":funcName,"status":"SUCCESS","message":"FINISH"}
                        if CallBack:CallBack(data)
                        return ret
                    else:
                ...
                data = {"func_name":funcName,"status":"FAILURE","message":err.rsp.Excpt}
                if CallBack:CallBack(data)
                if main: return
                raise TraceError(err.rsp.back())
            
            except:
                data = {"func_name":funcName,"status":"FAILURE","message":str(sys.exc_info()[1])}
                if CallBack:CallBack(data)
                if main: return
