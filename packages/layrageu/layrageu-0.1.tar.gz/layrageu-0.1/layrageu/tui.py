...
Usage:
  tui.py send <document> [--cw=title] [--preview] [--nopager] [--verbose]
...
class Layrageu:
    COMMANDS = ["preview", "send", "configure"]
    ...
    def cmd_send(self) -> int:
        ...
        # NOTE Right now, Layrageu is mono-instance, but we have the code
        # to be multi-instance. Some work on configuration tools is just
        # needed.
    ...
    def cmd_configure(self) -> int:
        ...
        opt_errors = False
        has_instance = self.settings.has_instance(self.arguments["--name"])
...
def main():
    ...
    app = Layrageu(ARGUMENTS, debug_mode)
