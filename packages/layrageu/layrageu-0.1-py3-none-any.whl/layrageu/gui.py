...
from layrageu import TOOT_VISIBILITIES
...
def validate_instance(url, usermail, init_tootv, other_tootv):
    """
...
class FirstTimeSettings(tkinter.Toplevel):
    ...
    def validate(self):
        ...
        validated, message = validate_instance(
...
class Layrageu(ttk.Frame):
    ...
    def __init__(
        ...
        settings_dir = Path(user_config_dir("layrageu")).resolve(False)
        settings_dir.mkdir(exist_ok=True)

        settings_file = settings_dir / "settings.ini"
        as_new = not settings_file.exists()

        self.settings = Settings(settings_file, as_new)
    ...
    def init_menu(self) -> NoReturn:
        ...
        instances_menu = new_menu(
            app_menu,
            [
                ["Configure", self.open_settings],
                # TODO ["New", self.new_instance],
            ],
        )
    ...
    def refresh_settings(self):
        """
        Refresh settings when Settings window is destroyed with
    ...
    def init_ui(self) -> NoReturn:
        ...
        send_btn.grid(column=2, row=4, columnspan=3, padx=5, sticky=tkinter.EW)

        prev_btn = ttk.Button(
            self.main_frame, text="Preview", command=self.preview
        )
        prev_btn.grid(column=1, row=4, padx=5)
    ...
    def send(self) -> bool:
        ...
        result = send_toots(
            instance_name, self.settings, self.toots, password, content_warning
        )
...
def main() -> NoReturn:
    ...
    # --- Tkinter / PAREOtools configuration startup ---------------------
