...
class LoxSwitch:
    ...
    def set_value(self, stateName, value):
        if self._device_type == "Switch" and stateName == "active":
            ...
            self.async_update()
        elif self._device_type == "TimedSwitch" and stateName == "deactivationDelayTotal":
            _LOGGER.debug("id:'{0}', name:'{1}', [SetValue Switch] - deactivationDelayTotal={2}".format(self._id, self._name, value))
            if value == 0: 
                self._state = False
            elif value == -1:
                self._state = True
