...
async def request_oauth_dep(
    ...
    sub = get_origin_from_subdomain(sub, OrbroConnect.host)
    origin = origin if origin is not None else sub
    logger.debug('request_oauth_dep: header info sub -> {sub} / origin -> {origin}'.format(origin=origin, sub=sub))

    try:
        r = await OrbroConnect.token_util.async_verify_oauth_token_by_remote(token, origin)
        res = json.loads(r.text)
        return res
    except Exception as e:
        raise HTTPException(status_code=401, detail=str(e))
