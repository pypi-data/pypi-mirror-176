...
class mainFormat(qtWidget.QWidget):
    ...
    def mullerPlotAction(self):
        ...
        if fname == "":
            ...
            return
        self._th_muller = (Process(target=external_plots.mullerPlot, args=(fname, self._file_path.text() + "/Figures")))
    ...
    def cloneHistAction(self):
        ...
        if all(map(lambda x: x.endswith('.txt'), fname)):
            self._th_hist = (Process(target=external_plots.binnedHist, args=(fname, self._file_path.text(), name_id)))
            ...
            self._th_hist = (Process(target=external_plots.singleHist, args=(fname, self._file_path.text(), name_id)))
            # external_plots.singleHist(fname, self._file_path.text(), name_id)
            self._th_hist.start()
            self.status.setText("mutations histograms ongoing")
        elif all(map(lambda x: x.endswith('.mtx'), fname)):
            self._th_hist = (Process(target=external_plots.matrixHist, args=(fname, self._file_path.text(), name_id)))
