    ...
    for box_to_write in picking_results:
        ...
        if "is_3d" in box_to_write and box_to_write["is_3d"] == True:
            ...
            for boxes_index, boxes in enumerate(box_set):
                ...
                if not is_tomo:
                    if outdir is not None:
                        filename = os.path.basename(csparc_path)
                        csparc_path = os.path.join(outdir, "CRYOSPARC", filename)

                    # Create directory if it does not existes
                    if not os.path.exists(os.path.dirname(csparc_path)):
                        os.makedirs(os.path.dirname(csparc_path))
                    CoordsIO.write_cryosparc_star_file(path=csparc_path, coordinates=boxes, micrograph_name=os.path.basename(box_to_write['img_path']))
