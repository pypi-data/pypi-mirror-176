...
def apply_fft_mask(img, mask):
    ...
    print("SHAPE MASK:", mask.shape, img_fft.shape)
...
def filter_single_image_from_np_array(image, filter_cutoff, resize_to_shape=None, channel_specifiction=0):
    ...
    for i in range(image.shape[channel_specifiction]):
        ...
        mask_size_0 = fft.next_fast_len(img.shape[0]) # next_power_of2(img.shape[0])
        mask_size_1 = fft.next_fast_len(img.shape[1]) # next_power_of2(img.shape[1] )
