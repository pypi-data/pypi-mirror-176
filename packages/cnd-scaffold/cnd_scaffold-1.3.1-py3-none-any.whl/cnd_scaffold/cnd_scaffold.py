...
class CndScaffold:
    ...
    def __init__(self, source, target, data_to_replace, cnd_io, print, cnd_io_target=None):
        self._print = print
        self._source = source
        self._target = target
        self._cnd_io = cnd_io
    ...
    def _load_files(self, step):
        ...
        for item in self.yaml_structure[step]:
            repeat = {'values': [''], 'key': ''}
            target = item['name']
            content = self._cnd_io.pull_file(self._source['project_id'], f'{self.yaml_structure["source_folder"]}/{item["name"]}', branch=self._source['branch'])
            if content is False:
            ...
            for value in repeat['values']:
                self._cnd_io_target.commit_file(
                    self._target['project_id'],
                    f"{self._target['folder']}/{self._replace_content(target, {repeat['key']: value})}",
                    self._replace_content(content, {repeat['key']: value}),
                    self._target['branch']
                )
        return len(self._cnd_io_target._files[self._target['project_id']][self._target['branch']])
