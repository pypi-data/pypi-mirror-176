...
from stqe.host.lsm import yield_lsm_config, translate_cache_policy
...
def volume_write_cache_policy_update_success():
    ...
    for config in yield_lsm_config():
        ...
        ret, data = atomic_run("Getting write cache policy of volume %s." % vol,
        ...
        for policy in ["WB", "AUTO", "WT", "wb", "auto", "wt"]:
            atomic_run("Updating policy of volume %s to be %s with protocol %s." % (vol, policy, config['protocol']),
                       command=lsm.volume_write_cache_policy_update,
                       vol=vol,
                       policy=policy,
                       errors=errors)

        atomic_run("Returning write policy of volume %s to former state '%s' with protocol %s." %
