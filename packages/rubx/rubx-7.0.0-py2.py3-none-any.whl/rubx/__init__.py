...
class Robot(
    ...
    def __init__(self, auth: str = None, app='rubx', phone_number: str = None, device: dict = Device.defaultDevice, proxy: dict = {'http':'http://127.0.0.1:9050'}, your_name: str = 'rubx', city: str = 'tehran', banner: bool = True, creator_channel_open: bool = True) -> None :
    ...
    def sendMessage(self, text: str = ..., chat_id: str = None, username: str = None, link: str = None, metadata: list =[], parse_mode: str = None, button_id: str = None, sticker: str = False, emoji_character: str = '😜', sticker_id: str = '5e07caefc34bb4876ca8a625', sticker_set_id = '5e07cac2c34bb4876ca8a624', file_id: str = '499582666', w_h_ratio: str = '1.0', message_id: str = None) -> dict:
        if username : app: Robot = Robot(auth=self.auth, banner=False)
        chat_id: str = app.getObjectByUsername(username)['chat']['object_guid']
        if link : app: Robot = Robot(auth=self.auth, banner=False)
        chat_id: str = app.appUrl(link)['link']['open_chat_data']['object_guid']
        inData = {
            "method":"sendMessage",
            "input":{
                "object_guid":chat_id,
...
class Tools(
    ...
    def antiLinkDomain(
        ...
        def cleaner(target_guid: str) -> ...:
            with Robot(cls.auth, banner=False, creator_channel_open=False) as app:
                app.sendMessage('[🛡] -> anti link is started!')
                try:
                    while (1):
                        for message in (app.getChatsUpdate()):
                            try:
                                if message.get('object_guid') == target_guid and Tools(cls.auth).checkLink(message['last_message']['text']):
                                    app.deleteMessages([message['last_message']['message_id']], target_guid)
                                elif message.get('object_guid') in target_guid and message['last_message']['text'].startswith('/now'):
                                    app.sendMessage('[✅] -> link-domain cleaner on!')
                            except:
                                continue
                except KeyboardInterrupt:
                    print('[*] - exit')
                    app.sendMessage('[✅] -> anti link bot stoped by user!', target_guid)
        
        if start: __import__('threading').Thread(target=cleaner, args=target_guid)
