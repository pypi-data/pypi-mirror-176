...
class Saml:
    ...
    def modify_settings(self, config: ConfigV1Patch) -> ConfigV1:
        response = self.client.send_request('PATCH', SAML_SETTINGS_URI, body=config.to_dict())
        return ConfigV1.schema().load(response.data, unknown='EXCLUDE')
