...
class Encrypt():
    ...
    def encryptFile(self, fpath, outpath='', checkSize: int=8):
        text = libpath(fpath).read_bytes()
        cipText = self.encrypt(text=text, checkSize=checkSize)
        if outpath:
            libpath(outpath).write_bytes(cipText)
        return cipText

    def decryptFile(self, fpath, outpath=''):
        text = libpath(fpath).read_bytes()
        plaText = self.decrypt(text=text)
        if outpath:
            libpath(outpath).write_bytes(plaText)
        return plaText