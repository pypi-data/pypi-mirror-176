...
class Basher:
    ...
    def __init__(
        ...
        self.message = msg
        self.debug = debug
        self.debug_errors_only = False
        if 'debug' in podarr.__arguments__:
            self.debug = True
        if 'debugerrors' in podarr.__arguments__:
            self.debug_errors_only = True
        self.shell = shell
        self.delay = 0.2
        self.yellow = "\033[93m"
        ...
        self.drained_data = []
        self.drainer_thread = threading.Thread(
            target=self.drained_data.extend, args=(self.result.stdout,))
        self.drainer_thread.daemon = True
        self.drainer_thread.start()
    ...
    def print_debug(self):
        ...
        self.timer_end = time()
        if self.debug or (self.debug_errors_only and not self.return_bool):
            if self.wait:
                ...
                if self.return_bool and not self.debug_errors_only:
                    print('-----STDOUT-----')
                    print(self.stdout)
                    print('----------------')
    ...
    def descriptor(self) -> None:
        ...
        self.drained_data = b''.join(self.drained_data)
        if self.return_code == 0:
            ...
            if self.decode:
                ...
                self.stdout = str(self.drained_data)
        else:
            self.return_bool = False
            if self.message is not None:
            ...
            if self.decode:
                ...
                self.stderr = str(self.drained_data)
