    ...
    async def json_dump(data_info):
        ...
        if data_info['data_type'] == 'Coco':
            if jpg.is_file():
                ...
                # 针对特殊目录手动修改并组合
                coco_image_dir = PathTest.initialize(data_info['image_dir']).parent
                self_path = data_info['image_file'].split('/')
                copy_path = os.path.join(data_info['input_dir'], coco_image_dir, self_path[2], '00.images',
                                         self_path[3])
                # 修改labelme_dir和image_dir
                data_info['image_dir'] = os.path.join(os.path.dirname(data_info['image_dir']), self_path[2],
                                                      os.path.basename(data_info['image_dir']))
                data_info['labelme_dir'] = os.path.join(os.path.dirname(data_info['labelme_dir']), self_path[2],
                                                        os.path.basename(data_info['labelme_dir']))
                # 同时修改json路径imagesPath的相对路径
                data_info['labelme_info']['imagePath'] = os.path.join('../', '00.images', self_path[3]).replace('\\',
                                                                                                                '/')
                # 组合保存图片的文件夹路径
                save_images_dir = os.path.join(data_info['output_dir'], data_info['image_dir'])
                os.makedirs(save_images_dir, exist_ok=True)
