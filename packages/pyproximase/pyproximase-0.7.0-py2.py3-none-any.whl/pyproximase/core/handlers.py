...
from .types import DeCollectionStats
...
class BaseHandler:
    ...
    def _parse_response(pb_or_txt,
        ...
        if isinstance(pb, common_pb2.Status):
            ...
            return ProximaSeStatus.from_pb(pb)
        status = ProximaSeStatus.from_pb(pb.status)
    ...
    @classmethod
    def _parse_collection_stats(self, response, message=None):
        if message is not None:
            response = Parse(response, message)
        status, collection_stats = self._parse_response(
                                        response, CollectionStats, 'collection_stats')
        status, de_collection_stats = self._parse_response(
                                        response, DeCollectionStats, 'de_collection_stats')
        if collection_stats is not  None:
            return status, collection_stats
        return status, de_collection_stats
...
class HttpHandler(BaseHandler):
    ...
    _QUERY_BY_SQL_URL = "/v1/collection/sql"
    ...
    def describe_collection(self, collection_name):
        ...
        return self._parse_response(
            ...
            proxima_se_pb2.DescribeCollectionResponse())
    ...
    def list_collections(self, list_condition):
        ...
        pb_rsp = self._parse_response(
            rsp, message=proxima_se_pb2.ListCollectionsResponse())
    ...
    def stats_collection(self, collection):
        rsp = self._send_request(self._STATS_COLLECTION_URL % collection,
                                 method='GET')
        return self._parse_collection_stats(rsp, proxima_se_pb2.StatsCollectionResponse())
    ...
    def query(self, query_request, **kwargs):
        timer = kwargs.get('timer')
        rsp = self._send_request(self._QUERY_URL % query_request.collection_name,
                                 query_request, method='POST', **kwargs)
        return self._parse_query_response(timer, rsp)

    def query_by_sql(self, sql_request, **kwargs):
        timer = kwargs.get('timer')
        rsp = self._send_request(self._QUERY_BY_SQL_URL,
                                 sql_request, method='POST', **kwargs)
        return self._parse_query_response(timer, rsp)
    ...
    def get_document_by_key(self, req):
        ...
        return self._parse_response(rsp, Document, 'document',
                                    proxima_se_pb2.GetDocumentResponse())

    def get_version(self):
        rsp = self._send_request(self._VERSION_URL, method='GET')
        status, pb = self._parse_response(rsp, message=proxima_se_pb2.GetVersionResponse())
    ...
    def _send_request(self, url, body=None, method='POST', **kwargs):
        ...
        try:
            ...
            return r.text
        except ProximaSeException:
            raise
        except requests.RequestException as e:
            raise ProximaSeException(
                f'Unexpected network error, url=[{full_url}]') from e
        except Exception as e:
            raise ProximaSeException('Unexpected exception') from e

    def _parse_query_response(self, timer, rsp):
        _Timer.begin_stage_helper(timer, _TimerStage.deserialization.name)
        root = json.loads(rsp)
        _Timer.end_stage_helper(timer, _TimerStage.deserialization.name, start_stage_name=_TimerStage.build_py_obj.name)
        status = root['status']
        se_status = ProximaSeStatus(status['code'], status['reason'])
...
class GrpcHandler(BaseHandler):
    def __init__(self, host, port, timeout):
        ...
        self._stub = proxima_se_pb2_grpc.ProximaServiceStub(self._channel)
        self._timeout = timeout
    ...
    def stats_collection(self, collection_name):
        rsp = self._stub.stats_collection(
            _build_collection_name(collection_name), timeout=self._timeout)
        return self._parse_collection_stats(rsp)
    ...
    def query_by_sql(self, sql_request, **kwargs):
        timer = kwargs.get('timer')
        _Timer.begin_stage_helper(timer, _TimerStage.rpc.name)
        rsp = self._stub.query_by_sql(sql_request, timeout=self._timeout)
        _Timer.end_stage_helper(timer, _TimerStage.rpc.name)
        return self._parse_response(rsp, QueryResponse, **kwargs)
    ...
    def get_version(self):
        rsp = self._stub.get_version(proxima_se_pb2.GetVersionRequest(), timeout=self._timeout)
...
class AsyncGrpcHandler(BaseHandler):
    ...
    async def query_by_sql(self, sql_request, **kwargs):
        timer = kwargs.get('timer')
        _Timer.begin_stage_helper(timer, _TimerStage.rpc.name)
        rsp = await self._stub.query_by_sql(sql_request, timeout=self._timeout)
        _Timer.end_stage_helper(timer, _TimerStage.rpc.name)
        return self._parse_response(rsp, QueryResponse, **kwargs)
    ...
    def get_version(self):
        ...
        stub = proxima_se_pb2_grpc.ProximaServiceStub(channel)
        rsp = stub.get_version(proxima_se_pb2.GetVersionRequest(), timeout=self._timeout)
        status, pb = self._parse_response(rsp)
        channel.close()
        return status, pb.version
        ...
        # rsp = loop.run_until_complete(self._stub.get_version(proxima_se_pb2.GetVersionRequest()))
        # status, pb = self._parse_response(rsp)
        # return status, pb.version
