...
Note that because this project is based on Tkinter Labels, it solves the macOS 
issue of Buttons not showing change in foreground/fg and background/bg color. 
There are other solutions to that issue, but **TTWidgets** solves it, as well.
Simply use a TTButton in lieu of a Tkinter Button on macOS, and that issue is
worked around.
...
if Platform_s == "Darwin":
    ...
    RMB = 3
B1_Motion_sc = "<B1-Motion>"
BR_Motion_sc = "<B%d-Motion>" % RMB
Button_1_sc = "<Button-1>"
Button_R_sc = "<Button-%d>" % RMB
ButtonRelease_1_sc = "<ButtonRelease-1>"
ButtonRelease_R_sc = "<ButtonRelease-%d>" % RMB

Chunk = collections.namedtuple("Chunk", "tag attrs text")
BBox = collections.namedtuple("BBox", "x y w h", defaults=(None, 0, 0))
...
overstrike_as = os_s = "os"
overstrike_s = "overstrike"  # Not a Widget option. In Text tags and Font_d.
padx_as = px_s = "px"
padx_s = "padx"
pady_as = py_s = "py"
pady_s = "pady"
...
underline_s = "underline"  # Widget (But Lab but not LB) option, not in Frame. In Text tags and Font_d
upper_as = up_s = "up"
upper_s = "upper"
vis_s = visual_as = "vis"
...
class TTWidget(tk.Frame):
    ...
    def _press(self, event=None, **kw):
        ...
        if self == event.widget:
            ...
            self._bx_bindings[B1_Motion_sc] = event.widget.bind(
                B1_Motion_sc, self._motion
            )
            self._bx_bindings[BR_Motion_sc] = event.widget.bind(
                BR_Motion_sc, self._motion
            )
        else:
            pass
        self._fired_b = False
    ...
    def _procreate(self, master=None, **options):
        ...
        OLD: If only a single child widget would be created, the
        creation of the child widget is skipped and the internal state 
        widget is mapped instead.
        """
        ...
        # self.emulation_b = True # is_tagged_text(text_b) and len(text_chunks) > 1
        if not self.emulation_b: #  len(text_chunks) == 1: #
        ...
        if self.emulation_b: # text_b and is_tagged_text(text_b) and len(text_chunks) > 1:
            text = text_b or ""
...
class TTToolTip:
    ...
    def __init__(self, widget=None, text="...", **kw):
        ...
        self.anchor = kw.pop(anchor_s, self.custom_defs[anchor_s])
        # 'widget' or 'mouse'/'cursor'
        self.base = kw.pop("base", self.custom_defs["base"])
        ...
        if self.widget:
            if self.tag:
                if isinstance(self.widget, (tk.Canvas, tk.Text, tk_scrolledtext.ScrolledText, )):
                    self.widget.tag_bind(self.tag, "<Enter>", self._enter)
                    self.widget.tag_bind(self.tag, "<Leave>", self._leave)
                    self.widget.tag_bind(self.tag, "<ButtonPress>", self._leave)
                    self.widget.tag_bind(self.tag,
    ...
    def _showtip(self, event=None, **kw):
        if not self.widget:
            return
        ...
        elif self.tag: # tag but not mouse.
            x0 = self.widget.winfo_rootx()
            y0 = self.widget.winfo_rooty()
            if isinstance(self.widget, (tk.Text, tk_scrolledtext.ScrolledText, )):
                ranges = self.widget.tag_ranges(self.tag)
                if si is None or ei is None:
                    if index is None:
                        index = self.widget.index(tk.CURRENT)
                    if index is True:
                        si, ei = ranges[:2]
                    else:
                        for si, ei in zip(ranges[::2], ranges[1::2]):
                            if self.widget.compare(si, "<=", index) and self.widget.compare(index, "<=", ei):
                                break
                            else:
                                si = ei = None
                if si and ei:
                    sbox = BBox(*self.widget.bbox(si))
                    ebox = BBox(*self.widget.bbox(ei))
                    if self.anchor == tk.NW:
                        x, y = sbox[:2]
                    ...
                    try:
                        ...
                        print(f"_SHOWTIP({self=}, {event=}): {x0=!r}, {x=!r}, {self.offsetx=!r}, {y=!r}, {self.anchor=!r}, ")
                        raise
                    y0 += y + self.offsety
            elif isinstance(self.widget, tk.Canvas):
                bbox = self.widget.bbox(self.tag)  # get item origin?
                x, y, w, h = bbox
                if self.anchor == tk.NW:
                    pass
                ...
                raise Exception("Unexpected Widget ", type(self.widget))
        else:
            x0 = (
    ...
    def showtip(self, event=None, **kw):
        """"""
        return self._showtip(event, **kw)
...
if __name__ == "__main__":
    ...
    def build_the_gui():
        ...
        debugText = tk_scrolledtext.ScrolledText(root, width=150, height=10)
        ...
        debugText.pack(side=tk.BOTTOM, fill=tk.BOTH, expand=True)
        for bn in but_nums:
    ...
    cbut = TTButton(text="Compound <t fg=red b>Button</t>", bg='khaki')
    sbut = TTButton(text="Simple Button", bg='cyan')
    clab = TTLabel(text="<t bg=khaki>Compound</t> Label", bg='lime')
    slab = TTLabel(text="Simple Label", bg='lime')
    for w in (cbut, sbut, clab, slab):
