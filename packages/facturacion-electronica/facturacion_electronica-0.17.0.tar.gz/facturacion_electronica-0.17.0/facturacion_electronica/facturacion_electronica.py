        ...
        for dte in dtes:
            ...
            try:
                if dte.sii_xml_request:
                    ...
                    dte.timbrar()
                resp.update({
                    'sii_xml_dte': dte.sii_xml_request,
...
def timbrar_y_enviar(vals):
    ...
    try:
        ...
        _logger.warning("Error en timbrar_y_enviar", exc_info=True)
        respuesta["errores"] = [str(e)]
    del envios[key]
    return respuesta
...
def timbrar_y_enviar_cesion(vals):
    ...
    try:
        ...
        _logger.warning("Error en timbrar_y_enviar_cesion", exc_info=True)
    del envios[key]
    return respuesta
...
def consulta_estado_envio(vals):
    firma = Firma(vals["firma_electronica"])
    emisor = Emisor(vals["Emisor"])
    conex = Conexion(emisor, firma, vals.get('api', False))
    if vals.get('cesion'):
        conex.cesion = vals['cesion']
    return conex.consulta_estado_envio(vals["codigo_envio"])

def reenvio_correo_envio(vals):
    firma = Firma(vals["firma_electronica"])
    emisor = Emisor(vals["Emisor"])
    conex = Conexion(emisor, firma, vals.get('api', False))
    return conex.reenvio_correo_envio(vals["codigo_envio"])
...
def consulta_estado_dte(vals):
    ...
    for d in vals['Documento']:
        for r in d['documentos']:
            ...
            respuesta[documento.ID] = conexion.consulta_estado_dte(
                documento)
    return respuesta
