...
def limpiar_items(xml):
    xml =re.sub(r"<items[a-zA-Z]*>", r"", xml)
    xml =re.sub(r"</items[a-zA-Z]*>", r"", xml)
    xml =re.sub(r"<item>", r"", xml)
    xml =re.sub(r"</item>", r"", xml)
    xml =re.sub(r"<item/>", r"", xml)
    return xml
...
def validar_xml(some_xml_string, validacion='doc'):
    ...
    try:
        ...
        _logger.warning(etree.tostring(xml_doc, encoding="iso-8859-1"))
