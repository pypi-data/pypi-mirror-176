...
class Envio(object):
    ...
    @property
    def TmstFirmaEnv(self):
        if not hasattr(self, '_tmst_firma_env'):
            return util.time_stamp()
        return self._tmst_firma_env

    @TmstFirmaEnv.setter
    def TmstFirmaEnv(self, val):
        self._tmst_firma_env = val
...
xsi:schemaLocation="http://www.sii.cl/SiiDte RespuestaEnvioDTE_v10.xsd" >
    ...
    def generate_xml_send(self):
        ...
        for dte in self.Documento:
            try:
                ...
                _logger.warning("Error al timbrar", exc_info=True)
                self.errores = {
    ...
    def do_dte_send(self):
        ...
        if self.errores or not self.sii_xml_request:
            self.errores = 'No se creó xml'
            result['errores'] = self.errores
            return result
        detalles = []
        for r in self.Documento:
            ...
            detalles.append({
                    ...
                    'MntIVA': r.MntIVA,
                    'MntTotal': r.MntTotal,
                    'ImptoReten': r.ImptoReten,
                    'sii_barcode_img': r.sii_barcode_img,
                    'sii_xml_dte': r.sii_xml_request
                })
        if self.conexion:
