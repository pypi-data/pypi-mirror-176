...
class Documento(DTE):
    ...
    def __init__(self, vals, emisor=False, resumen=False, tipo_dte=33):
        self._comisiones = []
        if not vals.get('Emisor') and emisor:
    ...
    def Comisiones(self):
        comisiones = []
        for c in self._comisiones:
            comision = collections.OrderedDict()
            comision['NroLinCom'] = c.NroLinCom
            comision['TipoMovim'] = c.TipoMovim
            comision['Glosa'] = c.Glosa
            if c.TasaComision:
                comision['TasaComision'] = c.TasaComision
            comision['ValComNeto'] = c.ValComNeto
            comision['ValComExe'] = c.ValComExe
            comision['ValComIVA'] = c.ValComIVA
            comisiones.append({"Comisiones": comision})
        return comisiones
 ...
 o servicio(s) prestado(s) ha(n) sido recibido(s).'
    ...
    def Detalle(self):
        ...
        for line in self._lineas_detalle:
            ...
            if line.CdgItem:
                lines['itemsCDG'] = line.CdgItem
            if line.TpoDocLiq:
                lines['TpoDocLiq'] = line.TpoDocLiq
            elif self.es_liquidacion:
                raise UserError("Debe Incluir el Tipo de Documento en la Línea")
            for t in line.Impuesto:
                taxInclude = t.price_include
            if line.IndExe:
                lines['IndExe'] = line.IndExe
...
#                lines['RUTMandante']
            ...
            if line.DscItem:
                lines['DscItem'] = line.DscItem
            #lines['InfoTicket']
            if line.QtyItem > 0 and not line.no_product:
    ...
    def Detalle(self, vals=[]):
        ...
        for d in vals:
            NroLinDet += 1
            if NroLinDet > 1 and self.no_product:
                raise UserError("NO puede ir más de una línea en código rectificación texto")
            ...
            if self.no_product and linea.MontoItem != 0:
                raise UserError("Documento rectificación texto debe ser valor 0")
    ...
    @property
    def es_nota(self):
        return self.TipoDTE in [55, 56, 60, 61] or self.es_nc_exportacion
    ...
    def IdDoc(self):
        ...
        if self.IndMntNeto:
            IdDoc['IndMntNeto'] = self.IndMntNeto
        #if self.es_boleta():
    ...
    def ImptoReten(self):
        if not hasattr(self, '_imp_reten'):
            ...
            for li in self.impuestos:
                if li.tax_id.es_retencion:
                    _imp_reten -= int(decimal.Decimal(
                        li.MontoReten).to_integral_value())
                elif li.tax_id.es_adicional:
                    _imp_reten += int(decimal.Decimal(
                        li.MontoImp).to_integral_value())
            return self._aplicar_dsc_rcg(_imp_reten)
    ...
    def MontoNF(self, val):
        ...
        decimal.getcontext().rounding = decimal.ROUND_HALF_UP
        self._monto_nf = int(decimal.Decimal(val).to_integral_value())
    ...
    def MntExe(self):
        if not hasattr(self, '_mnt_exe'):
            ...
            for li in self.impuestos:
                if li.tax_id.TasaImp == 0:
                    _mnt_exe += li.MontoImp
            return self._aplicar_dsc_rcg(_mnt_exe, False, True)
        ...
        if not hasattr(self, '_mnt_iva'):
            ...
            for li in self.impuestos:
                if li.tax_id.TasaImp == self.TasaIVA:
                    ...
                    li._compute_tax()
                    _mnt_iva += li.MontoImp
            decimal.getcontext().rounding = decimal.ROUND_HALF_UP
            _mnt_iva = int(decimal.Decimal(_mnt_iva).to_integral_value())
            return self._aplicar_dsc_rcg(_mnt_iva)
        ...
        if not hasattr(self, '_mnt_neto'):
            ...
            for li in self.impuestos:
                if li.tax_id.TasaImp == self.TasaIVA:
                    _mnt_neto += li.base
            decimal.getcontext().rounding = decimal.ROUND_HALF_UP
            _mnt_neto = int(decimal.Decimal(_mnt_neto).to_integral_value())
            return self._aplicar_dsc_rcg(_mnt_neto)
    ...
    def MntTotal(self):
        if not hasattr(self, '_mnt_total'):
            return self.MntExe + self.MntNeto + self.MntIVA + self.ImptoReten - self.ValComExe - self.ValComNeto - self.ValComIVA
    ...
    def MntTotOtrMnda(self):
        if not hasattr(self, '_mnt_total_otr_mnda'):
            return self.MntExeOtrMnda + self.MntNetoOtrMnda + self.IVAOtrMnda + self.ImptoRetenOtrMnda
    ...
    def Receptor(self):
        ...
        if not self.es_boleta() and not self.es_exportacion:
            ...
            if not Receptor['GiroRecep'] and not self.es_nota:
                raise UserError("Debe Especificar Giro")
...
llevar estrictamente las palabras Dice: y debe decir:')
            ...
            if not self.es_boleta():
                ...
                for i in self.impuestos:
                    if i.tax_id.es_adicional:
                        ...
                        mnt_imp = int(decimal.Decimal(i.MontoImp).to_integral_value())
                        Totales['ImptoReten']['MontoImp'] = 1 if mnt_imp < 0 else mnt_imp
        elif self.es_exportacion and self.MntIVA:
            raise UserError("Todos los productos deben ser Exentos")
        if self.CredEC:
            Totales['CredEC'] = self.CredEC
        if self.Comisiones:
            Totales['Comisiones'] = collections.OrderedDict()
            if self.ValComNeto:
                Totales['Comisiones']['ValComNeto'] = self.ValComNeto
            if self.ValComExe:
                Totales['Comisiones']['ValComExe'] = self.ValComExe
            if self.ValComIVA:
                Totales['Comisiones']['ValComIVA'] = self.ValComIVA
    ...
    def Totales(self, vals={}):
        if vals.get('Comisiones', False):
            vals.update(vasl['Comisiones'].copy())
            del vals['Comisiones']
        util.set_from_keys(self, vals)
    ...
    def TpoCambio(self, val):
        self._tpo_cambio = round(float(val), 10)
...
, en su ficha de receptor")
    ...
    def get_agrupados(self, monto, Afectos=True, Exentos=False):
        ...
        for gdr in self._gdrs:
            if (Afectos and not gdr.IndExeDR) or (Exentos and gdr.IndExeDR):
