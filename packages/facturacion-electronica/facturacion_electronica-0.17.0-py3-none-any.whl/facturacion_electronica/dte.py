...
class DTE(object):
    ...
    def firmar(self, message, uri, type='doc'):
        string = util.limpiar_items(message)
        if self.firma.firma:
...
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX</RSR><MNT>10000</MNT><IT1>IIIIIII\
    ...
    def _dte(self):
        if not self.MntIVA and self.TipoDTE in [33]:
            raise UserError("Debe ir IVA")
 ...
 deben incluir una referencia por obligación" % (self.TipoDTE, self.Folio))
        ...
        if self.DscRcgGlobal:
            if self.es_liquidacion:
                raise UserError("No Puede llevar Descuento Global en Liquidación")
            dte['itemsDscRcgGlobal'] = self.DscRcgGlobal
        if self.Referencia:
            dte['itemsRefs'] = self.Referencia
        if self.Comisiones:
            dte['itemsComisiones'] = self.Comisiones
        return dte
