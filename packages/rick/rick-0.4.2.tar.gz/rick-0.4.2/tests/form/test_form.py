...
class SampleForm_Fieldset_a(Form):
    ...
    def init(self):
        ...
        fs.field('text', 'name', 'Full Name', validators="required|minlen:4|maxlen:8") \
            .field('text', 'age', 'Age', validators="required|numeric|between:9,125")

        fs = self.fieldset('fs2', 'fs2_label')
        fs.field('text', 'other_name', 'Full Name', validators="required|minlen:4|maxlen:8") \
...
sampleform_fieldset_a_payload = {
    ...
    'no_error_2': {'name': 'john', 'age': '24', 'other_name': 'john2', 'other_age': '17'},
    'missing_field': {'name': 'john'},
    'minlen_error': {'name': 'j', 'age': 32, 'other_name': 'j'},
    'maxlen_error': {'name': 'john_connor', 'age': 17, 'other_name': 'john', 'other_age': 14},
    'no_numeric_error': {'name': 'john', 'age': 'abc'},
}
sampleform_fieldset_a_errors = {
    'no_error': {},
    'no_error_2': {},
    'missing_field': {'age': {'required': 'value required'}, 'other_age': {'required': 'value required'},
                      'other_name': {'required': 'value required'}},
    'minlen_error': {'name': {'minlen': 'minimum allowed length is 4'}, 'other_age': {'required': 'value required'},
                     'other_name': {'minlen': 'minimum allowed length is 4'}},
    'maxlen_error': {'name': {'maxlen': 'maximum allowed length is 8'}},
    'no_numeric_error': {'age': {'between': 'must be between 9 and 125', 'numeric': 'only digits allowed'},
                         'other_age': {'required': 'value required'}, 'other_name': {'required': 'value required'}},
}
...
@pytest.mark.parametrize("fixture", [(sampleform_simple_payload, sampleform_simple_errors)])
...
@pytest.mark.parametrize("fixture", [(sampleform_fieldset_a_payload, sampleform_fieldset_a_errors)])
