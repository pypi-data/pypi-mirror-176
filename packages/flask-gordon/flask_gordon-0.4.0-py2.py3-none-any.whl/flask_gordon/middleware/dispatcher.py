...
Unlike the :class:`werkzeug.middleware.DispatcherMiddleware` a call to
`/api` will not be routed to the app handling the prefix `/api`.
A call to `/api/` will though.
...
.. code-block:: python
    ...
    from flask_gordon.middleware import DispatcherMiddleware
    ...
    app = Flask(__name__)
    app.wsgi_app = DispatcherMiddleware(app.wsgi_app, paths)
...
class DispatcherMiddleware:
    ...
    def __init__(
        self,
        app: "WSGIApplication",
        mounts: t.Dict[str, "WSGIApplication"] = None,
    ) -> None:
        self.app = app
