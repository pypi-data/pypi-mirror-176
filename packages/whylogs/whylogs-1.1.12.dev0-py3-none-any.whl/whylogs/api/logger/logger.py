...
class Logger(ABC):
    ...
    def log(
        ...
        *,
        ...
        if active_schema.segments:
            if self._segment_cache is None:
                self._segment_cache = SegmentCache(active_schema)
            else:
                if not self._segment_cache.is_compatible(active_schema):
                    raise ValueError("You cannot redefine a schema's segmentation while having previously defined a different segmentation schema in this logger.")
