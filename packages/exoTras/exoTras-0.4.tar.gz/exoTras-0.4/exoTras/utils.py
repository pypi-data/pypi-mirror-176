...
def read_project(output_path, names_list):
    ...
    for i in (names_list):
        ...
        if os.path.exists(output_path + '/tmp_out/' + i):
            ...
            # exec('g{} = sc.read_h5ad("{}/{}/{}/raw_{}.h5ad")'.format(i, str(output_path), 'tmp_out', i, i), d1)
            adata_list.append(sc.read_h5ad(str(output_path) + '/tmp_out/'+ str(i) + '/raw_' + str(i) + '.h5ad'))
...
def count_genes(genes_out, out_path):
    ...
    pd.Series(cal_gene_out).value_counts().head(15).index.to_series().to_csv(str(out_path) + '/exo_genes.csv')
    genes = pd.read_csv(str(out_path) + '/exo_genes.csv')
...
def get_exo_list(species, score_t='1e-15'):
    ...
    ##exo gene symbols
    if species == 'Homo':
        exo_list = [x.strip() for x in open(Path(__file__).parent / 'id_homo_rna_exo')]
    elif species == 'Mus':
        exo_list = [x.strip() for x in open(Path(__file__).parent / 'id_mus_rna_exo')]
        score_t = '1e-10'
