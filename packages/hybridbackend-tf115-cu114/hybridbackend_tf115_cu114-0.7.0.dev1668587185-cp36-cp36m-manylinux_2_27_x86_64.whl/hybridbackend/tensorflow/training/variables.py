...
class EmbeddingLookupRewritingForVariables(EmbeddingLookupRewriting):
  ...
  def build_sharded_weights(
      self, shard, num_shards, collections, fn, name, *args, **kwargs):
    ...
    if shape_dims is None or len(shape) != 2:
      logging.vlog(1, f'Variable {name} is not an embedding weights')
      return fn(name, *args, **kwargs)

    bucket_size, dimension_size = shape
    if bucket_size <= num_shards:
      logging.vlog(1, f'Embedding weights {name} is very small')
      return fn(name, *args, **kwargs)

    if bucket_size <= Context.get().options.batch_size:
      logging.vlog(1, f'Embedding weights {name} is small')
      return fn(name, *args, **kwargs)
  ...
  def end(self):
    ...
    _shared_emb._get_dense_tensor_internal =\
      self._prev_shared_emb_get_dense_tensor_internal  # pylint: disable=protected-access
