...
class Local:
    ...
    def send_web_message(self, chatId: str, message: str = None, messageType: int = 0, icon: str = None,
        ...
        req = self.session.post(webApi("/add-chat-message"), json=data, headers=self.web_headers)
        try:
    ...
    def sendWebActive(self):
        ...
        req = self.session.post(webApi("/community/stats/web-user-active-time"), json=data, headers=self.web_headers, proxies=self.proxies)
        if req.status_code != 200: return CheckExceptions(req.json())
        return req.json()