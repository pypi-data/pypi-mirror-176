...
class canvas(object):
    ...
    """
    def __init__(self, device, background_color='black' ,background=None, dither=False):
        self.draw = None
        self.device = device
        self.dither = dither
        ...
        if self.background is None:
            self.image = Image.new("RGB" if self.dither else self.device.mode, self.device.size,color=background_color)
        else:
            assert(self.background.size == self.device.size)
    ...
    def clear(self):
        if self.background is None:
            self.image = Image.new("RGB" if self.dither else self.device.mode, self.device.size)
        else:
            assert(self.background.size == self.device.size)
            self.image = self.background.copy()
        self.draw = ImageDraw.Draw(self.image)
