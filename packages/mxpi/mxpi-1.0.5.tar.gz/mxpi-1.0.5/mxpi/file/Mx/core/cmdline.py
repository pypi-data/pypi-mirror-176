...
def get_library_version(module_name):
    ...
    :param module_name: The module name, e.g. ``oled``.
...
def get_interface_types():
    ...
    :rtype: list
    """
    return get_choices('core.interface.serial') + get_choices('core.interface.parallel')
...
def get_display_types():
    ...
    for namespace in get_supported_libraries():
        display_types[namespace] = get_choices(f'{namespace}.device')
...
def get_transformer_choices():
    ...
    :rtype: list
    """
    from emulator.render import transformer
...
class make_interface(object):
    ...
    def noop(self):
        from core.interface.serial import noop
        return noop()

    def i2c(self):
        from core.interface.serial import i2c
        return i2c(port=self.opts.i2c_port, address=self.opts.i2c_address)

    def bitbang(self):
        from core.interface.serial import bitbang
    ...
    def spi(self):
        from core.interface.serial import spi
        GPIO = self.__init_alternative_GPIO()
    ...
    def gpio_cs_spi(self):
        from core.interface.serial import gpio_cs_spi
        GPIO = self.__init_alternative_GPIO()
    ...
    def ftdi_spi(self):
        from core.interface.serial import ftdi_spi
...
def create_device(args, display_types=None):
    ...
    if args.display in display_types.get('core', []):
        import core.device
        Device = getattr(core.device, args.display)
        framebuffer = getattr(core.framebuffer, args.framebuffer)(num_segments=args.num_segments, debug=args.debug)
        ...
        import lcd.device
        Device = getattr(lcd.device, args.display)
        interface = getattr(make_interface(args), args.interface)()
        ...
        try:
            import lcd.aux
            lcd.aux.backlight(**backlight_params).enable(True)
        except ImportError:  # pragma: no cover
            pass

    elif args.display in display_types.get('led_matrix', []):
        import led_matrix.device
        from core.interface.serial import noop
        Device = getattr(led_matrix.device, args.display)
        ...
        Device = getattr(emulator.device, args.display)
...
def create_parser(description):
    ...
    framebuffer_choices = get_choices('core.framebuffer')
