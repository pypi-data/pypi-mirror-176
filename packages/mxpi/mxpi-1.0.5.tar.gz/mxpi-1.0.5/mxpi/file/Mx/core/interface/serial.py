...
class i2c(object):
    ...
    :type address: int
    :raises core.error.DeviceAddressError: I2C device address is invalid.
    :raises core.error.DeviceNotFoundError: I2C device could not be found.
    :raises core.error.DevicePermissionError: Permission to access I2C device
        denied.
    ...
    def __init__(self, bus=None, port=1, address=0x3C):
        ...
        try:
            ...
            raise Mx.core.error.DeviceAddressError(
        ...
        try:
            ...
            if e.errno == errno.ENOENT:
                # FileNotFoundError
                raise Mx.core.error.DeviceNotFoundError(
                    f'I2C device not found: {e.filename}')
            elif e.errno in [errno.EPERM, errno.EACCES]:
                # PermissionError
                raise Mx.core.error.DevicePermissionError(
...
class spi(bitbang):
    ...
    :param reset_release_time: The number of seconds to delay afer reset. Some devices may require
        ...
        initialization sequence (default:0)
    :type reset_release_time: float
    :raises core.error.DeviceNotFoundError: SPI device could not be found.
    :raises core.error.UnsupportedPlatform: GPIO access not available.
    """
    def __init__(self, spi=None, gpio=None, port=0, device=0,
        ...
        try:
            ...
            if e.errno == errno.ENOENT:
                raise Mx.core.error.DeviceNotFoundError('SPI device not found')
            else:  # pragma: no cover
                raise
