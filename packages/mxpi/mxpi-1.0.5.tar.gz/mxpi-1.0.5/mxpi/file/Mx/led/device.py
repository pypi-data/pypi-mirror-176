...
# Example usage:
#
#   from core.interface.serial import spi, noop
#   from core.render import canvas
#   from led.device import max7219
#
...
import Mx.led.const
from Mx.core.interface.serial import noop
from Mx.core.device import device
from Mx.core.render import canvas
from Mx.core.util import observable
from Mx.core.virtual import sevensegment
from Mx.led.segment_mapper import dot_muncher, regular
...
class max7219(device):
    ...
    def __init__(self, serial_interface=None, width=8, height=8, cascaded=None, rotate=0,
                 ...
                 **kwargs):
        super(max7219, self).__init__(Mx.led.const.max7219, serial_interface)
        ...
        if width <= 0 or width % 8 != 0 or height <= 0 or height % 8 != 0:
            raise core.error.DeviceDisplayModeError(
...
class apa102(device):
    ...
    def __init__(self, serial_interface=None, width=8, height=1, cascaded=None,
                 rotate=0, mapping=None, **kwargs):
        super(apa102, self).__init__(core.const.common, serial_interface or self.__bitbang__())
...
class neosegment(sevensegment):
    """
    Extends the :py:class:`core.virtual.sevensegment` class specifically
...
class unicornhathd(device):
    ...
    def __init__(self, serial_interface=None, rotate=0, **kwargs):
        super(unicornhathd, self).__init__(core.const.common, serial_interface)
