...
class CertoraBuildGenerator:
    ...
    def build(self, args: argparse.Namespace) -> None:
        ...
        for i, build_arg_contract_file in enumerate(sorted(self.input_config.files)):
            ...
            # Build sources tree
            sources = self.collect_sources(args)
            try:
                self.build_source_tree(sources)
    ...
    def build_source_tree(self, sources: Set[Path]) -> None:
        ...
        # location. In order to be able to rerun the cetoraRun, also the current working directory should be mapped
    ...
    def collect_sources(self, args: argparse.Namespace) \
            -> Set[Path]:
...
def build(args: argparse.Namespace, ignore_spec_syntax_check: bool = False) -> None:
    ...
    try:
        ...
        # Build sources tree
        sources = certora_build_generator.collect_sources(args)
        try:
            certora_build_generator.build_source_tree(sources)
        except Exception as e:
