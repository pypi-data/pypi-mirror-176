...
class OpenSearchKNN(BaseANN):
    def __init__(self, metric, dimension, method_param):
        ...
        self.param_string = "-".join(
            k + "-" + str(v) for k, v in self.method_param.items()
        ).lower()
    ...
    def fit(self, X):
        ...
        (_, errors) = bulk(
            self.es, gen(), chunk_size=500, max_retries=2, request_timeout=10
        )
        ...
        res = urlopen(
            Request(
                "http://localhost:9200/_plugins/_knn/warmup/" + self.name + "?pretty"
            )
        )
