...
class QG(BaseANN):
    def __init__(self, metric, object_type, epsilon, param):
        ...
        self._max_edge_size = (
            int(param["max_edge"]) if "max_edge" in param.keys() else 128
        )
        self._metric = metrics[metric]
        self._object_type = object_type
        self._edge_size_for_search = (
            int(param["search_edge"]) if "search_edge" in param.keys() else -2
        )
        self._tree_disabled = (
            (param["tree"] == False) if "tree" in param.keys() else False
        )
