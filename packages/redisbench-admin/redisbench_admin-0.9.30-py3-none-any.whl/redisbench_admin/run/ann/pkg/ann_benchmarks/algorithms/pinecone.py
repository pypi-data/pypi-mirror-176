...
class Pinecone(BaseANN):
    ...
    def fit(self, X, offset=0, limit=None):
        ...
        batch_size = min(
            1000, 2 * 1024 * 1024 // (sys.getsizeof(bulk[-1]))
        )  # bulk[-1] should be the largest (longest name)

        for batch in [
            bulk[i : i + batch_size] for i in range(0, len(bulk), batch_size)
        ]:
