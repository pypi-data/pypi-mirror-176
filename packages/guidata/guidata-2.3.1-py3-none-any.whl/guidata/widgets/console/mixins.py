...
class BaseEditMixin(object):
    ...
    def _format_signature(self, text):
        formatted_lines = []
        name = text.split("(")[0]
        rows = textwrap.wrap(text, width=50, subsequent_indent=" " * (len(name) + 1))
        for r in rows:
    ...
    def show_calltip(
        ...
        format2 = "<div style='font-family: \"%s\"; font-size: %spt'>" % (
            family,
            size - 1 if size > 9 else size,
        )
        tiptext = (
            format1 + ("<b>%s</b></div>" % title) + "<hr>" + format2 + text + "</div>"
        )
    ...
    def get_text_line(self, line_nb):
        ...
        try:
            return str(self.toPlainText()).splitlines()[line_nb]
        except IndexError:
    ...
    def get_current_word(self):
        ...
        if cursor.hasSelection():
            ...
            cursor.setPosition(min([cursor.selectionStart(), cursor.selectionEnd()]))
        else:
    ...
    def find_text(
        self, text, changed=True, forward=True, case=False, words=False, regexp=False
    ):
        """Find text"""
        cursor = self.textCursor()
        ...
        if forward:
            ...
            if changed:
                if str(cursor.selectedText()):
                    new_position = min([cursor.selectionStart(), cursor.selectionEnd()])
        ...
        for move in moves:
            ...
            if regexp and "\\n" in text:
                ...
                found_cursor = self.find_multiline_pattern(pattern, cursor, findflag)
            else:
    ...
    def get_number_matches(self, pattern, source_text="", case=False, regexp=False):
    ...
    def get_match_number(self, pattern, case=False, regexp=False):
        ...
        match_number = self.get_number_matches(
            pattern, source_text=source_text, case=case, regexp=regexp
        )
        return match_number
...
class GetHelpMixin(object):
    ...
    def show_object_info(self, text, call=False, force=False):
        ...
        if (
            help_enabled
            and (self.help is not None)
            and (self.help.dockwidget.isVisible())
        ):
...
class BrowseHistoryMixin(object):
    ...
    def browse_history(self, backward):
        ...
        text, self.histidx = self.find_in_history(tocursor, self.histidx, backward)
        if text is not None:
