...
def _handle_boolean(*args, **metadata):
    ...
    elif "default" in metadata and metadata.get("default") is None:
        metadata["choices"] = [True, False]
        return _create_argument(*args, type=_str_to_bool, **metadata)
...
def get_argument_creator(
    ...
    elif isinstance(argument_type, collections.abc.Container) and not isinstance(
        argument_type, str
    ):
        if any(
            isinstance(element, collections.abc.Container)
