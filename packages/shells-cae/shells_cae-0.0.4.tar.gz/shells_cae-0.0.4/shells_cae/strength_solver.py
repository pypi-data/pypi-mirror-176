...
class StressSolver:
    ...
    def preprocessor(self, data: dict, global_state: dict):
        ...
        self.preprocessed_data['alpha_n']=data['gun_char']['theta_n']
        self.preprocessed_data['alpha_k']=data['gun_char']['theta_k']
    ...
    def run(self, data: dict, state: dict):
        ...
        q = p_data['q']
        n = p_data['n']
        alpha_n = p_data['alpha_n']
        alpha_k = p_data['alpha_k']
        Ln = p_data['Ln']
        C = p_data['C']
        mu_c = p_data['mu_c']
        rho_w = p_data['rho_w']
...
class BottomThickSolver:
    ...
    def run(self, data: dict, state: dict):
        ...
        results = {'hd': hd, 'h1_sh': h1_sh, 'h3_sh': h3_sh, 'f': f, 'pkr': pkr, 'poc': poc, 'p_sd': p_sd, 'qd': qd,
                   'K': K}
        results['h2_sh'] = h2_sh
...
class RampCorpSolver:
    ...
    def run(self, data: dict, state: dict):
        ...
        R5 = p_data['res_press_mass']['R'][5]
        Pm = p_data['Pmax']
        Pc_5 = p_data['res_stress']['Pc_sh'][5]
        sigma_tsh = p_data['sigma_tsh']
        lambd = p_data['lambd']
        ...
        q_oc = np.abs(sigma_x5) / sigma_tsh - (Prasch - Pc_5 * ro_5 ** 2) / (sigma_tsh * (1 - ro_5 ** 2))

        P_rad = ((Prasch - Pc_5) * np.sqrt(3)) / (sigma_tsh * (1 - ro_5 ** 2))
        ...
        k = -1 * (q_oc / q_oc)  # меняем знак для последнего множителя формул
        # Если мы в упругой зоне, считаем упругий прогиб
        if P_rad < Pkr1:
            Wh5_sh = (np.sqrt(3) / 2) * eps_sh * r5 * (P_rad - (q_oc / (ro_5 * np.sqrt(3))))
        # Если мы в сложно-напряженном состоянии
        elif Pkr1 <= P_rad < Pkr2:
            Wh5_sh = (np.sqrt(3) / 2) * eps_sh * r5 * (1 / np.sqrt(1 + u ** 2 * ro_5 ** 4)) * (
        ...
        results = {'P_rad': P_rad, 'Pkr1': Pkr1, 'Pkr2': Pkr2, 'Pkr3': Pkr3, 'W5': W5, 'WH5': WH5, 'q_oc': q_oc,
                   'k_s': k_s, 'l': l, 'Wh5_sh': Wh5_sh}
...
class BeltZoneSolver:
    ...
    # 1. Остаточные деформации корпуса снаряда в зоне ведущего пояска
        ...
        # Определение текущего и остаточного значения прогиба стенки
        W0_curr = calc_coef(lambd, qN_g=None, qPR=None)


        if W0_curr < a0 * eps_sh:
            res['stat_deform'] = 'elastic'
            curr_lambda = 0
            W0_curr = calc_coef(curr_lambda, qN_g=None, qPR=None)
            if W0_curr <= W5:
...
class APFSDStrengthSolver(ABCSolver):
    ...
    def get_sigma_x(self, x, Fg):
        ...
        if la >= x > (x_db_0 + l_db):
            sigma = -(m_bt + (m_k + m_pl) * (1. - x / la)) / m_total
            sigma *= s * p_max / sk
            return sigma
