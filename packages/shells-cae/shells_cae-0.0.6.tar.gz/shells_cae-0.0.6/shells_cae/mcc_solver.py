...
class OFSMCCSolver:
    ...
    def gyro_stab(self, q, A, B, mu, xc, p_data):
        ...
        theta_k = p_data['gun']['theta_k']
        eta_k = round(np.pi / (np.tan(theta_k)))

        d = p_data['gun']['d']
        fi = 0.57
        L = np.max(p_data['corpus_coord'][0])
    ...
    def run(self, data: dict, state: dict):
        ...
        if abs(q - p_data['q_sh']) > 1E-2 and correct_mass_status:
            ...
            sigma_0, h = self.gyro_stab(q, A, B, mu, xc, p_data)
...
class APFSDMccSolver(ABCSolver):
    ...
    def _compute_driving_band(self, p_data):
        ...
        rlen = p_data['len'] * la
        mass = 0.25 * p_data['form_koef'] * np.pi * p_data['rho_db'] * rlen * p_data['d'] ** 2
