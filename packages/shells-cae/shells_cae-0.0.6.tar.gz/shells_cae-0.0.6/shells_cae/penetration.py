...
class PenetrationHeadSolver:
    ...
    def preprocessor(self, data: dict, global_state: dict):
        ...
        self.preprocessed_data['V'] = global_state['point_mass_ebh_solver']['v_array'][-1]
    ...
    def gch_penetration(self, data, sec_res):
        ...
        results = {'x_with_fuse': x_sec, 't': t, 'V': V, 'S': S, 'kf': kf, 'Px': Px,}
...
# Расчёт наседающих масс по сечениям
    ...
    def stress_solver(self, data, pen_res, sec_res, press_mass_res):
        ...
        for i in range(sec_count):
            ...
            # Сила сопротивления преграды
            Fn[i] = px * np.pi * (Rx**2 - Rn**2)
            # Сжимающая сила
            if i != 9:
                # формула для гч
        ...
        results = {'F': Fn, 'S': Sn, 'pc': pc, 'N': Nn, 'sigma_x': sigma_x, 'sigma_r': sigma_r,
