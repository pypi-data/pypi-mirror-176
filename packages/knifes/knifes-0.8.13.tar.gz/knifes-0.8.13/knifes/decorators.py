...
def func_cache(cache_key_prefix: str, timeout=3600):
    def outer_wrapper(func):
        def wrapper(*args, **kwargs):
            ...
            key = cache_key_prefix + md5(str(args[0]))  # 避免args[0]过长
            result = cache.get(key)  # 尝试读取缓存
            if result:
                return pickle.loads(result)    # 使用pickle支持枚举、自定义类等
            result = func(*args, **kwargs)
            cache.set(key, pickle.dumps(result), timeout=timeout)  # 写缓存
            return result
        return wrapper
    return outer_wrapper


def update_func_cache(cache_key_prefix: str, args_0, result, timeout=3600):
    key = cache_key_prefix + md5(str(args_0))
