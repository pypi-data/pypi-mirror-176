...
class Etsy():
    ...
    def request_handler(self, url, headers, method='get', data=None, params=None, count=0):
        '''
        请求
        :param url:
        :param headers:
        ...
        try:
            ...
            if count > 5:
                return None
            return self.request_handler(url, headers, method, data, count + 1)
    ...
    def get_search_products(self, page):
        ...
        try:
            # 获取listid
            resp = self.request_handler(url, headers, method='post', data=payload)
            if resp == None:
            ...
            current_html_goods = e.xpath('//ul/li')
            ...
            html_resp = self.request_handler(url, headers, method='post', data=payload)
            if html_resp == None:
    ...
    def get_shop_categroy_and_shop_id(self):
        ...
        resp = self.request_handler(shop_url, headers=headers, method='get')
        if resp == None:
            data['msg'] = '获取店铺分类失败'
            return data
    ...
    def get_shop_produts(self,shop_id, page,limit=36, section_id=None):
        ...
        resp = self.request_handler(url, headers=headers, method='get', params=params)
        if resp == None:
            data['msg'] = '请求失败，请重新请求！'
            return data
        ...
        for good in all:
            try:
                good_id = good.xpath('.//a/@data-listing-id')[0]
                good_list.append({"good_id": good_id, 'shop_id': shop_id,'html': etree.tostring(good, encoding='utf-8').decode()})
            except Exception as error:
                pass
    ...
    def parse_product(self,html):
        ...
        # 标题
        good_dict['title'] = ''.join(e.xpath('//h1[@class="wt-text-body-03 wt-line-height-tight wt-break-word"]/text()')).strip()
        ...
        try:
            ...
            good_dict['price'] = 0
        # 是否是bestseller
        good_dict['is_bestseller'] = 1 if ''.join(e.xpath('//span[@class="wt-display-inline-block"]/text()')).strip() == 'Bestseller' else 0

        # 描述
        good_dict['desc'] = ''.join(e.xpath('//div[@data-id="description-text"]//text()')).strip()
        # 图片
        try:
            good_dict['img_url'] = e.xpath('//img[@class="wt-max-width-full wt-horizontal-center wt-vertical-center carousel-image wt-rounded"]/@src')[0]
        except Exception as error:
            good_dict['img_url'] = ''
        ...
        # 关注数
        good_dict['favorites'] = ''.join(re.findall('<a rel="nofollow" class="wt-text-link" href=".*?>.*?(\d+).*?</a>', html,re.S)).strip()
        ...
        try:
            ...
            pass
        try:
            good_dict['category_etsy_raw'] = ' > '.join(e.xpath('//div[@class="wt-text-caption wt-text-center-xs wt-text-left-lg"]/a/text()')[1:])
        except Exception as error:
    ...
    def get_product_reviews(self,page_num,listing_id,shop_id):
        ...
        try:
            ...
            url = "https://www.etsy.com/api/v3/ajax/bespoke/member/neu/specs/search_async_recs"
            resp =self.request_handler(url,headers=headers,method='post',data=payload)
            if resp == None:
