...
class NMEC:
    ...
    def model_with_changepoint(self, df: pd, cp_type="SLR"):
        ...
        if cp_type.upper() in ["3PH", "3PC"]:
            df = my_model.df
            train_estimate, coeffs = my_model.train(df.dropna().copy())
            ...
            train_estimate, coeffs = my_model.train(df_occ.dropna().copy())
            print(coeffs)
            if df_unocc is not None:
                train_estimate = my_model.train(
    ...
    def example(self, model_type="SLR"):
