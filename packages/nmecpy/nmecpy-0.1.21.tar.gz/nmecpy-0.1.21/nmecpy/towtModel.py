...
class TOWTModel(Model):
    ...
    def train(self, interval=None, knot_temps=None):
        ...
        occ_x_train[occ_check.index[occ_check].tolist()[-1]] = 0
        ...
        if train[self.occupancy_col].nunique() > 1:
            ...
            unocc_x_train[unocc_check.index[unocc_check].tolist()[-1]] = 0
    ...
    def predict(self, df):
        ...
        if 1 in df[self.occupancy_col].unique():
        ...
        if 0 in df[self.occupancy_col].unique():
