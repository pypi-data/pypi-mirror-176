...
class obj:
    ...
    #   ---- 等待网页元素出现或消失
    def getEle(web, xpaths, x元素等待时长=10, x等待消失=0,eleindex=0):
        '''

        :param web:
        :param xpaths: 可以是列表
        :param x元素等待时长:
        :param x等待消失: 默认0 表示要等元素出现，如果=1，表示要等元素消失
        :param eleindex: 搜到的元素结果第几个？
        :return: 等待元素出现，返回元素 等待元素消失，返回True或False
        '''
        ...
        flag = 0
        for i in range(x元素等待时长 * 2):
            for xpath in xpaths:
                if x等待消失 == 1:
                    try:
                        ele = web.find_elements(by="xpath", value=xpath)[eleindex]
                    except:
                        return True,None
                if x等待消失 == 0:
                    try:
            ...
            time.sleep(0.5)
        if x等待消失 == 1:
            return False, "[等待元素消失]失败：等待超时。"
        if flag == 1:
            return ele,None
        else:
            return False, "[查找元素]失败：等待超时。"
    ...
    #   ---- 取元素文本
    def getEleText(web, xpaths, x元素等待时长=10,eleindex=0):
        '''
