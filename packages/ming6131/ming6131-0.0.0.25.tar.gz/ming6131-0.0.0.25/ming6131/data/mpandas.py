...
class obj:
        ...
        if self.file_extension == "csv":
            ...
            if not os.path.exists(self.path):
                ...
                self.df.to_excel(self.path, sheet_name=self.sheet_name,
                                 index=self.index)  # index默认是True，导致第一列是空的,设置为False后可以去掉第一列。
            ...
            #raise Exception("指定索引必须包含于self.cols")
    ...
    def add(self, data:dict,
        ...
        if self.index:
            self.df.set_index(self.index)
        return

    def save(self):
        if not self.path:
            return
        indexs = self.df.index.names
        ...
        if fe == "xlsx":
            ...
            self.df.to_csv(path_or_buf=self.path,header=1,encoding="utf-8")
        elif fe == "json":
