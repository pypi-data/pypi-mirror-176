...
class SocketOption(betterproto.Message):
    ...
    different values on different platforms. [#next-free-field: 7]
    """
...
class BindConfig(betterproto.Message):
    ...
    `source_address` field. This enables to specify multiple source addresses.
    Currently, only one extra address can be supported, and the extra address
    should have a different IP version with the address in the `source_address`
    field. The address which has the same IP version with the target host's
    address IP version will be used as bind address. If more than one extra
    address specified, only the first address matched IP version will be
    returned. If there is no same IP version address found, the address in the
    `source_address` will be returned.
    """
...
class Node(betterproto.Message):
    ...
    dynamic_parameters: Dict[
        str, "____xds_core_v3__.ContextParams"
...
class QuicProtocolOptions(betterproto.Message):
    ...
    QUICHE) and defaults to 65536 (2^16). NOTE: 16384 (2^14) is the minimum
    window size supported in Google QUIC. If configured smaller than it, we
    will use 16384 instead. QUICHE IETF Quic implementation supports 1 bytes
    window. We only support increasing the default window size now, so it's
    also the minimum. This field also acts as a soft limit on the number of
    bytes Envoy will buffer per-stream in the QUIC stream send and receive
    buffers. Once the buffer reaches this pointer, watermark callbacks will
    fire to stop the flow of data to the stream buffers.
    """
    ...
    QUICHE) and defaults to 65536 (2^16). window. Currently, this has the same
    minimum/default as ``initial_stream_window_size``. NOTE: 16384 (2^14) is
...
class HttpProtocolOptions(betterproto.Message):
    ...
    :ref:`drain_timeout <envoy_v3_api_field_extensions.filters.network.http_con
    nection_manager.v3.HttpConnectionManager.drain_timeout>`.
    """
...
class SubstitutionFormatString(betterproto.Message):
    ...
    plain text or JSON format. [#next-free-field: 7]
    """
