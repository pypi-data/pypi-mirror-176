    ...
    if rank == 0:
        ...
        for name, works in pluginInformation.get_plugin_items():
            ...
            if works is True:
                ...
                not_working.append(name)
        logging.info("Working plugins: %s", str(working))
        if len(not_working) > 0:
            logging.warning("Not working plugins: %s", str(not_working))
    else:
...
def test_papi() -> None:
    ...
    if plugin_p.isWorking() is False:
        if rank == 0:
            logging.warning("Could not load PAPI plugin")
        else:
...
def test_nv() -> None:
    ...
    if plugin_n.isWorking() is False:
        if rank == 0:
            logging.warning("Could not load Nvidia plugin")
        else:
...
def test_linuxNet() -> None:
    ...
    if plugin_net.isWorking() is False:
        if rank == 0:
            logging.warning("Could not load Linux network plugin")
        else:
...
def test_cpu() -> None:
    ...
    if plugin_cpu.isWorking() is False:
        if rank == 0:
            logging.warning("Could not load Linux CPU plugin")
        else:
...
def test_mem() -> None:
    ...
    if plugin_mem.isWorking() is False:
        if rank == 0:
            logging.warning("Could not load Linux memory plugin")
        else:
...
def test_disk() -> None:
    ...
    if plugin_disk.isWorking() is False:
        if rank == 0:
            logging.warning("Could not load Linux disk plugin")
        else:
...
def test_slurm() -> None:
    #pylint: disable=import-outside-toplevel
    from JuMonC.tasks.Slurm import _SlurmPlugin
    #pylint: enable=import-outside-toplevel
            
    plugin_s = _SlurmPlugin()
    if plugin_s.isWorking() is False:
        if rank == 0:
            logging.warning("Could not load Slurm plugin")
        else:
            logging.debug("Could not load Slurm plugin")
    

def test_job() -> None:
    #pylint: disable=import-outside-toplevel
    from JuMonC.tasks.job import _JOBPlugin
    #pylint: enable=import-outside-toplevel
