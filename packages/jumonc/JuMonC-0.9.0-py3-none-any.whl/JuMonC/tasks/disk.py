...
class _diskPlugin(Plugin.Plugin): 
    ...
    def __init__(self) -> None:
        ...
        if self.isWorking():
            if isinstance(self._psutil, ModuleType):
                ...
                logging.debug("IO initial values: %s", str(self._initValues))
            else:
    ...
    def getStatusData(self,
        ...
        if dataType == "write_count":
            return [self.convertNumber(self.getWriteCount(id=self.wc_mpi_id), "write count", "", overrideHumanReadableWithValue)]
        if dataType == "read_count":
            return [self.convertNumber(self.getReadCount(id=self.wc_mpi_id), "read count", "", overrideHumanReadableWithValue)]
        if dataType == "write_bytes":
            return [self.convertNumber(self.getWriteBytes(id=self.wc_mpi_id), "write", "B", overrideHumanReadableWithValue)]
        if dataType == "read_bytes":
            return [self.convertNumber(self.getReadBytes(id=self.wc_mpi_id), "read", "B", overrideHumanReadableWithValue)]
        if dataType == "write_time":
            return [self.convertNumber(self.getWriteTime(id=self.wc_mpi_id), "write", "ms", overrideHumanReadableWithValue)]
        if dataType == "read_time":
            return [self.convertNumber(self.getReadTime(id=self.wc_mpi_id), "read", "ms", overrideHumanReadableWithValue)]
        if dataType == "write_merged_count":
            return [self.convertNumber(self.getWriteMergedCount(id=self.wc_mpi_id), "merged write", "", overrideHumanReadableWithValue)]
        if dataType == "read_merged_count":
            return [self.convertNumber(self.getReadMergedCount(id=self.wc_mpi_id), "merged read", "", overrideHumanReadableWithValue)]
        if dataType == "busy_time":
            return [self.convertNumber(self.getBusyTime(id=self.wc_mpi_id), "busy", "ms", overrideHumanReadableWithValue)]
        return []
