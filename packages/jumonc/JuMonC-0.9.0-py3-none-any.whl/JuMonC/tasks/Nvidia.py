...
class _NvidiaPlugin(Plugin.Plugin):
        ...
        if isinstance(self._pynvml, ModuleType):
            ...
            for i in range(repetitions):
                for i in range(len(self.handles)):
                    result[i] = result[i] + self._pynvml.nvmlDeviceGetPcieThroughput(self.handles[i], self._status_nvml_id[name])
            for i in range(len(self.handles)):
    ...
    def getStatusData(self, dataType:str) -> Dict[str, Any]:
        logging.debug("Get GPU Status data for %s", dataType)
        if dataType in self._status:
            return self._getInfo(name=dataType, id=self.info_mpi_id)
        if dataType in self._status_nvml_id.keys():
            return self._getPcieThroughputInfo(name=dataType, id=self.pci_mpi_id)
        return {}
