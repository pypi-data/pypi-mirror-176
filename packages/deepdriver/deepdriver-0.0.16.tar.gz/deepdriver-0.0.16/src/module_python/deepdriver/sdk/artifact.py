...
class ArtifactEntry:
    ...
    def upload(self, upload_type: str, root_path: str, run_id: int, artifact_id: int, last_file_yn: str, artifact_digest: str, entry_list: List[grpc_ArtifactEntry]) -> bool:
        ...
        return interface.upload_file(upload_type, self.local_path, root_path, self.path, run_id, artifact_id, last_file_yn, artifact_digest, self.digest, entry_list)
...
class Artifact:
    ...
    def upload(self, run: Run) -> bool:
        ...
        id = interface.upload_artifact(run.run_id, self, artifact_digest, entry_list)
        if id is None:
            return False
        self.id = id
        ...
        for i, entry in enumerate(self.entry_list):
            ...
            succeeded = entry.upload(upload_type, root_path, run.run_id, self.id, last_file_yn, artifact_digest, entry_list)
            if not succeeded:
                return False
        return True
