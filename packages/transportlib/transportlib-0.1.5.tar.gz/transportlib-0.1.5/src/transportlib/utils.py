...
def write_run_log(
    ...
    write_log_sql = """
    INSERT INTO ctrl.transportlib_run_log (task_name,start_time,finish_time,items_retrieved,status) VALUES(%s,%s,%s,%s,%s)
    """
    ...
    cur.execute(
        write_log_sql,
        (task_name, start_time, finish_time, num_items, status)
    )
    conn.commit()
