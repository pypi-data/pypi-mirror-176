...
from gEcon.estimation.estimate import split_param_dict, build_Z_matrix, build_Q_and_H
...
from gEcon.sampling.prior_utilities import get_initial_time_index


def simulate_trajectories_from_posterior(model, posterior, n_samples=1000, n_simulations=100, simulation_length=40):
    simulations = []
    ...
    for i, idx in enumerate(random_idx):
        ...
        try:
            ...
            data = model.simulate(simulation_length=simulation_length,
...
def kalman_filter_from_posterior(model, data, posterior, n_samples=1000, filter_type='univariate'):
    ...
    pred_coords = {'sample': np.arange(n_samples),
                   'time': np.r_[np.array(get_initial_time_index(data), dtype='datetime64'), data.index.values],
                   'variable': model_var_names}

    cov_coords = {'sample': np.arange(n_samples),
                  'time': data.index.values,
                  'variable': model_var_names,
                  'variable2': model_var_names}

    pred_cov_coords = {'sample': np.arange(n_samples),
                       'time': np.r_[np.array(get_initial_time_index(data), dtype='datetime64'), data.index.values],
                       'variable': model_var_names,
                       'variable2': model_var_names}

    kf_data = xr.Dataset({'Filtered_State': xr.DataArray(data=np.stack([results[i][0] for i in range(n_samples)]),
                                                         coords=coords),
                          'Predicted_State': xr.DataArray(data=np.stack([results[i][1] for i in range(n_samples)]),
                                                          coords=pred_coords),
                          'Smoothed_State': xr.DataArray(data=np.stack([results[i][5] for i in range(n_samples)]),
                                                         coords=coords),
                          'Filtered_Cov': xr.DataArray(data=np.stack([results[i][2] for i in range(n_samples)]),
                                                       coords=cov_coords),
                          'Predicted_Cov': xr.DataArray(data=np.stack([results[i][3] for i in range(n_samples)]),
                                                        coords=pred_cov_coords),
                          'Smoothed_Cov': xr.DataArray(data=np.stack([results[i][6] for i in range(n_samples)]),
                                                       coords=cov_coords),
                          'loglikelihood': xr.DataArray(data=np.stack([results[i][4] for i in range(n_samples)]),
                                                        coords={'sample': np.arange(n_samples),
                                                                'time': data.index.values})})
