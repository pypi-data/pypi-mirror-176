...
# import stringcase
# from cityfront.generator.models import Inflector, Prose
...
# def get_project(
#     *, url: str | None = None, path: Path | None = None
# ) -> "ProjectContext":
#     """Get Project
#     Args:
#         url (str):
...
#         Project
#     """
#     assert url or path, "url or path is required"
...
#         config=Config(),
#     )
#     return ProjectContext(project=project)


# class ProseBuilder:
#     def __init__(self) -> None:
#         self._current_common: str | None = None

#     def common_path(self, paths: Iterator[str]) -> str:
#         return os.path.commonpath(list(map(lambda x: Path(x), paths)))

#     @property
#     def current(self) -> Path:
#         if not self._current_common:
#             raise Exception("No common path found")
#         return Path(self._current_common)

#     def is_valid_path(self, path: str) -> bool:
#         if not path or path == "/":
#             return False

#         common_file = Path(path)
#         parents = list(common_file.parents)

#         if not parents:
#             return False

#         def is_rel(x: Path, y: str) -> bool:
#             return (x.is_relative_to(y)) and (str(x.relative_to(x)) in ["/", "", "."])

#         if is_rel(common_file, API_V1):
#             return False
#         elif is_rel(common_file, API_STR):
#             return False
#         self._current_common = common_file

#         return True

#     def mutual_prefix(self, tag: str, common_path: Path) -> str:
#         pre_common = str(common_path).strip().strip("/").lower()
#         pre_tag = str(tag).strip().strip("/").lower()
#         return os.path.commonprefix([pre_common, pre_tag])

#     def collect_paths(self, collection: EndpointCollection):
#         return pipe(collection.endpoints, map(lambda x: str(x.path)), list)

#     def get_inflector(self, input: str) -> Inflector:
#         return Inflector(name=input)

#     def build_prose(self, word: str) -> Prose:
#         _inflector = self.get_inflector(word)
#         return Prose(
#             folder_format=_inflector.snake_case,
#             repo_format=_inflector.repo_class,
#             file_format=_inflector.table_name,
#             table_format=_inflector.table_name,
#             class_format=_inflector.class_name,
#             service_format=_inflector.service_name,
#         )

#     def get_prose(self, tag: str, collection: EndpointCollection) -> Prose:
#         paths = self.collect_paths(collection)
#         common_path = self.common_path(paths)
#         if not self.is_valid_path(common_path):
#             return self.build_prose(tag)

#         mutual_pref = self.mutual_prefix(tag, self.current)
#         return self.build_prose(mutual_pref)


# class ProjectContext:
#     def __init__(self, *, project: Project) -> None:
#         self.project = project
#         self.end_col = ProseBuilder()

#     @property
#     def endpoints(self) -> Dict[PythonIdentifier, EndpointCollection]:
#         """
#         > The `endpoints` property returns a dictionary of endpoint collections, keyed by tag

#         Returns:
#         A dictionary of EndpointCollection objects, keyed by tag.
#         """
#         return self.project.openapi.endpoint_collections_by_tag

#     @property
#     def models(self) -> Iterator[ModelProperty]:
#         return self.project.openapi.models

#     def proses(self) -> Dict[PythonIdentifier, Prose]:
#         return self.get_all_prose()

#     def prose_by_tag(self, tag: str) -> Prose:
#         with logger.catch(
#             message="Error getting prose for tag: {}".format(tag),
#             default=None,
#             exception=Exception,
#         ):
#             return self.end_col.get_prose(tag, self.endpoints[tag])

#     def get_all_prose(self) -> Prose:

#         prose_map = {}

#         for tag in self.endpoints.keys():
#             tag_prose = self.prose_by_tag(tag)
#             if not tag_prose:
#                 logger.info(f"{tag} has no prose necessary")
#                 continue
#             prose_map[tag] = tag_prose
#         return prose_map
