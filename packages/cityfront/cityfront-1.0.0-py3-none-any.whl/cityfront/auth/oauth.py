...
from cityfront.utils import is_async
from cityfront.exception import AuthExpiredError
...
def create_device_code(
    appwrite: "AppWrite", client_id: str, scopes: Optional[List[str]] = None
) -> Generator[httpx.Request, httpx.Response, Dict[str, Any]]:
    """Create a device code for OAuth."""
    base_url = get_oauth_base_url(appwrite.config.base_url)
    ...
    response = yield httpx.Request(
        ...
        json=body,
        headers={
            "User-Agent": appwrite.config.user_agent,
            "Accept": "application/json",
        },
    )
    response = appwrite._check(response)
...
def exchange_device_code(
    appwrite: "AppWrite", client_id: str, device_code: str
) -> Generator[httpx.Request, httpx.Response, Dict[str, Any]]:
...
def refresh_token(
    appwrite: "AppWrite", client_id: str, client_secret: str, refresh_token: str
) -> Generator[httpx.Request, httpx.Response, Dict[str, Any]]:
    """Refresh token."""
    base_url = get_oauth_base_url(appwrite.config.base_url)
    url = base_url.copy_with(raw_path=base_url.raw_path + b"login/oauth/access_token")
...
class OAuthWebAuth(httpx.Auth):
    ...
    def _exchange_code(self) -> httpx.Request:
        base_url = self.appwrite.config.base_url
        ...
        return httpx.Request(
            ...
            json=body,
            headers={
                "User-Agent": self.appwrite.config.user_agent,
                "Accept": "application/json",
            },
        )
...
class OAuthDeviceAuth(httpx.Auth):
    ...
    def sync_auth_flow(
        ...
        if require_bypass(request.url):
            ...
            return
        if not (token := self._token):
            flow = create_device_code(self.appwrite, self.client_id, self.scopes)
            ...
            while True:
                ...
                flow = exchange_device_code(self.appwrite, self.client_id, device_code)
...
class OAuthWebAuthStrategy(BaseAuthStrategy):
    ...
    def get_auth_flow(self, appwrite: "AppWrite") -> httpx.Auth:
        return OAuthWebAuth(
            appwrite, self.client_id, self.client_secret, self.code, self.redirect_uri
        )
...
class OAuthDeviceAuthStrategy(BaseAuthStrategy):
    ...
    def get_auth_flow(self, appwrite: "AppWrite") -> httpx.Auth:
        return OAuthDeviceAuth(appwrite, self.client_id, self.on_verification)
