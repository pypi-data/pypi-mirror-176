...
from cityfront.exception import AuthCredentialError
from cityfront.cache import DEFAULT_CACHE, BaseCache
from cityfront.utils import UNSET, Unset, exclude_unset

with suppress():
    from cityfront.rest import (
        BasicError,
        ValidationError,
        InstallationToken,
...
if TYPE_CHECKING:
    from cityfront import AppWrite, Response
    from cityfront.rest.types import AppPermissionsType
...
class AppAuth(httpx.Auth):
    ...
    JWT_CACHE_KEY = "cityfront:auth:app:jwt"
    INSTALLATION_CACHE_KEY = "cityfront:auth:app:installation:{installation_id}:{permissions}:{repositories}:{repository_ids}"
    ...
    def _create_jwt(self) -> str:
        ...
        if jwt is None:
            raise RuntimeError(
                ...
                "with `pip install cityfront[auth-app]`"
            )
    ...
    def _build_installation_auth_request(self) -> httpx.Request:
        ...
        return httpx.Request(
            ...
            headers={
                ...
                "Accept": self.appwrite.config.accept,
            },
        )
