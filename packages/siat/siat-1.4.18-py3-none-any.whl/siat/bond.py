...
def interbank_bond_deal(num,option='1'):
    ...
    if option=='1':
        df.sort_values(by=['加权收益率'],ascending=[False],inplace=True)
        if lang == 'Chinese':
            optiontxt=texttranslate("收益率从高到低")
        else:
    ...
    if option=='4':
        df.sort_values(by=['涨跌'],ascending=[False],inplace=True)
        if lang == 'Chinese':
    ...
    if option=='5':
        df.sort_values(by=['涨跌'],ascending=[True],inplace=True)
        if lang == 'Chinese':
    ...
    if option=='6':
        df.sort_values(by=['成交净价'],ascending=[False],inplace=True)
        if lang == 'Chinese':
    ...
    if option=='7':
        df.sort_values(by=['成交净价'],ascending=[True],inplace=True)
        if lang == 'Chinese':
    ...
    if lang == 'Chinese':
        print("\n"+texttranslate("银行间市场债券成交价（")+optiontxt+texttranslate("，前")+str(num)+texttranslate("名）"))
    else:
    ...
    if lang == 'English':
        df.rename(columns={'债券简称':'Bond Name','成交净价':'Price(Net)','最新收益率':'Latest Yield(bp)','涨跌':'Change(bp)'},inplace=True)
    print(df.head(num).to_string(index=False))
    ...
    if lang == 'Chinese':
        footnote1="\n"+texttranslate("注：收益率为百分比，涨跌幅为基点")
        footnote2="\n"+texttranslate("来源：银行间市场交易商协会(NAFMII)，")+today  
    else:
        footnote1=""
        footnote2="\nSource: NAFMII, "+today  
...
def kpmg_rnpm1_cr(k1,theta,i1, \
    ...
    footnote1="\n"+"票息率"+"（100bp = 1%）-->" 
    footnote2="\n"+"债券票面利率初始值"+str(k1*100)+"%，违约回收率"+str(theta*100)+"%，"
    footnote3="无风险利率"+str(i1*100)+'%'
...
def kpmg_rnpm1_rf(k1,theta,i1, \
    ...
    footnote1="\n"+"无风险利率"+"（100bp = 1%）-->" 
    footnote2="\n"+"债券票面利率"+str(k1*100)+"%，违约回收率"+str(theta*100)+"%，"
    footnote3="无风险利率初始值"+str(i1*100)+'%'
    footnote=footnote1+footnote2+footnote3
...
def kpmg_rnpm1_rrd(k1,theta,i1, \
    ...
    footnote1="\n"+"违约回收率 -->" 
    footnote2="\n"+"债券票面利率"+str(k1*100)+"%，违约回收率初始值"+str(theta*100)+"%，"
    footnote3="无风险利率"+str(i1*100)+'%'
    footnote=footnote1+footnote2+footnote3
...
if __name__ == "__main__":
    df=kpmg_rnpm1_rrd(k1,theta,i1,loc1='best',loc2='best')
