...
from __fluig_services_base import dataset
from __fluig_services_base.dataset import DatasetProvider
from __fluig_services_base.interfaces.dataset import (DatasetAvailableParams,
                                                      DatasetDisableParams,
                                                      DatasetQuery,
                                                      DatasetQueryParams,
                                                      StructConstraint,
                                                      StructField, StructOrder)
from fluig_services.infraestruture.server import ClientFluig
from utils.commomns import convert_zeep_object
from utils.exceptions import (RequestGetDatasetErrorException,
                              RequestGetDatasetIsEmptyException)
...
class Dataset(DatasetProvider):
    ...
    def get_dataset(
        ...
        *,
        ...
        try:
            self._logger.attribute_info.info("Start request dataset")
            ...
            if query_result:
                ...
                self._logger.attribute_info.info("Finish request dataset success")
                return serialize_result

            else:
                self._logger.attribute_warning.warning("Response Dataset is empty")
                return []

        except Exception as error:
            self._logger.attribute_error.error(error)

    def get_all_available_dataset(self, dataset: DatasetAvailableParams) -> List[dict]:
    ...
    def get_available_datasets(self, dataset: DatasetAvailableParams) -> List[dict]:
    ...
    def disable_dataset(self, dataset: DatasetDisableParams) -> None:
