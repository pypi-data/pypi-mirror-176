...
def stage_name_already_in_use(stage_name):
    """! Checks if a stage with the given name already exists
    """
    if os.path.exists('batch_files/stages/' + stage_name):
        click.echo('Stage \'' + stage_name + '\' already exists. Please use a different name.')
        return True
    return False


def generate_stage(stage_name):
    """! Generating new stage (with the relevant project_name)\n
    How to generate stage :
    > dsf-cli g stage my-new-stage
    @verbatim
    Args:
    ...
    if stage_name_already_in_use(stage_name):
        return
    create_stage_files_from_template(stage_name)
    click.echo('Stage created: ' + stage_name)
...
def create_pipeline_file(project_name, folder, pipeline_type, create_init_file=True):
    ...
    Args:
        ...
        create_init_file(=True): Create init file - if doesnt exist.
    @endverbatim
    """

    data = read_template_file(pipeline_type)
    replace_in_template_and_create_file(project_name, folder, pipeline_type, data, create_init_file)
...
def create_stage_files_from_template(stage_name):
    """! Create all needed folders and files for the new stage creation
    @verbatim
    Args:
    ...
    stage_directory_name = 'batch_files/stages/' + stage_name
    os.mkdir(stage_directory_name)
    replace_in_stage_template_and_create_file(stage_name, stage_directory_name, data)
...
def replace_in_stage_template_and_create_file(stage_name, folder, data):
