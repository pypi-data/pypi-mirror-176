...
class Telegram_Data:
    ...
    def telegram_insert(self, data):
        ...
        message_user_id = f"{user_id}/{message_id}"

        new_post = tg.TelegramPost(

            post_id=f"{user_id}/{message_id}",
        ...
        if "fwd_from.from_id.channel_id" in data and \
            ...
            forward_userid = data.get("fwd_from.from_id.channel_id")
            forward_messageid = data.get("fwd_from.channel_post")

            new_forward = tg.TelegramReferencedPost(
                post_id = message_user_id,
                referred_post_id = f"{forward_userid}/{forward_messageid}",
                referred_channel_id = data.get("fwd_from.from_id.channel_id"),
                referred_post_number = data.get("fwd_from.channel_post"),
                type = "forward"
            )

            log.info(f'Found referenced post in {message_user_id}.')
        ...
        if "entities" in data:
            ...
            for entity in data.get("entities"):
                ...
                #         post_id=data.get("user.id") + "/" + data.get("id")
                #     )
        ...
        new_user = tg.TelegramChannel(
            ...
            username=data.get("user.username") or "",
            ...
            # else "",
            verified=data.get("user.verified") == 'True'
            if "user.verified" in data
            else False,
            restricted=data.get("user.restricted") == 'True'
            if "user.restricted" in data
            else False,
            # location=data_dict.get("user.location")
            # if "user.location" in data_dict
            # else "",
            signatures=data.get("user.signatures") == 'True'
            if "user.signatures" in data
            else False,
            creator=data.get("user.creator") == 'True'
            if "user.creator" in data
            else False,
            megagroup=data.get("user.megagroup") == 'True'
            if "user.megagroup" in data
            else False,
            gigagroup=data.get("user.gigagroup") == 'True'
            if "user.gigagroup" in data
            else False,
            scam=data.get("user.scam") == 'True'
            if "user.scam" in data
            else False,
            min=data.get("user.min") == 'True'
            if "user.min" in data
            else False,
            has_geo=data.get("user.has_geo") == 'True'
            if "user.has_geo" in data
            else False,
            has_link=data.get("user.has_link") == 'True'
            if "user.has_link" in data
            else False,
        )
