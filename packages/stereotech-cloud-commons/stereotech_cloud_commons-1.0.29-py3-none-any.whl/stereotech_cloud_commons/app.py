...
class NotificationDefinition:
    def __init__(self, evt_name: str, notify_name: Optional[str] = None, transports: List[str] = ['websocket']):
...
class DynamicRequestHandler(AuthorizedRequestHandler):
    ...
    async def _do_local_request(self,
        ...
        return await self.callback(
            WebRequest(self.request.path, args, self.request.method,
                       ...
                       user=self.current_user, scopes=self.scopes))
    ...
    async def _process_http_request(self) -> None:
        ...
        if (
            ...
            and not isinstance(self.current_user, str)
            and not set(self.current_user.get('role', {}).get("scopes", []))
            & set(self.scopes[self.request.method.lower()])
            and len(self.scopes[self.request.method.lower()]) != 0
        ):
