...
class DMS:
    ...
    def __repr__(self):
        return f'{self.degrees:.3f} {self.minutes:.3f} {self.seconds:.3f}'
...
class LLA:
    ...
    def is_valid(self):
        ...
        return self.height < ALT_THRESHOLD_M and self.height > -ALT_THRESHOLD_M
...
class Coordinates(object):
    ...
    def dms(self, a: float = WGS84_A, e: float = WGS84_E) -> LLA:
        """
        >>> coordinates = Coordinates(4889803.653, 170755.706, 4078049.851)
...
class CoordinatesDisplacement(object):
    ...
    def get_axu(self, direction:ENU) -> AXU:
        ...
        >>> velocity = roktools.geodetic.ENU(1.0, 1.0, 1)
        >>> error = roktools.geodetic.ENU(1.0, 1.0, 0)
        >>> error = roktools.geodetic.CoordinatesDisplacement(enu=error)
        >>> error.get_axu(velocity)
        1.414 0.000 0.000

        >>> error = roktools.geodetic.ENU(1.0, 0.0, 0)
        >>> error = roktools.geodetic.CoordinatesDisplacement(enu=error)
        >>> error.get_axu(velocity)
        0.707 -0.707 0.000
        ...
        >>> error = roktools.geodetic.ENU(0.0, -1.0, 0)
        >>> error = roktools.geodetic.CoordinatesDisplacement(enu=error)
        >>> error.get_axu(velocity)
        -0.707 -0.707 0.000
        """
