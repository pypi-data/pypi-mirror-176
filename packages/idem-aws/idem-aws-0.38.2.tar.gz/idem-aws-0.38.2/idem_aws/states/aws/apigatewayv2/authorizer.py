...
async def search(hub, ctx, name, api_id: str, resource_id: str) -> Dict[str, Any]:
    """
    Use an un-managed API Gateway v2 authorizer resource as a data-source. This function has been deprecated.
    Please use exec.run with aws.apigatewayv2.authorizer.get instead.
    ...
    hub.log.warning(
        f"aws.apigatewayv2.authorizer.search '{name}' state has been deprecated. Please use exec.run with aws.apigatewayv2.authorizer.get instead."
    )
    result = dict(comment=(), old_state=None, new_state=None, name=name, result=True)
