...
async def search(hub, ctx, name: str, filters: List = None, resource_id: str = None):
    ...
    This function has been deprecated.  Please use exec.run with aws.ec2.spot_instance_request.get or
    aws.ec2.spot_instance_request.list instead.
    """
    hub.log.warning(
        f"aws.ec2.spot_instance_request.search '{name}' state has been deprecated. Please use exec.run with"
        "aws.ec2.spot_instance_request.get or aws.ec2.spot_instance_request.list instead."
    )
    result = dict(
        comment=[], old_state=ctx.old_state, new_state=None, name=name, result=True
    )
