...
# sources: code.proto, status.proto
...
class Code(betterproto.Enum):
    """
    The canonical error codes for gRPC APIs. Sometimes multiple error codes may
    apply.  Services should return the most specific error code that applies.
    For example, prefer `OUT_OF_RANGE` over `FAILED_PRECONDITION` if both codes
    apply. Similarly prefer `NOT_FOUND` or `ALREADY_EXISTS` over
    `FAILED_PRECONDITION`.
    """
    ...
    # such as gradual feature rollout or undocumented whitelist, `NOT_FOUND` may
    ...
    # test-and-set fails, indicating the      client should restart a read-
    # modify-write sequence).  (c) Use `FAILED_PRECONDITION` if the client should
    # not retry until      the system state has been explicitly fixed.  E.g., if
...
class Status(betterproto.Message):
    ...
    details: "betterproto_lib_google_protobuf.Any" = betterproto.message_field(3)
