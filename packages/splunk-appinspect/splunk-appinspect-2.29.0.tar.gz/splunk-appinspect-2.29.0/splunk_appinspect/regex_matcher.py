...
class JSSplunkReactUIMatcher(RegexMatcher):
    def __init__(self):

        possible_splunkreactui_regex_patterns = [RegexBundle(r"@splunk\/react-ui")]
        super(JSSplunkReactUIMatcher, self).__init__(possible_splunkreactui_regex_patterns)


class JSSplunkDashboardCoreMatcher(RegexMatcher):
    def __init__(self):

        possible_splunkdashboardcore_regex_patterns = [RegexBundle(r"@splunk\/dashboard-core")]
...
class JSSplunkVisualizationsMatcher(RegexMatcher):
    def __init__(self):
        ...
        possible_splunkvisualizations_regex_patterns = [RegexBundle(r"@splunk\/visualizations")]
        super(JSSplunkVisualizationsMatcher, self).__init__(possible_splunkvisualizations_regex_patterns)


class FrameMatcher(RegexMatcher):
    def __init__(self):

        possible_iframe_regex_patterns = [
            RegexBundle(r'(<iframe[^>]*src=[\'"]([^\'">]*)[\'"][^>]*>)')
        ]
        super(FrameMatcher, self).__init__(possible_iframe_regex_patterns)
...
class SecretDisclosureInAllFilesMatcher(RegexMatcher):
    def __init__(self):
        secret_patterns = [
            ...
            RegexBundle(
                r"(?i)\"client_secret\":\s*\"[a-zA-Z0-9-_]{24}\""
            ),  # Google Oauth
