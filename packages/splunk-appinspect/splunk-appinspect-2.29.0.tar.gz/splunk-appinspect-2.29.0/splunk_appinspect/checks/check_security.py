...
def check_for_secret_disclosure(app, reporter):
    ...
    for result, file_path, lineno in matcher.match_results_iterator(
        ...
        if _secret_disclosure_values_whitelist(result):
            continue
        if _conf_options_secret_disclosure_whitelist(file_path, result):
            continue
        reporter_output = (
            "The following line will be inspected during code review."
            "Possible secret disclosure found."
...
def _conf_options_secret_disclosure_whitelist(file_path, line_message):
    """Checks if the line message matched by the secret disclosure regex
    is a whitelisted configuration option.
    :param file_path: the file path of this matched line
    :param line_message: the line message matched by secret_patterns regex in check_for_secret_disclosure
    :return: True or False
    """

    # List of all configuration files and the options that have been whitelisted
    conf_options_whitelist = {
        os.path.join("default", "savedsearches.conf"): [
                "action.email.show_password"
        ]
    }

    # Iterate through each configuration file that have whitelisted options and
    # check to see if it matches the line message
    for configuration_file, whitelist_options in conf_options_whitelist.items():
        if file_path == configuration_file:
            for whitelist_option in whitelist_options:
                if re.search(whitelist_option, line_message):
                    return True
...
@splunk_appinspect.tags("cloud", "private_app", "splunk_9_0", "future", "private_victoria", "private_classic", "pura")
