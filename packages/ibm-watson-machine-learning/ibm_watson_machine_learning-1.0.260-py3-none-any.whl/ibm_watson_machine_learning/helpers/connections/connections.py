...
class DataConnection(BaseDataConnection):
    ...
    def read(self,
        ...
        sample_size_limit: int, optional
            Upper limit for overall data that should be downloaded in bytes, Default: 1 GB. If more than one of:
            sample_size_limit, sample_rows_limit, sample_percentage_limit are set, then downloaded data is limited to the lowest threshold.
        ...
        sample_percentage_limit: float, optional
            ...
            This parameter is ignored, when `sampling_type` parameter is set to `first_n_records`.
            Must be a float number between 0 and 1. If more than one of:
            sample_size_limit, sample_rows_limit, sample_percentage_limit are set, then downloaded data is limited to the lowest threshold.
        ...
        try:
            use_flight = kwargs.get(
                ...
                bool((self._wml_client is not None or 'USER_ACCESS_TOKEN' in os.environ or 'RUNTIME_ENV_ACCESS_TOKEN_FILE' in os.environ) and (
                        self._wml_client.ICP_40 or self._wml_client.ICP_45 or self._wml_client.ICP_46 or
