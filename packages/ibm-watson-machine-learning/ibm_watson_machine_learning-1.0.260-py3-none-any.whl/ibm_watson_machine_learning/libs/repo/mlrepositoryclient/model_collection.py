...
class ModelCollection:
    ...
    def _prepare_model_input(artifact):
        ...
        if artifact.meta.prop(MetaNames.TRAINING_DATA_REFERENCES) is not None:
            ...
            if artifact.meta.prop(MetaNames.TRAINING_DATA_SCHEMA) and artifact.meta.prop(MetaNames.INPUT_DATA_SCHEMA) is None:
                training_data_list=[]
                training_data_ref_obj = artifact.meta.prop(MetaNames.TRAINING_DATA_SCHEMA)

                if artifact.meta.prop(MetaNames.LABEL_FIELD):
                    training_data_ref_obj['id'] = str(MetaNames.INPUT_DATA_SCHEMA)
                    training_data_ref_obj['fields'] = [f for f in training_data_ref_obj['fields'] if f['name'] != artifact.meta.prop(MetaNames.LABEL_FIELD)]

                artifact.meta.add(MetaNames.INPUT_DATA_SCHEMA, training_data_ref_obj)
