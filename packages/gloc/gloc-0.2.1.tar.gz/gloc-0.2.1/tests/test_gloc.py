...
class TestLocation(unittest.TestCase):
    ...
    def test_address_param(self):
        self.assertEqual(
            "address=Skånegatan%2059,%20412%2052%20Göteborg,%20Sweden",
    ...
    def test_create_address_string(self):
        self.assertEqual(
            ...
            gloc.formatted_address("Skånegatan 59", "412 52", "Göteborg", "Sweden")
        )
    ...
    def test_from_api_response(self):
        l1 = Location.from_api_response(mock_data.GÖTAPLATSEN_FROM_ADDRESS)
        l2 = Location.from_api_response(mock_data.GÖTAPLATSEN_FROM_COORDINATES)
        self.assertLess(l1.distance_to(l2), 0.1)

    def test_city(self):
        location = Location.from_api_response(mock_data.DOWNING_STREET)
        self.assertEqual(location.city, "London")

    def test_distance(self):
        l1 = Location.from_api_response(mock_data.STOCKHOLM)
        l2 = Location.from_api_response(mock_data.GÖTEBORG)
        self.assertAlmostEqual(l1.distance_to(l2), 400, delta=10)

    def test_no_location(self):
        self.assertIsNone(Location.from_api_response(mock_data.NORTH_POLE).country)
