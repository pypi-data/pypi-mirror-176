...
from cubicweb.schema_exporters import TypescriptSchemaExporter, JSONSchemaExporter
...
eperson = schema.add_entity_type(
    EntityType(
        ...
        __permissions__=PERSONNE_PERMISSIONS,
    )
)
...
schema.add_relation_type(RelationType("colabore_avec", symmetric=True))
done["colabore_avec"] = True
schema.add_relation_def(RelationDefinition("Personne", "colabore_avec", "Societe"))
...
class CubicWebSchemaTC(TestCase):
    ...
    def test_relation_perms(self):
        rconcerne = schema.relation_schema_for("concerne").relation_definition(
            "Personne", "Societe"
        )
...
class SchemaReaderClassTest(TestCase):
    ...
    def test_knownValues_load_schema(self):
        ...
        eschema = schema.entity_schema_for("CWUser")
        rels = sorted(str(r) for r in list(eschema.subject_relations.values()))
        ...
        rels = sorted(r.type for r in list(eschema.object_relations.values()))
        self.assertListEqual(
            rels,
            [
                "bookmarked_by",
        ...
        rschema = schema.relation_schema_for("relation_type")
        properties = rschema.relation_definition("CWAttribute", "CWRType")
    ...
    def test_fulltext_container(self):
        schema = loader.load(config)
        self.assertIn("has_text", list(schema["CWUser"].subject_relations.values()))
        self.assertNotIn(
            "has_text", list(schema["EmailAddress"].subject_relations.values())
        )

    def test_permission_settings(self):
        schema = loader.load(config)
        aschema = schema["TrInfo"].relation_definition("comment")
    ...
    def test_relation_perm_overriding(self):
        ...
        config.bootstrap_cubes()
        schema = loader.load(config)
        rdef = next(iter(schema["in_group"].relation_definitions.values()))
        self.assertEqual(
            rdef.permissions,
            {"read": ("managers",), "add": ("managers",), "delete": ("managers",)},
        )
        rdef = next(iter(schema["cw_for_source"].relation_definitions.values()))
        self.assertEqual(
            rdef.permissions,
            {
                "read": ("managers", "users"),
    ...
    def test_computed_attribute(self):
        ...
        rdef = good_schema["Company"].relation_definition("total_salary")
...
class SchemaReaderComputedRelationAndAttributesTest(TestCase):
    def test_infer_computed_relation(self):
        ...
        self.assertEqual(
            [("Person", "Service")],
            list(schema["produces_and_buys"].relation_definitions.keys()),
        )
        self.assertEqual(
        ...
        self.assertCountEqual(
            ...
            list(schema["reproduce"].relation_definitions.keys()),
        )
        # check relation definitions are marked infered
        rdef = schema["produces_and_buys"].relation_definitions[("Person", "Service")]
...
class CompositeSchemaTC(CubicWebTC):
    composites = {
        "Ami": [("comments", "Comment", "Ami", "object")],
        ...
        "CWSource": [
            ...
            ("cw_source", "Comment", "CWSource", "object"),
            ...
            ("cw_source", "Ville", "CWSource", "object"),
        ...
        "Personne": [
            ("comments", "Comment", "Personne", "object"),
        ],
        ...
        for etype in self.composites:
            with self.subTest(etype=etype):
                self.assertEqual(
                    ...
                    sorted(
                        [
                            (r.relation_type.type, r.subject.type, r.object.type, role)
...
class TypescriptSchemaExporterTC(TestCase):
    def test_contains_json_export(self):
        exporter = TypescriptSchemaExporter("TestSchema")
        json_exporter = JSONSchemaExporter()

        json_export = json_exporter.export(schema)
        typescript_export = exporter.export(schema)

        assert json_export in typescript_export

    def test_default_name_generates_default_export(self):
        exporter = TypescriptSchemaExporter("default")

        export = exporter.export(schema)

        assert "export default" in export

    def test_custom_name_generates_named_export(self):
        exporter = TypescriptSchemaExporter("MyTestSchema")

        export = exporter.export(schema)

        assert "export interface MyTestSchema" in export


class JSONSchemaExporterTC(TestCase):
    def setUp(self):
        self.json_exporter = JSONSchemaExporter()
        self.schema_json = json.loads(self.json_exporter.export(schema))

    def test_generates_valid_json(self):
        assert "entities" in self.schema_json
        assert "relations_definitions" in self.schema_json

    def test_same_number_of_entities_as_in_schema(self):
        entities_count = len(schema.entities())

        assert len(self.schema_json["entities"]) == entities_count

    def test_same_number_of_relations_definitions_as_in_schema(self):
        relations_definitions_count = sum(
            len(rel.relation_definitions) for rel in schema.relations()
        )

        assert (
            len(self.schema_json["relations_definitions"])
            == relations_definitions_count
        )

    def test_entity(self):
        personne = next(
            e for e in self.schema_json["entities"] if e["type"] == "Personne"
        )
    ...
    def test_final_entity(self):
        string = next(e for e in self.schema_json["entities"] if e["type"] == "String")
    ...
    def test_attribute_definition(self):
        nom = next(
            e for e in self.schema_json["relations_definitions"] if e["type"] == "nom"
        )
    ...
    def test_relation_definition(self):
        travaille = next(
            e
            for e in self.schema_json["relations_definitions"]
            if e["type"] == "travaille"
        )
    ...
    def test_symetric_relation(self):
        rel1 = {
            "type": "colabore_avec",
            "description": "",
            "final": False,
        ...
        self.assertIn(rel1, self.schema_json["relations_definitions"])
        self.assertIn(rel2, self.schema_json["relations_definitions"])
