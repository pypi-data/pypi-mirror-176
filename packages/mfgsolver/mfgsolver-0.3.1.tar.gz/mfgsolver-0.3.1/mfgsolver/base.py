...
class MfgBase(object):
    ...
    def __init__(self, Q_guess=None, nv=0.05, Nx=200, Nt=200, T=1.0, bc="periodic"):
        self.Nx = Nx
        self.Nt = Nt
        if bc == "periodic":
            self.xs = np.linspace(0, 1, Nx + 1)[:-1]  # grid on space
        elif bc == "dirichlet":
            self.xs = np.linspace(0, 1, Nx + 1)[1:-1]  # grid on space
        elif bc == "neumann":
            self.xs = np.linspace(0, 1, Nx + 1)  # grid on space
...
class MfgBase2D(object):
    ...
    def __init__(
        ...
        self.Nt = Nt
        if bc == "periodic":
            self.xs1 = np.linspace(0, 1, Nx + 1)[:-1]  # grid on space
            self.xs2 = np.linspace(0, 1, Nx + 1)[:-1]  # grid on space
        elif bc == "dirichlet":
            self.xs1 = np.linspace(0, 1, Nx + 1)[1:-1]  # grid on space
            self.xs2 = np.linspace(0, 1, Nx + 1)[1:-1]  # grid on space
        elif bc == "neumann":
            self.xs1 = np.linspace(0, 1, Nx + 1)  # grid on space
            self.xs2 = np.linspace(0, 1, Nx + 1)  # grid on space
    ...
    def build_Laplace(self, bc="periodic"):
        if bc == "periodic":
            ...
            dim_one_laplace = diags([1, -2, 1], [-1, 0, 1], format="csr")
            dim_two_laplace = sparse.kron(
                dim_one_laplace, sparse.eye(self.Nx)
            ) + sparse.kron(sparse.eye(self.Nx), dim_one_laplace)
            dim_two_laplace = self.nv / self.dx**2 * dim_two_laplace
