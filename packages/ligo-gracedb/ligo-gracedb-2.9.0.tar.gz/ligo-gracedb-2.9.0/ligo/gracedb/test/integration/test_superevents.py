...
def test_basic_creation_and_retrieval(client, create_event):
    event = create_event()
    response = client.create_superevent(1, 2, 3,
...
def test_creation_with_already_used_preferred_event(client, create_event):
    ...
    with pytest.raises(HTTPError):
        response = client.create_superevent(1, 2, 3, event, category='T')
...
def test_update(client, create_event, create_superevent, params):
    ...
    response = client.update_superevent(superevent['superevent_id'], **params)
...
def test_event_addition_and_removal(client, create_event, create_superevent):
    ...
    response = client.add_event_to_superevent(superevent['superevent_id'],
                                              new_event['graceid'])
    ...
    # Remove event
    response = client.remove_event_from_superevent(superevent['superevent_id'],
                                                   new_event['graceid'])
    ...
    with pytest.raises(HTTPError):
        client.remove_event_from_superevent(
...
def test_pipeline_preferred_add_remove(client, create_event,
                                       create_event_spiir, create_superevent):
    superevent = create_superevent()
    spiir_event = create_event_spiir()

    sid = superevent['superevent_id']
    spiir_id = spiir_event['graceid']

    # Confirm that the preferred event is in the pipeline_preferred
    # response:

    assert superevent['preferred_event'] == \
        superevent['pipeline_preferred_events']['gstlal']['graceid']

    # The superevent has the gstlal event as preferred. Try and add
    # the spiir event without being part of the superevent. It should
    # HTTPError 400

    with pytest.raises(HTTPError):
        client.add_pipeline_preferred_event(sid, spiir_id)

    # Now add the spiir event to the superevent and verify that it 201's:
    response = client.add_event_to_superevent(sid, spiir_id)
    assert response.status_code == 201

    # Now add the spiir event to the superevent's pipeline_preferred_events
    # list.
    response = client.add_pipeline_preferred_event(sid, spiir_id)
    # Check results
    assert response.status_code == 201
    data = response.json()
    assert data['graceid'] == spiir_id

    response = client.superevent(sid)
    data = response.json()
    assert response.status_code == 200
    assert data['pipeline_preferred_events']['spiir']['graceid'] == spiir_id

    # Now the spiir event should remove without error:
    response = client.remove_pipeline_preferred_event(sid, spiir_id)
    # Check results:
    assert response.status_code == 204
    response = client.superevent(sid)
    data = response.json()
    assert response.status_code == 200
    assert len(data['pipeline_preferred_events']) == 1

    # Trying to remove again should result in a 404 error:
    with pytest.raises(HTTPError):
        client.remove_pipeline_preferred_event(sid, spiir_id)

    # Add the spiir event back to the pipeline_preferred_events list:
    response = client.add_pipeline_preferred_event(sid, spiir_id)
    assert response.status_code == 201

    # Now remove the spiir event from the superevent's event list:
    response = client.remove_event_from_superevent(sid, spiir_id)
    assert response.status_code == 204

    # Check that the spiir event is no longer in the pipeline_preferred_evnts
    # list:
    response = client.superevent(sid)
    data = response.json()
    assert response.status_code == 200
