...
def sector_from_coord(coord: Tuple[float, float], sector_dims: Tuple[float, float]) -> Tuple[int, int]:
    ...
    """
    return math.floor(coord[1] / sector_dims[1]), math.floor(coord[0] / sector_dims[0])
...
def rect_sector_from_coord(coord: Tuple[float, float], area_rect: Tuple[float, float], sector_def: Tuple[int, int]) -> (float, float):
    ...
    row, column = sector_from_coord(coord, sec_dims)
...
def coord_of_sector(sector_def: Tuple[int, int],
                    ...
                    area_rect: Tuple[float, float] = None,
    ...
    if area_rect is not None:
        ...
        raise ValueError(f"at least one of area_rect or sector_dims must be provided")
...
def coord_of_sector_from_area(area_rect: Tuple[float, float], sector_def: Tuple[int, int], sector: Tuple[int, int], cardinality: CardinalPosition):
    ...
    """
    sector_attr = rect_sector_attributes(area_rect, sector_def)
    return coord_of_sector(area_rect=area_rect,
                           sector=sector,
...
if __name__ == "__main__":
    from cooptools.sectors import utils as utils
    sector_def = utils.square_sector_def(1000) # should yield 3x3
    print (sector_def)
