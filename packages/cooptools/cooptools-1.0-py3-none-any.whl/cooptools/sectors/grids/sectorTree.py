...
class SectorTree:
    ...
    def _add_child_layer(self, grid_pos: Tuple[int, int]):
        # get the top left coord of the grid pos
        grid_pos_tl_coord = sec_u.coord_of_sector(area_rect=(self._area.width, self._area.height),
        ...
        child_rect = Rectangle(grid_pos_tl_coord[0] + self._area.x,
                               grid_pos_tl_coord[1] + self._area.y,
                               self._area.height / self.grid.nRows,
                               self._area.width / self.grid.nColumns)
    ...
    def add_update_client(self, client, pos: Tuple[float, float]):
        ...
        grid_pos = sec_u.rect_sector_from_coord(coord=pos_in_lvl,
                                                area_rect=(self._area.width, self._area.height),
                                                sector_def=self.grid.Shape)
    ...
    def _sector_corners_nearby(self, radius: float, pt: Tuple[float, float]):
        ...
        for pos, sector in self.MySectors:
            tl = self._within_radius_of_point(sector.TopLeft.as_tuple(), radius, pt)
            tr = self._within_radius_of_point(sector.TopRight.as_tuple(), radius, pt)
            bl = self._within_radius_of_point(sector.BottomLeft.as_tuple(), radius, pt)
            br = self._within_radius_of_point(sector.BottomRight.as_tuple(), radius, pt)
    ...
    def _sectors_potentially_overlaps_radius(self, radius: float, pt: Tuple[float, float]):
        ...
        for pos, sector_area in self.MySectors:
            ret[pos] = False
            if sector_area.BoundingCircleRadius + radius > (Vector2.from_tuple(pt) - sector_area.Center).length():
                ret[pos] = True
        return ret
    ...
    def MySectors(self) -> List[Tuple[Tuple[float, float], Rectangle]]:
        ...
        for pos, _ in self.grid.grid_enumerator:
            mine.append((pos.as_tuple(), Rectangle(pos.y * sec_def[0] + self._area.x, pos.x * sec_def[1] + self._area.y, height=sec_def[1], width=sec_def[0])))
...
if __name__ == "__main__":
    ...
    qt = SectorTree(area_rect=Rectangle(0, 0, h, w),
                    shape=(3, 3),
                    capacity=1,
                    max_lvls=3)
