...
def rect_corners(rect: Tuple[float, float, float, float]) -> Dict[CardinalPosition, Tuple[float, float]]:
    x, y, w, h = rect

    partial_pos = partial(CardinalPosition.alignment_from_top_left, dims=(w, h), top_left=(x, y))

    return {
        CardinalPosition.TOP_LEFT: partial_pos(cardinality=CardinalPosition.TOP_LEFT),
        CardinalPosition.TOP_RIGHT: partial_pos(cardinality=CardinalPosition.TOP_RIGHT),
        CardinalPosition.BOTTOM_RIGHT: partial_pos(cardinality=CardinalPosition.BOTTOM_RIGHT),
        CardinalPosition.BOTTOM_LEFT: partial_pos(cardinality=CardinalPosition.BOTTOM_LEFT)
    }
