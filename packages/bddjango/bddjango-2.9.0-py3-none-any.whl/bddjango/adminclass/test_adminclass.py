        ...
        try:
            if request.method == "POST":
                    ...
                    for i in range(len(title_ls)):
                        ...
                        if attr_field_name in ['DateField', 'DateTimeField']:
                            if attr_field_name == 'DateTimeField':
                                ...
                                ts = content_ls[i]
                                res = conv_date_field_str_format(ts=ts) or conv_date_time_field_str_format(ts=content_ls[i])
                                if res:
                                    try:
                                        if '.' in res:
                                            res_dt = datetime.datetime.strptime(res, common_date_time_formats.ms_dt)
                                        elif ':' in res:
                                            res_dt = datetime.datetime.strptime(res, common_date_time_formats.s_dt)
                                        else:
                                            res_dt = datetime.datetime.strptime(res, common_date_time_formats.only_date)
                                        res = res_dt.strftime(common_date_time_formats.only_date)
                                    except Exception as e:
                                        raise ValueError(f'字段[{title_i}]日期格式错误! 标准格式: [%Y-%m-%d]. error: {e}')
                                content_ls[i] = res
                ...
                if self.use_bulk_create:
                    ...
                    def check_df_format(df, md_dc_ls, start=0, end=None, n=None, tqdm_i=None):
                    ...
                    def save_md_dc_ls(md_dc_ls, start=0, end=None, n=None, tqdm_i=None):
                    ...
                    # save_md_dc_ls(md_dc_ls, start=0, end=1000, n=None, tqdm_i=save_tqdm)
                    # threading.Thread()

                    # self.model.objects.count()
                    # self.model.objects.all().delete()
                    use_threading_to_create_data = True     # 使用多线程来保存数据
                    n = len(md_dc_ls)
                    # interval = 50000  # 10万条开启一个线程
                    interval = 500  # 10万条开启一个线程
                    if n < interval:
                    ...
                    # if use_threading_to_create_data:
                        a = np.arange(n)
                        s = slice(0, len(a), interval)
                        start_end_ls = list(a[s])
                        thread_ls = []
                        for start_end_i in start_end_ls:
                            print(start_end_i, start_end_i + interval)
                            # break
                            thread_i = threading.Thread(target=save_md_dc_ls, args=(md_dc_ls, start_end_i, start_end_i + interval, n))
                            thread_ls.append(thread_i)
                            thread_i.start()
                            break

                    # count_0 = self.model.objects.count()
                    # # tqdm_i = save_tqdm
                    # for i in range(len(md_dc_ls)):
                    #     if i and i % 100000 == 0:
                    #         t_import.sleep(1)
