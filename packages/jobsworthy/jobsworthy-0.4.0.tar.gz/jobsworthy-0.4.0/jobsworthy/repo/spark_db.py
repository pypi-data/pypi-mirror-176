...
class DbNamingConventionProtocol(Protocol):
    ...
    def db_table_name(self, table_name) -> str:
        ...
        Used when reading using the Spark session

        e.g. spark.read.format('delta').load(self.db_table_name("table")
...
class DbNamingConventionCallerDefined(DbNamingConventionProtocol):
    ...
    def __init__(self, job_config: config.JobConfig):
    ...
    def db_path(self):
        """
        Usded to define the location of a Hive table.  The DB Path is formatted as follows:
