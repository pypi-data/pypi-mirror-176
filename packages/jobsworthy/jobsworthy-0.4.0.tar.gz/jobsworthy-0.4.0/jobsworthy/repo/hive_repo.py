...
class HiveRepo:
    ...
    def write_stream(self, stream, partition_cols: tuple = tuple(), trigger: dict = None):
        ...
        self.streamQ = self.write_stream_to_table(stream, self.table_name, trigger_condition, partition_cols)
        return self
    ...
    def write_stream_temporary(self, stream, partition_cols: tuple = tuple()) -> DataFrame:
        if not hasattr(self, "temp_table_name") or not self.__class__.temp_table_name:
            raise error.RepoConfigError('temp_table_name class property not provided')
        ...
        self.streamQ = self.write_stream_to_table(stream, self.temp_table_name, trigger_condition, partition_cols)
    ...
    def await_termination(self, other_streamQ=None):
        target_stream = other_streamQ if other_streamQ else self.streamQ
...
class DeltaStreamUpserter:
    ...
    def __init__(self, repo, partition_cols):
        self.repo = repo
    ...
    def stream_to_temp(self, stream) -> monad.EitherMonad:
        self.streamQ = self.temp_repo.write_stream_to_table(stream,
                                                            self.temp_table_name(self.temp_repo.table_name),
                                                            self.repo.__class__.default_stream_trigger_condition,
                                                            self.partition_cols)
        return monad.Right(self.streamQ)

    def await_termination(self, streamQ) -> monad.EitherMonad[pyspark.sql.streaming.StreamingQuery]:
        self.temp_repo.await_termination(streamQ)
        return monad.Right(streamQ)

    def read_temp(self, _streamQ) -> monad.EitherMonad[DataFrame]:
