...
class CodeDeploymentMetadata(
    ...
    def get_grpc_inject_env_vars_cli_env(self, deployment_name: str, location_name: str, instance):
        return {
            "DAGSTER_LOCATION_NAME": location_name,
            "DAGSTER_INJECT_ENV_VARS_FROM_INSTANCE": "1",
            "DAGSTER_INSTANCE_REF": serialize_dagster_namedtuple(
                instance.ref_for_deployment(deployment_name)
            ),
        }

    def get_grpc_server_env(
        self, port: int, deployment_name: str, location_name: str, instance
    ) -> Dict[str, str]:
        return merge_dicts(
            self.get_grpc_inject_env_vars_cli_env(deployment_name, location_name, instance),
            {
