...
def _build_upload_pex_if_needed(location: str, kwargs: Dict[str, Any]) -> Dict[str, Any]:
    # build and upload the python executable, return the modified kwargs with pex_tag
    python_source_dir = kwargs.pop("upload_python_executable", None)
    if not python_source_dir:
        return kwargs
    python_version = pex_builder.util.parse_python_version(kwargs.pop("python_version", "3.8"))
    ui.print(
        f"Building Python executable for directory {python_source_dir!r} for Python {python_version}."
    )
    ...
    with tempfile.TemporaryDirectory() as temp_folder:
        ...
        if not kwargs.get("image"):
            kwargs["image"] = pex_builder.deploy.get_base_image_for(build)
...
def add_command(
    ...
    **kwargs,
    ...
    kwargs = _build_upload_pex_if_needed(location, kwargs)
    ...
    while True:
        ...
        if all(
            ...
            if error_locations:
                ...
                ui.print(
                    f"Agent synced changes to {list_locations(locations)}. Changes should now be visible in Dagster Cloud."
                )
                break
        ...
        if iterations % 3 == 0:
            ui.print(
                f"Still waiting for agent to sync changes to {list_locations(locations)}. This can take a few minutes."
            )
