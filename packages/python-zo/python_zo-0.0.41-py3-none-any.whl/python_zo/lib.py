...
def mix_all(args, mappings, aliases, tags):
	...
	for x, y in permutations(args, 2):
		y_map = {yy['key']: yy for yy in y['data'] if y['kind'] != 'gql' or yy.get('kind') != 'OperationDefinition'}
		for v in x['data']:
			k = v['key']
...
def render_template(env, schemas, args):
	...
	if args.cmd == 'scaffold':
		...
		schema_map = {s['kind']: {x['key']: x for x in s['data'] if s['kind'] != "gql" or x.get('kind') != "OperationDefinition"} for s in schemas}
		for s in schemas:
