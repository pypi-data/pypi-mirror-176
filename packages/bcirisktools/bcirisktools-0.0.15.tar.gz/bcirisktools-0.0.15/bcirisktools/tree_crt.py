...
def get_report(summaries, report_name="Reporte"):
    ...
    for i, feature in enumerate(summaries.keys()):
        ...
        values = [float(i.split(",")[0].replace("(", "").replace("[", "")) for i in summary[["Categoria"]].iloc[:-1, 0]]
        summary = pd.concat([summary, pd.Series(values)], axis=1).sort_values(0).drop(columns=[0])
